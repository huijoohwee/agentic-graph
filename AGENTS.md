...
- Use `agentic-graph` for public product paths and generated artifacts, `agentic-*` for deployable Cloudflare service identities, and `AGENTIC_OS_*` plus `/agentic-os/...` for internal runtime protocol names. Legacy `agenticgraph` and `knowgrph` identifiers may appear only in bounded compatibility or retirement evidence; do not introduce new product or runtime dependencies on them.
