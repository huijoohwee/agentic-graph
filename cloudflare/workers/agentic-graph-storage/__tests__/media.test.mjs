...
// agentic-graph-widget-canvas-media spec · Task 7.2
...
const AGENTIC_OS_MEDIA_ROUTE_PREFIX = "/api/storage/media/";
const AGENTIC_OS_STORAGE_API_VERSION = "2026-05-04";

const MEDIA_CORS_HEADERS = {
  "access-control-allow-origin": "*",
  "access-control-allow-methods": "GET,HEAD,PUT,POST,OPTIONS",
  "access-control-allow-headers":
    "content-type,authorization,content-hash,x-agentic-graph-content-hash",
...
function readMediaObjectKey(pathname) {
  const suffix = pathname.slice(AGENTIC_OS_MEDIA_ROUTE_PREFIX.length);
...
async function handleMediaWrite(request, env, authProvider) {
  ...
  if (!bucket) return authErrorResponse(500, "server_error", "missing Cloudflare R2 binding AGENTIC_OS_STORAGE_BLOB_BUCKET");

  const contentType = normalizeString(request.headers.get("content-type")) || "application/octet-stream";
  const contentHash = normalizeString(request.headers.get("content-hash") || request.headers.get("x-agentic-graph-content-hash")) || null;
...
async function handleMediaRead(request, env, authProvider) {
  ...
  headers.set("x-agentic-graph-storage-object-key", objectKey);
...
const TEST_URL = `https://airvio.co${AGENTIC_OS_MEDIA_ROUTE_PREFIX}${TEST_OBJECT_KEY}`;
...
function makeEnv(bucket) {
  return { AGENTIC_OS_STORAGE_BLOB_BUCKET: bucket };
}
...
test("Read returns 200 for an authorized request when the object exists", async () => {
  ...
  assert.equal(response.headers.get("x-agentic-graph-storage-object-key"), TEST_OBJECT_KEY);
});
