...
const CORS_HEADERS = {
  ...
  'access-control-allow-headers': 'content-type,authorization,x-client-request-id,x-agentic-graph-session-token,x-agentic-graph-media-capability,x-agentic-graph-content-hash,x-agentic-graph-content-kind,x-agentic-graph-content-sha256,x-agentic-graph-file-sync-meta',
  'access-control-expose-headers': 'x-agentic-graph-file-sync-meta',
...
const handleCanvasRoomProxy = async (
  ...
  const route = readAgenticGraphCanvasRoomProxyIdentity(request, AGENTIC_OS_STORAGE_ROUTE_PATHS.canvasRoomPrefix)
  ...
  const namespace = env.AGENTIC_OS_CANVAS_ROOM
  if (!namespace) return errorResponse(500, 'server_error', 'missing Cloudflare Durable Object binding AGENTIC_OS_CANVAS_ROOM')
  ...
  headers.set('x-agentic-graph-room-workspace-id', route.workspaceId)
  headers.set('x-agentic-graph-room-id', route.roomId)
  headers.set('x-agentic-graph-user-id', auth.value.user.id)
  headers.set('x-agentic-graph-session-id', auth.value.session.id)
  if (devicePrincipalId) headers.set('x-agentic-graph-device-principal-id', devicePrincipalId)
  headers.set('x-agentic-graph-user-display-name', normalizeString(auth.value.user.displayName) || normalizeString(auth.value.user.email) || auth.value.user.id)
  headers.set('x-agentic-graph-room-role', membership.membership.role)
  ...
  if (side) {
    headers.set('x-agentic-graph-room-membership-id', side.membershipId)
    headers.set('x-agentic-graph-room-transaction-side', side.transactionSide)
  }
  const roomUrl = `https://agentic-graph.internal${targetPath}?workspaceId=${encodeURIComponent(route.workspaceId)}&roomId=${encodeURIComponent(route.roomId)}`
...
const handlePush = async (
  ...
  if (body.mutations.length > AGENTIC_OS_STORAGE_SYNC_LIMITS.maxPushMutations) {
    return errorResponse(
      413,
      'bad_request',
      `storage push exceeds the ${AGENTIC_OS_STORAGE_SYNC_LIMITS.maxPushMutations} mutation limit`,
    )
  }
  if (body.mutations.some(mutation => jsonByteLength(mutation) > AGENTIC_OS_STORAGE_SYNC_LIMITS.maxMutationBytes)) {
    return errorResponse(
      413,
      'bad_request',
...
const handleExport = async (
  ...
  const encodedWorkspaceId = pathname.slice(AGENTIC_OS_STORAGE_ROUTE_PATHS.exportPrefix.length)
  ...
  async fetch(request: Request, env: AgenticGraphStorageWorkerEnv): Promise<Response> {
    ...
    try {
      if (url.pathname === '/livez' || url.pathname === '/api/storage/livez') {
        ...
        const response = json(200, { ok: true, service: 'agentic-storage', status: 'live' })
      ...
      if (request.method === 'POST' && url.pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.collabSave) {
      ...
      if (url.pathname.startsWith(AGENTIC_OS_STORAGE_ROUTE_PATHS.canvasRoomPrefix)) {
        return await handleCanvasRoomProxy(request, env, db)
      }
      if (isAgenticGraphStorageChatRoute(url.pathname)) {
        if (request.method === 'GET' && url.pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.chatSession) {
          return await handleChatSession(request, db)
        }
        if (request.method === 'GET' && url.pathname.startsWith(AGENTIC_OS_STORAGE_ROUTE_PATHS.chatPoliciesPrefix)) {
          return await handleChatPolicies(request, db)
        }
        if (request.method === 'GET' && url.pathname.startsWith(AGENTIC_OS_STORAGE_ROUTE_PATHS.chatAuditPrefix)) {
          return await handleChatAudit(request, db)
        }
        if (request.method === 'POST' && url.pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.chatRelay) {
      ...
      if (request.method === 'POST' && url.pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.push) {
      ...
      if (request.method === 'POST' && url.pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.pull) {
        const auth = await authenticateAgenticGraphStorageSnapshotRequest(request, env, db)
        if (auth.ok === false) return auth.response
        return await handlePull(request, db, auth.principal)
      }
      if (request.method === 'GET' && url.pathname.startsWith(AGENTIC_OS_STORAGE_ROUTE_PATHS.exportPrefix)) {
        const auth = await authenticateAgenticGraphStorageSnapshotRequest(request, env, db)
        if (auth.ok === false) return auth.response
