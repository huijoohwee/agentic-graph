...
export const AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS = {
...
export const readBoundedPublishedMarkdown = async (
  ...
  if (contentBytes > AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxDocumentBytes) {
    throw new AgenticGraphStorageDocumentReadLimitError(
      `document exceeds the ${AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxDocumentBytes} byte response limit`,
    )
  }
  if (contentBytes > 0) {
    ...
    if (utf8Bytes(markdown) > AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxDocumentBytes) {
  ...
  if (chunkCount > AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxDocumentChunks) {
    throw new AgenticGraphStorageDocumentReadLimitError(
      `document exceeds the ${AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxDocumentChunks} chunk response limit`,
    )
  }
  if (chunkBytes > AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxDocumentBytes) {
    throw new AgenticGraphStorageDocumentReadLimitError(
      `document exceeds the ${AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxDocumentBytes} byte response limit`,
    )
  }
  const chunks = await queryAll<PublishedChunkRow>(db,
    `select id, chunk_order, markdown from document_chunks
     ...
     limit ?`, [workspaceId, document.id, AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxDocumentChunks + 1])
  if (chunks.length > AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxDocumentChunks) {
  ...
  const rows = await queryAll<CrawlerDocumentRow>(db,
    `select documents.id, documents.canonical_path, documents.title, documents.doc_type,
     ...
     limit ?`, [
      ...
      AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxCrawlerRows + 1,
    ])
  const page = rows.slice(0, AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxCrawlerRows)
  let metadataBytes = 0
  for (const row of page) {
    metadataBytes += utf8Bytes(JSON.stringify(row))
    if (metadataBytes > AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxCrawlerMetadataBytes) {
      throw new AgenticGraphStorageDocumentReadLimitError(
        `crawler page exceeds the ${AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxCrawlerMetadataBytes} byte metadata limit`,
      )
    }
  }
...
export const assertBoundedCrawlerResponse = (body: string): void => {
  if (utf8Bytes(body) > AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxCrawlerResponseBytes) {
    throw new AgenticGraphStorageDocumentReadLimitError(
      `crawler response exceeds the ${AGENTIC_OS_STORAGE_DOCUMENT_READ_LIMITS.maxCrawlerResponseBytes} byte limit`,
    )
  }
}
