import {
  AGENTIC_OS_RUNTIME_IDENTITY_ROOM_ID,
...
const CANVAS_ROOM_INTERNAL_HEADERS = {
  workspaceId: 'x-agentic-graph-room-workspace-id',
  roomId: 'x-agentic-graph-room-id',
  userId: 'x-agentic-graph-user-id',
  sessionId: 'x-agentic-graph-session-id',
  devicePrincipalId: 'x-agentic-graph-device-principal-id',
  displayName: 'x-agentic-graph-user-display-name',
  role: 'x-agentic-graph-room-role',
  membershipId: 'x-agentic-graph-room-membership-id',
  transactionSide: 'x-agentic-graph-room-transaction-side',
} as const
...
const RUNTIME_IDENTITY_ATTESTATION_SCHEMA = 'agentic-graph-runtime-identity-attestation/v1'
...
export class AgenticGraphCanvasSyncRoom {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    return json(404, { ok: false, apiVersion: AGENTIC_OS_STORAGE_API_VERSION, error: 'canvas room route not found' })
  }
  async webSocketMessage(ws: WebSocket, message: ArrayBuffer | string): Promise<void> {
    ...
    if (payload.type === 'runtime.identity.challenge.request') {
      if (attachment.roomId !== AGENTIC_OS_RUNTIME_IDENTITY_ROOM_ID) {
  ...
  private async handleConnect(request: Request): Promise<Response> {
    if (request.method !== 'GET') {
      return json(405, { ok: false, apiVersion: AGENTIC_OS_STORAGE_API_VERSION, error: 'unsupported canvas room connect method' })
    }
    if (!isWebSocketUpgrade(request)) {
      return json(426, { ok: false, apiVersion: AGENTIC_OS_STORAGE_API_VERSION, error: 'canvas room connect requires websocket upgrade' })
    }
    if (typeof this.state.acceptWebSocket !== 'function') {
      return json(500, { ok: false, apiVersion: AGENTIC_OS_STORAGE_API_VERSION, error: 'canvas room websocket accept is unavailable' })
    }
    const attachment = this.readConnectionAttachment(request)
    if (!attachment) {
      return json(401, { ok: false, apiVersion: AGENTIC_OS_STORAGE_API_VERSION, error: 'missing authenticated canvas room identity' })
    }
    if (
      attachment.roomId === AGENTIC_OS_RUNTIME_IDENTITY_ROOM_ID
      ...
      return json(409, { ok: false, apiVersion: AGENTIC_OS_STORAGE_API_VERSION, error: 'authenticated session is already bound to another device principal' })
    }
    ...
    const latestAsset = attachment.roomId === AGENTIC_OS_RUNTIME_IDENTITY_ROOM_ID
      ? null
  ...
  private async handleStatus(request: Request): Promise<Response> {
    if (request.method !== 'GET') {
      return json(405, { ok: false, apiVersion: AGENTIC_OS_STORAGE_API_VERSION, error: 'unsupported canvas room status method' })
    }
    ...
    if (!workspaceId || !roomId) {
      return json(400, { ok: false, apiVersion: AGENTIC_OS_STORAGE_API_VERSION, error: 'workspaceId and roomId are required for canvas room status' })
    }
  ...
  private async handleAssetSync(request: Request): Promise<Response> {
    if (request.method !== 'POST') {
      return json(405, { ok: false, apiVersion: AGENTIC_OS_STORAGE_API_VERSION, error: 'unsupported canvas room route method' })
    }
    const body = await readJsonBody(request)
    if (!body) {
      return json(400, { ok: false, apiVersion: AGENTIC_OS_STORAGE_API_VERSION, error: 'invalid canvas room asset payload' })
    }
    ...
    if (!workspaceId || !roomId || !artifactId || !contentHash) {
      return json(400, { ok: false, apiVersion: AGENTIC_OS_STORAGE_API_VERSION, error: 'missing canvas room asset identity' })
    }
    if (roomId === AGENTIC_OS_RUNTIME_IDENTITY_ROOM_ID) {
      return json(409, { ok: false, apiVersion: AGENTIC_OS_STORAGE_API_VERSION, error: 'identity room cannot persist assets' })
    }
  ...
  private readConnectionAttachment(request: Request): CanvasRoomConnectionAttachment | null {
    ...
    if (
      ...
      || !displayName
      || !isChatRole(roleRaw)
      || (roomId === AGENTIC_OS_RUNTIME_IDENTITY_ROOM_ID && !SHA256_PATTERN.test(devicePrincipalId))
    ) return null
    return {
      workspaceId,
      roomId,
