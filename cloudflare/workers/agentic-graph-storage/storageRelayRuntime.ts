...
const readRelayWorkspaceId = (env: AgenticGraphStorageWorkerEnv): string =>
  readEnvText(env.AGENTIC_OS_STORAGE_REMOTE_RELAY_WORKSPACE_ID)
  || AGENTIC_OS_STORAGE_DEFAULT_WORKSPACE_ID

const readGitPathPrefixes = (env: AgenticGraphStorageWorkerEnv): readonly string[] => {
  const configured = readEnvText(env.AGENTIC_OS_STORAGE_GIT_ALLOWED_PATH_PREFIXES)
...
const readGoogleAccessTokenSource = (env: AgenticGraphStorageWorkerEnv): StorageRelayAccessTokenSource | null => {
  const clientId = readEnvText(env.AGENTIC_OS_STORAGE_GOOGLE_DRIVE_CLIENT_ID)
  const clientSecret = readEnvText(env.AGENTIC_OS_STORAGE_GOOGLE_DRIVE_CLIENT_SECRET)
  const refreshToken = readEnvText(env.AGENTIC_OS_STORAGE_GOOGLE_DRIVE_REFRESH_TOKEN)
  return readAccessTokenSource({
    staticAccessToken: readEnvText(env.AGENTIC_OS_STORAGE_GOOGLE_DRIVE_ACCESS_TOKEN),
...
const readOneDriveAccessTokenSource = (env: AgenticGraphStorageWorkerEnv): StorageRelayAccessTokenSource | null => {
  const tenantId = readEnvText(env.AGENTIC_OS_STORAGE_ONEDRIVE_TENANT_ID)
  const clientId = readEnvText(env.AGENTIC_OS_STORAGE_ONEDRIVE_CLIENT_ID)
  const clientSecret = readEnvText(env.AGENTIC_OS_STORAGE_ONEDRIVE_CLIENT_SECRET)
  const refreshToken = readEnvText(env.AGENTIC_OS_STORAGE_ONEDRIVE_REFRESH_TOKEN)
  return readAccessTokenSource({
    staticAccessToken: readEnvText(env.AGENTIC_OS_STORAGE_ONEDRIVE_ACCESS_TOKEN),
...
const createGitRegistry = (env: AgenticGraphStorageWorkerEnv): GitRemoteRegistry => {
  const token = readEnvText(env.AGENTIC_OS_STORAGE_GITHUB_TOKEN)
  const owner = readEnvText(env.AGENTIC_OS_STORAGE_GITHUB_OWNER)
  const branch = readEnvText(env.AGENTIC_OS_STORAGE_GITHUB_BRANCH)
...
const createFileSyncRegistry = (env: AgenticGraphStorageWorkerEnv): FileSyncProviderRegistry => {
  ...
  const googleRootId = readEnvText(env.AGENTIC_OS_STORAGE_GOOGLE_DRIVE_ROOT_ID)
  if (googleAccessToken && googleRootId) {
    registrations.push({
      ...
      provider: new GoogleDriveFileSyncProvider({
        ...
        driveId: readEnvText(env.AGENTIC_OS_STORAGE_GOOGLE_DRIVE_ID) || null,
      }),
    })
  }
  const oneDriveAccessToken = readOneDriveAccessTokenSource(env)
  const oneDriveDriveId = readEnvText(env.AGENTIC_OS_STORAGE_ONEDRIVE_DRIVE_ID)
  const oneDriveRootId = readEnvText(env.AGENTIC_OS_STORAGE_ONEDRIVE_ROOT_ID)
...
export const isAgenticGraphStorageRelayRoute = (pathname: string): boolean =>
  pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.relayCapabilities
  || pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.gitRelay
  || pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.fileSyncRelay
...
const handleRelayCapabilities = async (args: {
  ...
  try {
    ...
    return storageRelayJsonResponse(200, {
      ...
      workspaceId,
      devOnly: true,
      relayEnabled: args.env.AGENTIC_OS_STORAGE_DEV_REMOTE_RELAY_ENABLED === 'true',
      gitRemotes,
      fileProviders,
      fileSigningReady: readEnvText(args.env.AGENTIC_OS_STORAGE_SIGNING_SECRET).length >= 16,
    })
  } finally {
    operation.dispose()
  }
...
export const handleStorageRelayRequest = async (args: {
  ...
  try {
    ...
    if (args.pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.relayCapabilities) {
    ...
    if (args.pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.gitRelay) {
    ...
    if (args.pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.fileSyncRelay) {
      const secret = String(args.env.AGENTIC_OS_STORAGE_SIGNING_SECRET || '')
