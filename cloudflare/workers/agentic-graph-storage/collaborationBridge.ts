...
const AGENTIC_OS_COLLABORATION_AWARENESS_STALE_MS = 2 * 60_000
...
const readGitHubBridgeConfig = (
  ...
  return {
    token: normalizeString(env.AGENTIC_OS_STORAGE_GITHUB_TOKEN),
    owner: normalizeString(env.AGENTIC_OS_STORAGE_GITHUB_OWNER),
    repo: authority.repository,
    branch: normalizeString(env.AGENTIC_OS_STORAGE_GITHUB_BRANCH) || 'main',
    committerName: normalizeString(env.AGENTIC_OS_STORAGE_GITHUB_COMMITTER_NAME),
    committerEmail: normalizeString(env.AGENTIC_OS_STORAGE_GITHUB_COMMITTER_EMAIL),
  }
}
...
const readPocketBaseBridgeConfig = (env: AgenticGraphStorageWorkerEnv): {
  ...
  baseUrl: normalizeString(env.AGENTIC_OS_STORAGE_POCKETBASE_URL),
  token: normalizeString(env.AGENTIC_OS_STORAGE_POCKETBASE_TOKEN),
})
...
const commitCollaborationSnapshotToGitHub = async (args: {
  ...
  const headers = {
    ...
    'user-agent': 'agentic-graph-storage-collaboration-bridge',
...
const readPocketBaseCollaborationSnapshot = async (args: {
  ...
  const freshItems = items.filter(item => {
    ...
    return lastSeenAtMs > 0 && currentMs - lastSeenAtMs <= AGENTIC_OS_COLLABORATION_AWARENESS_STALE_MS
  })
  return {
