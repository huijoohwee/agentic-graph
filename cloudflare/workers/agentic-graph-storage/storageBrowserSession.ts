...
} from '../agentic-graph-travel-operator-gateway/access-jwt'
import {
...
const buildBrowserSessionCookie = (token: string, maxAgeSeconds: number): string =>
  `${AGENTIC_OS_STORAGE_BROWSER_SESSION_COOKIE_NAME}=${token}; Path=/; Max-Age=${maxAgeSeconds}; Secure; HttpOnly; SameSite=Strict`

const buildClearedBrowserSessionCookie = (): string =>
  `${AGENTIC_OS_STORAGE_BROWSER_SESSION_COOKIE_NAME}=; Path=/; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT; Secure; HttpOnly; SameSite=Strict`
...
export const handleAgenticGraphStorageBrowserSessionRoute = async (args: {
  ...
  if (pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.browserSession) {
  ...
  if (pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.browserLogout) {
