...
const readDocumentRoute = (pathname: string): { workspaceId: string; canonicalPath: string } | null => {
  if (pathname.startsWith(AGENTIC_OS_STORAGE_ROUTE_PATHS.defaultDocPrefix)) {
    const canonicalPath = decode(pathname.slice(AGENTIC_OS_STORAGE_ROUTE_PATHS.defaultDocPrefix.length))
    return canonicalPath ? { workspaceId: AGENTIC_OS_STORAGE_DEFAULT_WORKSPACE_ID, canonicalPath } : null
  }
  if (!pathname.startsWith(AGENTIC_OS_STORAGE_ROUTE_PATHS.docPrefix)) return null
  const suffix = pathname.slice(AGENTIC_OS_STORAGE_ROUTE_PATHS.docPrefix.length)
