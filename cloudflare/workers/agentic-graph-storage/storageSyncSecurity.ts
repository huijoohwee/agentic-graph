...
export const AGENTIC_OS_STORAGE_SYNC_MAX_REQUEST_BYTES = 16 * 1_024 * 1_024
...
const CORS_HEADERS = {
  ...
  'access-control-allow-headers': 'content-type,authorization,x-agentic-graph-session-token',
...
export const isAgenticGraphStorageLocalRuntime = (env: AgenticGraphStorageWorkerEnv): boolean =>
  String(env.AGENTIC_OS_STORAGE_LOCAL_RUNTIME || '').trim() === 'true'
...
export const readBoundedAgenticGraphStorageSyncJson = async (
  ...
  if (declaredText !== null) {
    ...
    if (declaredLength > AGENTIC_OS_STORAGE_SYNC_MAX_REQUEST_BYTES) {
      await cancelAgenticGraphStorageRequestBody(request.body)
  ...
  try {
    while (true) {
      ...
      if (total > AGENTIC_OS_STORAGE_SYNC_MAX_REQUEST_BYTES) {
