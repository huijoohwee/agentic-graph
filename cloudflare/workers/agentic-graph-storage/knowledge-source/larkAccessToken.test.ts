...
const tenantEnv = (secret = 'tenant-secret'): AgenticGraphStorageWorkerEnv => ({
  DB: null,
  AGENTIC_OS_STORAGE_LARK_IDENTITY_MODE: 'tenant-app',
  AGENTIC_OS_STORAGE_LARK_APP_ID: 'tenant-app-id',
  AGENTIC_OS_STORAGE_LARK_APP_SECRET: secret,
})
...
test('externally managed user OAuth token requires safe expiry metadata before any fetch', async () => {
  ...
  try {
    await assert.rejects(createLarkAccessTokenSource({
      ...
      AGENTIC_OS_STORAGE_LARK_USER_ACCESS_TOKEN: 'externally-managed-token',
      AGENTIC_OS_STORAGE_LARK_USER_ACCESS_TOKEN_EXPIRES_AT_MS: String(5 * 60_000),
