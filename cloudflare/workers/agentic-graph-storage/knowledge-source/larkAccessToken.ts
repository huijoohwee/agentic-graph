...
export const createLarkAccessTokenSource = async (
  ...
  const mode = readKnowledgeSourceText(env.AGENTIC_OS_STORAGE_LARK_IDENTITY_MODE)
  if (isKnowledgeSourcePlaceholder(mode)) {
    throw new KnowledgeSourceError({ code: 'identity_unresolved', status: 503 })
  }
  if (mode === 'tenant-app') {
    const appId = assertSecret(env.AGENTIC_OS_STORAGE_LARK_APP_ID)
    const appSecret = assertSecret(env.AGENTIC_OS_STORAGE_LARK_APP_SECRET)
  ...
  if (mode === 'user-oauth') {
    const accessToken = assertSecret(env.AGENTIC_OS_STORAGE_LARK_USER_ACCESS_TOKEN)
    const expiresAtText = readKnowledgeSourceText(
      env.AGENTIC_OS_STORAGE_LARK_USER_ACCESS_TOKEN_EXPIRES_AT_MS,
    )
