...
export const normalizeAgenticGraphClientDeviceId = (value: unknown): string => {
...
export async function deriveAuthenticatedDevicePrincipalId(args: {
  ...
  const normalizedDeviceId = normalizeAgenticGraphClientDeviceId(args.deviceId)
  ...
  const payload = new TextEncoder().encode([
    'agentic-graph-authenticated-device-principal/v1',
    workspaceId,
    userId,
    normalizedDeviceId,
  ].join('\n'))
