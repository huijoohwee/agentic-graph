...
const CORS_HEADERS = {
  ...
  'access-control-allow-headers': 'content-type,authorization,x-agentic-graph-media-capability,x-agentic-graph-content-hash,x-agentic-graph-content-kind',
...
export const handleSecuredAgenticGraphStorageDataRoute = async (args: {
  ...
  if (isAgenticGraphStorageMediaRoute(args.pathname)) {
    if (String(args.env.AGENTIC_OS_STORAGE_LOCAL_RUNTIME || '').trim() !== 'true') {
