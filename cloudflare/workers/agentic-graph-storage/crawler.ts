...
import {
  ...
  AGENTIC_OS_MARKDOWN_CONTENT_MANIFEST_SCHEMA,
  AGENTIC_OS_MARKDOWN_CONTENT_MANIFEST_SUFFIX,
...
const readCrawlerRoute = (pathname: string): CrawlerRoute | null => {
  if (pathname === AGENTIC_OS_MARKDOWN_CONTENT_MANIFEST_PATH) {
    return { workspaceId: AGENTIC_OS_STORAGE_DEFAULT_WORKSPACE_ID, format: 'manifest' }
  }
  if (pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.sourceFilesLlms) {
    return { workspaceId: AGENTIC_OS_STORAGE_DEFAULT_WORKSPACE_ID, format: 'llms' }
  }
  if (pathname === AGENTIC_OS_STORAGE_ROUTE_PATHS.sourceFilesIndex) {
    return { workspaceId: AGENTIC_OS_STORAGE_DEFAULT_WORKSPACE_ID, format: 'index' }
  }
  if (!pathname.startsWith(AGENTIC_OS_STORAGE_ROUTE_PATHS.sourceFilesIndexPrefix)) return null
  const suffix = pathname.slice(AGENTIC_OS_STORAGE_ROUTE_PATHS.sourceFilesIndexPrefix.length)
  const format = suffix.endsWith(SOURCE_FILES_LLM_SUFFIX)
    ? 'llms'
    : suffix.endsWith(AGENTIC_OS_MARKDOWN_CONTENT_MANIFEST_SUFFIX) ? 'manifest' : 'index'
  const routeSuffix = format === 'llms' ? SOURCE_FILES_LLM_SUFFIX : format === 'manifest' ? AGENTIC_OS_MARKDOWN_CONTENT_MANIFEST_SUFFIX : ''
...
export const buildMarkdownContentManifest = (args: {
  ...
  schema: AGENTIC_OS_MARKDOWN_CONTENT_MANIFEST_SCHEMA,
  ...
  documents: args.documents.filter(isDiscoverableCrawlerDocument).map(document => {
    ...
    const canonicalPath = args.workspaceId === AGENTIC_OS_STORAGE_DEFAULT_WORKSPACE_ID
      ? `/agentic-graph/doc-default/${encodedPath}`
      : `/agentic-graph/doc/${encodeURIComponent(args.workspaceId)}/${encodedPath}`
    return {
      id: document.id,
...
const buildSourceFilesLlmsText = (args: {
  ...
  const lines = [
    '# agentic-graph Source Files',
    '',
    '> Markdown Source Files from the agentic-graph Editor Workspace storage boundary.',
    '',
