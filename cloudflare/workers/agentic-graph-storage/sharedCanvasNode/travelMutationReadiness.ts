...
export const probeTravelMutationTriggerReadiness = async (
  ...
  if (!local.ok || !env.AGENTIC_OS_TRAVEL_COMMERCE || local.dispatchTimeoutMs == null) {
  ...
  try {
    ...
    const fetchReady = async (path: string, authorization?: string): Promise<Response> => {
      ...
      return env.AGENTIC_OS_TRAVEL_COMMERCE!.fetch(new Request(
        `https://agentic-travel-commerce.internal${path}`,
    ...
    const token = env.AGENTIC_OS_TRAVEL_COMMERCE_API_TOKEN!.trim()
