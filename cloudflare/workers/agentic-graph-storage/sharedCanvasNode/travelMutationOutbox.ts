...
export const TRAVEL_MUTATION_OUTBOX_SCHEMA = 'agentic-graph-shared-canvas-travel-mutation-outbox/v1'
...
export class TravelMutationOutbox {
  ...
  private async dispatch(
    ...
    const binding = this.env.AGENTIC_OS_TRAVEL_COMMERCE
    const token = typeof this.env.AGENTIC_OS_TRAVEL_COMMERCE_API_TOKEN === 'string'
      ? this.env.AGENTIC_OS_TRAVEL_COMMERCE_API_TOKEN.trim()
      : ''
    ...
    const fetchTravel = async (path: string, method: 'PUT' | 'POST', body: unknown): Promise<Response> => {
      ...
      return binding.fetch(new Request(`https://agentic-travel-commerce.internal${path}`, {
        method,
        headers: {
