export const TRAVEL_BUNDLE_MAP_SCHEMA = 'agentic-graph-shared-canvas-travel-bundle-map/v1'
...
export type TravelMutationTriggerEnv = {
  AGENTIC_OS_TRAVEL_COMMERCE?: TravelCommerceServiceBinding
  AGENTIC_OS_TRAVEL_COMMERCE_API_TOKEN?: string
...
export const inspectTravelMutationTriggerReadiness = (
  ...
  const serviceBinding = hasTravelServiceBinding(env.AGENTIC_OS_TRAVEL_COMMERCE) ? 'ready' : 'missing'
  const token = typeof env.AGENTIC_OS_TRAVEL_COMMERCE_API_TOKEN === 'string'
    ? env.AGENTIC_OS_TRAVEL_COMMERCE_API_TOKEN.trim()
    : ''
