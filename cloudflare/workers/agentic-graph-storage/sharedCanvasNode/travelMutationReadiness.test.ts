...
const AGENTIC_OS_CANVAS_ROOM = { idFromName: () => 'room-id', get: () => ({ fetch: async () => new Response() }) }
...
const triggerEnv = (fetch: (request: Request) => Promise<Response>) => ({
  ...
  AGENTIC_OS_TRAVEL_COMMERCE: { fetch },
})
...
test('storage readiness fails closed when the travel mutation trigger is not operator-configured', async () => {
  const response = await createAgenticGraphStorageWorker().fetch(
    ...
    { DB, AGENTIC_OS_CANVAS_ROOM } as unknown as AgenticGraphStorageWorkerEnv,
  )
...
test('storage readiness performs a bounded downstream readiness probe and requires body-level ok', async () => {
  ...
  const base = {
    ...
    AGENTIC_OS_STORAGE_SIGNING_SECRET: signingSecret,
    AGENTIC_OS_STORAGE_ACCESS_ISSUER: 'https://storage.cloudflareaccess.com',
    AGENTIC_OS_STORAGE_ACCESS_AUDIENCE: 'abcdefghijklmnop',
    AGENTIC_OS_TRAVEL_COMMERCE_API_TOKEN: token,
    SHARED_NODE_TRAVEL_BUNDLE_MAP_JSON: map,
    SHARED_NODE_TRAVEL_DISPATCH_TIMEOUT_MS: '4000',
  }
  ...
  assert.deepEqual(calls.map(request => [request.method, request.url]), [[
    'GET', 'https://agentic-travel-commerce.internal/readyz',
  ], [
    'GET', 'https://agentic-travel-commerce.internal/v1/runtime',
  ]])
  ...
  const missingSigningSecret = await createAgenticGraphStorageWorker().fetch(
    new Request('https://storage.internal/readyz'),
    { ...base, AGENTIC_OS_STORAGE_SIGNING_SECRET: undefined } as unknown as AgenticGraphStorageWorkerEnv,
  )
  ...
  const falsePositive = await createAgenticGraphStorageWorker().fetch(
    ...
    {
      ...base,
      AGENTIC_OS_TRAVEL_COMMERCE: { fetch: async () => Response.json({ ok: false }, { status: 200 }) },
    } as unknown as AgenticGraphStorageWorkerEnv,
  )
