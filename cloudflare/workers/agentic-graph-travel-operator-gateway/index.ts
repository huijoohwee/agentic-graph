...
export const OPERATOR_GATEWAY_BASE_PATH = '/agentic-os/control-plane/travel/reconciliation'
const CONTROL_CONTRACT = 'agentic-graph.travel-reconciliation-control/v1'
...
const readiness = async (
  ...
  if (!config.ok) {
    return json(503, {
      ok: false,
      service: 'agentic-travel-operator-gateway',
...
const forwardDecision = async (
  ...
  try {
    const response = await config.service.fetch(new Request(
      `https://agentic-travel-commerce.internal/v1/bundles/${encodeURIComponent(identifiers.bundleId)}`
    ...
    async fetch(request: Request, env: OperatorGatewayEnv): Promise<Response> {
      ...
      if (pathname === `${OPERATOR_GATEWAY_BASE_PATH}/livez`) {
        ...
        return request.method === 'HEAD'
          ...
          : json(200, { ok: true, service: 'agentic-travel-operator-gateway', status: 'live' })
      }
