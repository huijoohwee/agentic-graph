...
const handleCreateSession = async (
  ...
  if (stripeCheckoutPayload) {
    await writeTraceEvent(db, {
      ...
      eventType: 'agentic-graph.commerce.stripe_checkout_session',
      payload: {
        tool: 'agentic-graph.commerce.stripe_checkout_session',
  ...
  if (parsed.session.paymentRail === AGENTIC_COMMERCE_SOLANA_PAY_KEY) {
    await writeTraceEvent(db, {
      sessionId: parsed.session.id,
      eventType: 'agentic-graph.commerce.solana_pay_request',
      payload: {
        tool: 'agentic-graph.commerce.solana_pay_request',
        session_id: parsed.session.id,
...
const handleOpenboxIngest = async (
  ...
  if (sessionId && await readSession(db, sessionId)) {
    await writeTraceEvent(db, {
      sessionId,
      eventType: 'agentic-graph.commerce.openbox_ingest',
      payload: {
        tool: 'agentic-graph.commerce.openbox_ingest',
...
const handleCompleteSession = async (
  ...
  if (session.payment_rail === 'fiat' && session.status !== 'complete') {
    ...
    await writeTraceEvent(db, {
      sessionId,
      eventType: authorization.ok ? 'agentic-graph.commerce.delegate_payment' : 'agentic-graph.commerce.payment_failed',
      payload: {
        tool: 'agentic-graph.commerce.delegate_payment',
        session_id: sessionId,
        ok: authorization.ok,
