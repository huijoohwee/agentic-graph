...
test('Stripe create uses one stable key, explicit API version, and Request-Id correlation', async () => {
  ...
  const adapter = createStripePaymentRailAdapter({
    ...
    async fetch(input, init) {
      ...
      return new Response(JSON.stringify({
        ...
        amount_total: 1200,
        currency: 'sgd',
        client_reference_id: `agentic-graph:stripe:create:${record.clientIntentKey}`,
      }), {
        status: 200,
  ...
  assert.equal(
    ...
    `agentic-graph:stripe:create:${record.clientIntentKey}`,
  )
...
test('StraitsX uses the exact dynamic PayNow JSON:API contract and minimizes instructions', async () => {
      ...
      return new Response(JSON.stringify({
        data: {
          ...
          attributes: {
            ...
            currency: 'SGD',
            amount: '12.01',
            referenceId: `agentic-graph:straitsx:create:${record.clientIntentKey}`,
            senderInformation: {
  ...
  assert.equal(
    ...
    `agentic-graph:straitsx:create:${record.clientIntentKey}`,
  )
