    ...
    async read(record) {
      return {
        ...
        clientIntentReference:
          `agentic-graph:${record.rail}:create:${record.clientIntentKey}`,
...
test('concurrent provider reads cannot regress an intent after paid wins the revision CAS', async () => {
  ...
  pendingReads[0].resolve({
    ...
    clientIntentReference:
      `agentic-graph:${paidRecord.rail}:create:${paidRecord.clientIntentKey}`,
  ...
  pendingReads[1].resolve({
    ...
    clientIntentReference:
      `agentic-graph:${staleRecord.rail}:create:${staleRecord.clientIntentKey}`,
