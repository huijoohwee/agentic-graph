...
const storeReadiness = Object.freeze({
  ok: true,
  storage: 'sqlite',
  contract: 'agentic-graph.net-settlement/v1',
})

const executorReadiness = Object.freeze({
  ok: true,
  contract: 'agentic-graph.net-settlement-effect/v1',
  providerBacked: true,
...
describe('Issuance Service net-settlement provider contract', () => {
  ...
  it('rejects malformed amounts, caller spoofing, and mismatched keys before storage', async () => {
    ...
    expect((await settle(payload, { 'x-agentic-graph-component': 'Reopt_Worker' })).status).toBe(403)
