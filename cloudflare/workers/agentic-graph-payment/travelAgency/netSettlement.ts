...
export class NetSettlementStore {
  ...
  private ready(): Readonly<{ ok: true; storage: 'sqlite'; contract: 'agentic-graph.net-settlement/v1' }> {
    this.ctx.storage.sql.exec('SELECT idempotency_key FROM net_settlement_effects LIMIT 1').toArray()
    return Object.freeze({ ok: true, storage: 'sqlite', contract: 'agentic-graph.net-settlement/v1' })
  }
    ...
    try {
      response = await this.executor.fetch(new Request('https://net-settlement-executor.internal/v1/net-settlements', {
        ...
        headers: {
          ...
          'x-agentic-graph-component': COMPONENT,
        },
...
const readiness = async (
  ...
  try {
    ...
    if (storeResult.storage !== 'sqlite' || storeResult.contract !== 'agentic-graph.net-settlement/v1') {
      throw new Error('net settlement store readiness response is malformed')
    }
    if (
      executorResult.contract !== 'agentic-graph.net-settlement-effect/v1'
...
export const handleNetSettlementRoute = async (
  ...
  if (pathname === PAYMENT_LIVE_PATH) {
    ...
    return request.method === 'HEAD'
      ...
      : json(200, { ok: true, service: 'agentic-payment', status: 'live' }, headers)
  }
  ...
  if (request.headers.get('x-agentic-graph-component') !== COMPONENT) {
