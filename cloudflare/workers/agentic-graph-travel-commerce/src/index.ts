  ...
  async fetch(request: Request, env: TravelCommerceEnv, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (request.method === 'GET' && segments[3] === 'events' && segments.length === 4) {
        ...
        const headers = new Headers({ 'sec-websocket-protocol': 'agentic-graph.v1' })
...
async function authorized(request: Request, secret: string): Promise<boolean> {
  ...
  const protocol = request.headers.get('sec-websocket-protocol')
    ...
    .find((value) => value.startsWith('agentic-graph.auth.'))
  const protocolSecret = protocol ? decodeBase64Url(protocol.slice('agentic-graph.auth.'.length)) : null
