# agenticgraph-mcp Worker

Cloudflare Worker that hosts the Agents SDK `McpAgent` for the
**agenticgraph ↔ agentic-canvas-os MCP connector** control plane. Exposes the
Director tool (`agenticgraph.video_remix.run`) plus the five stage tools
(`research`, `storyboard`, `render`, `publish`, `checkout`), the read-only
OS status tool (`agenticgraph.os.status`), the receipt-fenced run-note mutation
(`agenticgraph.run_manifest.note.update`), and the read-only Agentic Canvas OS docs
invocation tool (`agenticgraph.agentic_canvas_os.docs.invoke`) over the
**MCP Streamable HTTP** transport at `https://airvio.co/agenticgraph/control-plane/mcp`.

Migrates the deployed plain-JSON-RPC HTTP MCP at
`cloudflare/pages/agenticgraph-agent-ready.mjs` per **ADR-7** and
spec task 1.1 of `.kiro/specs/agenticgraph-acos-mcp-connector` (Requirement
...
| File | Responsibility |
|---|---|
| `index.ts` | Worker entry. Defines `AgenticGraphMcpAgent` (Agents SDK `McpAgent`), registers 10 tools with Streamable HTTP, routes `/agenticgraph/control-plane/mcp[*]`, and forwards request metadata and the idempotency header to the durable dispatcher. |
...
| `wrangler.toml` | Worker bindings: route `airvio.co/agenticgraph/control-plane/mcp[*]`, Durable Object classes `AgenticGraphMcpAgent` (transport sessions) and `RunManifestStore` (Run_Manifest persistence). |
...
|---|---|---|
| `agenticgraph.video_remix.run` | n/a (Director enforces gates per stage) | input + output (Run_Manifest) |
| `agenticgraph.video_remix.research` | `paid-model-call` | input + output (Evidence_Pack) |
| `agenticgraph.video_remix.storyboard` | `paid-model-call` | input + output (Kgc_Document + flow) |
| `agenticgraph.video_remix.render` | `render-action` | input + output (assets + ledgerEventIds) |
| `agenticgraph.video_remix.publish` | `cloud-deploy` | input + output (publishedUrls) |
| `agenticgraph.video_remix.checkout` | `payment-action` | input + output (Stripe sessionId, payoutSettled) |
| `agenticgraph.run_manifest.note.update` | Agentic signed review plus native receipt metadata | input + output (operator note, revision, execution receipt) |
| `agenticgraph.os.status` | n/a (read-only) | input + output (Agentic OS status views) |
| `agenticgraph.agentic_canvas_os.docs.invoke` | n/a (read-only) | input + output (`/`, `#`, `@` docs invocation lookup) |
...
`agenticgraph.run_manifest.note.update` accepts only `{ run_id, note }`. The model
...
separate `agenticgraph-mcp-dev` Worker also returned one accepted provider-driven
...
`agenticgraph-mcp-dev.huijoohwee.workers.dev`. It repeats the non-inheritable
...
`wrangler secret put AGENTICGRAPH_AGENT_RUNTIME_BEARER_TOKEN --env dev`, then run
...
`agenticgraph-tool-execution-receipt/v1`, status `applied`. This read-back is a
completed historical proof, not permission to repeat the run.

## Durable Run_Manifest persistence (Property 25 / R14.2)

When the Director tool (`agenticgraph.video_remix.run`) returns a Run_Manifest,
...
```json
{
  "persistence": {
    "persisted": true,
    "persistedAt": "2024-...Z",
    "readBackEndpoint": "/agenticgraph/control-plane/mcp/runs/<runId>",
    "deadlineMs": 2000,
    "error": null
  }
}
...
```bash
curl -s https://airvio.co/agenticgraph/control-plane/mcp/runs/<runId>
```
...
The Director tool (`agenticgraph.video_remix.run`) reuses the existing gate
...
```bash
SESSION_ID="$(curl -si -X POST http://localhost:8787/agenticgraph/control-plane/mcp \
...
curl -sX POST http://localhost:8787/agenticgraph/control-plane/mcp \
...
The route `airvio.co/agenticgraph/control-plane/mcp[*]` routes to this Worker
without shadowing the published Pages agent-ready endpoint at
`/agenticgraph/mcp`. Keep the Pages site on the public discovery/read-only MCP
...
- Task 1.2 (this task) added the `RunManifestStore` Durable Object and the
  `GET /agenticgraph/control-plane/mcp/runs/{id}` read-back route.
...
- Task 1.4 will refine the tool listing endpoint to mirror the
  `tools/list` payload at `/agenticgraph/control-plane/mcp/tools` for non-MCP discovery
