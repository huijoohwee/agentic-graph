...
// (agentic-graph-acos-mcp-connector spec, task 1.6 — R14.6 / Property 1, boundary
// slice).
//
...
const STAGE_TOOL_NAMES = Object.values(AGENTIC_OS_MCP_STAGE_TOOL_NAMES);

for (const stageName of STAGE_TOOL_NAMES) {
  const gateId = AGENTIC_OS_MCP_STAGE_GATES[stageName];
...
test("R14.6: an approved stage call still does not write RUN_MANIFEST_STORE at the boundary", async () => {
  ...
  const { namespace, calls } = createSpyNamespace();
  const result = await dispatchAgenticGraphMcpToolCall({
    toolName: AGENTIC_OS_MCP_STAGE_TOOL_NAMES.research,
    args: {
...
test("R14.6 contrast: a Director run DOES write RUN_MANIFEST_STORE (gated persistence is Director-only)", async () => {
  ...
  const result = await dispatchAgenticGraphMcpToolCall({
    toolName: AGENTIC_OS_MCP_DIRECTOR_TOOL_NAME,
    args: {
