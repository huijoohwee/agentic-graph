...
// (agentic-graph-acos-mcp-connector spec, task 1.2).
//
...
test("buildPersistenceRecord encodes runId, persistedAt, and contractVersion", () => {
  ...
  assert.equal(record.contractVersion, "agentic-graph.video_remix/v0.1");
...
test("RunManifestPersistence put then get returns the latest persisted state", async () => {
  ...
  assert.equal(readBack.contractVersion, "agentic-graph.video_remix/v0.1");
...
test("RunManifestPersistence stores under documented keys", async () => {
  ...
  assert.equal(
    ...
    "agentic-graph.video_remix/v0.1",
  );
