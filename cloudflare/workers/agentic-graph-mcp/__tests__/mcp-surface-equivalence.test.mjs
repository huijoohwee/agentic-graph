...
// agentic-graph-widget-canvas-media spec · Task 14
...
test("R9.1: MCP Director surface produces same state as direct runVideoRemix", () => {
  ...
  // MCP surface path
  const mcpResult = executeAgenticGraphMcpTool(AGENTIC_OS_MCP_DIRECTOR_TOOL_NAME, EQUIVALENCE_INPUT);
...
test("R9.2: dry-run via MCP surface and runtime are equivalent", () => {
  ...
  const mcpResult    = executeAgenticGraphMcpTool(AGENTIC_OS_MCP_DIRECTOR_TOOL_NAME, dryRunInput);
...
test("MCP stage tool without approval returns approval_required (R14.6)", () => {
  const result = executeAgenticGraphMcpTool(AGENTIC_OS_MCP_STAGE_TOOL_NAMES.render, {
    shots: [{ shotId: "s1" }],
...
test("MCP stage tool with matching gate approved returns ok (boundary passes to Director)", () => {
  const result = executeAgenticGraphMcpTool(AGENTIC_OS_MCP_STAGE_TOOL_NAMES.research, {
    referenceUrl: "https://example.com/ref.mp4",
    approvals: [
...
test("MCP surface and runtime produce same failure records for blocked runs", () => {
  ...
  const mcpResult    = executeAgenticGraphMcpTool(AGENTIC_OS_MCP_DIRECTOR_TOOL_NAME, blockedInput);
...
test("R9.1: Director output schema includes render field for artifact references", () => {
  ...
  const director = definitions.find(d => d.name === AGENTIC_OS_MCP_DIRECTOR_TOOL_NAME);
