...
import {
  ...
  AGENTIC_OS_MCP_DIRECTOR_TOOL_NAME,
...
export interface AgenticGraphMcpEnv extends Env {
  AGENTIC_OS_AGENT_MODEL_ID?: string;
  // task 12.5 env gating: live stage clients are enabled when AGENTIC_OS_LIVE_CLIENTS
  ...
  AGENTIC_OS_PAYMENT_URL?: string;
  AGENTIC_OS_PAYMENT_API_KEY?: string;
  AGENTIC_OS_MEDIA_BUCKET?: string;
  AGENTIC_OS_MEDIA_R2?: R2Bucket;
...
//
// task 1.4: the tool surface must list `agentic-graph.video_remix.run` plus each
// stage tool with BOTH an input schema and an output schema. The HTTP
// `GET /agentic-os/control-plane/mcp/tools` listing already returns both (it serializes the
...
async function dispatchToolCall(
  ...
  const liveClients = resolveStageClients({
    AGENTIC_OS_LIVE_CLIENTS: env?.AGENTIC_OS_LIVE_CLIENTS,
    ...
    AGENTIC_OS_PAYMENT_URL: env?.AGENTIC_OS_PAYMENT_URL,
    AGENTIC_OS_PAYMENT_API_KEY: env?.AGENTIC_OS_PAYMENT_API_KEY,
    AGENTIC_OS_MEDIA_BUCKET: env?.AGENTIC_OS_MEDIA_BUCKET,
  }, { r2Client: env?.AGENTIC_OS_MEDIA_R2 });
...
export class AgenticGraphMcpAgent extends McpAgent<AgenticGraphMcpEnv> {
  server = new McpServer({
    name: "agentic-graph-control-plane",
    version: "0.1.0",
  });
  ...
  async init(): Promise<void> {
    ...
    // tool as the HTTP `GET /agentic-os/control-plane/mcp/tools` listing (R14.4 / Property 26).
    ...
    register("agentic-graph.video_remix.research", RESEARCH_INPUT, RESEARCH_OUTPUT);
    register(
      "agentic-graph.video_remix.storyboard",
      STORYBOARD_INPUT,
      STORYBOARD_OUTPUT,
    );
    register("agentic-graph.video_remix.render", RENDER_INPUT, RENDER_OUTPUT);
    register("agentic-graph.video_remix.publish", PUBLISH_INPUT, PUBLISH_OUTPUT);
    register("agentic-graph.video_remix.checkout", CHECKOUT_INPUT, CHECKOUT_OUTPUT);
    register(RUN_NOTE_TOOL_NAME, RUN_NOTE_INPUT, RUN_NOTE_OUTPUT);
    register(AGENTIC_OS_OS_STATUS_TOOL_NAME, OS_STATUS_INPUT, OS_STATUS_OUTPUT);
...
function buildHealthBody(env: AgenticGraphMcpEnv): Record<string, unknown> {
  return {
    status: "pass",
    service: "agentic-mcp-worker",
    contractVersion: AGENTIC_OS_MCP_CONTRACT_VERSION,
    transport: "mcp/streamable-http",
    endpoint: `${(env.AGENTIC_OS_MCP_PUBLIC_BASE_URL ?? "https://airvio.co").replace(/\/+$/, "")}${MCP_PATH}`,
  ...
  async fetch(
    ...
    if (request.method === "GET" && pathname === AGENTS_PATH) {
      return runtimeJsonResponse({
        contractVersion: "agentic-graph.agent-definition-registry/v1",
