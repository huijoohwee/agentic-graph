// =============================================================================
// Property-based tests — agentic-graph control-plane McpAgent worker tier
...
import {
  ...
  AGENTIC_OS_MCP_STAGE_TOOL_NAMES,
...
// -----------------------------------------------------------------------------
// Feature: agentic-graph-acos-mcp-connector, Property 25: For any Director run state change, after the Mcp_Agent persists the updated Run_Manifest a subsequent GET /runs/{id} for that run returns the latest persisted state.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Feature: agentic-graph-acos-mcp-connector, Property 26: For any remote client request for the tool surface, the returned list includes agentic-graph.video_remix.run and each stage tool, and every listed tool includes both its input schema and its output schema.
// -----------------------------------------------------------------------------
test("Property 26: tool listing exposes input and output schemas", () => {
  const expectedNames = [AGENTIC_OS_MCP_DIRECTOR_TOOL_NAME, ...Object.values(AGENTIC_OS_MCP_STAGE_TOOL_NAMES)];
...
// -----------------------------------------------------------------------------
// Feature: agentic-graph-acos-mcp-connector, Property 27: For any stage transition during a Director run, the Mcp_Agent emits an observability diagnostic containing the run identifier, the originating stage id, the destination stage id, a UTC timestamp, and the transition outcome status.
// -----------------------------------------------------------------------------
