...
export const AGENTS_PATH = "/agentic-os/control-plane/agents";
...
async function timingSafeTokenMatch(
  ...
  const message = encoder.encode("agentic-graph-agent-runtime");
...
export async function authorizeRuntimeRequest(
  ...
  const expected = String(
    env.AGENTIC_OS_AGENT_RUNTIME_BEARER_TOKEN || "",
  ).trim();
  if (!expected) {
    return {
      ok: false,
