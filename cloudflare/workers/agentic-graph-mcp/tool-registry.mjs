//   * the tool list exposed at airvio.co/agentic-os/control-plane/mcp,
...
//
// Director semantics for `agentic-graph.video_remix.run` are reused from
...
export const AGENTIC_OS_MCP_STAGE_TOOL_NAMES = Object.freeze({
  research: "agentic-graph.video_remix.research",
  storyboard: "agentic-graph.video_remix.storyboard",
  render: "agentic-graph.video_remix.render",
  publish: "agentic-graph.video_remix.publish",
  checkout: "agentic-graph.video_remix.checkout",
});
...
export const AGENTIC_OS_MCP_STAGE_GATES = Object.freeze({
  [AGENTIC_OS_MCP_STAGE_TOOL_NAMES.research]: "paid-model-call",
  [AGENTIC_OS_MCP_STAGE_TOOL_NAMES.storyboard]: "paid-model-call",
  // render-action: runtime gate id; reconcile vs design glossary in task 4.1.
  [AGENTIC_OS_MCP_STAGE_TOOL_NAMES.render]: "render-action",
  [AGENTIC_OS_MCP_STAGE_TOOL_NAMES.publish]: "cloud-deploy",
  [AGENTIC_OS_MCP_STAGE_TOOL_NAMES.checkout]: "payment-action",
});
...
const STAGE_TOOL_DEFINITIONS = Object.freeze([
  {
    name: AGENTIC_OS_MCP_STAGE_TOOL_NAMES.research,
    title: "agentic-graph Video Remix - Research Stage",
    description:
  ...
  {
    name: AGENTIC_OS_MCP_STAGE_TOOL_NAMES.render,
    title: "agentic-graph Video Remix - Render Stage",
    description:
  ...
  {
    name: AGENTIC_OS_MCP_STAGE_TOOL_NAMES.publish,
    title: "agentic-graph Video Remix - Publish Stage",
    description:
  ...
  {
    name: AGENTIC_OS_MCP_STAGE_TOOL_NAMES.checkout,
    title: "agentic-graph Video Remix - Checkout Stage",
    description:
...
const DIRECTOR_TOOL_DEFINITION = Object.freeze({
  name: AGENTIC_OS_MCP_DIRECTOR_TOOL_NAME,
  title: "agentic-graph Video Remix - Director Run",
  description:
 ...
 * airvio.co/agentic-os/control-plane/mcp. Property 26 / R14.4: every entry carries both
...
export function gateIdForTool(toolName) {
  return AGENTIC_OS_MCP_STAGE_GATES[toolName] ?? null;
}
...
export function executeAgenticGraphMcpTool(toolName, rawArgs = {}) {
  ...
  if (toolName === AGENTIC_OS_MCP_DIRECTOR_TOOL_NAME) {
  ...
  if (toolName === AGENTIC_OS_OS_STATUS_TOOL_NAME) return executeCloudflareOsStatusTool(args, { toolDefinitions: buildAgenticGraphMcpToolDefinitions() });

  const gateId = AGENTIC_OS_MCP_STAGE_GATES[toolName];
  if (!gateId) {
    return {
      ok: false,
      structuredContent: {
  ...
  const envelope = {
    ...
    runManifestStateChanged: false,
    note:
      "Approval_Token accepted at the McpAgent boundary. Invoke `agentic-graph.video_remix.run` to execute the Director-owned full pipeline.",
  };
  return {
    ok: true,
    structuredContent: envelope,
