    ...
    *,
    ...
    lines = [
        ...
        *[f"  - {yaml_inline_string(level)}" for level in SUPERAGENT_TASK_LEVELS],
        "kgSuperAgentMessageGateway: \"CLI or agentic-graph.superagent.run local MCP\"",
        ...
        *flow_visual_lines("rich-media-panel"),
        ...
        f"# {title}",
        "",
        "This file is a generated agentic-graph frontmatter-flow workspace for the rich media pipeline.",
        "",
        f"- Run id: `{run_id}`",
        ...
        "- Message gateway: `CLI` or `agentic-graph.superagent.run` local MCP",
...
def render_final_report(*, state: JsonDict, verification: JsonDict, output_dir: str) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "```bash",
            "python3 -m agentic_graph_parser superagent --input <brief.md> --output-dir <run-output-dir>",
            "```",
            "",
        ]
    )
