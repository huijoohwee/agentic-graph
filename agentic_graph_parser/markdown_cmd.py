...
from .doc_markdown import build_agentic_graph_doc_markdown
...
def main(argv: Optional[Sequence[str]] = None, *, parser_script_path: Optional[str] = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Base IRI for agentic-graph relationship terms (e.g. hasSection, next)",
    )
    parser.add_argument(
    ...
    parser_entrypoint = (
        ...
        else f"{sys.executable} -m agentic_graph_parser"
    )
    ...
    doc_text = build_agentic_graph_doc_markdown(
        title=title,
        graph_id=gid,
        markdown_path=input_path,
        graph_jsonld_path=graph_out_path,
