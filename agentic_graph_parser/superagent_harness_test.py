...
class SuperAgentHarnessTests(unittest.TestCase):
    ...
    def test_external_xr_validation_document_is_runtime_input_only(self) -> None:
        ...
        external_name = "-".join(["agentic-graph", "xr", "demo"]) + ".md"
    ...
    def test_neutral_e2e_run_produces_trace_canvas_and_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(proof["schema_version"], "agentic-graph.superagent.proof.v1")
            self.assertEqual(proof["harness_contract"]["codex_integration"]["mcp_tool"], "agentic-graph.superagent.run")
    ...
    def test_generated_state_is_not_fixture_story_locked(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotIn("-".join(["agentic-graph", "video", "demo"]) + ".md", serialized)
            self.assertNotIn("-".join(["agentic-graph", "xr", "demo"]) + ".md", serialized)
