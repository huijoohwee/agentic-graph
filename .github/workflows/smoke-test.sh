...
agent_ready_base_url="${AGENTIC_OS_AGENT_READY_BASE_URL:-$configured_public_origin}"
...
for attempt in 1 2 3 4 5; do
  if AGENTIC_OS_AGENT_READY_BASE_URL="$agent_ready_base_url" \
    AGENTIC_OS_AGENT_READY_INCLUDE_X402=false \
  ...
  echo "[agentic-graph] release smoke attempt $attempt failed; retrying after Pages propagation"
  sleep 15
done
