---
title: "agentic-graph Client-Side Parsers: Universal Format Specification"
doc_type: "Document"
date: "2026-05-27"
lang: "en-US"
...
# agentic-graph Client-Side Parsers: Universal Format Specification
...
- Runtime-ready Canvas documents declare `kgParserRoutingContract.version: "agentic-graph-parser-routing/v1"` in opening frontmatter when parser output must route deterministically across Storyboard and Mermaid surfaces.
...
Implementation note: `agentic_graph_parser youtube` uses a robust native implementation to fetch transcripts via the YouTube `timedtext` API (parsing XML/JSON) or InnerTube API (Android client emulation), with fallbacks to `yt-dlp` and `whisper`. It avoids external dependency breakage (`youtube-transcript-api`) and handles session cookies/signatures natively.
...
Pipeline map:
- Import `docs/documents/agentic-graph-pipeline-map.graph.json` into Canvas (as JSON) to visualize the runtime + Python pipeline call graph.
...
|--------------------------------------|----------------------------------------------------------|-------------------------|----------------------|
| `agentic-graph-parser-document.md`        | Parser registry, format specs, line mapping              | docs:update, doc:lint, tests | Component Documenter |
| `agentic-graph-semantic-document.md`      | Semantic extraction, entity/edge processing              | docs:update, doc:lint, tests | Component Documenter |
| `agentic-graph-mermaid-frontmatter-document.md` | Mermaid parsing, frontmatter handling          | docs:update, doc:lint, tests | Component Documenter |
