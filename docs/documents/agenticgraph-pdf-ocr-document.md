...
- `AGENTICGRAPH_PDF_MODE=online`: prefer online enrichment providers (if configured)
- `AGENTICGRAPH_PDF_PROVIDER=native|docling-remote`: explicit provider selection
- `AGENTICGRAPH_DOCLING_ENDPOINT`: HTTP endpoint for Docling-based PDF→Markdown conversion
- `AGENTICGRAPH_PDF_PROVIDER_FALLBACK_TO_NATIVE=1`: if Docling conversion fails, fall back to native conversion

### OCR (server-side)

- `AGENTICGRAPH_PDF_OCR_ENABLE=1`: enable OCR enhancement during conversion
- `AGENTICGRAPH_PDF_OCR_ENDPOINT`: HTTP endpoint that accepts `{ filename, prompt, imageBase64 }` and returns `{ ok, markdown }`
- `AGENTICGRAPH_PDF_OCR_MODE=fallback|always`: apply OCR only when extracted text is sparse vs always
- `AGENTICGRAPH_PDF_OCR_MIN_TEXT_CHARS`: sparse-text threshold used by `fallback`
- `AGENTICGRAPH_PDF_OCR_MAX_IMAGES_PER_PAGE`: max page images sent to OCR
- `AGENTICGRAPH_PDF_OCR_TIMEOUT_MS`: request timeout
- `AGENTICGRAPH_PDF_OCR_PROMPT`: optional prompt forwarded to the OCR endpoint
