...
`knowgrph-storage` boundary; Git-backed Markdown/frontmatter remains the
...
| `cloudflare/workers/knowgrph-storage/knowledge-source/` | Owns server identity, allowlist admission, exact Lark reads, complete snapshot digests, opaque handoff, and typed failures. | Read-only; no source acceptance, write-back, or deploy authority. |
...
Knowgrph for review without leaking credentials/configured provider identifiers, accepting
...
| `PRD-LARK-08` | Own provider reads only in `knowgrph-storage`; keep Git-backed Markdown/frontmatter authoritative. | Must |
| `PRD-LARK-09` | Resolve identity/resources only from `KNOWGRPH_STORAGE_LARK_*` and `knowgrph-knowledge-source-allowlist/v1`. | Must |
...
| `PRD-LARK-12` | Emit only complete `knowgrph-knowledge-source-snapshot/v1` envelopes and require Canvas to bounded-stream, validate, and recompute content/envelope digests before mutation. | Must |
...
| `PRD-LARK-08` | Given a provider read, when ownership is traced, then only `knowgrph-storage` contacts Lark and the result remains a candidate projection. | `VCC-LARK-08` |
...
| Managed 12-month incremental Knowgrph TCO | USD 0 for browser-local source path; Lark/Worker cost unmeasured | Measure provider/Worker request and egress cost before delivery. |
...
The public API version is `knowgrph-knowledge-source/v1` and the only Knowgrph
routes are:

- `POST /api/storage/knowledge-source/handoff` to authenticate the Knowgrph
...
The `knowgrph-knowledge-source-allowlist/v1` resource variants and exact reads
are:
...
`KNOWGRPH_STORAGE_SIGNING_SECRET` seals each five-minute AES-GCM AEAD handoff. In
...
Success uses `knowgrph-knowledge-source-snapshot/v1` with `complete: true`,
...
1. The Knowgrph target is selected from the canonical MCP installation
   contract.
...
5. `knowgrph-storage` revalidates the binding, performs exact bounded
...
| Acquire | Lark provider behind `knowgrph-storage` | Allowlisted Base/Wiki/Doc plus Base fields/minimum | Stable-revision/total bounded pages/content | Request memory only | Entire read fails on incomplete, hidden/missing field, low count, or drift |
...
| Discover | Resolve config/opaque handoff | Ingest | Auth/allowlist guard | `knowgrph-storage` knowledge-source runtime |
...
Keep provider reads server-only in provider-neutral `knowgrph-storage`; keep
...
| `VCC-LARK-08` | From repository root, standalone: `TSX_TSCONFIG_PATH=canvas/tsconfig.json node --import tsx --test --test-concurrency=1 cloudflare/workers/knowgrph-storage/knowledge-source/*.test.ts` | Provider-neutral handoff/read ownership passes focused fixtures and no provider result becomes accepted source. | Mocked transport; not yet wired into `storage:relay:test`; no live/delivery claim. | None recorded |
| `VCC-LARK-09` | Run the focused knowledge-source suite in `VCC-LARK-08`. | Only server env plus `knowgrph-knowledge-source-allowlist/v1` select identity/resources; caller overrides fail. | No secrets in fixtures/output. | None recorded |
