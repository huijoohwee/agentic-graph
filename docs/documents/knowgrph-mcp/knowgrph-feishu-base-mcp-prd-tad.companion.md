...
| Server public contract | `canvas/src/lib/storage/knowgrphStorageSyncContract.ts` and `knowgrphStorageRoutePaths.ts` | `knowgrph-knowledge-source/v1`, complete snapshot types, and the handoff/read paths. | Lark-specific source authority |
| Server runtime | `cloudflare/workers/knowgrph-storage/knowledge-source/` | Server identity, versioned allowlist, Lark read provider, provenance/digests, opaque handoff, and typed block/failure results. | Write-back or source acceptance |
...
| Allowlist | `knowgrph-knowledge-source-allowlist/v1` with `{ schema, revision, sources[] }` |
...
| Snapshot | `knowgrph-knowledge-source-snapshot/v1`, `complete: true`, with allowlist provenance, page/field/record/document/byte counts, content/envelope digests, sanitized snapshot, and warnings |

Identity is selected only by `KNOWGRPH_STORAGE_LARK_IDENTITY_MODE`. A
`tenant-app` configuration uses server-held
`KNOWGRPH_STORAGE_LARK_APP_ID`/`KNOWGRPH_STORAGE_LARK_APP_SECRET`; a
`user-oauth` configuration uses an external broker/operator-managed server-held
`KNOWGRPH_STORAGE_LARK_USER_ACCESS_TOKEN` plus required absolute expiry and
...
`KNOWGRPH_STORAGE_LARK_SOURCE_ALLOWLIST_JSON`. The browser cannot select an
identity mode or replace a Base/table/view binding. The server-only
`KNOWGRPH_STORAGE_SIGNING_SECRET` seals the five-minute AES-GCM AEAD handoff.
...
| Resolve source handoff | `knowgrph-storage` knowledge-source runtime | Authenticated member receives a five-minute AEAD bearer capability | No Lark content read when configuration blocks |
| Read allowlisted Base | `knowgrph-storage` Lark provider | Creates one complete in-memory snapshot envelope | Read-only Lark API calls |
...
network endpoints. `knowgrph-storage` is the provider-neutral read authority;
...
| `VCC-FB-C-05` | Standalone `cloudflare/workers/knowgrph-storage/knowledge-source/*.test.ts` invocation | Membership-gated issuance, Base revision/total/approved-field/minimum invariants, exact POST search, zero-fetch blockers, redaction, complete digests, and AEAD handoff hold under fixtures. This file set is not yet wired into `storage:relay:test`. | None recorded |
