...
constraints:
  ...
  - "one active shared XR or Geo renderer; Flight adds projection only, never a second renderer, Canvas, world, map provider, or scene owner"
clean_room_policy:
...
Knowgrph gains browser-local mission-based flight training on one composed **Geo+XR Mode** surface. The selected authored environment, Flight actors, waypoints, camera, and input share the existing React Three Fiber XR Canvas; Geo supplies four planar/volumetric and Classic/Modern presentation policies without painting a second world or starting an external map runtime. One shared training projection appears in FloatingPanel **Media**, **Animation**, **Motion Control**, **Game Mode**, **Flight Sim**, and **Camera**. Media environment cards stage their existing authored environment before opening the shared **Geo** panel, and the Flight workspace seed selects Geo+XR directly so that same environment remains the live mission world. It opens from a source-backed run-ready document, the shared surface catalog, browser WebMCP, or the strict `/flight.sim @canvas #flight` invocation. Desktop keyboard/pointer, mobile touch, standard gamepad, and optional Motion Control input arm deterministic native Agentic ECS missions with in-repo flight dynamics, AABB terrain collision, local day/night presentation, bounded practice failures, visible and spoken coaching, scored outcomes, selectable camera source, and Decisions-only WorkspaceFs persistence.
...
|---|---|---|---|
| Enter | Apply the source-backed flight seed or invoke `/flight.sim @canvas #flight operation=open` | Run-ready activation | Flight Sim retains the selected authored environment and Flight actors in one shared XR Canvas |
...
- One Geo+XR presentation path that maps 2D Classic/Modern and 3D Classic/Modern to four distinct policies for the selected authored environment in the existing R3F Canvas; plain Geo retains its separate interactive DOM/SVG Flight projection.
...
Given Geo+XR owns the active surface, entering Flight Sim retains the selected authored environment, aircraft, waypoint/objective actors, and HUD in the same XR Canvas. The combined mode suppresses only the duplicate XR atmosphere; the Geo DOM host paints no screen-space world or terrain, while authored XR subjects, lights, mission actors, camera, deterministic physics, and colliders remain in the shared Canvas. No fallback scene, second renderer, alternate rendered XR world, renderer branch, map dependency, or Flight-owned camera is introduced. The Flight presentation adapter converts the shared procedural aircraft's `-Y` nose axis to the flight model's `-Z` forward axis before applying pitch, yaw, and roll, so the visible nose, simulated velocity, waypoint travel, and Fixed Follow view direction remain aligned. The shared Camera catalog supplies exactly `fixed-follow` / `free-orbit`; the canonical Physics-controller camera hook alone mutates the camera and OrbitControls, using north-up plan framing for both 2D policies and preserving the player's Chase/Cockpit/Survey choice for both 3D policies. Timeline camera-marks may temporarily take framing ownership before returning to the last selected option. Every telemetry surface renders normalized compass heading from the same flight-model projection. While FloatingPanel is open at its default top-right position, the pointer-transparent HUD uses the same responsive width expression to clear the panel while keeping mobile controls above bottom choreography surfaces.

Plain Geospatial Mode remains an exclusive, interactive Geo surface and may use the existing transparent DOM/SVG Flight projection without mounting the XR Canvas. In Geo+XR Mode the Flight/XR input surface owns pointer input and Geo supplies presentation state. The combined bridge requests the `shared-xr-stage` policy: the screen-space SVG world and MapLibre stay unmounted; `2d-classic`, `2d-modern`, `3d-classic`, and `3d-modern` vary camera plan/volume, grid, wash, and light presentation while retaining identical selected terrain geometry and colliders. Standalone Geo retains its existing provider-backed modes. City and FPS gameplay retain their existing 3D precedence. A Geo or Geo+XR Flight handoff never clears the selected environment or adds a map provider, renderer, camera, world, network call, token, copied asset, or external dependency.
...
| Rendering | `ThreeGraph.impl.tsx`, `xrGeoEnvironmentPresentation.ts`, and the policy bridge in `CanvasViewportGeospatialOverlay.tsx` | In Geo+XR, retain one authored environment and Flight in one XR Canvas; Geo changes presentation state and paints no second world; retain exclusive plain-Geo projection separately |
...
Flight Sim uses the native Agentic ECS for ephemeral runtime state and projects onto the active shared surface. Geo+XR uses `CanvasViewportGeospatialOverlay` only as the presentation-policy bridge, keeps its screen-space world transparent, and mounts the selected authored environment plus Flight inside the existing `ThreeGraph` React Three Fiber Canvas. Plain Geo remains an exclusive renderer path. A second Canvas, second camera owner, bitECS, Babylon.js, or another ECS is rejected because it duplicates an existing repository owner.
...
| `npm run game-flight-sim:browser-smoke` | Mandatory two-run exact-HEAD XR proof | Child-isolated retained authored XR Canvas, four Geo presentation policies, north-up planar framing, no screen-space Geo world or MapLibre, first frame under 3 s, desktop/WebMCP input, trusted Chromium-emulated touch at 375x812, ordered route completion, lifecycle/failure/Exit disposal, camera and Timeline round-trip, zero blocked/non-local requests, and cleanup-before-transactional-evidence publication |
...
| Exact-main Geo+XR four-view acceptance | Mandatory composed-surface proof | One selected authored environment and one R3F Canvas survive all four view selections; policies are distinct; 2D is north-up; 3D preserves Flight camera choice; world SVG, MapLibre, duplicate terrain, and remote requests remain absent |
