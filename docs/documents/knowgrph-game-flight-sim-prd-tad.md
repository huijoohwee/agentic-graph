...
Knowgrph gains browser-local mission-based flight training on one composed **Geo+XR Mode** surface. The existing local Geo renderer owns the visible background while the existing React Three Fiber XR Canvas stays transparent above it and owns the Flight actors, waypoints, camera, and input; no second Canvas or external map runtime is introduced. One shared training projection appears in FloatingPanel **Media**, **Animation**, **Motion Control**, **Game Mode**, **Flight Sim**, and **Camera**. Media environment cards stage their existing authored environment before opening the shared **Geo** panel, and the Flight workspace seed selects Geo+XR directly so the same environment is visible beneath the live mission. It opens from a source-backed run-ready document, the shared surface catalog, browser WebMCP, or the strict `/flight.sim @canvas #flight` invocation. Desktop keyboard/pointer, mobile touch, standard gamepad, and optional Motion Control input arm deterministic native Agentic ECS missions with in-repo flight dynamics, AABB terrain collision, local day/night presentation, bounded practice failures, visible and spoken coaching, scored outcomes, selectable camera source, and Decisions-only WorkspaceFs persistence.
...
|---|---|---|---|
| Enter | Apply the source-backed flight seed or invoke `/flight.sim @canvas #flight operation=open` | Run-ready activation | Flight Sim composes the local Geo background with the one shared transparent XR Canvas |
...
- One Geo presentation path that retains the existing interactive Geo renderer and mounts exactly one transparent Flight route/aircraft/input overlay without a competing R3F Canvas.
...
Given Geo+XR owns the active surface, entering Flight Sim keeps the local Geo renderer visibly mounted below the same transparent XR Canvas and overlays the aircraft and waypoint/objective actors plus the HUD. The combined mode suppresses only the duplicate XR atmosphere and terrain; authored XR subjects, lights, mission actors, camera, and deterministic physics remain in the shared Canvas. No fallback scene, second renderer, alternate rendered XR world, renderer branch, map dependency, or Flight-owned camera is introduced. The Flight presentation adapter converts the shared procedural aircraft's `-Y` nose axis to the flight model's `-Z` forward axis before applying pitch, yaw, and roll, so the visible nose, simulated velocity, waypoint travel, and Fixed Follow view direction remain aligned. The shared Camera catalog supplies exactly `fixed-follow` / `free-orbit`, the canonical Physics-controller camera hook consumes the pure aircraft follow/framing descriptor and alone mutates the camera and OrbitControls, and Timeline camera-marks may temporarily take framing ownership before returning to the last selected option; immersive first-person entry is not required to fly. Every telemetry surface renders normalized compass heading from the same flight-model projection instead of displaying raw signed yaw. While FloatingPanel is open at its default top-right position, the pointer-transparent HUD uses the same responsive width expression to clear the panel for top telemetry, navigation, and bottom lifecycle actions, while retaining the established layer that keeps mobile flight controls above bottom choreography surfaces.

Plain Geospatial Mode remains an exclusive, interactive Geo surface and may use the existing transparent DOM/SVG Flight projection without mounting the XR Canvas. In Geo+XR Mode the Flight/XR input surface owns pointer input and Geo is the visible background. The combined bridge requests a local-only Geo render policy: committed inline basemap geometry supplies visibly distinct 3D Classic and 3D Modern presentations while MapLibre remains unstarted, so the Flight fence sees no remote style or tile request. Standalone Geo retains its existing provider-backed modes. City and FPS gameplay retain their existing 3D precedence. A Geo or Geo+XR Flight handoff never clears the selected environment or adds a map provider, renderer, camera, world, network call, token, copied asset, or external dependency.
...
| Rendering | Shared surface policy over `ThreeGraph.impl.tsx` and `CanvasViewportGeospatialOverlay.tsx` | In Geo+XR, keep Geo below one transparent XR Canvas; retain exclusive plain-Geo projection as a separate surface |
...
Flight Sim uses the native Agentic ECS for ephemeral runtime state and projects onto the active shared surface. Geo+XR retains `CanvasViewportGeospatialOverlay` as the background and mounts the dedicated Flight stage inside the existing transparent `ThreeGraph` React Three Fiber Canvas. Plain Geo remains an exclusive renderer path. A second Canvas, second camera owner, bitECS, Babylon.js, or another ECS is rejected because it duplicates an existing repository owner.
...
| `npm run game-flight-sim:browser-smoke` | Mandatory two-run exact-HEAD XR proof | Child-isolated retained authored XR Canvas, first frame under 3 s, desktop/WebMCP input, trusted Chromium-emulated touch at 375x812, ordered three-waypoint plus landing-pad completion through public production runtime APIs, lifecycle/failure/Exit disposal, fixed-follow/free-orbit plus Timeline round-trip, zero blocked/non-local requests, and cleanup-before-transactional-evidence publication |
