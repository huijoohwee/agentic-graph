# AgenticGraph Canvas Frontend: Universal Technical Specification

> Canonical index document. Keep this file sub-600; continue interaction and operations detail in `agenticgraph-frontend-document-interactions-and-operations.md`.
...
## AgenticGraph Canvas Architecture
...
- Implementation notes and guardrails live in `docs/documents/agenticgraph-graph-data-table-fast-grid-document.md`.
...
**Storyboard Widget viewport settle retry**: After workspace-open overlay transitions, the Flow canvas retries centroid-centered fit if the initial viewport settle produces an offscreen or degenerate transform. See `agenticgraph-storyboard-widget-pan-zoom-overlay-failsafe-document.md`.
...
## Continuation: See `agenticgraph-frontend-document-interactions-and-operations.md`.
