---
title: "AgenticGraph Docs/Sheets/Slides Export Runtime Readiness"
id: "md:agenticgraph-docs-sheets-slides-runtime-readiness"
...
doc_path: "docs/documents/agenticgraph-docs-sheets-slides-runtime-readiness.md"
parent: "docs/documents/agenticgraph-docs-sheets-slides-prd-tad.md"
live_provider_run_proven: false
production_release_authorized: false
---

# AgenticGraph Docs/Sheets/Slides Export Runtime Readiness
...
1. Human OAuth access token: `AGENTICGRAPH_GOOGLE_ACCESS_TOKEN`.
2. Human OAuth refresh: `AGENTICGRAPH_GOOGLE_CLIENT_ID`,
   `AGENTICGRAPH_GOOGLE_CLIENT_SECRET`, and
   `AGENTICGRAPH_GOOGLE_REFRESH_TOKEN`.
3. Service account: `AGENTICGRAPH_GOOGLE_SERVICE_ACCOUNT_JSON` plus either
   `AGENTICGRAPH_GOOGLE_IMPERSONATED_USER` for Workspace domain-wide delegation or
   `AGENTICGRAPH_GOOGLE_SHARED_DRIVE_FOLDER_ID` for a destination folder in a shared
   drive. `AGENTICGRAPH_GOOGLE_DRIVE_FOLDER_ID` may constrain only human-OAuth output.
...
1. `AGENTICGRAPH_MICROSOFT_ACCESS_TOKEN`; or
2. `AGENTICGRAPH_MICROSOFT_CLIENT_ID` and
...
`AGENTICGRAPH_MICROSOFT_TENANT` and `AGENTICGRAPH_MICROSOFT_SCOPE`. Set
`AGENTICGRAPH_MICROSOFT_ONEDRIVE_FOLDER` when acceptance artifacts should be grouped
...
- `AGENTICGRAPH_EXPORT_MICROSOFT_FALLBACK_ENABLED` defaults to enabled.
- `AGENTICGRAPH_EXPORT_FLEET_PATH` selects an isolated ledger.
- `AGENTICGRAPH_ROOT` selects the repository root for bounded artifact resolution.

## Focused local verification

Run from the AgenticGraph repository root. These commands do not require provider
...
```bash
AGENTICGRAPH_ROOT="$(pwd)" node ./mcp/server.js
```
...
- `agenticgraph-docs-sheets-slides-prd-tad.md`
- `agenticgraph-api-document.md`
- `../../mcp/README.md`
