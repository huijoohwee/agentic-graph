---
title: "Knowgrph Agentic OS — Reference implementation PRD/TAD"
...
source_references:
  ...
  worker_dispatcher: "cloudflare/workers/knowgrph-mcp/os-status-tool.mjs"
  worker_registry: "cloudflare/workers/knowgrph-mcp/tool-registry.mjs"
  worker_entrypoint: "cloudflare/workers/knowgrph-mcp/index.ts"
  worker_authorization: "cloudflare/workers/knowgrph-mcp/agent-runtime-http.ts"
  worker_configuration: "cloudflare/workers/knowgrph-mcp/wrangler.toml"
---

# Knowgrph Agentic OS — Reference implementation PRD/TAD
...
| Worker status dispatcher | Implement five projections and two invalid-view paths | `cloudflare/workers/knowgrph-mcp/os-status-tool.mjs` | `spec-complete` | `undocumented` |
| Worker registry | Build ten definitions | `cloudflare/workers/knowgrph-mcp/tool-registry.mjs` | `spec-complete` | `undocumented` |
| Worker authorization | Enforce the static bearer boundary | `cloudflare/workers/knowgrph-mcp/agent-runtime-http.ts` | `spec-complete` | `undocumented` |
| Worker entrypoint | Order public inspection and protected MCP handling | `cloudflare/workers/knowgrph-mcp/index.ts` | `spec-complete` | `undocumented` |
...
solely in [the MCP install contract](knowgrph-mcp-install-contract.md). The
...
|---|---|---|---|---|---|
| `knowgrph.superagent.run` | Tool identity | Agent runtime | Source-owned agent-run schema | Protected control plane | Harness-dependent |
| `knowgrph.video_remix.run` | Tool identity | Video Remix Director | Source-owned run schema | Protected control plane | Harness-dependent |
| `knowgrph.video_remix.research` | Tool identity | Video Remix stage runtime | Source-owned research schema | Protected control plane and approval gate | Harness-dependent |
| `knowgrph.video_remix.storyboard` | Tool identity | Video Remix stage runtime | Source-owned storyboard schema | Protected control plane and approval gate | Harness-dependent |
| `knowgrph.video_remix.render` | Tool identity | Video Remix stage runtime | Source-owned render schema | Protected control plane and approval gate | Harness-dependent |
| `knowgrph.video_remix.publish` | Tool identity | Video Remix stage runtime | Source-owned publish schema | Protected control plane and approval gate | Harness-dependent |
| `knowgrph.video_remix.checkout` | Tool identity | Video Remix stage runtime | Source-owned checkout schema | Protected control plane and approval gate | Harness-dependent |
| `knowgrph.run_manifest.note.update` | Tool identity | Run-manifest note runtime | Source-owned note schema | Protected control plane | 0 model tokens |
| `knowgrph.os.status` | Tool identity | Agentic OS reference implementation | `{ view: enum(7), cloudflareMcpUrl?: string }` | Local stdio trust or protected control plane | 0 |
| `knowgrph.agentic_canvas_os.docs.invoke` | Tool identity | Agentic Canvas OS docs resolver | Source-owned docs-resolution schema | Protected control plane | 0 model tokens |
...
| VCC-AOS-07 | The registry returns exactly ten unique names matching the capability catalog | `node --test cloudflare/workers/knowgrph-mcp/__tests__/tool-registry.test.mjs cloudflare/workers/knowgrph-mcp/__tests__/mcp-surface-equivalence.test.mjs` | No tool owner is duplicated |
...
- Follow-on sequencing: [Agentic OS follow-on reference implementation](knowgrph-agentic-os-follow-on-prd-tad.md)
- Video target contract: [Agentic video workflow reference implementation companion](knowgrph-agentic-os-video-agent-prd-tad.companion.md)
- Superseded nested path: [MCP compatibility record](knowgrph-mcp/knowgrph-mcp-agentic-os-prd-tad.md)
- Superseded nested companion path: [MCP companion compatibility record](knowgrph-mcp/knowgrph-mcp-agentic-os-prd-tad.companion.md)
