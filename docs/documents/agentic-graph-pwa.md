# agentic-graph Progressive Web App (PWA)

**Context**: Browser-based knowledge graph canvas deployed on Cloudflare Pages at `airvio.co/agentic-graph`.
...
|-------|-------|-----------|
| `name` | `agentic-graph` | Full app name |
| `short_name` | `agentic-graph` | Home screen label |
| `id` | `/agentic-graph/` (prod) or `/` (dev) | Scope isolation per deployment |
...
rewrite from `/` to `/agentic-graph/` cannot bind the page to an apex-root manifest by accident.
...
agentic-graph-owned caches plus every scoped `/agentic-graph` HTML response and every stale
`/agentic-graph/assets/*` entry wherever found. This removes response-typed HTML poison stored
...
published `/agentic-graph/` app shell and fails closed if that shell is unavailable or invalid.
...
target no longer exists; the root Function and canonical `/agentic-graph/` application remain the
only Home owners.

The generated `sw.js` entry and both mutable imports,
`agentic-graph-service-worker-revision.js` and `agentic-graph-chat-stream-sw.js`, use `no-store`
...
|------|--------|-------|-----------|
| `/agentic-graph/sw.js` | `Cache-Control` | `no-cache, no-store, must-revalidate` | Prevent stale agentic-graph SW |
| `/agentic-graph/agentic-graph-service-worker-revision.js` | `Cache-Control` | `no-cache, no-store, must-revalidate` | Bind the active worker to one exact release revision |
| `/agentic-graph/agentic-graph-chat-stream-sw.js` | `Cache-Control` | `no-cache, no-store, must-revalidate` | Prevent stale imported worker lifecycle code |
| `/agentic-graph/manifest.webmanifest` | `Cache-Control` | `no-cache, no-store, must-revalidate` | Instant manifest updates |
| `/agentic-graph/apple-touch-icon.png` | `Cache-Control` | `public, max-age=86400` | 1-day icon cache |
...
`content/agentic-graph` are not authoritative and should be excluded from publish sync.
...
| Title | `<meta name="apple-mobile-web-app-title" content="agentic-graph" />` |
...
| Headers | Prevent stale SW, imported worker, and manifest | Set `no-store` on `sw.js`, `agentic-graph-chat-stream-sw.js`, and `manifest.webmanifest`; add `Service-Worker-Allowed: /` |
