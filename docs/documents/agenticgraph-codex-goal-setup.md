...
AgenticGraph keeps its durable Codex objective in the repo-root `goal` file. That goal is the local contract for long-running harness work: it names the objective, constraints, validation loop, proof artifacts, and stopping condition.

The goal loop is the native AgenticGraph long-horizon SuperAgent message gateway for local research, code, and create work. It uses repo-owned run memory, traces, role-scoped agent contracts, tools, skills, and sandbox/workspace artifacts through `agenticgraph_parser`; DeerFlow may be cited only as conceptual inspiration or used as an optional local gateway provider, never as copied runtime architecture or a required deploy target.
...
Then use `/goal` from Codex while working in `$GITHUB_ROOT/agenticgraph`, or run the same repo-owned goal loop through the local harness:
