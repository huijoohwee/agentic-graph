...
id: "md:agenticgraph-lark-app-mcp-prd-tad.companion"
...
doc_path: "docs/documents/agenticgraph-mcp/agenticgraph-lark-app-mcp-prd-tad.companion.md"
guideline_version: "1.7.0"
reference_implementation_label: "reference implementation"
parent: "agenticgraph-lark-app-mcp-prd-tad.md"
...
[the parent contract](agenticgraph-lark-app-mcp-prd-tad.md). It does not redefine
...
| Public storage contract | `canvas/src/lib/storage/agenticgraphStorageSyncContract.ts`, `agenticgraphStorageRoutePaths.ts`, and `agenticgraphStorageWorkerEnvContract.ts` | Defines the stable API/schema, handoff/read routes, complete envelope, and server env boundary. | Does not make Canvas a provider client. |
| Read-only server runtime | `cloudflare/workers/agenticgraph-storage/knowledge-source/` | Resolves identity/allowlist, performs exact Lark reads, emits complete digested snapshots, and signs opaque handoffs. | No Lark write, source acceptance, or deploy authority. |
...
[the MCP installation contract](../agenticgraph-mcp-install-contract.md) as the
...
`agenticgraph-storage` is the provider-neutral runtime authority for read access,
...
| Canvas bounded-streams and accepts only a closed `agenticgraph-knowledge-source-snapshot/v1` with `complete: true`, consistent counts, and recomputed digests. | Knowledge-source read client |
...
| Identity | `AGENTICGRAPH_STORAGE_LARK_IDENTITY_MODE`; tenant-app app ID/secret or externally managed user OAuth access token plus expiry remains server-only |
| Allowlist | `AGENTICGRAPH_STORAGE_LARK_SOURCE_ALLOWLIST_JSON` using `agenticgraph-knowledge-source-allowlist/v1` |
| Opaque-token custody | Existing server-only `AGENTICGRAPH_STORAGE_SIGNING_SECRET`; five-minute AES-GCM AEAD bearer capability |
...
`agenticgraph-knowledge-source-snapshot/v1`. The envelope binds identity mode,
...
`agenticgraph-storage` read-route admission and grants no publish authority.
