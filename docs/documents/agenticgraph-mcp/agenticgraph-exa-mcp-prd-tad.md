...
id: "md:agenticgraph-exa-mcp-prd-tad"
...
doc_path: "docs/documents/agenticgraph-mcp/agenticgraph-exa-mcp-prd-tad.md"
...
| Engage | Copies a config and configures an MCP host. | Host outside AgenticGraph | A secret may be pasted into browser state. | Copy only non-secret material. |
...
| Managed 12-month incremental AgenticGraph TCO | USD 0 for source-only UI; upstream fees unmeasured | Keep USD 0 until a provider budget is approved. |
...
own a AgenticGraph Invocation Register. AgenticGraph endpoint selection remains solely
in [the MCP installation contract](../agenticgraph-mcp-install-contract.md).
...
AgenticGraph route ownership. No separate provider endpoint register is authored
here.
