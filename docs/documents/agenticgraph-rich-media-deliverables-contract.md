...
The preset instruction contains existing source-backed `/`, `@`, and `#` tokens. On explicit Run, the browser posts only normalized invocation tokens to same-origin Dev/Preview `POST /__agenticgraph_mcp_agentic_os_docs_invoke`.

The bridge:

- accepts JSON only and rejects cross-site requests;
- bounds the body to 32 KiB and tokens to 12;
- invokes only `agenticgraph.agentic_canvas_os.docs.invoke` through the local stdio MCP server;
