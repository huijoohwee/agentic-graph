...
- Persists per-URL artifacts to the repo-local ignored workspace output dir (default `.agentic-graph-workspace/...`):
  - `raw.html` (guaranteed)
...
```bash
npm --prefix agentic-graph/canvas run webpage:markdown -- --url "https://example.com/" --out "./webpage.md"
```
