---
title: "agentic-graph Vdeoxpln PRD/TAD"
...
# agentic-graph Vdeoxpln PRD/TAD

## Document Map

This combined PRD/TAD defines a agentic-graph-native vdeoxpln system that enhances existing repo
...
below is original to agentic-graph and must be rooted in current agentic-graph owners.
...
copied into agentic-graph. The reviewed snapshot did not include a license file, so the reference remains
conceptual only. agentic-graph must implement any similar capability through its own existing source
owners, typed contracts, and validation harnesses.

## Executive Summary

agentic-graph already has source-owned vdeoxpln surfaces:
...
| Agent-ready tool contract | `canvas/src/features/agent-ready/agentic-graph-agent-ready-tool-contract.mjs` | Shared read-only tool contract for Pages and browser WebMCP. | Vdeoxpln reference this contract for deployed inspection capabilities. |
...
| Pages agent-ready route | `cloudflare/pages/agentic-graph-agent-ready.mjs` | Publishes health, MCP, A2A, OpenAPI, HTML fallback WebMCP, and agent-skills metadata. | Published skill metadata is generated from canonical vdeoxpln definitions. |
...
| Publish sync | `scripts/sync-pages-agentic-graph.mjs` | Syncs source-owned Pages and static artifacts into the production mirror. | Prod mirror receives generated vdeoxpln artifacts only through sync. |
...
agentic-graph is already agent-ready, but its skill surfaces are distributed across local skill files,
...
If agentic-graph defines vdeoxpln as semantic, manifest-governed projections over existing import,
...
|---|---|---|---|---|
| Discover | Agent inspects available agentic-graph skills. | Pages agent-skills, WebMCP, local MCP docs | Skill names and capabilities may differ by surface. | Publish one normalized vdeoxpln index. |
...
```mermaid
flowchart LR
  SourceOwners["agentic-graph source owners"] --> Registry["Canonical vdeoxpln registry"]
...
flowchart LR
  ...
  Cloudflare --> Agents["airvio.co/agentic-graph agent-ready discovery"]
```
...
Decision: agentic-graph may adopt the general pattern of manifest-governed, file-backed, staged
skill workflows, but it must not copy PaperMotion implementation artifacts.

Rationale: agentic-graph already has different owners, topology, and deployment surfaces. Copying would
introduce licensing uncertainty, stale fixtures, and repo-specific assumptions.

TCO/FOSS: original implementation over existing agentic-graph owners avoids new dependencies.
...
```bash
test -f docs/documents/agentic-graph-vdeoxpln-prd-tad.md
sed -n '1,80p' docs/documents/agentic-graph-vdeoxpln-prd-tad.md
```
