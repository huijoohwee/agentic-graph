...
| API | Credit Ledger Service | Owns all credit-token balance mutations; atomic debit via Durable Object with D1 audit persistence. | `StrytreeCreditLedgerActor`; `cloudflare/workers/agentic-graph-payment/strytreeApi.ts` |
...
| API | Unlock Transaction Service | Debits wallet and grants entitlement atomically. | `cloudflare/workers/agentic-graph-payment/strytreeApi.ts` |
| API | Generation Harness | Validates external video provider payload, debits/refunds, enqueues job, handles fallback. | `cloudflare/workers/agentic-graph-payment/strytreeApi.ts` |
| API | Candidate Run Service | Validates candidate fan-out, quotes cost, creates candidate run rows, and coordinates publish of one selected candidate. | `cloudflare/workers/agentic-graph-payment/strytreeApi.ts` |
