---
title: "agentic-graph — AI Voice Studio PRD-TAD-ADR-MVP-GTM"
id: "md:agentic-graph-ai-voice-studio-prd-tad"
...
traceability:
  ...
  doc_path: "docs/documents/agentic-graph-ai-voice-studio-prd-tad-adr-mvp-gtm.md"
source_references:
...
continuity_id: "PLAN-AGENTIC-GRAPH-AI-VOICE-STUDIO-PRD-TAD-ADR-MVP-GTM"
...
# agentic-graph — AI Voice Studio PRD-TAD-ADR-MVP-GTM

## Outcome and claim boundary

agentic-graph now owns one provider-neutral voice studio across browser UI and local stdio MCP. It supports three exact intent routes:
...
On 2026-07-24, the agentic-graph-authored Voice Studio contract, runtime, tests,
...
| agentic-graph shared schema | `contracts/voice-studio.schema.js` | Closed input/output schemas and pure validation |
| Local MCP descriptor | `mcp/voice-studio-tool-contract.js` | `agentic-graph.voice.studio` |
...
| Registry | agentic-graph vdeoxpln | discoverability and routing metadata |

### MCP operation model

The single MCP tool accepts `operation: clone | dictate | create` using
`agentic-graph-voice-studio-request/v1`; results use
`agentic-graph-voice-studio-result/v1`.
...
## ADR: share the domain contract and inject live execution owners

Keep browser and MCP operations on the shared clone/dictate/create vocabulary. The browser owns a session-only manifest and disclosed system-voice preview; live execution depends on separately injected rights, authorization, adapter, settled-cost and read-back owners. A separate provider-specific product model would duplicate state and blur these evidence boundaries. The canonical stdio server remains unconfigured for live execution; this decision records the existing architecture, without asserting provider-backed cloning.

## Planning revision — reference implementation

All five roles below consume `PLAN-AGENTIC-GRAPH-AI-VOICE-STUDIO-PRD-TAD-ADR-MVP-GTM@0.1.1`. Existing source and runtime observations retain their original revisions and scope; this documentation update renews no deployment or demand evidence. The guideline is [v2.7.0](https://github.com/huijoohwee/huijoohwee.github.io/blob/e8d2a10a8d3e5735c43edf350a22523df05fdf91/guidelines/prd-tad-adr-mvp-gtm-guidelines.md); the shared maturity rubric loads on demand.

| Role | Owning content at this revision |
|---|---|
| PRD | [PRD](agentic-graph-ai-voice-studio-prd-tad-adr-mvp-gtm.md#prd) |
| TAD | [TAD](agentic-graph-ai-voice-studio-prd-tad-adr-mvp-gtm.md#tad) |
| ADR | [ADR: share the domain contract and inject live execution owners](agentic-graph-ai-voice-studio-prd-tad-adr-mvp-gtm.md#adr-share-the-domain-contract-and-inject-live-execution-owners) |
| MVP | [MVP — reference implementation](agentic-graph-ai-voice-studio-prd-tad-adr-mvp-gtm.md#mvp--reference-implementation) |
| GTM | [GTM — reference implementation](agentic-graph-ai-voice-studio-prd-tad-adr-mvp-gtm.md#gtm--reference-implementation) |

## MVP — reference implementation

Reuse the minimum scope, acceptance conditions and component owners identified above. The demonstration must follow the documented entry, permitted action, durable outcome and readback, including its stated failure/recovery path. Use `npm run voice-studio:check` for its actual coverage and the named feature checks in the specification; attach exact source, command, result and authoring/mirror/delivery surface to each VCC before advancing readiness. A source locator or structural check alone proves no user outcome.
...
Source review is bounded to repository `7fb85741121d8c2886027e4a630d013ba91c1027`. Confirmed: these referenced artifacts exist at that revision: [`contracts/voice-studio.schema.js`](https://github.com/huijoohwee/agentic-graph/blob/7fb85741121d8c2886027e4a630d013ba91c1027/contracts/voice-studio.schema.js), [`mcp/voice-studio-tool-contract.js`](https://github.com/huijoohwee/agentic-graph/blob/7fb85741121d8c2886027e4a630d013ba91c1027/mcp/voice-studio-tool-contract.js), [`mcp/voice-studio-runtime.js`](https://github.com/huijoohwee/agentic-graph/blob/7fb85741121d8c2886027e4a630d013ba91c1027/mcp/voice-studio-runtime.js). Their existence does not confirm every behavior asserted by the specification.
