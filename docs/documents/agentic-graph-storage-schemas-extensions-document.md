---
title: "agentic-graph Storage Schema Extensions"
id: "md:agentic-graph-storage-schemas-extensions-document"
version: "1.4.0"
updated: "2026-07-23"
...
# agentic-graph Storage Schema Extensions

**Context**: Deferred schema extensions for agentic-graph storage and sync.
**Intent**: Keep the shipped D1 baseline in `agentic-graph-storage-schemas-document.md` while preserving extension contracts in a focused owner.
...
**Version**: 1.4.0
**Date**: 2026-07-23
**Canonical baseline**: `agentic-graph-storage-schemas-document.md`
...
Repository routing is not a deferred schema extension. The shipped collaboration bridge resolves `agentic-graph-docs` or `workspace-docs` from the canonical document path, validates the client-supplied target, and uses target-specific server configuration. Repository credentials, local mirror paths, and online/offline preferences must not be stored in these D1 tables.
...
Explorer ownership is derived from the shared repository-authority contract, not stored in PocketBase or D1. `workspace-docs` displays `GitHub/huijoohwee/docs`; `agentic-graph-docs` displays `GitHub/agentic-graph/docs`; `/docs/workspace-seeds/**` displays the narrower `GitHub/agentic-graph/docs/workspace-seeds` boundary; IndexedDB displays as offline fallback. Agentic runtime projections and rejected Huijoohwee seed duplicates never become selectable write authorities.

No schema field may override the seed owner. Local mirror requests carry only a logical `/docs/workspace-seeds/**` `workspacePath`; the server bridge derives the corresponding `$GITHUB_ROOT/agentic-graph/docs/workspace-seeds/**` host target. PocketBase room metadata, D1 rows, browser settings, import payloads, client absolute paths, and client mutation-root environment variables cannot select `huijoohwee/docs/workspace-seeds` or another host path.

Seed inventory authority is runtime metadata, not a persisted schema extension. Entries read from the local canonical directory carry `agentic-graph-workspace-seeds-local`; entries loaded from the revision-pinned seven-file app artifact carry `agentic-graph-workspace-seeds-bundled`. Only a complete, non-empty authority-marked inventory can replace and prune the cached `/docs/workspace-seeds/**` subtree.
