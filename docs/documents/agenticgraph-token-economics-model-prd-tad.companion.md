---
title: "AgenticGraph Token Economics Model — TAD Companion"
...
parent: "agenticgraph-token-economics-model-prd-tad.md"
tags:
  - "token-economics"
  - "tad"
  - "kgc"
...
# AgenticGraph Token Economics Model — TAD Companion
...
**Interfaces**: `MCP tool: agenticgraph.tem.query` — input: `{ query: string }` → output: `NLQResult`
...
4. **react-force-graph (MIT)**: 3D capable; existing AgenticGraph dependency. Heavyweight for a 14-node 2D graph; adds WebGL dependency.
...
**Trigger**: A AgenticGraph harness completes and appends a `cost_log` entry to `harness-proof.json`.
...
**Future TEM renderer extension**: Existing AgenticGraph React/canvas surface or portable static artifact. No API key in browser. Deploy through the same Pages pipeline as the app.
...
| Future | Renderer extension | Reuse existing AgenticGraph canvas surfaces | Planned |
...
| OQ-8 | Renderer extension: portable static artifact vs embedded in AgenticGraph React app? Proposed: embed first when it can reuse existing graph surfaces; export a portable artifact only through the same canonical renderer. | airvio | Sprint 4 |
