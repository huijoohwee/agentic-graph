# agentic-graph Super-Agent Harness

## Document Purpose

This is the source-side SSOT for agentic-graph's local long-horizon SuperAgent harness.
It records the runnable `agentic_graph_parser` harness, its local MCP entrypoint, and
the guardrails for using external SuperAgent references without copying them.

agentic-graph may use [bytedance/deer-flow](https://github.com/bytedance/deer-flow)
...
```bash
python3 -m agentic_graph_parser superagent --input docs/documents/my-input.md --output-dir data/outputs/superagent-neutral-example --run-id superagent-neutral-example
```
...
- a quick demo of agentic-graph as a runnable document system
...
| Primitive | agentic-graph owner | Boundary |
|---|---|---|
| Message gateway | `/goal`, `npm run goal:run`, local MCP `agentic-graph.superagent.run`, MainPanel/FloatingPanel handoff docs | Local/dev only unless a separate deploy proof exists. |
...
The canvas worker now emits both a standalone graph artifact and a agentic-graph workspace markdown artifact for the app route:
...
```bash
python3 -m agentic_graph_parser superagent --input docs/documents/my-input.md --output-dir data/outputs/superagent-checkpoint --run-id superagent-checkpoint --stop-after-step 2
```

Resume it:

```bash
python3 -m agentic_graph_parser superagent --resume --output-dir data/outputs/superagent-checkpoint
```

Exercise bounded recovery:

```bash
python3 -m agentic_graph_parser superagent --input docs/documents/my-input.md --output-dir data/outputs/superagent-retry --run-id superagent-retry --fail-once video.generate.byteplus_modelark_placeholder
```

## Codex And MCP

Codex can run the harness directly from `/goal` using `superagent` or the equivalent `run-goal` CLI alias. The local setup guide is [agentic-graph-codex-goal-setup.md](agentic-graph-codex-goal-setup.md), and the repo-owned goal loop is also available as `npm run goal:run`.
...
MCP clients can call `agentic-graph.superagent.run`, which wraps the same command and returns the trace, state, report, canvas artifact paths, and workspace frontmatter-flow artifact. The local stdio MCP tool contract is owned upstream by [local-tool-contract.js](../../mcp/local-tool-contract.js); usage and surface boundaries live in [README.md](../../mcp/README.md), [agentic-graph-mcp-service-prd-tad-adr-mvp-gtm.companion.md](agentic-graph-mcp/agentic-graph-mcp-service-prd-tad-adr-mvp-gtm.companion.md), and the planned Agentic OS dashboard contract in [agentic-graph-mcp-agentic-os-prd-tad-adr-mvp-gtm.md](agentic-graph-mcp/agentic-graph-mcp-agentic-os-prd-tad-adr-mvp-gtm.md).
