...
continuity_id: "PLAN-AGENTIC-GRAPH-STRYBLDR-PRD-TAD-ADR-MVP-GTM"
...
# agentic-graph Strybldr - PRD and TAD

## Document Map

This combined PRD/TAD follows `$GITHUB_ROOT/huijoohwee.github.io/guidelines/prd-tad-adr-mvp-gtm-guidelines.md`. It defines a 3-hour hackathon slice for agentic-graph Strybldr: import an image, reverse engineer it into editable storyboard element cards, let the user update those cards, and generate or hand off a bounded video request through the existing BytePlus ModelArk run owners.
...
Strybldr converts one imported image into a source-backed storyboard and video-generation handoff. The MVP must reuse agentic-graph’s existing Launch import, queryable-corpus media source units, Source Files parsing, Storyboard renderer, Floating Panel shell, settings/integrations, and BytePlus video-generation task owner.
...
If agentic-graph can import one image, produce editable element-level storyboard cards, and generate or prepare one bounded video handoff from the approved cards, then a solo dev can demo an AI-native image-to-video orchestration loop with high ROI, zero mandatory monthly TCO, and visible provenance.
...
| agentic-graph maintainer | Extend existing surfaces without drift. | Must reuse shared owners. |
...
- Given a PNG, JPEG, WebP, GIF, or AVIF image, when Import Image runs, then agentic-graph creates a corpus media source unit with original name, MIME hint, byte size, source path, and source-unit ID.
...
|---|---|---|
| Dev | `$GITHUB_ROOT/agentic-graph` | Implement, typecheck, focused tests, browser smoke. |
| Prod mirror | `$GITHUB_ROOT/huijoohwee/content/agentic-graph` | Sync built/static output only after validation. |
| Cloudflare | `https://airvio.co/agentic-graph` | Publish only with correct Wrangler credentials and live smoke. |
...
| Deploy smoke | Open `https://airvio.co/agentic-graph` after publish. | Route loads and no bad runtime requests. |
...
- `docs/documents/agentic-graph-strybldr-prd-tad-adr-mvp-gtm.md` has valid YAML frontmatter and stays under repository hygiene budgets.
...
All five roles below consume `PLAN-AGENTIC-GRAPH-STRYBLDR-PRD-TAD-ADR-MVP-GTM@0.2.2`. Existing source and runtime observations retain their original revisions and scope; this documentation update renews no deployment or demand evidence. The guideline is [v2.7.0](https://github.com/huijoohwee/huijoohwee.github.io/blob/e8d2a10a8d3e5735c43edf350a22523df05fdf91/guidelines/prd-tad-adr-mvp-gtm-guidelines.md); the shared maturity rubric loads on demand.

| Role | Owning content at this revision |
|---|---|
| PRD | [Product Requirements](agentic-graph-strybldr-prd-tad-adr-mvp-gtm.md#product-requirements) |
| TAD | [Technical Architecture](agentic-graph-strybldr-prd-tad-adr-mvp-gtm.md#technical-architecture) |
| ADR | [ADRs](agentic-graph-strybldr-prd-tad-adr-mvp-gtm.md#adrs) |
| MVP | [MVP — reference implementation](agentic-graph-strybldr-prd-tad-adr-mvp-gtm.md#mvp--reference-implementation) |
| GTM | [GTM — reference implementation](agentic-graph-strybldr-prd-tad-adr-mvp-gtm.md#gtm--reference-implementation) |
...
Source review is bounded to repository `7fb85741121d8c2886027e4a630d013ba91c1027`. Confirmed: these referenced artifacts exist at that revision: [`canvas/src/lib/toolbar/LaunchDropdown.impl.tsx`](https://github.com/huijoohwee/agentic-graph/blob/7fb85741121d8c2886027e4a630d013ba91c1027/canvas/src/lib/toolbar/LaunchDropdown.impl.tsx), [`canvas/src/features/markdown-workspace/useWorkspaceFileActions/importActions.ts`](https://github.com/huijoohwee/agentic-graph/blob/7fb85741121d8c2886027e4a630d013ba91c1027/canvas/src/features/markdown-workspace/useWorkspaceFileActions/importActions.ts), [`canvas/src/features/markdown-explorer/workspaceActionBridge.ts`](https://github.com/huijoohwee/agentic-graph/blob/7fb85741121d8c2886027e4a630d013ba91c1027/canvas/src/features/markdown-explorer/workspaceActionBridge.ts). Their existence does not confirm every behavior asserted by the specification.
