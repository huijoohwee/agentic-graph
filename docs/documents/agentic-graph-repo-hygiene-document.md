# agentic-graph Repo Hygiene (Artifacts + History Purge)
...
- `.agentic-graph-workspace/` is a local artifact root (PDF workspace, website-import artifacts, per-URL `raw.html`, etc).
- `.agentic-graph-workspace/` must never be committed.
- In this repo, `.agentic-graph-workspace/` is a repo-local ignored runtime root. It is disposable and may be removed between verification runs.
- Workflow preview artifacts are generated under `data/outputs/agentic-graph-workflow-preview/` and must stay ignored; source docs under `docs/documents/` remain canonical.

## Ignore Rules

- `agentic-graph/.gitignore` must include `.agentic-graph-workspace` and `.agentic-graph-workspace/`.
- `agentic-graph/.gitignore` must ignore `data/outputs/` and the removed tracked generated preview root `data/agentic-graph-workflow-preview/`.
- Runtime and sandbox-specific ignore rules must not reintroduce `.agentic-graph-workspace/` or generated preview outputs as tracked source.

## Cross-Repo Policy

Apply the same “no large local artifacts in git history” rule to these repos:

- `/GitHub/agentic-graph`
- `/GitHub/singabldr`
- `/GitHub/chatgrph`
...
python3 -m git_filter_repo --force \
  --path .agentic-graph-workspace \
  --path data/agentic-graph-workflow-preview \
  --path-glob 'git-error-*' \
  --invert-paths
```
...
```bash
git remote add origin https://github.com/<owner>/agentic-graph.git
```
...
- Never commit: `.agentic-graph-workspace/**`.
...
- Keep stale archives and dated generated reports out of active source: `docs/documents/deprecated/`, `docs/documents/agentic-graph-api-reference/_archive/`, `docs/documents/api-reference/`, root `test-report/`, root `todo-log_*.md`, and dated `docs/reports/prd-codebase-gap-report_*.md`.
...
- Keep Pages deploy content constrained: `content/agentic-graph` must only include built `assets`, entry `index.html`, favicon, settings, and small examples; scripts exclude cesium, vendor/mermaid bundles, demo, examples, and large test JSON from sync/publish.
...
- Before optimization or refactor work, clean unrelated pending changes in all repos, preserve only scoped code/test diffs, and treat `content/agentic-graph` artifacts as generated outputs mirrored from `canvas/dist` rather than hand-edited configuration state.
