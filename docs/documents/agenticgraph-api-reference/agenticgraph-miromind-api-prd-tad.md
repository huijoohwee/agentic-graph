---
title: "Reference implementation: AgenticGraph and MiroMind API PRD/TAD"
schema: kgc-computing-flow/v1
id: "md:agenticgraph-miromind-api-prd-tad"
...
# Reference implementation: AgenticGraph and MiroMind API PRD/TAD
...
AgenticGraph contains the source-owned core path that a MiroMind adapter must reuse:
...
2. MainPanel `MCP` explains how MiroMind's optional `mcp_servers` capability relates to the existing AgenticGraph MCP/readiness surfaces.
...
- No prod-only doc edits outside the `agenticgraph` source tree.
...
- Given MainPanel opens on `mcp`, when MiroMind is documented, then the text states that `mcp_servers` is an optional provider capability and remains distinct from AgenticGraph's own MCP runtime/readiness surfaces.
...
| Publish-surface doc divergence | possible | source-owned only in `agenticgraph` |
...
| Auth | Server-managed key from the proxy runtime (`AGENTICGRAPH_CHAT_PROXY_MIROMIND_API_KEY`, with `MIROMIND_API_KEY` retained as the current compatibility alias), or memory-only BYOK on explicit fallback |
...
- MiroMind `mcp_servers` is a provider-side request capability, not a replacement for AgenticGraph MCP.
- AgenticGraph's browser/local MCP readiness surfaces remain separate shared owners.
...
|---|---|---|---|---|---|---|
| Authoring | source, tests, and this specification in `$GITHUB_ROOT/agenticgraph` | scoped source/test/doc edits | developer checkout and local test process | VCCs stated; results not attached | `spec-complete` | `undocumented` |
...
- Source owner: `$GITHUB_ROOT/agenticgraph/docs/documents/...`.
- Possible generated mirror owner: `$GITHUB_ROOT/huijoohwee/content/agenticgraph`.
- Configured public route: `https://airvio.co/agenticgraph`.
...
- keeps AgenticGraph MCP and provider-side MCP concerns separated,
...
- Keep `miromindApi.api_key` defaulted to `Server-managed Key`; the local proxy prefers `AGENTICGRAPH_CHAT_PROXY_MIROMIND_API_KEY`, while the current Pages readiness host checks the `MIROMIND_API_KEY` compatibility name. Neither source declaration proves that a deployed runtime sees a value.
