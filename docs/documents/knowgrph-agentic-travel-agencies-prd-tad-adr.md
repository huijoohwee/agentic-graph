---
title: "Knowgrph Agentic Travel Agencies — Flight Booking, Comparison Shopping & Shared-Canvas Primitive"
...
# Knowgrph Agentic Travel Agencies — Combined PRD/TAD/ADR
...
- MCP Gateway federation / external-agent discovery surface for Knowgrph itself (separate Follow-on PRD/TAD per Agent-Platform Readiness)
...
- **[v0.5.0]** Telegram Bot API — **reference implementation** for Notification Dispatcher; genuinely $0 at any volume, created instantly via `@BotFather`, no approval process. **Footnote — Web Push**: native PWA push (VAPID) is a genuinely zero-cost complementary channel, worth keeping in view since Knowgrph is already a PWA, but it only reaches users who already have the app installed with notification permission granted — it doesn't substitute for Telegram/WhatsApp reaching someone who hasn't opened Knowgrph yet, so it isn't built out as its own component this revision.
...
| **[v0.2.0] Self-Custody Wallet Interface** | Executor (signing) | Wallet (Core.app, device-local) | *outside Knowgrph lanes* | Avalanche network directly (Path A), or Wallet-Linking Service (Path B entry) | Sync (signature request/response), on-chain broadcast | Device-local keys; on-chain state is public |
...
flowchart TB
  ...
  EO -- sync REST --> AVAX
  EO -- sync REST --> SNOW
  subgraph SelfCustody["Self-Custody zone (outside Knowgrph trust boundary)"]
    CORE[Core.app\nreference implementation]
  end
...
**Responsibility**: Component queries flight fares and normalizes results into Knowgrph's typed offer schema.
...
**Interfaces**: **MCP server, SSE transport** — sandbox: `https://card.straitsx.ai/sandbox/sse`, production: `https://card.straitsx.ai/production/sse`, **reference implementation: StraitsX Card MCP Gateway**. This is a materially better fit than the v0.1.0 generic-REST assumption: Knowgrph is already MCP-native (its own `/`, `@`, `#` invocation grammar), so this harness binds as a direct MCP tool call, not a wrapped REST client. Sandbox tools confirmed: `get_card_sandbox { wallet_address, cardholder_name, amount_sgd }`, `view_card_sandbox { card_opaque_id, settlement_tx, wallet_address }` → one-time iframe URL. Production tool names are **unconfirmed** — assumed analogous (`get_card`/`view_card`) but not verified (see Open Questions).
...
**Dependencies**: none from Knowgrph's side — this is explicitly outside Knowgrph's trust boundary; device-local keys, never held or seen by any Knowgrph component. **[v0.4.0]** For sandbox testing, the wallet needs (a) Fuji-testnet XSGD, funded via StraitsX's own whitelisting process (organizer/operator-funded, not self-serve), and (b) for **Path A transactions only**, Fuji-testnet AVAX for gas — **reference implementation: Avalanche Fuji Faucet** (`build.avax.network/console/primary-network/faucet`), up to 2 AVAX per claim, rate-limited to roughly one claim per 24h per address.
**Configuration**: N/A — user-controlled wallet, not a Knowgrph-configured service
**FOSS / Vendor**: Core.app is Ava Labs' wallet, free to use, no TCO to Knowgrph; MetaMask (FOSS-licensed, MIT-derived core) remains a drop-in alternative per ADR-4
**Token Budget**: N/A (non-AI, non-Knowgrph-hosted)
...
|---|---|---|---|---|
| Edge | Shared Canvas Node Store | `cloudflare/workers/knowgrph-storage/sharedCanvasNode/*`, `cloudflare/workers/knowgrph-storage/canvasSyncRoom.ts` | `dev-proven` | `undocumented` |
| Edge | Transaction-Side Authority | `cloudflare/workers/knowgrph-storage/travelAgencySide.ts`, `cloudflare/d1/migrations/0012_travel_agency.sql` | `dev-proven` | `undocumented` |
| Edge | Guardrail Gate | `cloudflare/workers/knowgrph-payment/travelAgency/guardrailGate.ts` | `dev-proven` | `undocumented` |
| Harness | Flight Intent Parser | `cloudflare/workers/knowgrph-payment/travelAgency/intentParser.ts` | `dev-proven` | `undocumented` |
| Harness | Flight Discovery Harness | `[...]` | `spec-complete` | `undocumented` |
| Harness | Shopping Discovery Harness | `[...]` | `spec-complete` | `undocumented` |
| Harness | Issuance Service | `cloudflare/workers/knowgrph-payment/travelAgency/issuanceService.ts` | `dev-proven-fail-closed` | `undocumented` |
| Harness | Settlement Verifier | `cloudflare/workers/knowgrph-payment/travelAgency/settlementVerifier.ts` | `dev-proven` | `undocumented` |
...
### Alternatives Considered
1. **Build a custom card-issuance program directly on Avalanche**: Pros — full control; Cons — requires a card-network/regulatory relationship Knowgrph does not have and should not attempt to build for a pilot-scale product.
...
### Context
v0.1.0 implicitly assumed every settlement passes through StraitsX custody. Surfacing Core.app as a first-class wallet exposed a real question: should Knowgrph support paying a merchant directly on-chain when StraitsX custody isn't otherwise needed?
...
### Rationale
The MCP/SSE transport is strictly better min-pivot-max-value than a hand-built REST wrapper would have been: Knowgrph's probe-tree already speaks MCP natively, so this harness requires no protocol-translation layer at all — the smallest possible integration surface for the largest possible fit. The x402/EIP-3009 funding mechanism is accepted as discovered, not chosen from alternatives, since it's dictated by the vendor's own gateway design, not a Knowgrph decision point.
