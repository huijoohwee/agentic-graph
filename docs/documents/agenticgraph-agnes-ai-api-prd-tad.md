---
title: "AgenticGraph - Agnes AI API Integration: MainPanel -> FloatingPanel Chat"
doc_type: "PRD/TAD"
...
# AgenticGraph - Agnes AI API Integration: MainPanel -> FloatingPanel Chat PRD/TAD

## Purpose

This document defines the implementation-accurate product and technical contract for adding **Agnes AI API** to AgenticGraph through the **existing shared chat pipeline**, not through a parallel provider runtime.
...
- Production Cloudflare Pages shared `__chat_proxy` now also recognizes Agnes and MiroMind at the publish-repo owner boundary, so deployed `/agenticgraph` requests no longer depend on an OpenAI/BytePlus-only proxy contract.
...
AgenticGraph already has a strong downstream contract for AI-assisted document generation, but every new provider risks introducing:
...
As a AgenticGraph developer using MainPanel Integrations, I want to configure Agnes AI API as a shared chat provider so that FloatingPanel Chat can generate canonical KGC markdown through the existing prompt, streaming, workspace, and renderer pipeline without introducing provider-specific churn.
...
- Dev/preview owner: `agenticgraph/canvas/vite.config.ts`
...
- accept `AGENTICGRAPH_CHAT_PROXY_AGNES_API_KEY` or `AGNES_API_KEY` for server-managed auth,
...
- preserve provider parity with MiroMind and other shared chat-completions providers instead of letting the published Pages Function lag behind the AgenticGraph Dev proxy.
...
- `huijoohwee/docs/agenticgraph-storyboard-demo.md`
- `huijoohwee/docs/agenticgraph-video-demo.md`
- `huijoohwee/docs/agenticgraph-animatic-demo.md`
...
- Shared provider/runtime tests in `agenticgraph/canvas` are passing for Agnes provider normalization, shared chat-completions option shaping, and MainPanel Integrations readiness.
...
- A real deployed Agnes BYOK smoke remains pending only because no `AGNES_API_KEY` or `AGENTICGRAPH_CHAT_PROXY_AGNES_API_KEY` is available in the current execution environment.
...
Agnes behavior is governed by the shared AgenticGraph implementation surfaces above. Archived upstream copies are not kept as runtime authority.
