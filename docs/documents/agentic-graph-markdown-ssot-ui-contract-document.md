---
title: "Reference implementation: agentic-graph Markdown SSOT UI Contract"
id: "md:agentic-graph-markdown-ssot-ui-contract-document"
doc_type: "Document"
version: "1.1.0"
date: "2026-07-30"
lang: "en-US"
...
doc_path: "docs/documents/agentic-graph-markdown-ssot-ui-contract-document.md"
---

# Reference implementation: agentic-graph Markdown SSOT UI Contract
...
- Shared markdown logic must live in `agentic-graph/grph-shared` and be imported via `grph-shared/*` exports (no `grph-shared/src/*` runtime imports).
...
- Canonical derivation API: `useActiveGraphRenderData()` in `agentic-graph/canvas/src/hooks/useActiveGraphData.ts`.
...
- **Canonical implementation**: `agentic-graph/canvas/src/features/markdown-workspace/MarkdownWorkspaceToolbar.tsx` (`MarkdownWorkspaceToolbar`).
- **Required behavior**:
...
- **LocalStorage (UI state)**: keys are SSOT in `agentic-graph/canvas/src/lib/config.ls.ts`.
...
- **Dexie (workspace + cached sources)**: `agentic-graph/canvas/src/features/source-files/sourceFilesDb.ts`
...
- Floating panel (Inspector / Props / Tool Menu) is the baseline typography ladder for non-markdown panels.
  ...
  - Shared selector: `agentic-graph/canvas/src/lib/ui/panelTypography.ts` (returns `PanelTypography`).
  - `PanelTypography` SSOT type: `agentic-graph/grph-shared/src/ui/panelTypography.ts`.
...
Shared markdown logic is contractually owned by `agentic-graph/grph-shared`.
