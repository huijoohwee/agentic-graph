---
title: "Reference implementation: AgenticGraph Agentic Game OS core"
id: "md:agenticgraph-game-mmorpg-prd-tad"
...
constraints:
  ...
  - "device-local, local-first, offline-first persistence through the existing AgenticGraph storage owner"
...
# Reference implementation: AgenticGraph Agentic Game OS core

## Outcome and claim boundary

This candidate implements the native shared Agentic Game OS core and a thin
AgenticGraph browser-persistence facade. It is a persistent single-player strategy
...
that do not already host the AgenticGraph persistence adapter. It stores the same
...
`agenticgraph.inspect_game_os` and `agenticgraph.control_local_world`. The latter accepts
...
`agenticgraph.game-os-operation-result/v1` with a zero-cost record and nullable typed
...
Each response uses `agenticgraph.game-os-status/v1`, normalized entries,
...
`AgenticGraph repository-authored neutral mesh fixture`, licence `CC0-1.0`, source
...
Run from the AgenticGraph repository root:
...
swift test --scratch-path /tmp/agenticgraph-game-os-swift-golden --filter GameOsPersistentStrategyGoldenTests
swift test --scratch-path /tmp/agenticgraph-game-os-swift-full
```
...
- Canvas continuity delegates durable reads and CAS writes to
  `agenticgraphStorageEnginePersistence`; its adapter contains no IndexedDB schema;
...
`@agenticgraph/agentic-ecs` package exposes only its five bounded mutation/query
...
| AgenticGraph IndexedDB/tool projection facade | focused-tested | undocumented | install Persistent Strategy through the canonical scene adapter and visual UI owner |
...
|---|---|---|---|---|
| `game-os-core-integrate` | isolated authoring candidate | protected AgenticGraph main | protected review and exact candidate evidence required | closed |
| `game-os-package-consume` | protected AgenticGraph package | GameXR frontend lane | pinned export plus separate repository claim required | closed |
