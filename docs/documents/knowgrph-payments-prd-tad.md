---
title: "Reference implementation - Knowgrph Payments - PRD & TAD"
doc_type: "PRD+TAD"
...
# Reference implementation: Knowgrph Payments PRD/TAD
...
This document is the source-checked PRD/TAD projection of the `knowgrph-payments`
requirements at `.kiro/specs/knowgrph-payments/requirements.md` version 0.4.0.
...
|---|---|---|
| Payments requirements SSOT | `.kiro/specs/knowgrph-payments/requirements.md` | Version 0.4.0 remains the normative owner and accepts the R1-R17 contracts projected here. |
...
| Seller-side ACP checkout, Web3 settlement, Solana Pay, OpenBOX, proof and trace runtime | `docs/documents/knowgrph-agentic-commerce-prd-tad.md` | Remains the Knowgrph-as-seller runtime owner and the authority for server-owned Knowgrph offers and prices. This document adds a distinct buyer-side, third-party-merchant purchase lifecycle; it does not route a StraitsX-issued card through ACP, relax seller price authority, or duplicate the commerce proof runtime. |
...
| PS-14 | As a Buyer_SG I want the exact approved XSGD amount moved on the intended network into my KYC-verified provider account so that the issued card is backed without Knowgrph taking custody. | JX-Discover | R14 |
...
catalog. Knowgrph's local policy marks every hosted provider tool confirmation-required, and
...
Already credited XSGD remains in the buyer's provider account; Knowgrph never broadcasts an
...
- Knowgrph custody of XSGD, private keys, seed phrases, provider KYC documents, PAN, CVV, or
  full card expiry.
...
|---|---|---|
| Existing `knowgrph-payment` Cloudflare Worker and its D1 binding | No-new-fixed-infra, existing free-tier binding | Already the payment trust boundary; reuse avoids a new tier. |
...
transport. Knowgrph registers every hosted tool as confirmation-required; spend-bearing and
...
key or KYC document enters Knowgrph state and no automatic return transfer exists.
...
| Agent host → payment tool | Existing MCP transport | Typed tool schema; verified agent create receives derived `payment-action:<tokenId-or-issuedAt>` reference, never the raw token | Knowgrph marks hosted tools confirmation-required; `Approval_Gate` also required for state-changing or spend-bearing calls | `approval_missing`, `schema_invalid`; zero-cost pre-dispatch rejection; agent-create and refund services are callable only through this approved host boundary |
...
| `Funding_Adapter` → Avalanche-compatible RPC/signer | JSON-RPC plus external signer request | EVM transaction/receipt and signer result | Buyer-controlled or approved external signer; Knowgrph holds no private key | Wrong chain/token/address/amount/gas/signer fails before broadcast; provider credit remains separately required |
...
| Idempotency | Scope this contract to API v1: every POST carries a key of at most 255 characters derived from Client_Intent_Key and free of personal data. Stripe replays the first status/body, including `500`, can prune a key after at least 24 hours, and rejects changed parameters. Knowgrph retains durable uniqueness beyond that window ([idempotent requests](https://docs.stripe.com/api/idempotent_requests)) |
...
| StraitsX REST tools | StraitsX_Rail_Adapter behind Knowgrph tools | R4, R9 | `dev-proven` | `undocumented` |
...
Knowgrph transports. Mark every hosted tool confirmation-required in local policy; additionally
...
2. Knowgrph-held wallet/private key [FOSS libraries]: creates custody, key security, recovery,
...
**Consequences**
- **Positive**: no Knowgrph key custody; wrong-chain/address failures occur before value moves.
...
| Worker | Existing payment trust boundary | `cloudflare/workers/knowgrph-payment/index.ts`, `payments.ts`, `stripeHostedCheckout.ts`, `paymentRuntimeFailures.ts`, `paymentRuntimeRoutes.ts` | `dev-proven` | `undocumented` | The sole Worker delegates provider-neutral routes while hosted-checkout, typed-failure, read-only `agentic_purchase_readiness`, and denial-only public agent/refund ownership stay split into bounded modules |
...
| Worker | Configuration owner | `cloudflare/workers/knowgrph-payment/wrangler.toml` | `dev-proven` | `undocumented` | Secret-name and visible-variable checks pass; configured secrets and provider grants remain external |
| Store | Payment runtime ledgers | `cloudflare/d1/migrations/0009_knowgrph_payment_runtime.sql` | `dev-proven` | `undocumented` | Additive intent, event, and cost schema passes focused local persistence checks |
| Store | Lifecycle, funding reservation, approval, opaque card, authorization, receipt, and disposal state | `cloudflare/d1/migrations/0010_knowgrph_agentic_purchase_lifecycle.sql` | `dev-proven` | `undocumented` | Real migration is exercised by deterministic SQLite race/restart tests; no remote migration was run |
| Operator | Source-bound readiness and local VCC | `check-knowgrph-payments-readiness.mjs`, `run-knowgrph-payments-local-vcc.mjs`, `knowgrph-agentic-purchase-readiness.mjs` | `dev-proven` | `undocumented` | Source and five-suite local gates pass; provider, browser, protected, mirror, and delivery gates remain separately blocked |
...
| Reference | Advisory local provider captures | `docs/documents/knowgrph-api-reference/` | `spec-complete` | `undocumented` | Source bindings recorded below; current official sources remain normative |
...
| MCP tools | `knowgrph.payment.rail.select`, `knowgrph.payment.intent.create`, `knowgrph.payment.status`, `knowgrph.payment.event.settle`, `knowgrph.payment.reconcile`, `knowgrph.payment.receipt.project`, `knowgrph.payment.refund`, `knowgrph.payment.readiness` | Agentic Canvas OS MCP gateway | Status/readiness are read-only; intent/refund require approval; execution is zero-model-call |
...
|---|---|---|---|
| [Stripe API](https://docs.stripe.com/api) | `docs/documents/knowgrph-api-reference/knowgrph-stripe-api-reference.md` | R1, R3, R10, R12 | Normative root; supplemented by exact official pages for v1 idempotency, errors, webhooks, keys, versions, and object states |
| [Stripe MCP](https://docs.stripe.com/mcp) | `docs/documents/knowgrph-api-reference/knowgrph-stripe-mcp-reference.md` | R9, ADR-4 | Normative current inventory/auth source; whole-server Public Preview and two stale resource-tool claims removed |
| [StraitsX introduction](https://docs.straitsx.com/docs/introduction) | `docs/documents/knowgrph-api-reference/knowgrph-straitsx-api-reference.md` | R4, R12, ADR-2 | Normative model/product root; Customer Profiles prerequisite and account-grant boundary retained |
| [StraitsX Say Hello](https://docs.straitsx.com/reference/say-hello) | `docs/documents/knowgrph-api-reference/knowgrph-straitsx-authentication-reference.md` | R1, R4, R11 | Normative connection probe; HTTP `200` is connectivity/authentication evidence only |
