---
title: "AgenticGraph Markdown Agent Discovery"
...
source_of_truth:
  ...
  - "cloudflare/workers/agenticgraph-storage/db.ts"
publish_policy: "Dev -> generated Prod mirror -> operator-authorized Cloudflare deploy"
---

# AgenticGraph Markdown Agent Discovery
...
2. `/agenticgraph/llms.txt` describes the AgenticGraph application surface.
...
Legacy `/agenticgraph/openapi.json` and `/agenticgraph/api-catalog.json` requests use permanent redirects to their canonical well-known resources. They must never return the SPA HTML fallback with HTTP 200.
