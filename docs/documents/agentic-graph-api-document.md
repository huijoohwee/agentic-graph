# agentic-graph API Surfaces (Dev/Preview)

agentic-graph Canvas is primarily a client-side app. Local “API” surfaces used by the UI are implemented as Vite middleware during **dev** and **preview** runs.
...
- Runtime boundary: iframe rendering remains owned by agentic-graph. External hosts may size or position the frame but must not reimplement canvas state.
...
- Home and MainPanel import: **Import canvas embed** is available on Home and under **MainPanel Settings → Canvas Embed**. Both entry points reuse the shared canvas-embed import panel and accept the generated iframe HTML or a JSON `postMessage` envelope with `type: "agentic-graph.canvas-embed.select"`, `version: 1`, and either `embedUrl` or `iframe`. The optional `sourcePath` preserves source identity. Generic imports normalize `kgPreview=1&kgLiveHero=1`. The canonical cold-start preset resolves the published Physics Playground identity through `XR_PHYSICS_DEMO_PUBLISHED_CANONICAL_PATH`; its source frontmatter selects XR/3D without renderer query overrides, and Settings exposes it as **Use Physics Playground background**.
...
- Path: `/agentic-os/control-plane/mcp` (POST, MCP initialize and `tools/call` for `agentic-graph.agentic_canvas_os.docs.invoke`).
...
- Implementation: [superagent_harness.py](../../agentic_graph_parser/superagent_harness.py), local stdio transport in [server.js](../../mcp/server.js), and local MCP tool-contract ownership in [local-tool-contract.js](../../mcp/local-tool-contract.js)
...
- MCP tools: `agentic-graph.sandbox.policy.validate` and `agentic-graph.sandbox.policy.authorize`
...
- Contract source: [README.md](../../mcp/README.md) and [agentic-graph-mcp-service-prd-tad-adr-mvp-gtm.companion.md](agentic-graph-mcp/agentic-graph-mcp-service-prd-tad-adr-mvp-gtm.companion.md)

### Agentic video workflow (local and control-plane MCP)

- MCP tool: `agentic-graph.video_remix.run`
...
- Automated image-prompt input/output: `workflow.imagePromptPolicy` controls enablement, prior-timeline/reference directives, coordinate precision, interaction distance, and strict spatial blocking. `workflow.imageGeneration` returns one canonical prompt, semantic input key, reuse state, and structured spatial plan per shot; unchanged checkpoint prompts are reused byte-for-byte, while `imagePrompt` reaches provider dispatch, AGENTIC_OS nodes, checkpoints, and VLM expectations without downstream recomputation.
...
- Schema owner: [workflow-contract.js](../../mcp/video-remix/workflow-contract.js), reused by [local-tool-contract.js](../../mcp/local-tool-contract.js) and the Cloudflare [tool-registry.mjs](../../cloudflare/workers/agentic-graph-mcp/tool-registry.mjs).
...
- Boundary: ViMax informs the capability checklist only. agentic-graph imports no ViMax code, prompt, provider configuration, fixture, test, or runtime dependency. Prod and Cloudflare remain deployment-gated.

### API-native browser MCP bridge

- MCP tool: `agentic-graph.browser_api.run`
...
- Implementation: [vite.config.ts](../../canvas/vite.config.ts)
- Validation: video-agent external test URLs are supplied through `AGENTIC_OS_VIDEO_AGENT_TEST_URLS` for focused tests, and through the user-configurable `VIDEO_AGENT_VALIDATION_CONFIG_STORAGE_KEY` / `VITE_AGENTIC_OS_VIDEO_AGENT_VALIDATION_URLS` runtime config for the app. Both paths are exercised by the same Toolbar Launch Import URL flow, which calls this endpoint and then materializes source-owned video markdown with video corpus metadata without committing URL fixtures in repo source. Director-style orchestration is represented only through the native `VIDEO_AGENT_REFERENCE_BOUNDARY` contract: inspiration-only, native agentic-graph implementation, no external code copy, and no external video-agent runtime dependency.
...
- Runtime contract: engine adapters are registered through `globalThis.agenticGraphHtmlVideoEngines` and resolved by `createHtmlVideoEngineRegistryFromRuntimeConfig()`. Missing engines fail closed with structured `engine_not_configured` errors; there is no endpoint-level or client-level fallback engine.
...
These middleware endpoints exist for local development and preview builds. The successor production target is split by concern; it is not evidence that the corresponding Worker has been deployed or cut over:

- Static SPA: `agentic-graph/canvas/dist` -> `huijoohwee/content/agentic-graph` -> Cloudflare Pages at `airvio.co/agentic-graph`
- Storage API: `cloudflare/workers/agentic-graph-storage` -> Cloudflare Worker `agentic-storage` at `airvio.co/api/storage/*`
...
- Payments API: `cloudflare/workers/agentic-graph-payment` -> Cloudflare Worker `agentic-payment` at `airvio.co/api/payments/*`
...
Route owner: `huijoohwee/functions/__chat_proxy/[[path]].js`. Public route: `airvio.co/__chat_proxy/*`. Shared helper owner: `huijoohwee/functions/api/_integrationHub.js`. Deploy owner: `npm run pages:deploy-cloudflare` from `agentic-graph`, which uploads the `huijoohwee` publish repo to Cloudflare Pages project `joohwee`.
...
`x-kg-ai-gateway-cache-ttl`. When `AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_BASE_URL` plus a matching
...
|---|---|---|
| `AGENTIC_OS_CHAT_PROXY_OPENAI_API_KEY` | Server-managed OpenAI proxy key | Cloudflare Pages project secret |
| `AGENTIC_OS_CHAT_PROXY_MIROMIND_API_KEY` | Server-managed MiroMind proxy key | Cloudflare Pages project secret |
| `AGENTIC_OS_CHAT_PROXY_AGNES_API_KEY` | Server-managed Agnes proxy key | Cloudflare Pages project secret |
| `AGENTIC_OS_CHAT_PROXY_SEALION_API_KEY` | Server-managed AI Singapore SEA-LION proxy key | Cloudflare Pages project secret |
| `AGENTIC_OS_CHAT_PROXY_BYTEPLUS_API_KEY` | Server-managed BytePlus proxy key | Cloudflare Pages project secret |
...
| `AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_BASE_URL` | Cloudflare AI Gateway universal base URL. **Legacy:** `https://gateway.ai.cloudflare.com/v1/<account_id>/<gateway_id>`. **Modern REST API:** `https://api.cloudflare.com/client/v4/accounts/<account_id>/ai` for `/run` and `/v1/chat/completions` | Cloudflare Pages project variable |
| `AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_TOKEN` | Server-managed Cloudflare AI Gateway token | Cloudflare Pages project secret |
| `AI_GATEWAY_TOKEN` | Fallback AI Gateway token alias | Cloudflare Pages project secret; cleanup debt only |
| `AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_GATEWAY_ID` | Optional explicit gateway id header override | Cloudflare Pages project variable |
| `AGENTIC_OS_CHAT_PROXY_UPSTREAM` | Optional upstream base override | Cloudflare Pages project variable |
| `AGENTIC_OS_CHAT_PROXY_TIMEOUT_MS` | Upstream timeout override | Cloudflare Pages project variable |
| `AGENTIC_OS_INTEGRATION_ALLOWED_HOSTS` | Canonical additional host allowlist | Cloudflare Pages project variable |
| `AGENTIC_OS_CHAT_PROXY_ALLOWED_HOSTS` | Legacy host allowlist alias | Cloudflare Pages project variable; cleanup debt only |

#### GitHub To Cloudflare Deploy Chain For This Route

1. Edit the production route owner in `huijoohwee/functions/__chat_proxy/[[path]].js` or update the canonical source/deploy logic in `agentic-graph`.
2. From `agentic-graph`, run `npm run pages:build-sync` so the publish repo mirror stays aligned.
3. From `agentic-graph`, run `npm run pages:functions:build` to build the Pages Functions bundle and regenerate `huijoohwee/_worker.js` plus `huijoohwee/_routes.json`.
4. From `agentic-graph`, run `npm run pages:deploy-cloudflare`, which executes `wrangler pages deploy ../huijoohwee --project-name=joohwee --branch=main --commit-dirty=true`.
...
### Storage Worker: `agentic-storage`

Route owner: `cloudflare/workers/agentic-graph-storage`; deployed Cloudflare Worker: `agentic-storage`. Cloudflare route: `airvio.co/api/storage/*`. The Worker owns the D1 query layer through `drizzle-orm` and schema through Wrangler SQL migrations; browser storage is cache-only and is not the canonical persistence surface.
...
| GET upgrade | `/api/storage/canvas-room/{workspaceId}/{roomId}` | Join an authenticated room; `roomId=runtime-identity:agentic-graph:main` is application-global and permits only ping, challenge, attestation, and server peer-lifecycle messages, with no identity or asset persistence |
...
| Browser env | Purpose |
|---|---|
| `VITE_AGENTIC_OS_STORAGE_BASE_URL` | Absolute origin for storage Worker browser calls |
| `VITE_AGENTIC_OS_STORAGE_WORKSPACE_ID` | Workspace identity resolved by the browser storage/chat client |
| `VITE_AGENTIC_OS_STORAGE_CHAT_SESSION_TOKEN` | Authenticated session token forwarded as `Authorization: Bearer ...` to chat routes and `agentic_os_session_token` on the canvas-room WebSocket upgrade |
...
### Payment Worker: `agentic-payment`

Route owner: `cloudflare/workers/agentic-graph-payment`; deployed Cloudflare Worker: `agentic-payment`. Cloudflare route: `airvio.co/api/payments/*`.
...
Required production runtime configuration lives on `agentic-payment`, not on Cloudflare Pages project variables:
...
|---|---|---|---|---|
| OpenAI chat proxy | Bearer API key | `AGENTIC_OS_CHAT_PROXY_OPENAI_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| BytePlus ModelArk chat proxy | Bearer API key | `AGENTIC_OS_CHAT_PROXY_BYTEPLUS_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| MiroMind chat proxy | Bearer API key | `AGENTIC_OS_CHAT_PROXY_MIROMIND_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| Agnes AI chat proxy | Bearer API key | `AGENTIC_OS_CHAT_PROXY_AGNES_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| AI Singapore SEA-LION chat proxy | Bearer API key for `https://api.sea-lion.ai/v1` | `AGENTIC_OS_CHAT_PROXY_SEALION_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| AI Singapore SEA-LION MCP sidecar | Bearer API key | `AGENTIC_OS_MCP_SEALION_API_KEY` on the local MCP host process | No browser path | Never |
| Qwen chat proxy | Bearer API key | `AGENTIC_OS_CHAT_PROXY_QWEN_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| Google Cloud Vertex AI chat proxy | OAuth access token | `AGENTIC_OS_CHAT_PROXY_GOOGLE_CLOUD_ACCESS_TOKEN` on the Vite or server proxy runtime | Yes | RAM only |
| Google Gemini chat proxy | Gemini API key via `x-goog-api-key` | `AGENTIC_OS_CHAT_PROXY_GEMINI_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| GrabMaps API | Bearer token | `AGENTIC_OS_GRABMAPS_API_TOKEN` on the API proxy runtime | Yes | RAM only |
| GrabMaps MCP | Bearer token | `AGENTIC_OS_GRABMAPS_MCP_TOKEN` on the MCP host process | No browser path | Never |
| Stripe Checkout and webhook | Restricted or secret API key | Worker secret `STRIPE_RESTRICTED_KEY` or `STRIPE_SECRET_KEY` on `agentic-payment` | No | Never |
| Solana Pay settlement | RPC URL or private binding | Worker secret/private binding `SOLANA_PAY_RPC_URL` on `agentic-payment` | No | Never |
...
| Hardcode API key in `.env.example` | Leaks a real or plausible key; misleads contributors | Use `${AGENTIC_OS_<PROVIDER>_API_KEY}` placeholder text |
