...
continuity_id: "PLAN-AGENTIC-GRAPH-SINGLE-FILE-HTML-PAGE-EXPORT"
...
composition of the export paths already present in `agentic-graph/canvas`.
...
A agentic-graph workspace is not just a graph. The useful shareable state is the document/source workspace plus
...
requiring a live agentic-graph session. Current separate HTML actions can export the document viewer or the canvas,
...
| Return | User reopens artifact offline. | Browser | Export should not require agentic-graph, Cloudflare, or source repo access. | Standalone HTML shell with both surfaces. |
...
- Add realtime sync from exported file back to agentic-graph.
...
- FORBID manual edits to `$GITHUB_ROOT/huijoohwee/content/agentic-graph`; production mirror updates come from the source-owned sync flow.

## Quality Attributes

| Attribute | Scenario | Validation |
|---|---|---|
| Portability | Open exported file without running agentic-graph. | Offline browser smoke has no critical console errors for core sections. |
...
Source changes begin in `$GITHUB_ROOT/agentic-graph`. The production mirror at
`$GITHUB_ROOT/huijoohwee/content/agentic-graph` is generated, not manually patched.
Cloudflare proof lives at `https://airvio.co/agentic-graph` after the normal Pages sync/deploy flow.
...
All five roles below consume `PLAN-AGENTIC-GRAPH-SINGLE-FILE-HTML-PAGE-EXPORT@1.1.3`. Existing source and runtime observations retain their original revisions and scope; this documentation update renews no deployment or demand evidence. The guideline is [v2.7.0](https://github.com/huijoohwee/huijoohwee.github.io/blob/e8d2a10a8d3e5735c43edf350a22523df05fdf91/guidelines/prd-tad-adr-mvp-gtm-guidelines.md); the shared maturity rubric loads on demand.

| Role | Owning content at this revision |
|---|---|
| PRD | [Product Requirements](agentic-graph-single-file-html-page-export.md#product-requirements) |
| TAD | [Technical Architecture](agentic-graph-single-file-html-page-export.md#technical-architecture) |
| ADR | [Implementation Decisions](agentic-graph-single-file-html-page-export.md#implementation-decisions) |
| MVP | [MVP — reference implementation](agentic-graph-single-file-html-page-export.md#mvp--reference-implementation) |
| GTM | [GTM — reference implementation](agentic-graph-single-file-html-page-export.md#gtm--reference-implementation) |
