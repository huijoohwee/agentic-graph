---
title: "Reference implementation: agentic-graph-crawler-prd-tad-adr-mvp-gtm section 1"
...
Cloudflare's current Pay Per Crawl flow lets verified AI crawlers send signed `crawler-exact-price` or `crawler-max-price` headers. Those headers are Cloudflare/Web Bot Auth concerns, not agentic-graph app state.
...
| Site Owner | Publishes agentic-graph content and controls crawler access policy | Make Source Files discoverable while allowing Cloudflare to negotiate Pay Per Crawl access when enabled | App-local payment or crawler-detection logic would conflict with zone-level policy and create stale behavior |
...
- **Given** a deployed agentic-graph app
...
flowchart TB
    ...
    crawler -->|"GET /agentic-graph/llms.txt or app HTML"| appStatic
...
| Contract | Storage route constants | `canvas/src/lib/storage/agentic-graph-storage-sync-contract.ts` | Centralize route paths, default workspace, Pay Per Crawl URL, and crawler header names | Implemented |
| Worker route contract | Worker contract mirror | `cloudflare/workers/agentic-graph-storage/contract.ts` | Reuse the same route and header semantics inside the Worker | Implemented |
| Worker router | Storage API entrypoint | `cloudflare/workers/agentic-graph-storage/index.ts` | Delegate crawler routes before generic storage handling | Implemented |
| Crawler handler | Source Files index builder | `cloudflare/workers/agentic-graph-storage/crawler.ts` | Read D1 documents and build markdown or text crawler responses | Implemented |
...
| Verification | Storage contract tests | `canvas/src/__tests__/agentic-graph-storage-contracts.test.ts` | Guard centralized routes and Pay Per Crawl header names | Implemented |
| Verification | Worker crawler tests | `canvas/src/__tests__/agentic-graph-storage-worker.test.ts` | Prove index, LLM text, deleted suppression, headers, and doc links | Implemented |
...
sequenceDiagram
    ...
    C->>S: GET /agentic-graph/llms.txt
