---
title: "Reference implementation: agentic-graph Cloud Platform Product and Technical Specification"
id: "md:agentic-graph-cloudflare-document"
...
doc_path: "docs/documents/agentic-graph-cloudflare-document.md"
...
# Reference implementation: agentic-graph Cloud Platform Product and Technical Specification

## Reference implementation: Authority and readiness

This combined PRD/TAD owns the platform boundary between agentic-graph source, its generated Pages
...
| Agent discovery requirements and surface counts | [Agent-ready PRD/TAD](agentic-graph-agent-ready-prd-tad.md) |
...
| Checkout, settlement, and payment-worker behavior | [Payments PRD/TAD](agentic-graph-payments-prd-tad.md) |
...
`canvas/src/lib/storage/agentic-graph-storage-route-paths.ts`; exact MCP tools remain owned by their
...
1. Before dispatch, the repository-owned evidence producer emits
   `agentic-graph-production-release-evidence/v1`, content-addressing every preserved frontier lane and the exact
...
|---|---|---|---|---|
| structured `push`, `pull`, and workspace `export` | `cloudflare/workers/agentic-graph-storage/index.ts` | D1 | no authentication or membership check; caller supplies workspace/device identity | an exposed deployment could mutate or disclose another workspace |
| generic blob read/write | `cloudflare/workers/agentic-graph-storage/blob.ts` | R2 | no authentication or entitlement check; same key is overwriteable | an exposed deployment could disclose or replace bytes |
| run-media read/write | `cloudflare/workers/agentic-graph-storage/media.ts`, `mediaAuth.ts` | R2 | token checks only `{runId, expiresAt}` decoded from unsigned base64url JSON | caller can forge a matching unexpired token |
| media-asset metadata GET at `/api/storage/media/assets` | `cloudflare/workers/agentic-graph-storage/mediaAssetSync.ts` | D1 | no authentication; caller supplies `workspaceId` | response exposes artifact/object/provenance metadata |
| media-asset metadata mutation at `/api/storage/media/assets` | `cloudflare/workers/agentic-graph-storage/mediaAssetSync.ts` | D1, R2, optional KV/room | same unsigned run token as run-media | not issuer-backed authorization |
...
- The verify job binds the exact `main` revision and
  ...
  clean agentic-graph and Agentic Canvas OS `main` checkouts at the candidate-bound fetched revisions
...
| Storage | `cloudflare/workers/agentic-graph-storage/` | `spec-complete` | `undocumented` |
| Payment | `cloudflare/workers/agentic-graph-payment/` | `spec-complete` | `undocumented` |
| MCP control | `cloudflare/workers/agentic-graph-mcp/` | `spec-complete` | `undocumented` |
| Research | `cloudflare/workers/agentic-graph-research/` | `spec-complete` | `undocumented` |
| Fetch proxy | `cloudflare/workers/agentic-graph-fetch-proxy/` | `spec-complete` | `undocumented` |
