...
ADR: captured timestamps remain historical; local import grants no session, paging, evaluation or execution capability. A 256-KB/32-span bound and 60-second in-memory handoff preserve zero/unknown resources, causal links, source identity and authored documents. Ordinary JSON keeps the existing importer. MVP: baseline `f4d4e766a3f092ba7991cef6c0eae2cddf966210`, 45 active minutes, 17 Graph files and 5 OS files, 100 KB edits; verify export/import, mobile/desktop D3 fit/selection, unchanged authored state and the existing complete agent lifecycle. GTM: join native preparation/check/CI/integration/cleanup/sync/runtime receipts using the OS workflow exporter; compare measured time/resource evidence without inferring savings or release authority. Rollback restores the prior source and OS pin; private records remain in `.workspace/.artifacts`.
