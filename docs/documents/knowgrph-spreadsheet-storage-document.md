...
- Source Files local mirroring cannot redirect a spreadsheet seed through the general docs-root setting: repository-local Dev reads use the exact running Knowgrph checkout or worktree, browser mutations carry only a logical `/docs/workspace-seeds/**` `workspacePath`, and the server bridge derives the authorized `$GITHUB_ROOT/knowgrph/docs/workspace-seeds/**` host target. Client absolute mutation paths or mutation-root environment variables, invalid logical paths, unkeyed writes, and root deletion are rejected.
