...
- Source Files local mirroring cannot redirect a spreadsheet seed through the general docs-root setting: repository-local Dev reads use the exact running Knowgrph checkout or worktree, supported mutations retain the authorized `$GITHUB_ROOT/knowgrph/docs/workspace-seeds/**` root, and the host bridge rejects mismatched paths and root deletion.
