---
title: "agentic-graph XR Invocation Runtime API"
id: "md:agentic-graph-xr-invocation-runtime-api"
...
# agentic-graph XR Invocation Runtime API
...
| MCP inspection and mutation | `agentic-graph.inspect_local_immersive_media` and `agentic-graph.control_local_immersive_media` |
...
`docs/documents/agentic-graph-ar-vr-xr-prd-tad-adr-mvp-gtm.md`. Runtime surfaces that expose XR entry,
...
|---|---|---|
| `agentic-graph-xr-capability-snapshot/v1` | Declares what the current browser can do | one exact `recommended_entry_mode` plus downgrade `reason_codes` |
| `agentic-graph-xr-capture-draft/v1` | Records the first-slice phone capture intent | first slice admits `capture_mode: monocular-camera` only |
| `agentic-graph-xr-viewer-handoff/v1` | Records how the source opened | exact `selected_mode`, `fallback_mode`, and `target_route` |
...
- XR physics delegates fixed stepping to agentic-graph's independently authored spatial engine. The Rapier repository informed domain-separation principles only; no external source, prose, schema, algorithm, example, fixture, package, compatibility layer, service, or runtime dependency is admitted.
