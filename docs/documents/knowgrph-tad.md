...
## Reference implementation: Knowgrph repository
...
| Offline transformer | `knowgrph_parser/` | Python CLI/parser/harness, not a web service |
| Local transport | `mcp/server.js` and `mcp/local-tool-contract.js` | stdio; broad descriptors with configuration-gated executors |
| Remote control adapter | `cloudflare/workers/knowgrph-mcp/` | ten-tool source registry; separate deployment and bearer boundary |
| Working store | browser IndexedDB/Dexie plus memory fallback | local documents/chunks/snapshots/outbox/cursor |
| Shared adapter | `cloudflare/workers/knowgrph-storage/` | D1/R2/Durable Object source owners; optional KV is not assumed live |
...
- Product requirements: `docs/documents/knowgrph-prd.md`
- Decision companion: `docs/documents/knowgrph-architecture-decisions.md`
- Protected release runbook: `docs/knowgrph-acos-deploy-runbook.md`
