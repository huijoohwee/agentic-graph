...
id: "md:agentic-graph-feishu-base-mcp-prd-tad.companion"
...
doc_path: "docs/documents/agentic-graph-mcp/agentic-graph-feishu-base-mcp-prd-tad-adr-mvp-gtm.companion.md"
guideline_version: "1.7.0"
reference_implementation_label: "reference implementation"
parent: "agentic-graph-feishu-base-mcp-prd-tad-adr-mvp-gtm.md"
parent_version: "0.5.0"
frontmatter_contract: "required"
continuity_id: "PLAN-AGENTIC-GRAPH-FEISHU-BASE-MCP-PRD-TAD-ADR-MVP-GTM"
...
[the parent contract](agentic-graph-feishu-base-mcp-prd-tad-adr-mvp-gtm.md). It is not a second
...
| Server public contract | `canvas/src/lib/storage/agentic-graph-storage-sync-contract.ts` and `agentic-graph-storage-route-paths.ts` | `agentic-graph-knowledge-source/v1`, complete snapshot types, and the handoff/read paths. | Lark-specific source authority |
| Server runtime | `cloudflare/workers/agentic-graph-storage/knowledge-source/` | Server identity, versioned allowlist, Lark read provider, provenance/digests, opaque handoff, and typed block/failure results. | Write-back or source acceptance |
...
|---|---|
| API | `agentic-graph-knowledge-source/v1` |
| Allowlist | `agentic-graph-knowledge-source-allowlist/v1` with `{ schema, revision, sources[] }` |
...
| Snapshot | `agentic-graph-knowledge-source-snapshot/v1`, `complete: true`, with allowlist provenance, page/field/record/document/byte counts, content/envelope digests, sanitized snapshot, and warnings |

Identity is selected only by `AGENTIC_OS_STORAGE_LARK_IDENTITY_MODE`. A
`tenant-app` configuration uses server-held
`AGENTIC_OS_STORAGE_LARK_APP_ID`/`AGENTIC_OS_STORAGE_LARK_APP_SECRET`; a
`user-oauth` configuration uses an external broker/operator-managed server-held
`AGENTIC_OS_STORAGE_LARK_USER_ACCESS_TOKEN` plus required absolute expiry and
...
`AGENTIC_OS_STORAGE_LARK_SOURCE_ALLOWLIST_JSON`. The browser cannot select an
identity mode or replace a Base/table/view binding. The server-only
`AGENTIC_OS_STORAGE_SIGNING_SECRET` seals the five-minute AES-GCM AEAD handoff.
...
| Resolve source handoff | `agentic-graph-storage` knowledge-source runtime | Authenticated member receives a five-minute AEAD bearer capability | No Lark content read when configuration blocks |
| Read allowlisted Base | `agentic-graph-storage` Lark provider | Creates one complete in-memory snapshot envelope | Read-only Lark API calls |
...
network endpoints. `agentic-graph-storage` is the provider-neutral read authority;
...
Canonical agentic-graph endpoint selection remains in
...
| `VCC-FB-C-05` | Standalone `cloudflare/workers/agentic-graph-storage/knowledge-source/*.test.ts` invocation | Membership-gated issuance, Base revision/total/approved-field/minimum invariants, exact POST search, zero-fetch blockers, redaction, complete digests, and AEAD handoff hold under fixtures. This file set is not yet wired into `storage:relay:test`. | None recorded |
