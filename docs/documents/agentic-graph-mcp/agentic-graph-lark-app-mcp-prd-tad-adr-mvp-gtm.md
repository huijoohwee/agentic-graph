...
doc_path: "docs/documents/agentic-graph-mcp/agentic-graph-lark-app-mcp-prd-tad-adr-mvp-gtm.md"
guideline_version: "1.7.0"
reference_implementation_label: "reference implementation"
companion: "agentic-graph-lark-app-mcp-prd-tad-adr-mvp-gtm.companion.md"
traceability:
...
`agentic-graph-storage` boundary; Git-backed Markdown/frontmatter remains the
...
| `cloudflare/workers/agentic-graph-storage/knowledge-source/` | Owns server identity, allowlist admission, exact Lark reads, complete snapshot digests, opaque handoff, and typed failures. | Read-only; no source acceptance, write-back, or deploy authority. |
...
agentic-graph Invocation Register and endpoint owner is
[the MCP installation contract](../agentic-graph-mcp-install-contract.md). The
...
agentic-graph for review without leaking credentials/configured provider identifiers, accepting
...
|---|---|---|
| `PRD-LARK-01` | Keep agentic-graph endpoint ownership solely in the canonical MCP installation contract. | Must |
...
| `PRD-LARK-08` | Own provider reads only in `agentic-graph-storage`; keep Git-backed Markdown/frontmatter authoritative. | Must |
| `PRD-LARK-09` | Resolve identity/resources only from `AGENTIC_OS_STORAGE_LARK_*` and `agentic-graph-knowledge-source-allowlist/v1`. | Must |
...
| `PRD-LARK-12` | Emit only complete `agentic-graph-knowledge-source-snapshot/v1` envelopes and require Canvas to bounded-stream, validate, and recompute content/envelope digests before mutation. | Must |
...
|---|---|---|
| `PRD-LARK-01` | Given this document, when an operator needs a agentic-graph endpoint, then exactly one link resolves to the canonical register and no endpoint is duplicated here. | `VCC-LARK-01` |
...
| `PRD-LARK-08` | Given a provider read, when ownership is traced, then only `agentic-graph-storage` contacts Lark and the result remains a candidate projection. | `VCC-LARK-08` |
...
| Managed 12-month incremental agentic-graph TCO | USD 0 for browser-local source path; Lark/Worker cost unmeasured | Measure provider/Worker request and egress cost before delivery. |
...
The public API version is `agentic-graph-knowledge-source/v1` and the only agentic-graph
routes are:

- `POST /api/storage/knowledge-source/handoff` to authenticate the agentic-graph
...
The `agentic-graph-knowledge-source-allowlist/v1` resource variants and exact reads
are:
...
`AGENTIC_OS_STORAGE_SIGNING_SECRET` seals each five-minute AES-GCM AEAD handoff. In
...
Success uses `agentic-graph-knowledge-source-snapshot/v1` with `complete: true`,
...
1. The agentic-graph target is selected from the canonical MCP installation
   contract.
...
5. `agentic-graph-storage` revalidates the binding, performs exact bounded
...
| Acquire | Lark provider behind `agentic-graph-storage` | Allowlisted Base/Wiki/Doc plus Base fields/minimum | Stable-revision/total bounded pages/content | Request memory only | Entire read fails on incomplete, hidden/missing field, low count, or drift |
...
| Discover | Resolve config/opaque handoff | Ingest | Auth/allowlist guard | `agentic-graph-storage` knowledge-source runtime |
...
Keep provider reads server-only in provider-neutral `agentic-graph-storage`; keep
...
|---|---|---|---|---|
| `VCC-LARK-01` | From repository root: `node -e 'const fs=require("node:fs"); const files=process.argv.slice(1); const text=files.map(f=>fs.readFileSync(f,"utf8")).join("\n"); const copied=["https:/","/airvio.co"].join(""); if(text.includes(copied) \|\| !text.includes("../agentic-graph-mcp-install-contract.md") \|\| !fs.existsSync("docs/documents/agentic-graph-mcp-install-contract.md")) process.exit(1)' docs/documents/agentic-graph-mcp/agentic-graph-lark-app-mcp-prd-tad-adr-mvp-gtm.md docs/documents/agentic-graph-mcp/agentic-graph-lark-app-mcp-prd-tad-adr-mvp-gtm.companion.md` | No copied target appears and the install-contract link resolves. | Deterministic source check. | None recorded |
...
| `VCC-LARK-08` | From repository root, standalone: `TSX_TSCONFIG_PATH=canvas/tsconfig.json node --import tsx --test --test-concurrency=1 cloudflare/workers/agentic-graph-storage/knowledge-source/*.test.ts` | Provider-neutral handoff/read ownership passes focused fixtures and no provider result becomes accepted source. | Mocked transport; not yet wired into `storage:relay:test`; no live/delivery claim. | None recorded |
| `VCC-LARK-09` | Run the focused knowledge-source suite in `VCC-LARK-08`. | Only server env plus `agentic-graph-knowledge-source-allowlist/v1` select identity/resources; caller overrides fail. | No secrets in fixtures/output. | None recorded |
...
See [the companion](agentic-graph-lark-app-mcp-prd-tad-adr-mvp-gtm.companion.md) for file-level
...
All five roles below consume `PLAN-AGENTIC-GRAPH-LARK-APP-MCP-PRD-TAD-ADR-MVP-GTM@0.5.1`. Existing source and runtime observations retain their original revisions and scope; this documentation update renews no deployment or demand evidence. The guideline is [v2.7.0](https://github.com/huijoohwee/huijoohwee.github.io/blob/e8d2a10a8d3e5735c43edf350a22523df05fdf91/guidelines/prd-tad-adr-mvp-gtm-guidelines.md); the shared maturity rubric loads on demand.

| Role | Owning content at this revision |
|---|---|
| PRD | [PRD](agentic-graph-lark-app-mcp-prd-tad-adr-mvp-gtm.md#prd) |
| TAD | [TAD](agentic-graph-lark-app-mcp-prd-tad-adr-mvp-gtm.md#tad) |
| ADR | [Architectural decision](agentic-graph-lark-app-mcp-prd-tad-adr-mvp-gtm.md#architectural-decision) |
| MVP | [MVP — reference implementation](agentic-graph-lark-app-mcp-prd-tad-adr-mvp-gtm.md#mvp--reference-implementation) |
| GTM | [GTM — reference implementation](agentic-graph-lark-app-mcp-prd-tad-adr-mvp-gtm.md#gtm--reference-implementation) |
...
Source review is bounded to repository `7fb85741121d8c2886027e4a630d013ba91c1027`. Confirmed: these referenced artifacts exist at that revision: [`grph-shared/src/search/larkAppMcpSsot.ts`](https://github.com/huijoohwee/agentic-graph/blob/7fb85741121d8c2886027e4a630d013ba91c1027/grph-shared/src/search/larkAppMcpSsot.ts), [`canvas/src/features/panels/views/larkAppMcpApiDocs.ts`](https://github.com/huijoohwee/agentic-graph/blob/7fb85741121d8c2886027e4a630d013ba91c1027/canvas/src/features/panels/views/larkAppMcpApiDocs.ts), [`canvas/src/features/panels/views/settingsMcpDocEntries.ts`](https://github.com/huijoohwee/agentic-graph/blob/7fb85741121d8c2886027e4a630d013ba91c1027/canvas/src/features/panels/views/settingsMcpDocEntries.ts). Their existence does not confirm every behavior asserted by the specification.
