...
doc_path: "docs/documents/agentic-graph-mcp/agentic-graph-lark-app-mcp-prd-tad.companion.md"
guideline_version: "1.7.0"
reference_implementation_label: "reference implementation"
parent: "agentic-graph-lark-app-mcp-prd-tad.md"
...
[the parent contract](agentic-graph-lark-app-mcp-prd-tad.md). It does not redefine
...
| Public storage contract | `canvas/src/lib/storage/agentic-graph-storage-sync-contract.ts`, `agentic-graph-storage-route-paths.ts`, and `agentic-graph-storage-worker-env-contract.ts` | Defines the stable API/schema, handoff/read routes, complete envelope, and server env boundary. | Does not make Canvas a provider client. |
| Read-only server runtime | `cloudflare/workers/agentic-graph-storage/knowledge-source/` | Resolves identity/allowlist, performs exact Lark reads, emits complete digested snapshots, and signs opaque handoffs. | No Lark write, source acceptance, or deploy authority. |
...
[the MCP installation contract](../agentic-graph-mcp-install-contract.md) as the
...
`agentic-graph-storage` is the provider-neutral runtime authority for read access,
...
| Canvas bounded-streams and accepts only a closed `agentic-graph-knowledge-source-snapshot/v1` with `complete: true`, consistent counts, and recomputed digests. | Knowledge-source read client |
...
| Identity | `AGENTIC_OS_STORAGE_LARK_IDENTITY_MODE`; tenant-app app ID/secret or externally managed user OAuth access token plus expiry remains server-only |
| Allowlist | `AGENTIC_OS_STORAGE_LARK_SOURCE_ALLOWLIST_JSON` using `agentic-graph-knowledge-source-allowlist/v1` |
| Opaque-token custody | Existing server-only `AGENTIC_OS_STORAGE_SIGNING_SECRET`; five-minute AES-GCM AEAD bearer capability |
...
`agentic-graph-knowledge-source-snapshot/v1`. The envelope binds identity mode,
...
`agentic-graph-storage` read-route admission and grants no publish authority.
