...
## Implementation successor — source-aligned image objects, 2026-09-25

**PRD / MVP:** the reported fidelity defect placed generated objects on an angled floor apart from their evidence. **Show space image on Canvas**, reviewed image builds and **Overlay objects on image** now present the photograph at its native aspect ratio with matching selectable Three.js geometry over the saved regions. Clicking a region selects its saved entity and outlines it in cyan. **Open 3D layout** retains the authored placement, dimensions, physics and export controls. This successor starts from `cf68ab0b3dba547b1aed3d864a4ce5b051ea799a` ([review #1262](https://github.com/huijoohwee/agentic-graph/pull/1262)); its containing commit binds the candidate. Existing commercial and device acceptance gaps remain open.

**TAD / ADR-020:** reuse the shared Canvas, Camera, image normalization, evidence store, procedural builders and bounded appearance atlas. An explicit photo source uses a flat image plane; ordinary immersive panorama sources retain their existing projection. A small display adapter reprojects the existing mesh vertices along image rays into their normalized evidence region, with shallow authored relief and photo UVs. The camera fits both image axes across viewport aspects. Clamp contour bevels to the evidence bounds so geometry cannot protrude beyond the source. This presentation adapter never edits the saved recipes, dimensions, placement or GLB construction. A photo-hash match gates overlays; unmatched images show only their own pixels. For repeated identical regions of the same image, the last saved binding is displayed while all historical entities remain in storage. Different regions remain separately selectable. Photo mode omits the floor and demo panorama markers; gravity requires the 3D layout.

**AI-native and race handling:** `operation=overlay` with `observationId`, or `/space.overlay @observation-id #image`, invokes the same browser-local owner as the visible editor action. Source normalization and saved-image hash matching use the shared import/perception reader without generating or confirming new entities. Source changes/unmount cancel the pending image presentation; saved-space revision is rechecked across asynchronous linking before activation. The image and generated geometry share one camera and move together. This adds browser WebMCP coverage; native headless host parity is not claimed.

**Resources / GTM:** the slice admits two new production helpers, totaling 71 lines / 4,656 source bytes, with no new package, model, API or infrastructure. Existing 20-object, 30,000-triangle, 4,194,304-atlas-pixel and 1,024-pixel evidence budgets stay in force. The image-overlay surface uses the existing compatible WebGL path; the separate 3D layout retains optional WebGPU. CPU generation remains local. Shallow relief is a display convention, not recovered depth, measured geometry or semantic recognition. Photographic detail is limited by saved evidence resolution; unseen surfaces remain authored approximations. The unvalidated operator-assisted $1 experiment and existing commercial evidence gaps are unchanged.

**Verification receipt:** 16 focused perception/appearance tests pass, including four image aspects, three viewport aspects, four procedural templates, a real contour touching the source edge, unchanged triangle counts and saved bindings, exact evidence isolation, repeated-region handling and bounded explicit image-data URLs. Twelve existing immersive-media, semantic-space and WebMCP scope regressions pass. Final TypeScript, changed-file hygiene and Vite production build pass; unrelated baseline bundle warnings remain. Local Chromium at `127.0.0.1:4179` shows the original photograph without the angled floor, selects the reviewed cloud region directly on the image, exposes its matching saved ID in the existing editor, opens the authored 3D layout and returns through structured WebMCP without changing the saved revision. Slash invocation also passed. Physical mobile Safari, cold offline cache closure and inferred object separation/depth remain unverified. No test photograph, filename or machine path enters product code.

**Release / next checkpoint:** publish this candidate through the native review lane and bind its own provider checks. Publication is not integration, cleanup or production authority. Preserve the lane until exact completion eligibility exists. The next bounded device check is a 20-minute phone/Safari overlay-selection and resize pass with zero new modules or dependencies; external device availability is a gate, not a promised completion time.
