---
title: "AgenticGraph Queryable Corpus Graph - PRD and TAD"
doc_type: "Combined PRD/TAD"
id: "agenticgraph-query-prd-tad"
version: "0.3.0"
...
| Native in-repo | Existing Toolbar, Workspace, Source Files, parser, and chat owners are enhanced. | No copied parser generator, copied skill, copied CLI, or duplicate graph runtime. |
...
AgenticGraph's implementation remains grounded in its existing browser workspace, Source Files, GraphData, KGC markdown, and Cloudflare deployment topology.
...
AgenticGraph can solve this by turning imported corpus material into one queryable graph. The graph becomes the compact retrieval surface for follow-up questions, path finding, impact analysis, onboarding, architecture review, schema inspection, and agent handoff.
...
| Share | User deploys or publishes queryable graph outputs. | Dev -> Prod -> Cloudflare | Local-only graphs are hard for agents to read. | Publish graph summaries through existing AgenticGraph artifact routes. |
...
- Given a folder containing supported and unsupported files, when the user selects Toolbar -> Launch -> Import folder, then AgenticGraph creates source units for supported files, records skipped files with reasons, preserves relative paths, and focuses the first query-ready artifact.
...
- Given SQL and infrastructure files, when extraction runs, then AgenticGraph emits neutral schema/config graph nodes with relationships to app code and scripts when references are observable.
...
- Given docs, papers, images, or videos, when import completes, then AgenticGraph stores a source artifact and emits graph fragments from local text/metadata first, with AI extraction only behind an explicit harness when needed.
...
- `/goal` translation: E2E import -> parse -> render -> chat readiness is verified by focused tests and browser smoke with no duplicate import bridge, parser stack, renderer fork, or copied external artifact.
...
| Won't | Copy or call another parser-generator or graph project's package, service, fixture, schema, command, or runtime | Violates the independent native implementation rule. |
...
1. Copy or vendor another parser-generator or graph runtime: rejected because it violates implementation policy and creates a parallel runtime.
...
Raw-folder prompting wastes tokens and encourages unsupported claims. AgenticGraph already has graph data, selected node state, and chat context packing.
...
Dev `$GITHUB_ROOT/agenticgraph` owns source edits, tests, docs, and Pages build; prod mirror `$GITHUB_ROOT/huijoohwee/content/agenticgraph` receives generated app payload only; Cloudflare `https://airvio.co/agenticgraph` requires live smoke after sync/deploy when claiming production completion.
...
| Live smoke | `curl -I https://airvio.co/agenticgraph/` plus browser check | Cloudflare route |
