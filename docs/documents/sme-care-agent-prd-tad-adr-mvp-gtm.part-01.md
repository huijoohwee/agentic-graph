---
title: "Reference implementation: sme-care-agent-prd-tad-adr-mvp-gtm section 1"
...
- **Neutral**: capabilities are named by function (Risk Exposure Graph store, Probe-Tree harness, Multilingual Adapter, Marketplace Matcher harness) rather than by brand. Where a concrete tool is named (e.g. an agent-graph canvas, a local model runtime, a regional multilingual model), it appears only as a **non-binding reference implementation** and may be swapped for any FOSS or licensed equivalent that satisfies the same interface.
