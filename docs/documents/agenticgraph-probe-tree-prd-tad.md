---
title: "AgenticGraph Probe-Tree — Combined PRD/TAD"
doc_type: "PRD/TAD"
version: "1.3.0"
date: "2026-07-30"
lang: "en-US"
...
invocation_id: "agenticgraph.probe-tree"
invocation_tokens:
  slash: "/agenticgraph.probe-tree"
  hash: "#agenticgraph.probe-tree"
  at: "@agenticgraph.probe-tree"
...
promptPreset:
  ...
  alias: "/agenticgraph-probe-tree-prompt-preset"
...
parent: "none"
---
# AgenticGraph Probe-Tree — Combined PRD/TAD
...
**Given** a Storyboard Widget Card containing `/agenticgraph.probe-tree`, `@agenticgraph.probe-tree`, or `#agenticgraph.probe-tree` in an authored Prompt, Summary, or Action field, including typed frontmatter cells, **When** the user runs the card, **Then** the native Canvas handler invokes bounded local `agenticgraph.probe.generate`, accepts only relevant `probe-tree-llm-response/v5`, derives source-verbatim anchors and all graph-owned fields in the runtime, replaces the stale descendant closure, materializes 2–4 Type 2 cards with canonical inferred `candidateOption` edges, preserves the viewport, and atomically publishes their evidence ledger.
...
**Configuration**: `k` (default 3), token budget ceiling, recall top-N, optional host-owned local model env (`AGENTICGRAPH_PROBE_TREE_MODEL`, `AGENTICGRAPH_PROBE_TREE_MODEL_URL`, `AGENTICGRAPH_PROBE_TREE_MODEL_TIMEOUT_MS`)
...
|---|---|---|---|
| Harness | Generation Harness | `mcp/probe-tree-runtime.js`, `mcp/probe-tree-model-adapter.js` | Implemented — `agenticgraph.probe.generate` with optional local Ollama adapter |
| Harness | Selection Handler | `mcp/probe-tree-runtime.js` | Implemented — `agenticgraph.probe.select` |
| Harness | Evolution Harness | `mcp/probe-tree-runtime.js` | Implemented — `agenticgraph.probe.evolve` |
...
- [x] Invocation routing proof: `/agenticgraph.probe-tree`, `#agenticgraph.probe-tree`, and `@agenticgraph.probe-tree` resolve through the shared FloatingPanel Chat invocation catalog to this source document
...
- [x] Multi-turn card proof: every materialized branch uses only the canonical `/agenticgraph.probe-tree` prompt, retains the `@` and `#` aliases as metadata rather than auto-inserting them, and increments a bounded depth that stops visibly at 8
