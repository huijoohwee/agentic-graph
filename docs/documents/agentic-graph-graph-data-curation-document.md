# agentic-graph Graph Data Curation (singabldr Extraction)

## Overview

agentic-graph hosts the end-to-end import → parse → store → render pipeline, while the Graph Data curation and document presentation surfaces are owned by the sibling repo `singabldr`.
...
- `agentic-graph/docs/documents/singabldr-graph-data-curation-document.md`
...
This boundary mirrors the earlier pattern used for Geospatial Mode extraction (implementation lives in a sibling repo; agentic-graph remains host-only).
...
**agentic-graph**
- `agentic-graph/canvas/src/*`: continues to host the app entrypoints, renderers, store orchestration, and pipeline logic.
...
agentic-graph integrates the extracted Graph Data code via:

- Local dependency wiring for `singabldr` (sibling repo).
- Runtime module resolution targets the installed package copy at `agentic-graph/canvas/node_modules/singabldr/src` (not direct sibling `../../singabldr/src` imports) to avoid cross-repo path coupling.
