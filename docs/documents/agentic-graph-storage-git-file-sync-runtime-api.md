---
title: "agentic-graph Browser Git and Multi-Provider File Sync Runtime API"
id: "md:agentic-graph-storage-git-file-sync-runtime-api"
...
owner: "agentic-graph storage runtime"
...
This API adds a agentic-graph-owned browser Git engine and provider-neutral file synchronization to the existing Storage Sync contract. It introduces no external Git implementation, rclone binary, file-sync library, browser credential field, Production mirror writer, or Cloudflare provisioning action.
...
|---|---|
| `agentic-graph.inspect_local_git_repository` | Read-only, local-only inspection of repositories, refs, object counts, retained operations, persistence, bounds, grammar, and the authenticated Worker capability result. It returns no object bytes or credentials. |
| `agentic-graph.control_local_git_repository` | Mutating/open-world Git clone, fetch, commit, or push. It accepts structured input or the exact Git invocation below. |
| `agentic-graph.inspect_local_file_sync` | Read-only, local-only inspection of Worker-advertised opaque providers, credential mode, ledger counts, retained transfers, persistence, bounds, and grammar. It returns no file bytes or credentials. |
| `agentic-graph.control_local_file_sync` | Mutating/open-world pull or push for one explicit prefix. It accepts structured input or the exact file-sync invocation below. |
...
| Tool | Behavior |
|---|---|
| `agentic-graph.git.run` | Validates Git input and returns a structured handoff to `agentic-graph.control_local_git_repository`. |
| `agentic-graph.file.sync` | Validates file-sync input and returns a structured handoff to `agentic-graph.control_local_file_sync`. |

The handoff schema is `agentic-graph-storage-stdio-handoff/v1` with `ok=false`, `status=blocked`, `errorCode=BROWSER_RUNTIME_REQUIRED`, `surface=local-stdio`, `executableSurface=browser-webmcp`, a credential-free normalized invocation, and the required browser tool name.
...
Copy `cloudflare/workers/agentic-graph-storage/.dev.vars.example` beside `wrangler.toml`, replace every placeholder with operator-owned Dev values, and run:
...
| Owner | Path |
|---|---|
| Persistence and outbox | `canvas/src/lib/storage/agentic-graph-storage-engine-persistence.ts` |
| Browser Git | `canvas/src/lib/storage/git/` |
| Git Save Bridge binding | `canvas/src/lib/storage/agentic-graph-storage-git-document-authority.ts` and `agentic-graph-storage-git-save-bridge.ts` |
| Browser provider sync | `canvas/src/lib/storage/file-sync/` |
| Invocation contract | `canvas/src/lib/storage/agentic-graph-storage-engine-mcp-contract.mjs` |
...
| Worker relays | `cloudflare/workers/agentic-graph-storage/storage-relay/` |
