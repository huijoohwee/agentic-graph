# AgenticGraph DeerFlow Setup Guide — Companion

Continuation of [agenticgraph-deerflow-setup-guide.md](agenticgraph-deerflow-setup-guide.md). Contains Step 7 (Verify and Validate), Troubleshooting, Proxy Architecture Reference, and Revision History.
...
The canonical fixture `agenticgraph-video-demo.md` must never be referenced by absolute filesystem path in source code. Validation:
...
const text = readDocsSsotFixtureText('agenticgraph-video-demo.md')
```
...
| DFI-C003 | Frontmatter metadata parsed | Open `agenticgraph-video-demo.md` and check graph nodes |
...
**Fix**: Ensure `AGENTICGRAPH_CHAT_PROXY_OPENAI_API_KEY` (or `OPENAI_API_KEY`) and `AGENTICGRAPH_CHAT_PROXY_BYTEPLUS_API_KEY` (or `BYTEPLUS_API_KEY`) are set as **Secrets** (not plain Variables)

### Fixture Not Found

**Symptom**: `agenticgraph-video-demo.md` not loaded as workspace seed
...
| `openai` | true | false | `Bearer <openai-key>` | `AGENTICGRAPH_CHAT_PROXY_OPENAI_API_KEY` or `OPENAI_API_KEY` |
| `byteplus-modelark` | false | true | `Bearer <byteplus-key>` | `AGENTICGRAPH_CHAT_PROXY_BYTEPLUS_API_KEY` or `BYTEPLUS_API_KEY` |
...
|----------|-----------|-------------|---------|
| `AGENTICGRAPH_CHAT_PROXY_OPENAI_API_KEY` | `process.env` | Cloudflare Secret | OpenAI provider auth |
| `OPENAI_API_KEY` | `process.env` | Cloudflare Secret | Fallback OpenAI key |
| `AGENTICGRAPH_CHAT_PROXY_BYTEPLUS_API_KEY` | `process.env` | Cloudflare Secret | BytePlus provider auth |
| `BYTEPLUS_API_KEY` | `process.env` | Cloudflare Secret | Fallback BytePlus key |
| `AGENTICGRAPH_CHAT_PROXY_UPSTREAM` | `process.env` | Cloudflare Variable | Default upstream URL |
| `AGENTICGRAPH_CHAT_PROXY_TIMEOUT_MS` | `process.env` | Cloudflare Variable | Proxy timeout |
| `AGENTICGRAPH_INTEGRATION_ALLOWED_HOSTS` | `process.env` | Cloudflare Variable | Allowed upstream hosts |
