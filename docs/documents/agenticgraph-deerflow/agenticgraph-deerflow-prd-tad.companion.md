# AgenticGraph DeerFlow Integration - TAD Companion

Continuation of [agenticgraph-deerflow-prd-tad.md](agenticgraph-deerflow-prd-tad.md). Contains Part II: Technical Architecture Documentation.
...
AgenticGraph's implemented DeerFlow baseline is a local-gateway integration. UI surfaces remain provider-neutral; DeerFlow-specific transport stays in DeerFlow gateway owners. DeerFlow may inform long-horizon harness concepts, but AgenticGraph's native SuperAgent implementation remains source-owned in `docs/documents/agenticgraph-superagent-harness.md`, `agenticgraph_parser/*`, and the local MCP tool contract.
...
| TAD-DF-C008 | Setup guide | Dev/Prod/Cloudflare Tunnel operator setup | `docs/documents/agenticgraph-deerflow/agenticgraph-deerflow-setup-guide.md` | Active |
...
- DeerFlow returns a manifest; AgenticGraph writes sanitized file names and text.
...
- AgenticGraph must not copy DeerFlow code, duplicate its architecture, or add DeerFlow-specific parser, renderer, memory, or graph-apply owners.
- Native SuperAgent work extends `agenticgraph_parser` and local MCP owners first, then documents the shipped source owner.
...
- Dev: run the AgenticGraph dev server and, when DeerFlow is needed, the local DeerFlow gateway.
...
- Keep future SuperAgent work on AgenticGraph's native harness owners; DeerFlow remains optional provider/inspiration rather than a copied runtime stack.
