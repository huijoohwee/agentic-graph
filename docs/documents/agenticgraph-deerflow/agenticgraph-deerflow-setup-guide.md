# AgenticGraph DeerFlow Setup Guide — From 0 to 1

**Document Version**: 4.0.0
**Date**: 2026-05-09
**Status**: Active
**Companion To**: `agenticgraph-deerflow-prd-tad.md`, `agenticgraph-deerflow-prd-tad-integration-contracts-and-patterns.md`, `agenticgraph-deerflow-prd-tad-delivery-validation.md`

---

## Document Purpose

**Context**: DeerFlow is an optional local gateway and conceptual SuperAgent reference. AgenticGraph's native long-horizon harness is source-owned in `docs/documents/agenticgraph-superagent-harness.md` and `agenticgraph_parser/*`; Canvas Storyboard Widget still consumes provider-neutral graph/media fields through existing widget, chat, and rich-media owners.
...
DeerFlow is a **first-class optional provider**, not a required dependency. AgenticGraph works fully without DeerFlow installed or running.
...
AgenticGraph's default experience remains provider-neutral. DeerFlow only activates when explicitly selected as `chatProvider = 'deerflow'` in Integrations settings. No DeerFlow gateway running = no impact on any other feature. The `dev:all` script gracefully skips DeerFlow startup if the repo is not found.

The native AgenticGraph SuperAgent harness can still run locally through
`python3 -m agenticgraph_parser superagent`, `npm run goal:run`, or the local MCP
tool `agenticgraph.superagent.run` without a DeerFlow gateway.
...
|-------------|---------|-----------|
| Node.js | >= 18 | Yes |
| AgenticGraph dev server | `npm -C canvas run dev` | Yes |
...
| Workspace docs root | Env var `VITE_WORKSPACE_INITIALIZATION_DOCS_ABS_ROOT` pointing to docs folder containing `agenticgraph-video-demo.md` | Only for demo fixture |
| Cloudflare Pages project | `airvio.co/agenticgraph` with env vars configured (see Step 1.5) | Only for prod deployment |
...
DeerFlow runs as a local gateway that proxies to underlying model providers. It must be running somewhere accessible to the AgenticGraph proxy (Vite middleware in dev, Cloudflare Pages Function in prod).

### Dev Mode: Run DeerFlow Locally

#### Option A: One-command startup (AgenticGraph + DeerFlow)
...
- Starts the AgenticGraph Vite dev server
...
# Terminal 2: Start AgenticGraph dev server
npm run dev
```
...
#### Step 0.3: Configure AgenticGraph endpoint for Prod

In AgenticGraph Integrations, set the DeerFlow endpoint URL to the tunnel URL:
...
2. Add: `AGENTICGRAPH_CHAT_PROXY_UPSTREAM` = `https://abcxyz-trycloudflare-com.trycloudflare.com`
...
cloudflared tunnel create agenticgraph-deerflow

# Route your domain to the tunnel
cloudflared tunnel route dns agenticgraph-deerflow deerflow.yourdomain.com

# Run the tunnel
cloudflared tunnel run agenticgraph-deerflow --url http://localhost:8001
```
...
| **LaunchAgent** (macOS) | Create `~/Library/LaunchAgents/com.agenticgraph.deerflow.plist` | Auto-starts on login |
| **systemd** (Linux) | Create `/etc/systemd/system/deerflow-tunnel.service` | Auto-starts on boot |

**macOS LaunchAgent example** (`~/Library/LaunchAgents/com.agenticgraph.deerflow.plist`):
...
<dict>
  <key>Label</key>
  <string>com.agenticgraph.deerflow</string>
  <key>ProgramArguments</key>
  <array>
    <string>/opt/homebrew/bin/cloudflared</string>
    <string>tunnel</string>
    <string>run</string>
    <string>agenticgraph-deerflow</string>
    <string>--url</string>
...
launchctl load ~/Library/LaunchAgents/com.agenticgraph.deerflow.plist
```
...
AgenticGraph supports two authentication modes. The mode is shared globally across all providers (BytePlus, OpenAI, DeerFlow).
...
1. Launch the AgenticGraph Canvas dev server: `npm -C canvas run dev`
...
export AGENTICGRAPH_CHAT_PROXY_OPENAI_API_KEY="sk-..."
# or fallback
export OPENAI_API_KEY="sk-..."

# BytePlus provider (used for image/video generation)
export AGENTICGRAPH_CHAT_PROXY_BYTEPLUS_API_KEY="..."
# or fallback
export BYTEPLUS_API_KEY="..."

# DeerFlow local gateway upstream (optional, client header takes priority)
export AGENTICGRAPH_CHAT_PROXY_UPSTREAM="http://localhost:8001"

# Proxy timeout (optional, 5000-180000 ms, default 90000)
export AGENTICGRAPH_CHAT_PROXY_TIMEOUT_MS="120000"
```

#### Prod Mode (Cloudflare Pages at `airvio.co/agenticgraph`)
...
|----------|------|---------|-----------|
| `AGENTICGRAPH_CHAT_PROXY_OPENAI_API_KEY` | **Secret** | OpenAI key for `serverManaged` mode | Yes (if using OpenAI/DeerFlow LLM) |
| `OPENAI_API_KEY` | **Secret** | Fallback OpenAI key | Fallback |
| `AGENTICGRAPH_CHAT_PROXY_BYTEPLUS_API_KEY` | **Secret** | BytePlus ModelArk key for `serverManaged` mode | Yes (if using BytePlus image/video) |
| `BYTEPLUS_API_KEY` | **Secret** | Fallback BytePlus key | Fallback |
| `AGENTICGRAPH_CHAT_PROXY_UPSTREAM` | Variable | Default upstream URL fallback | Optional |
| `AGENTICGRAPH_CHAT_PROXY_TIMEOUT_MS` | Variable | Proxy timeout (5,000-180,000; default 90,000) | Optional |
| `AGENTICGRAPH_INTEGRATION_ALLOWED_HOSTS` | Variable | CSV of allowed upstream hostnames | Optional |

**Key fallback chain** (identical in both environments):

```javascript
const envOpenAiApiKey = env.AGENTICGRAPH_CHAT_PROXY_OPENAI_API_KEY || env.OPENAI_API_KEY || ''
const envBytePlusApiKey = env.AGENTICGRAPH_CHAT_PROXY_BYTEPLUS_API_KEY || env.BYTEPLUS_API_KEY || ''
```
...
This means: in `serverManaged` mode, **no API key is needed for DeerFlow** at the AgenticGraph level. The DeerFlow gateway manages its own credentials for upstream LLM providers.
...
export AGENTICGRAPH_CHAT_PROXY_UPSTREAM="https://airvio.co"
```
...
The canonical fixture `agenticgraph-video-demo.md` is a path-agnostic workspace seed. It defines a 3-node DAG pipeline (Text -> Image -> Video) that exercises the full DeerFlow harness.
...
The docs folder must contain `agenticgraph-video-demo.md` (or `workspace-seeds/agenticgraph-video-demo.md`).

### 2.2 Open the Fixture in Canvas

1. In the AgenticGraph Workspace, open the file browser
2. Navigate to `agenticgraph-video-demo.md`
...
When an operator opts into DeerFlow gateway support for rich-media generation, parallel locale lanes remain AgenticGraph-authored Storyboard Widget branches and DeerFlow stays provider-side. Native long-horizon coordination belongs to the AgenticGraph SuperAgent harness; this setup guide does not make DeerFlow a parser, renderer, memory, or graph-apply owner.

```mermaid
flowchart TD
    BRIEF["Markdown Brief"] --> HARNESS["AgenticGraph SuperAgent Harness"]
...
Step 7 (Verify and Validate), Troubleshooting, Proxy Architecture Reference, and Revision History continue in [agenticgraph-deerflow-setup-guide.companion.md](agenticgraph-deerflow-setup-guide.companion.md).
