...
| --- | --- | --- | --- | --- |
| App source | Keep all AgenticGraph source, build config, and release logic in `agenticgraph`; forbid source copies inside `huijoohwee`. | `agenticgraph` | `huijoohwee/content/agenticgraph`, `huijoohwee/agenticgraph` | `airvio.co/agenticgraph` |
...
| --- | --- | --- |
| Static mirror drift | `npm run pages:check-sync` | Confirms `canvas/dist`, `huijoohwee/content/agenticgraph`, managed `huijoohwee/agenticgraph` files, and generated root `_headers` / `_redirects` agree while excluding mirrored nested control files. |
