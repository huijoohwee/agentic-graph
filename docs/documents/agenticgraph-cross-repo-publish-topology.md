# AgenticGraph Cross-Repo Publish Topology

Canonical companion for the clean end-state topology shared with `singabldr`.

For current remote MCP onboarding, start with
`docs/documents/agenticgraph-mcp-onboarding-index.md`, then use
`docs/documents/agenticgraph-mcp-install-contract.md` for the canonical
public-discovery vs control-plane endpoint boundary.
Map intent on `https://airvio.co/agenticgraph/mcp`, orchestrate agents on
`https://airvio.co/agenticgraph/control-plane/mcp` only for session-capable hosts,
and prove outcomes first with the source-side `README.md` or
`docs/documents/agenticgraph-superagent-harness.md` offline path.

## Scope

- Dev SSOT repo: `$GITHUB_ROOT/agenticgraph`
...
Protected Dev main + exact localhost review
  -> agenticgraph-production-release-evidence/v1
...
`agenticgraph-production-release-evidence/v1` bytes. Its exact frontier inventory entries and retained
...
AgenticGraph release paths. Only the protected release workflow may reconcile the verified candidate
...
`airvio.co/agenticgraph` are custom-domain transports with their own DNS, policy, cache, and route
...
- Source repo `agenticgraph` shipped commit `0ecd4deb5ee0ad92e561c4143c03995e4d867a8a` (`fix release D1 JSON parsing (#792)`).
...
- The protected release completed without rollback and re-proved the public route at `https://airvio.co/agenticgraph/` with matching `root-agent-ready-pages` and `agenticgraph-agent-ready-pages` route-owner markers plus immutable manifest digest `5abe443adec86ed395d0910597928b85371fd7e2d4a68a7a96bd9cb0f79f9ff3`.

### 2026-08-02 Canonical Main Advance Release Record

- Source repo `agenticgraph` shipped commit `32d2cfca34f7d5bf484b4a8f449083954a476bd8` (`chore(release): promote Agentic Canvas OS docs pin (#647)`).
...
- Publish repo `huijoohwee` shipped commit `5f9eff39339e1f1f0ea86ddaa11e48e49f1811cc` (`chore(release): promote agenticgraph 32d2cfca34f7`).
- The protected release completed without rollback and re-proved the public route at `https://airvio.co/agenticgraph/` through the verified deployment path.

### 2026-08-02 Earlier Canonical Main Advance Release Record

- Source repo `agenticgraph` shipped commit `027cd892b57e4247c1e8edb4c144b216c398379c` (`chore(release): promote Agentic Canvas OS runtime pin (#637)`).
...
- Publish repo `huijoohwee` shipped commit `dec0a40bb6718d70b6bdfd6d3196ec5bc6342df4` (`chore(release): promote agenticgraph 027cd892b57e`).
...
- Source repo `agenticgraph` shipped commit `7f097dbabb7285cf4a18a66a0e37170158b5a610` (`Promote Agentic Canvas OS runtime pin (#625)`).
...
- Publish repo `huijoohwee` shipped commit `154fb68b1bab81401f277ece6083200714f6f23f` (`chore(release): promote agenticgraph 7f097dbabb72`).
- The protected release completed without rollback and re-proved the public route at `https://airvio.co/agenticgraph/` through the verified deployment path.

### 2026-08-02 Even Earlier Protected Release Record

- Source repo `agenticgraph` shipped commit `af26f37477cb92e1a8306931262bb25dd4944f00` (`Promote Agentic Canvas OS ba45470e0365 (#622)`).
...
- Publish repo `huijoohwee` shipped commit `41043c2ff3bb8b49d0f54850dd2c83f4778cd44f` (`chore(release): promote agenticgraph af26f37477cb`).
- The protected release completed without rollback and re-proved the public route at `https://airvio.co/agenticgraph/` through the verified deployment path.

### 2026-07-11 Root Live Canvas Hero Release Record

- Source repo `agenticgraph` shipped `a86bdbc9` (`restore source-backed apex FlowCanvas hero`) and `ada81a16` (`isolate apex hero from unloaded persisted source text`).
- Publish repo `huijoohwee` shipped `88aa31070` (`deploy persisted-state resilient apex FlowCanvas hero`).
- Cloudflare Pages deployed the exact publish commit to `https://1b9d700b.joohwee.pages.dev`; the custom domain served the release at `https://airvio.co/` and `https://airvio.co/agenticgraph/`.
- The root route uses the published React app shell with `x-agenticgraph-root-alias=/agenticgraph/`; it is not a separately maintained launch page. The normal root path renders the interactive `workspace-readme.md` FlowCanvas and the same React Live Canvas Hero as Dev.
- Dev/Prod browser proof matched: the canvas region, hero headline, `/`, `#`, and `@` invocation controls, one `Enter AgenticGraph` link to `/agenticgraph/`, and no static launch-overlay marker. Route checks returned `200` for both root and app paths.

### 2026-06-29 Release Record

- Source repo `agenticgraph` shipped commit `530462d6` (`Stabilize storyboard runtime and sync docs`).
...
- Cloudflare Pages deploy ran through `npm run pages:deploy-cloudflare` from `agenticgraph` and completed with preview URL `https://0d3c18ba.joohwee.pages.dev`.
- Post-deploy route proof passed with `https://airvio.co/` -> `200`, `https://airvio.co/agenticgraph/` -> `200`, and `https://0d3c18ba.joohwee.pages.dev/agenticgraph/` -> `200`.
- The deploy also completed `storage:d1:seed:docs` with `applied=41`, `conflict=0`, and `rejected=0`.

### 2026-06-26 Release Record

- Source repo `agenticgraph` shipped commits `66926a74` (`feat: improve timeline preview animation runtime`) and `e97df37c` (`chore: refresh settings flow metadata`).
- Publish repo `huijoohwee` shipped commit `0e4ab538` (`chore: sync agenticgraph release surface`).
- Cloudflare Pages deploy ran through `npm run pages:deploy-cloudflare` from `agenticgraph` and completed with preview URL `https://bdc25ab1.joohwee.pages.dev`.
- Post-deploy route proof passed with `https://bdc25ab1.joohwee.pages.dev/agenticgraph/` -> `200` and `https://airvio.co/agenticgraph/` -> `200`.
- The deploy also completed `storage:d1:seed:docs` with `applied=39`, `conflict=0`, and `rejected=0`.

### 2026-06-15 Release Record

- Source repo `agenticgraph` shipped commit `fcd0ea5f` (`feat: modularize chat skill prompt handling`).
- Publish repo `huijoohwee` shipped commit `f0422135` (`chore: sync agenticgraph production publish`).
- Cloudflare Pages deploy ran through `npm run pages:deploy-cloudflare` from `agenticgraph` and completed with preview URL `https://84f45986.joohwee.pages.dev`.
- Post-deploy route proof passed with `https://airvio.co/agenticgraph` -> `308` and `https://airvio.co/agenticgraph/` -> `200`.
...
| --- | --- | --- | --- | --- |
| App source | Keep all AgenticGraph source, build config, and release logic in `agenticgraph`; forbid source copies inside `huijoohwee`. | `agenticgraph` | `huijoohwee/content/agenticgraph`, `huijoohwee/knowgrph` | `airvio.co/agenticgraph` |
| Publish boundary | Treat `huijoohwee` as artifact-only for AgenticGraph; allow deploy config, headers, redirects, and shared Functions there. | `agenticgraph` | `huijoohwee` | `airvio.co/agenticgraph` |
| Route ownership | Keep `/agenticgraph` assets, redirects, manifests, and shell logic isolated from Singabldr route assumptions. | `agenticgraph` | `huijoohwee/_redirects`, `huijoohwee/content/agenticgraph` | `airvio.co/agenticgraph` |
| Root launch alias | Keep `airvio.co/` on the published AgenticGraph React app shell. The generated root handler injects `x-agenticgraph-root-alias=/agenticgraph/`; `CanvasPage` and `useAgenticGraphLiveCanvasHero` consume that marker synchronously so Home owns first paint and resolves the canonical Physics Playground **Share canvas embed** runtime from `XR_PHYSICS_DEMO_PUBLISHED_CANONICAL_PATH` before outer Source Files hydration. The seed frontmatter owns XR/3D mode; no Home query may override its renderer. Inside that same-origin embed, `CanvasDocDeepLinkRuntime` keeps `kgShare` unconsumed until the embedded Source Files bootstrap readiness signal fires, then applies the shared document as the final authoritative selection so origin-scoped persisted documents cannot replace the apex background. The full `/agenticgraph/` workspace canvas must never flash or mount underneath Home. Explorer → Source Files → **Share canvas embed** selects and session-persists another source's same-origin interactive runtime, replaces the canonical background, and copies sandboxed iframe HTML for external websites. The async published URL remains the internal selection value while the external clipboard contract is owned by `canvasEmbedIframeMarkup.ts`. | `agenticgraph/cloudflare/pages/root-agent-ready-index.mjs`, `agenticgraph/canvas/src/pages/Canvas.tsx`, `agenticgraph/canvas/src/features/canvas/{CanvasDocDeepLinkRuntime.tsx,canvasEmbedIframeMarkup.ts,useAgenticGraphLiveCanvasHero.ts,liveCanvasHeroSourceSelection.ts,canvasDocDeepLink.ts}` | `huijoohwee/_worker.js`, `huijoohwee/knowgrph/**` | `airvio.co` → `airvio.co/agenticgraph/` |
| Embed selection route boundary | A persisted embed selection is Home background source state only. Its `sourcePath` and decoded share/local URL must resolve one document atomically. Conflicts and malformed first-party share routes are deleted from session state; a local Physics alias is rewritten to the canonical published share. The selection cannot make `/agenticgraph/` render the Live Canvas Hero; the workspace route always owns the interactive editor/canvas. | `agenticgraph/canvas/src/features/canvas/{liveCanvasHeroSourceSelection.ts,liveCanvasHeroSourceSelectionContract.mjs,useAgenticGraphLiveCanvasHero.ts}` | `huijoohwee/knowgrph/**` | `airvio.co/agenticgraph/` |
| Shared Pages Functions | Keep production chat-proxy and integration host policy in publish-repo shared Functions; validate provider onboarding there so Cloudflare Pages behavior matches Dev, including the optional Cloudflare AI Gateway draft route. | `huijoohwee/functions/{__chat_proxy,api/_integrationHub.js}` | `huijoohwee/functions/**` | `airvio.co/__chat_proxy/*`, `airvio.co/agenticgraph` |
| Release flow | Build and validate through `Integration Gate`; protected merge proves Dev only. Before dispatch, create content-addressed `agenticgraph-production-release-evidence/v1` for every preserved frontier lane and exact last-known-good Pages/mirror/D1. `turn:end` seals exact review evidence; the protected workflow revalidates both, builds once, and waits for exact-candidate terminal authorization. That workflow alone deploys Pages (Deployment v1), reconciles direct D1 state (State Reconciliation v1), separately proves immutable/stable/custom transports (Live Verification v2), and only then publishes the exact mirror (Publication v2). It validates the chain into `agentic-collaborative-release-lifecycle/v2`; only `production-complete` or verified `rolled-back` is terminal. Any predecessor, authority, artifact, transport, state, marker, or mirror drift fails closed. A failure restores only the bound last-known-good Pages target, records D1 disposition separately, requires restoration probes, and keeps the previous mirror unchanged. Direct local deploy/D1/probe/mirror/rollback commands are forbidden. Storage Worker, DNS, and payment deployments remain separately authorized operations. | `agenticgraph/.github/workflows/{integration,release}.yml`, `agenticgraph/scripts/{production-release-lifecycle,production-terminal-authorization}.mjs` | `huijoohwee` release artifact | immutable `pages.dev`, stable Pages, `airvio.co`, `airvio.co/agenticgraph` |
| Generated artifact storage | Keep FloatingPanel Chat KGC sessions under `/chat-log/{session}/`; write Markdown/text artifacts to the configured GitHub repository path `chat-log/{session}/{file}` first, then mirror searchable Markdown/manifests to D1 and generated image/video/binary bytes to R2 when runtime storage is enabled. A generated artifact is Cloudflare-persisted only when both the D1 manifest route and R2 blob route are readable. | `agenticgraph/canvas/src/features/{chat,source-files}`, `agenticgraph/cloudflare/pages/agenticgraph-agent-ready.mjs`, `agenticgraph/cloudflare/workers/agenticgraph-storage` | GitHub repository `chat-log/**` files; secondary Cloudflare Worker D1 rows + R2 `agenticgraph-storage-blobs` | `airvio.co/agenticgraph/api/workspace/github/write`, root alias `airvio.co/api/workspace/github/write`, `airvio.co/api/storage/{doc,blob}/*` |
| Drift control | Fix stale paths, route leakage, and runtime drift at the AgenticGraph source or shared publish config root; never patch generated outputs downstream. | `agenticgraph` | `huijoohwee` | `airvio.co/agenticgraph` |
...
| --- | --- | --- |
| Static mirror drift | `npm run pages:check-sync` | Confirms `canvas/dist`, `huijoohwee/content/agenticgraph`, managed `huijoohwee/knowgrph` files, and generated root `_headers` / `_redirects` agree while excluding mirrored nested control files. |
...
| AI Gateway draft-route readiness gate | `npm run ai-gateway:readiness:check` | Runs the focused OpenAI draft-route source proofs, publish sync drift validation, publish-repo `__chat_proxy` smoke, Cloudflare Pages project-config verification for `AGENTICGRAPH_CHAT_PROXY_AI_GATEWAY_BASE_URL`, Pages secret-list verification, and a bounded live Cloudflare-hosted transport smoke so the draft lane can be activated without guesswork. `-- --skip-live` skips only the live transport smoke; the Pages config and secret gates still fail closed until the `joohwee` project exposes the AI Gateway base URL plus an accepted AI Gateway secret. |
...
| Responsive parity release gate | `npm --prefix canvas run test:smoke:mobile-keyboard:browser`; `npm run pages:check-sync`; review `docs/documents/agenticgraph-feature-map.md` | Required before `pages:deploy-cloudflare` when a change affects mobile grammar reachability, heavy-runtime intent policy, or touch-first responsive behavior. Blocks release until the mobile keyboard proof, the route-and-action matrix, and the publish mirror all agree. |
| Mobile route-and-action evidence audit | Review `docs/documents/agenticgraph-feature-map.md` together with the focused mobile browser smoke before publish when a change alters phone workflow activation, fallback behavior, or heavy-runtime intent gates. | Confirms the documented immediate/deferred/fallback-safe matrix still matches the shipped mobile topology and proof path. |
| Static build + sync | `npm run pages:build-sync` | Rebuilds with `VITE_BASE_PATH=/agenticgraph/`, blocks personal home-directory paths in active source and built text assets, then syncs the Prod mirror. |
| Pages Functions build | `npm run pages:functions:build` | Generates the publish-repo `_worker.js`, including the root AgenticGraph app-shell alias handler. |
| Direct multi-unit deploy (not a release path) | `npm run pages:build-sync-cloudflare` | Mutates Pages/Workers/D1 outside the protected receipt chain and therefore must not be used for AgenticGraph production release. Separate unit runbooks may name bounded operator uses under separate authority. |
...
| Root + app route proof | `curl -I https://airvio.co/`; `curl -I https://airvio.co/agenticgraph/`; inspect the served root module asset | Confirms Cloudflare Pages is serving the pushed Prod mirror and root requests resolve through the published AgenticGraph app shell. |
| Live Canvas Hero parity | Open Dev and `https://airvio.co/` in parallel; verify the `AgenticGraph Live Canvas Hero` and interactive canvas regions, confirm `data-kg-live-canvas-hero-source` equals the path resolved by `XR_PHYSICS_DEMO_PUBLISHED_CANONICAL_PATH`, and verify the iframe `src` matches the canonical direct opaque share path without editor or renderer query overrides. Pre-seed a valid noncanonical share URL paired with the canonical Physics source path in a fresh browser context: Home must remove that conflict, recover the canonical token, and first-mount exactly one `native-controller` XR root/Canvas with no motion-reference, empty-world, or Game fallback owner. Open the canonical `src` and verify the source-derived XR/physics surface. In MainPanel Settings → Canvas Embed, confirm **Use Physics Playground background** resolves the same canonical share path. On a clean `/agenticgraph/` initialization, confirm the same physics seed opens after bootstrap; explicit share/deep-link targets must still win. | Confirms the public root, stale-session recovery, full workspace cold start, and Settings preset resolve one source-backed physics default without downstream mutation or renderer conflict. |
...
- Canonical storage & sync index: `agenticgraph-storage-sync-document.md`
- Storage schema appendix: `agenticgraph-storage-schemas-document.md`
