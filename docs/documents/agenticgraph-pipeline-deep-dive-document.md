---
title: "Reference implementation: AgenticGraph Import-to-Render Pipeline Deep Dive"
...
# Reference implementation: AgenticGraph Import-to-Render Pipeline Deep Dive
...
**Artifact**: `docs/assets/agenticgraph-canvas-import-parse-derive-layout-render-pipeline-map.jsonld`
...
- Optional: apply `data/config/schema/agenticgraph-universal-schema-config.jsonld` to get pipeline-friendly styles (`contains/calls/invokes/next`)
...
**YouTube import**:
- Status: Active. The import action calls `/__youtube_transcript` (Vite middleware) which runs `python3 -m agenticgraph_parser youtube --emit json` to produce:
...
- All Canvas pipeline docs under `agenticgraph/docs/documents` are indexed into a JSON-LD documentation graph at `huijoohwee.github.io/schema/AgenticRAG/agenticgraph-documents-map.graph.jsonld`.
...
- AgenticRAG schemas (`canvas.jsonld`, `pipeline.jsonld`, `markdown.jsonld`) describe the same import→parse→store→render pipeline and reference these AgenticGraph docs as implementation notes; changes to pipeline stages in code must be reflected in both the docs here and the mapped schema comments.
