...
All 17 authoring criteria remain open here. S1 source adapts native construction/controls to selected subjects; the exact installed source has passed its two registered focused cases. Its visual part inspector continuation edits the same recipe/session, preserves control bounds and source fencing, and adds no module or checkout. The next admitted S1 slice persists clip/rest-pose selection and repeat/hold behavior sampled by the shared Timeline, with source/reopen/seek proof before acceptance. See the historical companion’s S1 checkpoints for scoped budgets and limits; no full browser/export acceptance is claimed.
