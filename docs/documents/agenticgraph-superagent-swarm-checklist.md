# AgenticGraph: 0 → 1 “SuperAgent / Swarm Intelligence” Repo-Level Checklist

## Current Source-Truth Update

AgenticGraph now has a local, runnable SuperAgent harness baseline. The canonical
source-side contract is [agenticgraph-superagent-harness.md](agenticgraph-superagent-harness.md).
The runnable owners are `agenticgraph_parser/superagent_harness.py`,
`agenticgraph_parser/superagent_plan.py`, `agenticgraph_parser/superagent_tools.py`,
`agenticgraph_parser/superagent_verifier.py`, `mcp/server.js`, and
...
## Current snapshot: `agenticgraph` (high-level placement)

Based on current repo state:

- `agenticgraph` remains strong in KG/GraphRAG ingestion, parsing, Source Files,
  Storyboard Widget rendering, and visualization.
- `agenticgraph` now meets the local SuperAgent harness baseline:
  - CLI entrypoints: `python3 -m agenticgraph_parser superagent`,
    `python3 -m agenticgraph_parser run-goal`, `npm run goal:run`, and
    `npm run superagent:example`.
  - Local MCP entrypoint: `agenticgraph.superagent.run`.
...
- `agenticgraph` now has a Dev-source Level 3 swarm prediction baseline:
  ...
  - PRD/TAD: `docs/documents/agenticgraph-swarm-prediction-engine-prd-tad.md`
