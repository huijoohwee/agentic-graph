---
title: "agentic-graph — AI Agents Universal Memory Layer PRD/TAD"
...
traceability:
  ...
  doc_path: "docs/documents/agentic-graph-ai-agents-memory-layer-prd-tad.md"
source_references:
...
agentic-graph agents otherwise start cold each session. Users repeat preferences, prompt tokens bloat, and long-running research or build workflows lose context.
...
| Agent registry | `agentic-graph-memory-layer` vdeoxpln entry | `canvas/src/features/agent-ready/agentic-graph-vdeoxpln-contract.mjs` | Implemented |
...
| Tool | Mutation | Input |
|---|---|---|
| `agentic-graph.memory.add` | Local scoped write | `text` or `messages`, plus at least one of `user_id`, `agent_id`, `run_id`, `app_id` |
| `agentic-graph.memory.write` | Fenced local mutation | exact scope, memory action/evidence/operator, host authorization token, expected scope revision, idempotency key |
| `agentic-graph.memory.compact` | Fenced explicit compaction | exact scope, named memory entries with prior text, replacement, evidence, host authorization, and fences |
| `agentic-graph.memory.search` | Read-only | `query`, required exact four-part persistent scope, optional filters and `as_of_revision`; partial legacy scope is rejected |
| `agentic-graph.session.search` | Read-only | exact scope, query, optional session and snapshot revision |
| `agentic-graph.user.profile` | Read or fenced local mutation | inspect, or one allowlisted structured interaction preference plus evidence and host authorization |
| `agentic-graph.memory.invoke` | Route-dependent | exact source revision, complete canonical `/ # @` tuple, direct-tool arguments |
| `agentic-graph.memory.assemble_prompt` | Read-only | `base_system_message`, ranked `memories`, `max_memory_tokens` |

### Scope and Config

Persistent-memory scope requires exact `tenant_id`, `workspace_id`, `agent_id`, and `subject_id` values; omission never broadens a read. Each exact scope receives an opaque physical store with independent revisions, receipts, and quotas. The state root is configured by `AGENTIC_OS_MEMORY_STATE_DIR`, or resolved beneath the host state directory using digests of the Git common directory and `AGENTIC_OS_MEMORY_NAMESPACE`. Main and linked worktrees therefore share repository-scoped host state without writing into either checkout or returning a machine path.

Mutation arguments are not trusted approval evidence by themselves. The canonical local MCP runtime requires `AGENTIC_OS_MEMORY_APPROVAL_HMAC_KEY` and verifies a short-lived HMAC token over the exact mutation tool and request. The host mints that token with `mintPersistentMemoryAuthorization`; the key remains outside agent-visible arguments. Missing, expired, future, wrong-host, wrong-tool, or request-drifted tokens fail before persistence.

The original harness remains separately configured by `AGENTIC_OS_MEMORY_STORE_PATH`; its compatibility contract is not treated as the fenced persistent-memory runtime.
...
| Env | Purpose |
|---|---|
| `AGENTIC_OS_MEMORY_PROVIDER_MODE` | `local-json`, `mem0-platform`, `mem0-oss`, or `external-mcp` |
| `AGENTIC_OS_MEMORY_STORE_PATH` | Local Dev JSON store path |
| `AGENTIC_OS_MEMORY_STATE_DIR` | Optional persistent-memory state directory outside the repository |
| `AGENTIC_OS_MEMORY_NAMESPACE` | Host-owned local storage partition |
| `AGENTIC_OS_MEMORY_APPROVAL_HMAC_KEY` | Host-only secret of at least 32 bytes used to verify exact-request mutation authorizations |
...
| Search top-K | `agentic-graph.memory.search` returns deterministic bounded cited results with zero model calls |
| Prompt budget | `agentic-graph.memory.assemble_prompt` emits `injected_token_estimate` |
...
| Registry discovery | `agentic-graph-memory-layer` is present in vdeoxpln output |
...
Known wider check status on 2026-06-13: `npm -C canvas run check` currently fails on unrelated pre-existing type errors in `src/lib/canvas/widgets/replayContract.ts`, `src/lib/config.ls.owners.ts`, and `cloudflare/workers/agentic-graph-storage/media.ts`.
...
| No external runtime dependency | Persistent memory uses only local code and existing agentic-graph durability owners |
