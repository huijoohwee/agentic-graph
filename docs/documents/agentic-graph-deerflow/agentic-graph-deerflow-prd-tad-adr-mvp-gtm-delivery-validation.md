---
title: "agentic-graph DeerFlow Delivery and Validation"
...
# agentic-graph DeerFlow Delivery and Validation

**Document Version**: 1.2.0  
**Date**: 2026-05-29  
**Status**: Accepted and implemented baseline  
**Companion To**: `agentic-graph-deerflow-prd-tad-adr-mvp-gtm.md`, `agentic-graph-deerflow-prd-tad-adr-mvp-gtm-integration-contracts-and-patterns.md`
...
**SuperAgent boundary**: DeerFlow validates as an optional provider/gateway. agentic-graph's native long-horizon SuperAgent harness validates through `agentic_graph_parser`, `npm run goal:run`, local MCP `agentic-graph.superagent.run`, and the source-side contract in `agentic-graph-superagent-harness.md`. DeerFlow-inspired docs must not copy DeerFlow code or claim a DeerFlow-owned parser, renderer, memory, or graph-apply stack.
...
| DFD-007 | Operator setup | `agentic-graph-deerflow-setup-guide.md` | Active Dev/Prod/Cloudflare Tunnel guide |
| DFD-008 | Native SuperAgent distinction | `agentic-graph-superagent-harness.md` | DeerFlow remains optional provider/inspiration, not the owner of agentic-graph's local harness |
...
| SuperAgent inspiration boundary | DeerFlow references stay conceptual-reference-only and point to the native agentic-graph harness owner |
...
- [x] DeerFlow-inspired SuperAgent language is marked conceptual-reference-only and points to the native agentic-graph harness owner.
...
- DeerFlow is optional; agentic-graph must operate fully when no DeerFlow gateway is running.
...
- A copied DeerFlow SuperAgent stack is out of scope; native long-horizon harness work belongs to `agentic_graph_parser` and local MCP owners.
