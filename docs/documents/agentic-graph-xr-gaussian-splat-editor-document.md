---
title: "agentic-graph XR Gaussian Splat Editor"
...
# agentic-graph XR Gaussian Splat Editor
...
5. Download an independently serialized Gaussian PLY and provider-neutral edit manifest, or explicitly publish the optimized PLY through configured agentic-graph storage.

glTF and GLB remain separate Three.js model-asset formats. They are not treated as Gaussian encodings. SPZ sources remain recognizable import manifests but are reported as unsupported until agentic-graph owns an independently implemented codec.
...
| Optional publish | `uploadGeneratedWorkspaceBlobToAgenticGraphStorage` | Runs only after an explicit Publish action and only when repository-owned storage sync is configured. |
...
- Publish is an explicit user action. It uses only configured agentic-graph storage and falls back to local download when storage is unavailable.
