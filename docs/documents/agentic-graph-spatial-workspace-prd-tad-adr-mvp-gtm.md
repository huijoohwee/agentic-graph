...
The follow-up is branch `agent/device-cba000d3779d/spatial-acceptance`. Its automated full-app proof binds commit `e093342f3229ef1c6e050e1b97a34cfcf22d3a86` and tree `627a379d897d65fd27e147dd8a726c081f80ce9d`; artifact `/tmp/spatial-full-app-mobile/acceptance.json`. Subsequent specification and protected lifecycle-pin changes retain this source observation and receive their own required CI proof; the observation is not retroactively rebound.
...
| Native review | `SpatialWorkspaceReview.tsx` adds an explicit one-metre X preview and reuses `LearningOfflineControls`. It refreshes after bootstrap/indexing/layout fences clear, with one timer for the existing mutation deadline. Mobile review works while expensive 3D rendering remains opt-in. |
...
**Technical evidence, 2026-09-26:** 23 model/runtime/provenance tests pass, including hydration-fence release and actual parser/source reserialization. TypeScript and three browser-runtime policy tests pass. The cross-repository browser smoke passes at 1024/390 pixels with Canvas's generated client and the actual Graph registry: matching identities, stale rejection, offline apply/undo, four receipts, durable readback and zero page errors. The full-app rehearsal records five deliberate actions at both widths; 1024 px: 25.14 s to first value, 9.14 s installation, 3.17 s offline reload; 390 px: 13.40 s to first value, 4.34 s installation, 1.34 s offline reload. Both profiles verify preview/cancel byte preservation, apply, undo, two persisted receipts, installed offline cold reload, absent navigator/document tool surfaces, no script execution and no horizontal overflow. Mobile uses text review without opting into 3D. These are automated technical timings, not observed customer TTV.
...
The [pilot specification](agentic-graph-spatial-workspace-pilot-prd-tad-adr-mvp-gtm.md), `SPATIAL-WORKSPACE-PILOT-001@0.1.0`, owns the recommended solo-author, agent-builder and mobile-reviewer sessions. No person was recruited, no feedback invented and no outreach sent. Production remains governed by its existing explicit authorization boundary.
