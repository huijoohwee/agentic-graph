...
continuity_id: "PLAN-AGENTIC-GRAPH-AGNES-AI-API-PRD-TAD-ADR-MVP-GTM"
...
previous_document_version: "2.0.0"
prd_revision: "2.0.1"
tad_revision: "2.0.1"
adr_revision: "2.0.1"
mvp_revision: "2.0.1"
...
# agentic-graph - Agnes AI API Integration: MainPanel -> FloatingPanel Chat PRD-TAD-ADR-MVP-GTM

## Purpose

This document defines the implementation-accurate product and technical contract for adding **Agnes AI API** to agentic-graph through the **existing shared chat pipeline**, not through a parallel provider runtime.
...
agentic-graph already has a strong downstream contract for AI-assisted document generation, but every new provider risks introducing:
...
- Dev/preview owner: `agentic-graph/canvas/vite.config.ts`
...
- accept `AGENTIC_OS_CHAT_PROXY_AGNES_API_KEY` or `AGNES_API_KEY` for server-managed auth,
...
- preserve provider parity with MiroMind and other shared chat-completions providers instead of letting the published Pages Function lag behind the agentic-graph Dev proxy.
...
When `chatStorageTarget` is `chatAgenticGraph`, Agnes output must satisfy all of the following:
...
- `huijoohwee/docs/agentic-graph-storyboard-demo.md`
- `huijoohwee/docs/agentic-graph-video-demo.md`
- `huijoohwee/docs/agentic-graph-animatic-demo.md`
...
- Shared provider/runtime tests in `agentic-graph/canvas` are passing for Agnes provider normalization, shared chat-completions option shaping, and MainPanel Integrations readiness.
...
- A real deployed Agnes BYOK smoke remains pending only because no `AGNES_API_KEY` or `AGENTIC_OS_CHAT_PROXY_AGNES_API_KEY` is available in the current execution environment.
...
Agnes behavior is governed by the shared agentic-graph implementation surfaces above. Archived upstream copies are not kept as runtime authority.

## Planning revision — reference implementation

All five roles below consume `PLAN-AGENTIC-GRAPH-AGNES-AI-API-PRD-TAD-ADR-MVP-GTM@2.0.1`. Existing source and runtime observations retain their original revisions and scope; this documentation update renews no deployment or demand evidence. The guideline is [v2.7.0](https://github.com/huijoohwee/huijoohwee.github.io/blob/e8d2a10a8d3e5735c43edf350a22523df05fdf91/guidelines/prd-tad-adr-mvp-gtm-guidelines.md); the shared maturity rubric loads on demand.

| Role | Owning content at this revision |
|---|---|
| PRD | [Product Requirements](agentic-graph-agnes-ai-api-prd-tad-adr-mvp-gtm.md#product-requirements) |
| TAD | [Technical Contract](agentic-graph-agnes-ai-api-prd-tad-adr-mvp-gtm.md#technical-contract) |
| ADR | [Architecture Decisions](agentic-graph-agnes-ai-api-prd-tad-adr-mvp-gtm.md#architecture-decisions) |
| MVP | [MVP — reference implementation](agentic-graph-agnes-ai-api-prd-tad-adr-mvp-gtm.md#mvp--reference-implementation) |
| GTM | [GTM — reference implementation](agentic-graph-agnes-ai-api-prd-tad-adr-mvp-gtm.md#gtm--reference-implementation) |

## MVP — reference implementation

Reuse the minimum scope, acceptance conditions and component owners identified above. The demonstration must follow the documented entry, permitted action, durable outcome and readback, including its stated failure/recovery path. Use `npm run agnes:readiness:check` for its actual coverage and the named feature checks in the specification; attach exact source, command, result and authoring/mirror/delivery surface to each VCC before advancing readiness. A source locator or structural check alone proves no user outcome.
...
Source review is bounded to repository `7fb85741121d8c2886027e4a630d013ba91c1027`. Confirmed: these referenced artifacts exist at that revision: [`canvas/vite.config.ts`](https://github.com/huijoohwee/agentic-graph/blob/7fb85741121d8c2886027e4a630d013ba91c1027/canvas/vite.config.ts), [`scripts/check-agnes-readiness.mjs`](https://github.com/huijoohwee/agentic-graph/blob/7fb85741121d8c2886027e4a630d013ba91c1027/scripts/check-agnes-readiness.mjs), [`canvas/src/lib/chatEndpoint.ts`](https://github.com/huijoohwee/agentic-graph/blob/7fb85741121d8c2886027e4a630d013ba91c1027/canvas/src/lib/chatEndpoint.ts). Their existence does not confirm every behavior asserted by the specification.
