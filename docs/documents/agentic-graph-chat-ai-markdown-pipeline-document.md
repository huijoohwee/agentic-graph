...
- Base contract system prompt (`chatBaseKgcResponseContractPrompt.ts` for `chatAgenticGraph`, `chatBaseResponseContractPrompt.ts` for standard chat; re-exported by `chatResponseBaseContract.ts`)
...
When `chatStorageTarget=chatAgenticGraph`, the `kgc` block or renderable literal MCP structured surface is validated before final persistence:
...
- `chatAgenticGraph` creates one timestamped session folder and persists `kgc_<session>.md` there.
