---
title: "Reference implementation: agentic-graph Codebase Responsibility Flow PRD/TAD"
...
doc_path: "docs/documents/agentic-graph-codebase-responsibility-flow-prd-tad.md"
...
# Reference implementation: agentic-graph Codebase Responsibility Flow PRD/TAD

## Executive Summary

The Codebase Responsibility Flow is a generated ownership index that maps every entry in the
agentic-graph canvas `settingsRegistry` to its source provenance. It does not claim coverage of
...
The recommended tech-stack optimization keeps agentic-graph a **single browser-first, local-first SPA**
...
in `$GITHUB_ROOT/agentic-graph`. No deployment to the Prod mirror or Cloudflare occurs until the
...
| Discover | Searches the responsibility index by Area or Key | `docs/agentic-graph-codebase-responsibility-flow.md` | Manual grep returns scattered hits  | One row resolves the full ownership chain |
...
`docs/agentic-graph-codebase-responsibility-flow.md`, bounded parts under
`docs/agentic-graph-codebase-responsibility-flow/`,
...
**Component**: `TAD-CODE-INDEX` — Responsibility Index File (`docs/agentic-graph-codebase-responsibility-flow.md`)
...
live server round-trip the interaction primitive. agentic-graph is offline-first, local-first,
...
| Docs | This PRD/TAD | `docs/documents/agentic-graph-codebase-responsibility-flow-prd-tad.md` | `spec-complete` | `undocumented` |
