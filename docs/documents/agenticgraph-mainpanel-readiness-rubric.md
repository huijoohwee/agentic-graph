---
title: "AgenticGraph MainPanel Readiness Rubric"
...
# AgenticGraph MainPanel Readiness Rubric
...
| MainPanel MCP AgenticGraph-owned rows | Mixed `documented` and `browser-published` | Some rows are browser-readable readiness/setup surfaces rather than executable MCP routes |
| External bridge ids `agenticgraph.tool.catalog`, `agenticgraph.tool.search`, `agenticgraph.tool.describe`, `agenticgraph.tool.call` | `runtime-executable` on local stdio; Dev browser bridge for approved artifact calls | The runtime exposes only opaque host-approved capabilities. External writes additionally require a digest-bound, expiring, single-use approval after a separate browser confirmation. This is not Prod or Cloudflare proof. |
