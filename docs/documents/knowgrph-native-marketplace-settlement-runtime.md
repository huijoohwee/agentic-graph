...
This document is the operational contract for the Knowgrph native vendor settlement layer. The Bundle Graph Durable Object remains the authoritative store for bundle state, vendor splits, payout state, and ordered settlement events. D1 stores versioned vendor/rule reference data and non-authoritative reporting projections.
