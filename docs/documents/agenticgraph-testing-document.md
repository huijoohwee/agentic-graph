# AgenticGraph Testing Architecture
...
- Run `npm run test:smoke:xr-spatial-capture-fallback:source` for the repo-native XR fallback source bundle. It delegates to `scripts/run-xr-spatial-capture-fallback-source-smoke.mjs`, which chains the focused `canvas.xrMode.nativeSessionPolicy` source contract, the focused `xr.spatialCaptureFallback.browserSmokeContract` seam check, and the XR readiness contracts without launching a browser.
- Run `npm run xr:source-runner:test` to verify the repo-owned XR source-runner contract itself before or alongside the XR acceptance path.
- Run `npm run xr:review-ready` for the one-command review path that chains the source-runner contract and the local runtime/browser acceptance proof.
- Run `npm run test:smoke:xr-spatial-capture-fallback:browser` for the Dev-only XR fallback smoke that mounts `canvas/src/features/testing/XrSpatialCaptureFallbackSmokePage.tsx`, drives `canvas/scripts/verify_xr_spatial_capture_fallback_browser_smoke.mjs`, and writes `data/outputs/xr-spatial-capture-fallback-browser-smoke.json`.
- Run `npm run xr:runtime-ready` for the repo-owned XR fallback acceptance path. It chains `test:smoke:xr-spatial-capture-fallback:source` and `test:smoke:xr-spatial-capture-fallback:browser`; the canonical proof boundary is documented in `docs/documents/agenticgraph-xr-spatial-capture-fallback-readiness.md`.
- XR fallback scope is the browser-native capability slice only: the shared XR session policy must publish `agenticgraph-xr-capability-snapshot/v1`, the rendered spatial-capture entry owner must expose the exact DOM capability markers, and a non-immersive browser with camera capture available must settle to `monocular-capture` without reverse-engineering UI text.
...
```yaml
AGENTICGRAPH_EDA_MLP_INTERVIEW_MD_PATH:
  scope: test_global
  type: string (file path)
...
const relativePath = process.env.AGENTICGRAPH_EDA_MLP_INTERVIEW_MD_PATH;
...
|------------------------------------|----------------------------------------------|--------------------------------------------------|
| `AGENTICGRAPH_EDA_MLP_INTERVIEW_MD_PATH` | Interview markdown fixture path            | `../data/test-data/eda-mlp-interview-session.fixture.md` |
| `AGENTICGRAPH_TEST_DATA_DIR`           | Root directory for test fixtures             | `../data/test-data`                              |
