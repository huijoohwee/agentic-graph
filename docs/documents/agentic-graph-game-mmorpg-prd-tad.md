...
agentic-graph browser-persistence facade. It is a persistent single-player strategy
...
that do not already host the agentic-graph persistence adapter. It stores the same
...
`agentic-graph.inspect_game_os` and `agentic-graph.control_local_world`. The latter accepts
...
`agentic-graph.game-os-operation-result/v1` with a zero-cost record and nullable typed
...
Each response uses `agentic-graph.game-os-status/v1`, normalized entries,
...
`agentic-graph repository-authored neutral mesh fixture`, licence `CC0-1.0`, source
...
Run from the agentic-graph repository root:
...
swift test --scratch-path /tmp/agentic-graph-game-os-swift-golden --filter GameOsPersistentStrategyGoldenTests
swift test --scratch-path /tmp/agentic-graph-game-os-swift-full
```
...
- Canvas continuity delegates durable reads and CAS writes to
  `agentic-graph-storage-engine-persistence`; its adapter contains no IndexedDB schema;
...
`@agentic-graph/agentic-ecs` package exposes only its five bounded mutation/query
...
| agentic-graph IndexedDB/tool projection facade | focused-tested | undocumented | install Persistent Strategy through the canonical scene adapter and visual UI owner |
...
|---|---|---|---|---|
| `game-os-core-integrate` | isolated authoring candidate | protected agentic-graph main | protected review and exact candidate evidence required | closed |
| `game-os-package-consume` | protected agentic-graph package | GameXR frontend lane | pinned export plus separate repository claim required | closed |
