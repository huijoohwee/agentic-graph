...
2. **Q6 application registration:** the canonical browser contract and lazy registry now expose `inspect_local_python_learning` and `control_local_python_learning` over the existing runtime. Python files in Editor select a separate `pythonLearning` discovery scope: two learning tools, five core tools and one scope selector, preserving the 16-tool/32-KiB project budget. Existing floating-panel and mission scopes retain priority; the native scope selector exposes Python explicitly when either is active. The native schema validator runs before executor effects. The full production-build browser runner supplies a controlled `registerTool` host, verifies actual application registration, rejects missing/extra/stale bindings, checks request replay, and executes Run and explicit debrief Save while offline. This is application integration proof when its exact-candidate receipt passes; it is not browser-vendor API availability or a live external agent session. OS/Canvas invocation catalogs remain separate owners.
