...
doc_type: "Reference"
id: "knowgrph-stripe-mcp-reference"
version: "0.1.0"
...
ownership_boundary: "Upstream capture only. Knowgrph readiness, secret custody, and MainPanel wiring remain owned by docs/documents/knowgrph-mcp/knowgrph-stripe-mcp-service.md."
tags:
  - "stripe"
  - "mcp"
  - "payments"
...
| Concern | Owner |
|---|---|
| Upstream MCP capability capture (this file) | `docs/documents/knowgrph-api-reference/knowgrph-stripe-mcp-reference.md` |
| Knowgrph MCP readiness, secret custody, MainPanel wiring | `docs/documents/knowgrph-mcp/knowgrph-stripe-mcp-service.md` |
| Payment rail architecture and harness contracts | `docs/documents/knowgrph-payments-prd-tad.md` |
| Invocation surface and gateway federation contract | `agentic-canvas-os/docs/MCP-GATEWAY.md` |
| Normative acceptance criteria | `.kiro/specs/knowgrph-payments/requirements.md` |

This file introduces no transport tier. It documents one external transport that the existing
Knowgrph MCP federation may register.
...
| Local transport | `@stripe/mcp@latest`, per the existing Knowgrph service doc | `docs/documents/knowgrph-mcp/knowgrph-stripe-mcp-service.md` |
...
or move money; those are the tools Knowgrph keeps behind the existing approval gate with human
confirmation.
...
That design choice is why Knowgrph treats `stripe_api_write` as the highest-risk registered
tool rather than treating tool count as the risk signal.

## Upstream Safety Guidance (binding for Knowgrph)

| Guidance | Knowgrph position |
|---|---|
...
| Treasury tools that move money, pay bills, and manage cards are an opt-in extension requiring an access request | Excluded from Knowgrph scope this increment. |
...
| Tool execution by an external agent host | Host-side, not billed to Knowgrph | Logged per call by the Cost_Observer wherever Knowgrph mediates the call |
...
1. Upstream marks the server as public preview, so tool names and groupings can change without a
   Knowgrph-visible version pin. Re-capture before depending on any single tool name.
...
4. No StraitsX equivalent MCP surface is documented. See `knowgrph-straitsx-api-reference.md`.
