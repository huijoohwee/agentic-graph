...
doc_type: "Reference"
id: "knowgrph-straitsx-api-reference"
version: "0.1.0"
...
companion_reference: "docs/documents/knowgrph-api-reference/knowgrph-straitsx-authentication-reference.md"
acos_invocation:
...
([StraitsX API Guides](https://docs.straitsx.com/docs/introduction)). Knowgrph therefore treats
...
| Family | Capability summary (paraphrased) | Knowgrph relevance this increment |
|---|---|---|
...
## Rail Capability Map for Knowgrph

| Knowgrph need | Family | Endpoint group | Reference |
|---|---|---|---|
...
proof possible before any live credential exists, which is the basis of the Knowgrph per-rail
...
1. **Idempotency.** No request-level idempotency key header is documented for payment or payout
   creation. `not documented upstream`. Knowgrph compensates with a per-attempt request reference
...
2. **Callback authenticity.** Source-address allowlisting is documented; no signature header is
   ([Webhooks](https://docs.straitsx.com/reference/webhooks)). Knowgrph compensates by reading
