...
geo_xr_contract: "/docs/documents/knowgrph-geo-xr-mode-prd-tad-ard.md"
regional_profile_companion: "/docs/documents/knowgrph-adm0-singapore-prd-tad-ard.companion.md"
evidence_companion: "/docs/documents/knowgrph-game-city-building-sim-vcc-delivery.md"
---

# Reference implementation: Knowgrph City-Building Simulation PRD/TAD/ADR

## 1. Authority and contract boundaries

This document owns the City simulation product contract: parcel state,
...
| Discover | Inspect current metrics and grid | City Builder | No parcel may be selected yet | Explain the next valid action |
...
1. The parser validates source identity, initial grid, and geographic profile.
...
source's initial grid without writing it.
...
|---|---|---|
| `CityInitialState` | version, stable parcels, integer metrics | reject incomplete or duplicate parcel identity |
...
use whole safe integers. Each fixed tick visits parcels in stable id order.
...
|---|---|---|---|
| Determinism | fixed tick; at most 64 by 64 parcels | safe integers, stable order, atomic candidate | 02 |
...
| Accessibility | pointer, keyboard, touch, and coordinate parity | normalized action boundary | 05, 08 |
...
**Consequence:** generic surface arbitration and regional data can change
without changing City mechanics.

## 17. Evidence, readiness, and delivery delegation

The [City Simulation VCC and delivery register](./knowgrph-game-city-building-sim-vcc-delivery.md)
...
[split conformance inventory](./knowgrph-prd-tad-adr-conformance-report.md#reference-implementation-2026-07-31-split-conformance)
...
- the [ADM0 Singapore companion](./knowgrph-adm0-singapore-prd-tad-ard.companion.md)
...
| `knowgrph.inspect_local_city_sim` | Tool identity | City embedded-tool owner | empty object | browser-local read | 0 |
| `knowgrph.control_local_city_sim` | Tool identity | City embedded-tool owner | native invocation or one structured operation | browser-local approval-gated mutation | 0 |
