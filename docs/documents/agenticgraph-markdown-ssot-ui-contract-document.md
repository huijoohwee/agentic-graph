---
title: "Reference implementation: AgenticGraph Markdown SSOT UI Contract"
id: "md:agenticgraph-markdown-ssot-ui-contract-document"
doc_type: "Document"
version: "1.1.0"
date: "2026-07-30"
lang: "en-US"
...
doc_path: "docs/documents/agenticgraph-markdown-ssot-ui-contract-document.md"
---

# Reference implementation: AgenticGraph Markdown SSOT UI Contract
...
- Shared markdown logic must live in `agenticgraph/grph-shared` and be imported via `grph-shared/*` exports (no `grph-shared/src/*` runtime imports).
...
- Canonical derivation API: `useActiveGraphRenderData()` in `agenticgraph/canvas/src/hooks/useActiveGraphData.ts`.
...
- **Canonical implementation**: `agenticgraph/canvas/src/features/markdown-workspace/MarkdownWorkspaceToolbar.tsx` (`MarkdownWorkspaceToolbar`).
- **Required behavior**:
...
- **LocalStorage (UI state)**: keys are SSOT in `agenticgraph/canvas/src/lib/config.ls.ts`.
...
- **Dexie (workspace + cached sources)**: `agenticgraph/canvas/src/features/source-files/sourceFilesDb.ts`
...
- Floating panel (Inspector / Props / Tool Menu) is the baseline typography ladder for non-markdown panels.
  ...
  - Shared selector: `agenticgraph/canvas/src/lib/ui/panelTypography.ts` (returns `PanelTypography`).
  - `PanelTypography` SSOT type: `agenticgraph/grph-shared/src/ui/panelTypography.ts`.
...
Shared markdown logic is contractually owned by `agenticgraph/grph-shared`.
