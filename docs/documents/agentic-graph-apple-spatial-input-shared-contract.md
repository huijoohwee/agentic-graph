---
title: "agentic-graph Apple Spatial Input Shared Contract"
...
# agentic-graph Apple Spatial Input Shared Contract

## Outcome

agentic-graph is the root source and upstream owner for the portable backend in
...
agentic-graph `motionControlDeviceSensorRuntime` is a thin frontend adapter that keeps
the existing `agentic-graph.motion-control-device-sensors/v2` projection while
...
consumer of agentic-graph's backend and may differ only in frontend and visual
...
Its native adapter consumes the agentic-graph root SwiftPM products. Both routes are
pinned to protected agentic-graph revision
...
## agentic-graph camera-control boundary
...
and follow-target computation. agentic-graph's Fixed Follow, Free Orbit, Timeline,
