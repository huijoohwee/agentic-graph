---
title: "AgenticGraph Game Flight Sim PRD/TAD"
id: "md:agenticgraph-game-flight-sim-prd-tad"
...
normative_kiro_authority: ".kiro/specs/agenticgraph-game-flight-sim/"
readiness:
...
constraints:
  - "one bounded offline single-player flight mission projected into native MapLibre on the composed Geo+XR surface"
  - "Flight gameplay requires no sign-in, camera permission, passkey, model, outbound request, or Cloudflare service; Geo provider reads retain their independent owner"
  - "native AgenticGraph Agentic ECS with ephemeral runtime state; only validated Decisions persist through browser-local WorkspaceFs"
  ...
  - "native MapLibre owns visible Geo+XR Flight geometry; one retained transparent R3F Canvas owns simulation/input/readiness only and paints no competing world"
clean_room_policy:
...
source_references:
  agentic_ecs: "docs/documents/agenticgraph-agentic-entity-component-system-prd-tad.md"
  sibling_game_fps: "docs/documents/agenticgraph-game-fps-prd-tad.md"
  renderer_owner: "gympgrph/src/GeospatialHost.tsx"
  runtime_canvas_owner: "canvas/src/lib/three/ThreeGraph.impl.tsx"
  ...
  kiro_authority: ".kiro/specs/agenticgraph-game-flight-sim/"
  validation_seed: "docs/workspace-seeds/agenticgraph-game-flight-sim-demo.md"
  authored_world_source: "docs/workspace-seeds/agenticgraph-physics-playground-demo.md"
  clean_room_gate: "scripts/check-game-flight-sim-boundary.mjs"
---

# AgenticGraph Game Flight Sim PRD/TAD

Governed by the same solo-dev AI-native orientation as the sibling `agenticgraph-game-fps-prd-tad.md`: every decision is evaluated through the four compounding lenses (min-viable-max-value, TCO-zero, token economics, harness-first). Repository-owned focused and browser proof gates are registered and must bind the unchanged exact candidate at handoff. Protected integration, production, and Cloudflare deployment remain separate and unauthorized here.

## Status boundary

This extensioned `.md` PRD/TAD is a derived implementation/proof projection of the normative repository-tracked Kiro source of truth at `.kiro/specs/agenticgraph-game-flight-sim`. The tracked package inventory and hashes are readiness-gated; any optional root-workspace Kiro copy must remain byte-identical and is not an independent authority. `canvas/src/features/game-flight-sim/`, its Flight Sim panel, strict invocation, two browser WebMCP tools, asset admission, and Decisions-only WorkspaceFs adapter are source-owned in AgenticGraph. Local proof is not protected integration, production, or deployment proof; those states remain explicit below.

## Outcome

AgenticGraph gains browser-local mission-based flight training on one composed **Geo+XR Mode** surface. The selected native MapLibre 2D/3D Classic/Modern view remains the visible geospatial world and renders the georeferenced Flight route, waypoints, a source-generated fixed-pixel aircraft icon over pose-derived geometry, selected XR stage footprint, authored structures, and placed subjects. The existing React Three Fiber XR Canvas remains mounted above it only as a pointer-transparent simulation/input/readiness surface; its competing Flight and XR geometry are suppressed. The shared DOM HUD remains above both. One shared training projection appears in FloatingPanel **Media**, **Animation**, **Motion Control**, **Game Mode**, **Flight Sim**, and **Camera**. Media environment cards prepare Flight publication before opening the shared **Geo** panel, so a failed source or layer gate cannot strand the operator on a blank map. The Flight workspace seed selects Geo+XR directly without replacing the selected map view. It opens from a source-backed run-ready document, the shared surface catalog, browser WebMCP, or the strict `/flight.sim @canvas #flight` invocation. Desktop keyboard/pointer, mobile touch, standard gamepad, and optional Motion Control input arm deterministic native Agentic ECS missions with in-repo flight dynamics, AABB terrain collision, MapLibre/HUD day/night presentation, bounded practice failures, visible and spoken coaching, scored outcomes, selectable camera source, and Decisions-only WorkspaceFs persistence.

Core gameplay requires no camera, account, passkey, model, remote asset, Flight-owned transport, token, or Cloudflare service. Flight never replaces global browser transport functions; an explicit gameplay transport attempt is rejected before its executor runs. The independent Geo runtime retains its established MapLibre style/tile provider transport, and Motion Control retains its checked-in LiteRT asset owner. The required aircraft is admitted through an **img2threejs-style TypeScript module plus a small JSON scene spec** containing identity, procedural renderer shape, dimensions, collision size, color, and zero-call metadata. It is git-diffable, human-auditable, offline-loadable, rejects non-null `opaqueBinaryFallback`, and records a required-aircraft GLB fallback count of zero. The distinct optional beacon proves the Kiro opaque-fallback boundary with one self-contained, committed-local, CC0-1.0 GLB; remote references and missing/unreadable local bytes fail closed without a fetch. External references inform conceptual principles only. Contributors attest that all implementation, instructional content, and assets are source-authored; external project identity and URL are prohibited in product source and runtime metadata; and there is no external project dependency. The deterministic clean-room scanner cannot prove the absence of arbitrary derived code.

The four-mode projection consumes one selected
[ADM0 environment companion](./agenticgraph-adm0-singapore-prd-tad-ard.companion.md)
for regional anchor, presentation framing, camera policy, stage structures, and
placed subjects. Flight owns only mission-coordinate conversion, route,
aircraft, and camera intent; it cannot redefine regional facts. No second
geography, copied boundary, external asset, or external simulator dependency is
admitted.
...
|---|---|---|---|
| Enter | Apply the source-backed flight seed or invoke `/flight.sim @canvas #flight operation=open` | Run-ready activation | Geo retains native MapLibre and renders Flight layers while one transparent XR Canvas retains only simulation/input/readiness |
...
- One Geo+XR presentation path that retains native MapLibre 2D Classic/Modern and 3D Classic/Modern as the visible Flight projection while the existing transparent R3F Canvas paints no Flight or XR geometry; plain Geo retains its separate interactive DOM/SVG Flight projection.
...
- Strict native `/flight.sim @canvas #flight` invocation with typed `errorCode` plus offending `field`/`token` diagnostics, and browser-local `agenticgraph.inspect_local_flight_sim` / `agenticgraph.control_local_flight_sim` WebMCP with a 2,000 ms deadline and explicit timeout/unavailable results.
...
Given an invocation, exactly one `/flight.sim`, one `@canvas`, and one `#flight` token is accepted. Missing/mismatched/duplicate sigils, malformed or duplicate pairs, unknown keys, mixed structured/native input, invalid throttle, and unsupported lifecycle operations fail closed with stable `errorCode` and applicable `field`/`token` diagnostics. Browser agent-ready registration exposes only `agenticgraph.inspect_local_flight_sim` and `agenticgraph.control_local_flight_sim` for this surface; each completes or returns `FLIGHT_SIM_WEB_MCP_TIMEOUT` within 2,000 ms, and inactive inspection returns `FLIGHT_SIM_STATE_UNAVAILABLE` without mutation. It adds no stdio tool, HTTP mutation route, remote gateway, or deployment authority. The private Agentic ECS stdio lane remains exactly three tools.

#### AC-9: shared Canvas surface and renderer ownership

Given Geo+XR owns the active surface, entering Flight Sim retains the selected native MapLibre world and applies dedicated, topmost GeoJSON layers for the authored route, a conditional aircraft-to-active-objective course segment, waypoint/objective points, and fixed-pixel heading-aware aircraft. The course segment is derived from the same pure route-guidance state as the exclusive plain-Geo SVG overlay, north-up navigation inset, and compact mobile/desktop HUD cue; it exposes objective label, distance, bearing, and signed heading error, disappears after completion, and is guidance rather than predicted trajectory. The existing R3F Canvas remains mounted and pointer-transparent for simulation, input, WebGL readiness, and lifecycle integration, while all of its Flight and XR visual geometry is suppressed. The shared DOM HUD remains above both and keeps per-tick course guidance outside its polite objective-transition live region. No fallback scene, second R3F Canvas, alternate rendered XR world, Flight-owned map provider, or Flight-owned map camera is introduced. A typed deterministic projection converts local mission coordinates through the selected ADM0 companion. The shared Camera catalog supplies exactly `fixed-follow` / `free-orbit`: Fixed Follow drives MapLibre Chase, Cockpit, or Survey framing, Cockpit derives a collision-clear forward look-ahead from the canonical Flight target, and Free Orbit yields pan and zoom to MapLibre. Timeline camera-marks temporarily map their sampled authored pose into the visible MapLibre center, bearing, pitch, and zoom before returning to the last selected option. Every telemetry surface renders normalized compass heading from the same flight-model projection. While FloatingPanel is open at its default top-right position, the pointer-transparent HUD uses the same responsive width expression to clear the panel while keeping mobile controls above bottom choreography surfaces.

Plain Geospatial Mode remains an exclusive, interactive Geo surface and may use the existing transparent DOM/SVG Flight projection without mounting the XR Canvas. In Geo+XR Mode the Flight/XR input surface owns flight controls while Geo retains its native provider view. `2d`, `2d-modern`, `3d`, and `3d-modern` remain the actual MapLibre modes; the transparent R3F layer paints no environment, grid, wash, or duplicate terrain. City and FPS gameplay retain their existing 3D precedence. A Geo or Geo+XR Flight handoff never clears the selected map view or adds a Flight-owned provider, renderer, map camera, transport, token, copied asset, or external dependency.

#### AC-10: synchronous admission and resumable lifecycle

WebGL support is resolved synchronously before mission start; unsupported WebGL or unreadable Decisions keeps the mission stopped with a local error. Geo+XR entry waits for the visible MapLibre owner to apply the Flight layers and camera and commit an actual map render within the source-apply first-playable budget; the hidden R3F stage cannot acknowledge that preparation. Start then prepares a MapLibre-presented ready frame at tick zero within 100 milliseconds and waits for normalized desktop, pointer, touch, gamepad, Motion Control, or MCP input before fixed ticks begin. Blur and document hide always pause the clock without changing state. Fixed-follow pointer release pauses; free-orbit exits pointer lock without pausing. Stop then Start resumes the exact in-memory mission; malformed hydration blocks Start and Restart until **Reset local save** succeeds. The run-ready lifecycle captures the complete pre-document surface, including Geo enablement, before the Flight preset applies. The shared Physics base initializes beneath Flight without re-activating an already-active XR surface; if Flight frontmatter has not yet entered XR, the base uses the canonical surface transaction while preserving the pending Flight gameplay owner. Entry failure restores that surface without presenting an active Flight surface; restoration failure is reported and never presented as successful. Exit disposes the Flight mission’s ECS World, clears pending ephemeral run state, restores prior ownership through the canonical Geo bridge, and—when the prior surface was non-Geo—returns success only after MapLibre has been disposed for two committed frames. A prior Geo surface retains its shared map.

While Flight is active, optional Motion Control initialization reuses only the checked-in LiteRT runtime assets already prepared by the repository. Flight exposes no fetch, XMLHttpRequest, WebSocket, EventSource, sendBeacon, or Service Worker capability and does not replace global transport functions. Its explicit gameplay-call seam still rejects a transport attempt before the supplied executor runs. Geo provider and Motion Control asset requests retain their independent owners. After initialization, pose continues through the existing normalized input adapter; it does not gain Canvas, camera, renderer, flight-policy, persistence, or gameplay-network ownership.
...
| Flight dynamics | `packages/apple-spatial-input/src/flight.ts` | Shared deterministic integration and envelope projection; no external physics engine |
| Collision | `canvas/src/features/game-flight-sim/flightSimSpatialProfile.ts` | In-repo AABB terrain resolution against the canonical authored spatial profile |
...
| Rendering | `ThreeGraph.impl.tsx`, `CanvasViewportGeospatialOverlay.tsx`, `flightSimGeospatialProjection.ts`, and Gympgrph Flight overlay layers | In Geo+XR, native MapLibre renders the geospatial world and Flight geometry; one transparent XR Canvas retains simulation/input/readiness with all R3F visuals suppressed; retain exclusive plain-Geo projection separately |
| Camera/input arbitration | `xrNativeControllerCameraCatalog.ts`, `xrNativeControllerCameraRuntime.ts`, `flightGeoOverlayMapLibreCamera.ts`, Timeline camera-marks, and Motion Control adapter | Fixed Follow drives visible MapLibre Chase/Cockpit/Survey framing; Free Orbit yields pan/zoom to MapLibre; the transparent runtime Canvas retains capture/lifecycle integration and Motion Control contributes normalized input only |
| Flight transport capability | `flightSimExternalCallGuard.ts` | Reject explicit Flight gameplay transport attempts before their executors run; leave Geo provider and Motion Control asset transport with their existing independent owners |
...
| Activation | `docs/workspace-seeds/agenticgraph-game-flight-sim-demo.md` with Physics as `shared_xr_scene.source_authority` | Source-backed run-ready activation; overlay-only world ownership |
...
flowchart LR
  ...
  VIEW --> SURFACE{"Shared surface policy"}
  SURFACE --> THREE["Existing R3F Canvas"]
  SURFACE --> GEO["Existing interactive Geo renderer + transparent Flight overlay"]
...
Flight Sim uses the native Agentic ECS for ephemeral runtime state and projects onto the active shared surface. Geo+XR uses `CanvasViewportGeospatialOverlay` to publish deterministic Flight coordinates into the native MapLibre host, which renders all visible Flight geometry. The existing transparent `ThreeGraph` React Three Fiber Canvas retains only simulation/input/readiness and renders neither Flight actors nor XR environment geometry. Plain Geo remains an exclusive renderer path. A second R3F Canvas, duplicate terrain, Flight-owned map camera, bitECS, Babylon.js, or another ECS is rejected because it duplicates an existing repository owner.
...
| `npm run game-flight-sim:browser-smoke` | Mandatory two-run exact-HEAD XR proof | Child-isolated native MapLibre surface in all four Geo views; one transparent terrain-free Flight XR Canvas; exact selected ADM0 companion reference and visible camera center; planar 2D stage/asset footprints; native 3D structure and placed-subject extrusions; registered source-generated fixed-pixel aircraft images over pose-derived geometry; georeferenced route and active-objective course; mode-correct camera pitch; non-live mobile HUD course cue; first frame under 3 s; desktop/WebMCP input; trusted Chromium-emulated touch at 375x812; ordered route completion with guide removal; lifecycle/failure/Exit disposal; camera and Timeline round-trip; Geo-attributed provider requests only; and cleanup-before-transactional-evidence publication |
| Exact-main Media → Geo Flight acceptance | Mandatory handoff proof | Existing Geo renderer visibly retained beneath exactly one Flight route/aircraft/HUD/input overlay, no active R3F Flight stage, selected environment retained, and Geo panel still interactive |
| Exact-main Geo+XR four-view acceptance | Mandatory composed-surface proof | Native MapLibre remains visible for 2D Classic/Modern and 3D Classic/Modern and renders the same companion-authored stage, structures, selected subjects, route, course director, and native aircraft geometry; camera policy matches the selected companion; one transparent R3F Canvas retains simulation/input/readiness with zero competing Flight/XR visuals; map transforms remain interactive; provider requests remain Geo-owned |
...
| Protected baseline PR `#369` | Merged at `f5530e141b7d76ade09c091617e253f930f95eff` | Historical Flight runtime baseline only |
| Current four-mode Flight/Geo/XR candidate | Exact revision, PR, and proof status reported externally at handoff | No tracked-document integration, deployment, or public-runtime claim |
...
Each `agenticgraph-flight-sim-browser-run/v5` record carries `runIndex` 1/2 or 2/2, exact candidate/source identity, first-frame timing, trusted Chromium-emulated touch provenance, the four exact MapLibre view/style/projection results, rendered Flight layer and live-coordinate evidence, map interaction, ordered route transitions and terminal Decision, camera ownership, lifecycle/failure evidence, and separated Geo-provider/Flight-gameplay transport inventories. Gameplay-network proof invokes the explicit Flight guard with a real browser transport inside a never-invoked executor, then requires exact mission-state equivalence apart from the monotonic revision and runtime diagnostic, the exact runtime error/deadline observation, zero executor invocation, and zero observed transport; it does not replace Geo-owned browser transports globally. The aggregate `agenticgraph-flight-sim-browser-proof/v5` record requires both runs to share the same identity. Native Pointer Lock is attempted first; only the exact automation-host `WrongDocumentError` may activate the browser-interface contract harness, while production code retains native request and rejection diagnostics. The final handoff reports the exact evidence-bound SHA and source hash without embedding a self-invalidating commit hash in this tracked document.

Both commands are finite and local apart from ordinary build/test artifacts, access no paid model or runtime image-to-3D service, and do not deploy or mutate Cloudflare. The physics-playground seed (`docs/workspace-seeds/agenticgraph-physics-playground-demo.md`) is the canonical shared XR source authority: it selects the companion-authored regional terrain, selectable airplane/helicopter subjects, the Physics-controller camera with exactly fixed-follow/free-orbit, Motion Control boundary, and `/ @ #` MCP grammar.
...
| MCP-ready | Exactly `agenticgraph.inspect_local_flight_sim` and `agenticgraph.control_local_flight_sim` are browser-local WebMCP tools with a 2,000 ms deadline, explicit timeout/unavailable results, and no stdio, HTTP mutation route, remote gateway, or deployment authority. |
