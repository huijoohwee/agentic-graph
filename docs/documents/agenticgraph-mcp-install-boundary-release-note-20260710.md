# AgenticGraph MCP Install Boundary Release Note 2026-07-10

## Summary

AgenticGraph now publishes one explicit operator-facing MCP install rule for remote hosts.

Map intent. Orchestrate agents. Prove outcomes.

A source-backed canvas where `/` routes work, `#` sets meaning, and `@` binds context.

- Public install and discovery stay on `https://airvio.co/agenticgraph/mcp`
...
- The canonical onboarding and host recipes now live in
  `docs/documents/agenticgraph-mcp-install-contract.md`
...
overview, open `docs/documents/agenticgraph-mcp-onboarding-index.md`.
...
- Map intent: `https://airvio.co/agenticgraph/mcp` for basic remote MCP installation, discovery,
  read-only retrieval, prompt discovery, resource discovery, and inspection
- Orchestrate agents: `https://airvio.co/agenticgraph/control-plane/mcp` only when the host can
...
1. install `https://airvio.co/agenticgraph/mcp` first
2. add `https://airvio.co/agenticgraph/control-plane/mcp` only for session-capable hosts
3. execute grammar through `agenticgraph.agentic_canvas_os.docs.invoke`
...
- `https://airvio.co/agenticgraph/.well-known/mcp/apps/agenticgraph-agent-ready.html`
...
- Do not document `/agenticgraph/mcp` as if it already exposes the control-plane grammar tool.
