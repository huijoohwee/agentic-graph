---
title: "agentic-graph Docs/Sheets/Slides Export Runtime Readiness"
id: "md:agentic-graph-docs-sheets-slides-runtime-readiness"
...
# agentic-graph Docs/Sheets/Slides Export Runtime Readiness
...
1. Human OAuth access token: `AGENTIC_OS_GOOGLE_ACCESS_TOKEN`.
2. Human OAuth refresh: `AGENTIC_OS_GOOGLE_CLIENT_ID`,
   `AGENTIC_OS_GOOGLE_CLIENT_SECRET`, and
   `AGENTIC_OS_GOOGLE_REFRESH_TOKEN`.
3. Service account: `AGENTIC_OS_GOOGLE_SERVICE_ACCOUNT_JSON` plus either
   `AGENTIC_OS_GOOGLE_IMPERSONATED_USER` for Workspace domain-wide delegation or
   `AGENTIC_OS_GOOGLE_SHARED_DRIVE_FOLDER_ID` for a destination folder in a shared
   drive. `AGENTIC_OS_GOOGLE_DRIVE_FOLDER_ID` may constrain only human-OAuth output.
...
1. `AGENTIC_OS_MICROSOFT_ACCESS_TOKEN`; or
2. `AGENTIC_OS_MICROSOFT_CLIENT_ID` and
...
`AGENTIC_OS_MICROSOFT_TENANT` and `AGENTIC_OS_MICROSOFT_SCOPE`. Set
`AGENTIC_OS_MICROSOFT_ONEDRIVE_FOLDER` when acceptance artifacts should be grouped
...
- `AGENTIC_OS_EXPORT_MICROSOFT_FALLBACK_ENABLED` defaults to enabled.
- `AGENTIC_OS_EXPORT_FLEET_PATH` selects an isolated ledger.
- `AGENTIC_OS_ROOT` selects the repository root for bounded artifact resolution.

## Focused local verification

Run from the agentic-graph repository root. These commands do not require provider
...
```bash
AGENTIC_OS_ROOT="$(pwd)" node ./mcp/server.js
```
