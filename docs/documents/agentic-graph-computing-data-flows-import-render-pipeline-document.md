# agentic-graph Computing Data Flows: Import → Render Pipeline (End-to-End)
...
- Normalized E2E fixtures may attach `agentic-os:readingSummary` to node records when AGENTIC_OS-readable summaries are needed. Do not add a body-side `## AGENTIC_OS Reading Layer`, line-start `@node:...`, or line-start `@edge:...` section to mirror frontmatter.
...
- For published validation fixtures such as `agentic-graph-video-demo.md` and `agentic-graph-token-economics-model-demo.md`, a driver like `agent_token_take_rate` must render as exactly one editable KTV row with the output handle attached to that row when the field and port resolve to the same schema path.
...
| Parsing | Markdown parser returns `context=frontmatter-flow`, expected node/edge counts, and zero warnings. | Missing `flow:portTypes`, synthetic handles, malformed YAML, body `flow:` mirrors, or body-side `@node:` / `@edge:` AGENTIC_OS reading layers. |
...
- Publish-demo syntax guards should reject body-side `## AGENTIC_OS Reading Layer`, line-start `@node:`, line-start `@edge:`, and body `flow:` mirrors for frontmatter-driven Storyboard Widget documents.
...
- If `superagent_harness_template` is present, it must name native agentic-graph owners and the no-copy DeerFlow inspiration boundary.

The publish-side reusable template is `huijoohwee/docs/agentic-graph-storyboard-widget-computing-flow-template.md`; it is intentionally generic and should remain project-agnostic.
