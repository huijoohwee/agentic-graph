# agentic-graph — Telegram “Import URL” (FOSS + TCO) Recommendation + Implementation Plan

## Goal (as stated)

Telegram → user sends a URL → agentic-graph imports/parses it (reuse/expose existing **Import URL** pipeline) → return a shareable **airvio.co** (agentic-graph) link that opens the content in the Markdown editor.
...
## Existing agentic-graph capability (reuse points)

agentic-graph already has:
...
- Dev/preview “API” middleware endpoints pattern (Vite middleware) and an existing concept of artifact persistence for website imports (e.g., `.__agentic-graph-workspace/.../website-imports/...`).
...
- **Self-hostable**: single VM/container runs everything (agentic-graph static + ingest API + Telegram handler).
...
- Response: send a Telegram message containing the “Open in agentic-graph” link.
...
- Persist:
  - `.agentic-graph-workspace/telegram-imports/<docId>/page.md`
...
- Existing `agentic_graph_parser.webpage_cmd.py` (simple HTML → Markdown) can serve as a no-dependency fallback.
...
1) Add a small server (Python FastAPI recommended) under `agentic-graph/server/`:
   - `POST /api/import-url`
   - `GET /api/docs/<docId>`
   - `GET /r/<docId>`
2) Implement storage layout under `.agentic-graph-workspace/telegram-imports/`.
