# agentic-graph Graph Canvas Layout SSOT
...
For edge shapes (bezier/straight/step/smoothstep) and cross‑renderer edge path behavior, see the **Edge Types (Global SSOT)** section in `agentic-graph-renderer-document.md`; layout docs do not duplicate edge‑type details.
