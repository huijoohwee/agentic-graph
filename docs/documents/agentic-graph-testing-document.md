# agentic-graph Testing Architecture
...
- Run `npm run xr:runtime-ready` for the repo-owned XR fallback acceptance path. It chains `test:smoke:xr-spatial-capture-fallback:source` and `test:smoke:xr-spatial-capture-fallback:browser`; the canonical proof boundary is documented in `docs/documents/agentic-graph-xr-spatial-capture-fallback-readiness.md`.
- XR fallback scope is the browser-native capability slice only: the shared XR session policy must publish `agentic-graph-xr-capability-snapshot/v1`, the rendered spatial-capture entry owner must expose the exact DOM capability markers, and a non-immersive browser with camera capture available must settle to `monocular-capture` without reverse-engineering UI text.
...
```yaml
AGENTIC_OS_EDA_MLP_INTERVIEW_MD_PATH:
  scope: test_global
  type: string (file path)
...
const relativePath = process.env.AGENTIC_OS_EDA_MLP_INTERVIEW_MD_PATH;
...
|------------------------------------|----------------------------------------------|--------------------------------------------------|
| `AGENTIC_OS_EDA_MLP_INTERVIEW_MD_PATH` | Interview markdown fixture path            | `../data/test-data/eda-mlp-interview-session.fixture.md` |
| `AGENTIC_OS_TEST_DATA_DIR`           | Root directory for test fixtures             | `../data/test-data`                              |
