---
title: "AgenticGraph Strybldr - PRD and TAD"
doc_type: "Combined PRD/TAD"
id: "agenticgraph-strybldr-prd-tad"
version: "0.2.1"
...
# AgenticGraph Strybldr - PRD and TAD

## Document Map

This combined PRD/TAD follows `$GITHUB_ROOT/huijoohwee.github.io/guidelines/prd-tad-guidelines.md`. It defines a 3-hour hackathon slice for AgenticGraph Strybldr: import an image, reverse engineer it into editable storyboard element cards, let the user update those cards, and generate or hand off a bounded video request through the existing BytePlus ModelArk run owners.
...
Strybldr converts one imported image into a source-backed storyboard and video-generation handoff. The MVP must reuse AgenticGraph’s existing Launch import, queryable-corpus media source units, Source Files parsing, Storyboard renderer, Floating Panel shell, settings/integrations, and BytePlus video-generation task owner.
...
If AgenticGraph can import one image, produce editable element-level storyboard cards, and generate or prepare one bounded video handoff from the approved cards, then a solo dev can demo an AI-native image-to-video orchestration loop with high ROI, zero mandatory monthly TCO, and visible provenance.
...
| AgenticGraph maintainer | Extend existing surfaces without drift. | Must reuse shared owners. |
...
- Given a PNG, JPEG, WebP, GIF, or AVIF image, when Import Image runs, then AgenticGraph creates a corpus media source unit with original name, MIME hint, byte size, source path, and source-unit ID.
...
|---|---|---|
| Dev | `$GITHUB_ROOT/agenticgraph` | Implement, typecheck, focused tests, browser smoke. |
| Prod mirror | `$GITHUB_ROOT/huijoohwee/content/agenticgraph` | Sync built/static output only after validation. |
| Cloudflare | `https://airvio.co/agenticgraph` | Publish only with correct Wrangler credentials and live smoke. |
...
| Deploy smoke | Open `https://airvio.co/agenticgraph` after publish. | Route loads and no bad runtime requests. |
...
- `docs/documents/agenticgraph-strybldr-prd-tad.md` has valid YAML frontmatter and stays under repository hygiene budgets.
