# AgenticGraph API Surfaces (Dev/Preview)

AgenticGraph Canvas is primarily a client-side app. Local “API” surfaces used by the UI are implemented as Vite middleware during **dev** and **preview** runs.
...
- Canonical API contracts: [agenticgraph-image-to-threejs-api.md](agenticgraph-image-to-threejs-api.md) owns typed image projection and lifecycle; [agenticgraph-image-to-glb-api.md](agenticgraph-image-to-glb-api.md) owns compact connected-contour reconstruction, rigid pivots and sockets, the inspection clip, separated quality admission, GLB/editable glTF fidelity, and external-inspiration isolation.
...
- Runtime boundary: iframe rendering remains owned by AgenticGraph. External hosts may size or position the frame but must not reimplement canvas state.
...
- Home and MainPanel import: **Import canvas embed** is available on Home and under **MainPanel Settings → Canvas Embed**. Both entry points reuse the shared canvas-embed import panel and accept the generated iframe HTML or a JSON `postMessage` envelope with `type: "agenticgraph.canvas-embed.select"`, `version: 1`, and either `embedUrl` or `iframe`. The optional `sourcePath` preserves source identity. Generic imports normalize `kgPreview=1&kgLiveHero=1`. The canonical cold-start preset resolves the published Physics Playground identity through `XR_PHYSICS_DEMO_PUBLISHED_CANONICAL_PATH`; its source frontmatter selects XR/3D without renderer query overrides, and Settings exposes it as **Use Physics Playground background**.
...
- Path: `/agenticgraph/control-plane/mcp` (POST, MCP initialize and `tools/call` for `agenticgraph.agentic_canvas_os.docs.invoke`).
...
- Implementation: [superagent_harness.py](../../agenticgraph_parser/superagent_harness.py), local stdio transport in [server.js](../../mcp/server.js), and local MCP tool-contract ownership in [local-tool-contract.js](../../mcp/local-tool-contract.js)
...
- MCP tools: `agenticgraph.sandbox.policy.validate` and `agenticgraph.sandbox.policy.authorize`
...
- Contract source: [README.md](../../mcp/README.md) and [agenticgraph-mcp-service-prd-tad.companion.md](agenticgraph-mcp/agenticgraph-mcp-service-prd-tad.companion.md)

### Agentic video workflow (local and control-plane MCP)

- MCP tool: `agenticgraph.video_remix.run`
...
- Schema owner: [workflow-contract.js](../../mcp/video-remix/workflow-contract.js), reused by [local-tool-contract.js](../../mcp/local-tool-contract.js) and the Cloudflare [tool-registry.mjs](../../cloudflare/workers/agenticgraph-mcp/tool-registry.mjs).
...
- Boundary: ViMax informs the capability checklist only. AgenticGraph imports no ViMax code, prompt, provider configuration, fixture, test, or runtime dependency. Prod and Cloudflare remain deployment-gated.

### API-native browser MCP bridge

- MCP tool: `agenticgraph.browser_api.run`
...
- Implementation: [vite.config.ts](../../canvas/vite.config.ts)
- Validation: video-agent external test URLs are supplied through `AGENTICGRAPH_VIDEO_AGENT_TEST_URLS` for focused tests, and through the user-configurable `VIDEO_AGENT_VALIDATION_CONFIG_STORAGE_KEY` / `VITE_AGENTICGRAPH_VIDEO_AGENT_VALIDATION_URLS` runtime config for the app. Both paths are exercised by the same Toolbar Launch Import URL flow, which calls this endpoint and then materializes source-owned video markdown with video corpus metadata without committing URL fixtures in repo source. Director-style orchestration is represented only through the native `VIDEO_AGENT_REFERENCE_BOUNDARY` contract: inspiration-only, native AgenticGraph implementation, no external code copy, and no external video-agent runtime dependency.
...
Route owner: `huijoohwee/functions/__chat_proxy/[[path]].js`. Public route: `airvio.co/__chat_proxy/*`. Shared helper owner: `huijoohwee/functions/api/_integrationHub.js`. Deploy owner: `npm run pages:deploy-cloudflare` from `agenticgraph`, which uploads the `huijoohwee` publish repo to Cloudflare Pages project `joohwee`.
...
`x-kg-ai-gateway-cache-ttl`. When `AGENTICGRAPH_CHAT_PROXY_AI_GATEWAY_BASE_URL` plus a matching
...
|---|---|---|
| `AGENTICGRAPH_CHAT_PROXY_OPENAI_API_KEY` | Server-managed OpenAI proxy key | Cloudflare Pages project secret |
| `AGENTICGRAPH_CHAT_PROXY_MIROMIND_API_KEY` | Server-managed MiroMind proxy key | Cloudflare Pages project secret |
| `AGENTICGRAPH_CHAT_PROXY_AGNES_API_KEY` | Server-managed Agnes proxy key | Cloudflare Pages project secret |
| `AGENTICGRAPH_CHAT_PROXY_SEALION_API_KEY` | Server-managed AI Singapore SEA-LION proxy key | Cloudflare Pages project secret |
| `AGENTICGRAPH_CHAT_PROXY_BYTEPLUS_API_KEY` | Server-managed BytePlus proxy key | Cloudflare Pages project secret |
...
| `AGENTICGRAPH_CHAT_PROXY_AI_GATEWAY_BASE_URL` | Cloudflare AI Gateway universal base URL. **Legacy:** `https://gateway.ai.cloudflare.com/v1/<account_id>/<gateway_id>`. **Modern REST API:** `https://api.cloudflare.com/client/v4/accounts/<account_id>/ai` for `/run` and `/v1/chat/completions` | Cloudflare Pages project variable |
| `AGENTICGRAPH_CHAT_PROXY_AI_GATEWAY_TOKEN` | Server-managed Cloudflare AI Gateway token | Cloudflare Pages project secret |
| `AI_GATEWAY_TOKEN` | Fallback AI Gateway token alias | Cloudflare Pages project secret; cleanup debt only |
| `AGENTICGRAPH_CHAT_PROXY_AI_GATEWAY_GATEWAY_ID` | Optional explicit gateway id header override | Cloudflare Pages project variable |
| `AGENTICGRAPH_CHAT_PROXY_UPSTREAM` | Optional upstream base override | Cloudflare Pages project variable |
| `AGENTICGRAPH_CHAT_PROXY_TIMEOUT_MS` | Upstream timeout override | Cloudflare Pages project variable |
| `AGENTICGRAPH_INTEGRATION_ALLOWED_HOSTS` | Canonical additional host allowlist | Cloudflare Pages project variable |
| `AGENTICGRAPH_CHAT_PROXY_ALLOWED_HOSTS` | Legacy host allowlist alias | Cloudflare Pages project variable; cleanup debt only |

#### GitHub To Cloudflare Deploy Chain For This Route

1. Edit the production route owner in `huijoohwee/functions/__chat_proxy/[[path]].js` or update the canonical source/deploy logic in `agenticgraph`.
2. From `agenticgraph`, run `npm run pages:build-sync` so the publish repo mirror stays aligned.
3. From `agenticgraph`, run `npm run pages:functions:build` to build the Pages Functions bundle and regenerate `huijoohwee/_worker.js` plus `huijoohwee/_routes.json`.
4. From `agenticgraph`, run `npm run pages:deploy-cloudflare`, which executes `wrangler pages deploy ../huijoohwee --project-name=joohwee --branch=main --commit-dirty=true`.
...
### Storage Worker: `agenticgraph-storage`
...
| GET upgrade | `/api/storage/canvas-room/{workspaceId}/{roomId}` | Join an authenticated room; `roomId=runtime-identity:agenticgraph:main` is application-global and permits only ping, challenge, attestation, and server peer-lifecycle messages, with no identity or asset persistence |
...
| Browser env | Purpose |
|---|---|
| `VITE_AGENTICGRAPH_STORAGE_BASE_URL` | Absolute origin for storage Worker browser calls |
| `VITE_AGENTICGRAPH_STORAGE_WORKSPACE_ID` | Workspace identity resolved by the browser storage/chat client |
| `VITE_AGENTICGRAPH_STORAGE_CHAT_SESSION_TOKEN` | Authenticated session token forwarded as `Authorization: Bearer ...` to chat routes and `kg_session_token` on the canvas-room WebSocket upgrade |
...
### Payment Worker: `agenticgraph-payment`

Route owner: `cloudflare/workers/agenticgraph-payment`. Cloudflare route: `airvio.co/api/payments/*`.
...
Required production runtime configuration lives on `agenticgraph-payment`, not on Cloudflare Pages project variables:
...
|---|---|---|---|---|
| OpenAI chat proxy | Bearer API key | `AGENTICGRAPH_CHAT_PROXY_OPENAI_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| BytePlus ModelArk chat proxy | Bearer API key | `AGENTICGRAPH_CHAT_PROXY_BYTEPLUS_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| MiroMind chat proxy | Bearer API key | `AGENTICGRAPH_CHAT_PROXY_MIROMIND_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| Agnes AI chat proxy | Bearer API key | `AGENTICGRAPH_CHAT_PROXY_AGNES_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| AI Singapore SEA-LION chat proxy | Bearer API key for `https://api.sea-lion.ai/v1` | `AGENTICGRAPH_CHAT_PROXY_SEALION_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| AI Singapore SEA-LION MCP sidecar | Bearer API key | `AGENTICGRAPH_MCP_SEALION_API_KEY` on the local MCP host process | No browser path | Never |
| Qwen chat proxy | Bearer API key | `AGENTICGRAPH_CHAT_PROXY_QWEN_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| Google Cloud Vertex AI chat proxy | OAuth access token | `AGENTICGRAPH_CHAT_PROXY_GOOGLE_CLOUD_ACCESS_TOKEN` on the Vite or server proxy runtime | Yes | RAM only |
| Google Gemini chat proxy | Gemini API key via `x-goog-api-key` | `AGENTICGRAPH_CHAT_PROXY_GEMINI_API_KEY` on the Vite or server proxy runtime | Yes | RAM only |
| GrabMaps API | Bearer token | `AGENTICGRAPH_GRABMAPS_API_TOKEN` on the API proxy runtime | Yes | RAM only |
| GrabMaps MCP | Bearer token | `AGENTICGRAPH_GRABMAPS_MCP_TOKEN` on the MCP host process | No browser path | Never |
| Stripe Checkout and webhook | Restricted or secret API key | Worker secret `STRIPE_RESTRICTED_KEY` or `STRIPE_SECRET_KEY` on `agenticgraph-payment` | No | Never |
| Solana Pay settlement | RPC URL or private binding | Worker secret/private binding `SOLANA_PAY_RPC_URL` on `agenticgraph-payment` | No | Never |
...
| Proxy injection | Dev chat proxy | `x-kg-chat-api-key` header (BYOK) or `process.env.AGENTICGRAPH_*` (serverManaged) | `Authorization: Bearer <key>` | None | Return 400 with provider-specific error message if key missing |
...
| Hardcode API key in `.env.example` | Leaks a real or plausible key; misleads contributors | Use `${AGENTICGRAPH_<PROVIDER>_API_KEY}` placeholder text |
