---
title: "Reference implementation: agentic-graph Markdown Processing"
doc_type: "Document"
version: "1.1.0"
date: "2026-07-30"
lang: "en-US"
...
# Reference implementation: agentic-graph Markdown Processing
...
## Overview
The Markdown rendering engine in agentic-graph has been enhanced to support GitHub-style code blocks with semantic HTML structure and optimized token sharing architecture. This ensures high performance during rendering cycles and a consistent, accessible UI.

## SSOT UI Contract
Markdown Editor/Viewer/Presentation must share a single SSOT header and a single SSOT Contents sidebar/TOC structure.

- SSOT contract document: `agentic-graph/docs/documents/agentic-graph-markdown-ssot-ui-contract-document.md`
- Header SSOT: `agentic-graph/canvas/src/features/markdown-workspace/MarkdownWorkspaceToolbar.tsx`
- Runtime SSOT: `agentic-graph/canvas/src/lib/markdown-workspace-runtime/MarkdownWorkspaceRuntime.impl.tsx`
- Shared workspace UI types: `agentic-graph/canvas/src/features/markdown-explorer/workspaceUi.ts` + `agentic-graph/canvas/src/features/markdown-workspace/markdownUtils.ts`
- Sidebar SSOT: `canvas/src/features/markdown/ui/MarkdownPanelLayout.tsx` (frame + sections)
- Shared utils SSOT: `agentic-graph/grph-shared/src/markdown/*` (wikilinks/backlinks/slugify)
...
- **Markdown File Extensions (Graph Pipeline)**: Files ending in `.md`, `.markdown`, `.mmd`, or `.mdx` are treated as Markdown inputs for graph extraction (`isMarkdownLikeFileName` in `agentic-graph/grph-shared/src/markdown/mermaidInput.ts`).
...
- **Webpage SSOT conversion**: For HTML/CSS/JS → Markdown SSOT rules (URL resolution, lazy media src filling, and duplicate-content forbiddance), use `agentic-graph-webpage-proxy-and-srcdoc-iframe-document.md` + `agentic-graph-markdown-editor-mode-contract-document.md` as the canonical references.
...
For cross-feature smoke testing (Frontmatter + Mermaid + GeoJSON + Presentation), use a repo-local fixture under `data/test-data/` or pass an operator-owned fixture explicitly through `AG_MARKDOWN_SLIDE_DEMO_PATH` / `AG_EXTERNAL_FIXTURE_ROOT`.

For slide-demo interop (anchors/callouts/wikilinks), keep the bounded tests aligned with the same fixture:

- `agentic-graph/canvas/src/__tests__/markdown/markdownSlideDemoSupportInterop.test.ts`
- `agentic-graph/canvas/src/__tests__/markdown/markdownJsonLdSlideDemoInterop.test.ts`
