...
docs_dependency:
  ...
  ref: "e0ef770860905830157e64c455f0a342084b6d25"
  ...
  proof_tokens: ["/asset.create", "#procedural-asset", "/runtime-ready.check", "/release.complete", "/repository.pack", "/agentic-graph.probe-tree", "/application.compose", "/skill.evolve", "/agent.team", "/voice.studio", "/ecs.session-start", "/ecs.world-tick", "/ecs.decision-persist", "/agent.toolkit", "/adlc.observe", "/camera.select", "/motion.control", "/xr.stage", "/xr.place", "/xr.transform", "/xr.label", "/xr.remove", "/xr.physics", "/xr.present", "#runtime-ready", "#multi-agent-collaboration", "#repository-packing", "#agentic-graph.probe-tree", "#application-composition", "#skill-evolution", "#role-based-agent-team", "#voice-clone", "#speech-to-text", "#text-to-speech", "#agentic-ecs", "#agent-toolkit", "#adlc-observability", "#pose", "#transform", "#world", "#body", "#impulse", "#controller", "#reticle", "@operator", "@working-directory", "@repository-root", "@implementation-run", "@application-manifest", "@component-catalog", "@integration-profile", "@skill-catalog", "@skill-policy", "@agent-team", "@source.frontmatter", "@runtime-proof", "@agentic-graph.probe-tree", "@mcp-gateway", "@ecs-session", "@agent-toolkit-observer", "@scene", "@canvas", "@audio", "@text", "@voice-profile", "@approval-gate", "@cost-log", "/sandbox.policy.validate", "#agent-sandbox-policy", "@sandbox-policy"]
local_proof:
  provider_mode: "mock"
...
audio, text, profile, approval, cost, and proof bindings. agentic-graph independently
implements `agentic-graph.voice.studio` as a closed local stdio facade for `clone`,
...
The reviewed Function Calling proof uses the separate `env.dev` Worker named `agentic-mcp-dev`. That environment repeats the MCP and Run Manifest Durable Object bindings, keeps `AGENTIC_OS_LIVE_CLIENTS="0"`, serves only a `workers.dev` hostname, and declares no `airvio.co` route. Its Dev-only bearer authenticates both the Agentic service client and the proof manifest read-back. The top-level production routes and deploy command remain a separate gate.

The accepted 2026-07-19 proof is bound to Agentic Canvas OS revision `a7ac73f427c10957b37d016e6a55592b578c381f` and its canonical `LIVE-REVIEWED-FUNCTION-PROOF.md`. One recovered durable continuation completed one logical `gpt-5.6-luna` run in two Responses requests and one signed reviewed call. agentic-graph returned an `applied` native receipt, and authenticated read-back found the exact note at revision 1. Aggregate returned usage was 546 input and 55 output tokens with USD 0.000876 estimated cost. The evidence proves only this route-free Dev lane; no production route, Pages deployment, custom domain, or live stage client changed.
...
```bash
npx wrangler secret put AGENTIC_OS_AGENT_RUNTIME_BEARER_TOKEN \
  --config cloudflare/workers/agentic-graph-mcp/wrangler.toml
npx wrangler secret put AGENTIC_OS_AGENT_MODEL_ID \
  --config cloudflare/workers/agentic-graph-mcp/wrangler.toml
```
...
```bash
npx wrangler secret put AGENTIC_OS_AGENT_RUNTIME_BEARER_TOKEN \
  --config cloudflare/workers/agentic-graph-mcp/wrangler.toml --env dev
npm --prefix cloudflare/workers/agentic-graph-mcp run deploy:dev
```
