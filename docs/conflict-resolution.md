...
- `worktree:check` validates the contract's canonical source registry without fetching or starting Dev; `ci:integration` runs it first and the pre-push hook inherits the same fast failure. `status` is the informational lifecycle view and does not replace that fail-closed gate.
