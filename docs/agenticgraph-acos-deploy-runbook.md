...
doc_path: "docs/agenticgraph-acos-deploy-runbook.md"
---

# Reference implementation: AgenticGraph Protected Release Runbook
...
|---|---|---|---|---|---|
| `AGENTICGRAPH-AUTHORING-TO-MIRROR` | Authoring → Mirror | `ER-REL-B1`; result not recorded | `OI-REL-B1`: Production dispatch with exact `source_sha` and `local_review_candidate` | discard the candidate; rerun the verify job; compare candidate, manifest, and lifecycle digests | closed |
| `AGENTICGRAPH-MIRROR-TO-DELIVERY` | Mirror → Delivery | `ER-REL-B2`; result not recorded | `OI-REL-B2`: while the exact run is pending, use the interactive terminal command to submit the protected `production` approval and evidence | follow **Rollback**; rerun live smoke, revision, document-seed, and browser-fidelity checks | closed |
...
- the canonical AgenticGraph and Agentic Canvas OS checkouts are both clean on `main`, with
...
AgenticGraph canonical release-owner checkout must remain the same clean `main` checkout at that
...
Before `workflow_dispatch`, create one immutable `agenticgraph-production-release-evidence/v1` object
...
- immutable candidate-origin probes run first, the stable `joohwee.pages.dev` route runs as a
  distinct Pages transport, and `airvio.co` plus `airvio.co/agenticgraph` run as custom-domain
...
- Earlier recovery dispatch: run `30771147307` for source revision `32d2cfca34f7d5bf484b4a8f449083954a476bd8` failed closed at source-to-mirror parity because `huijoohwee.github.io/schema/AgenticRAG/agenticgraph-documents-map.graph.jsonld` was missing the relocated XR document node.
