...
This Git-backed file is the canonical Physics Playground and XR readiness seed. On upgrade, Source Files preserves the retired canonical Playground file as a byte-identical recovered note under `notes` before removing its old canonical row. Imported files remain user-owned. After changing it in Dev, restart the serving dev process, reload the page, and use **Source Files → Refresh**; generated demo instances are local working documents. Verify the expected heading or frontmatter change in the editor because Refresh alone may retain an earlier loaded seed. Preserve or export a customized seed before refreshing because canonical seed reconciliation replaces its working copy. A local save, a workspace snapshot upload, and a GitHub commit are distinct results.
