...
| `knowgrph-game-city-building-sim-demo.md` | `city-sim` | Geo+XR: native MapLibre with a transparent shared R3F semantic media stage | implementation candidate | Deterministic local grid/economy with City Builder coordinate parcel input; reuses existing Flight Geo store/layers for a read-only stopped aircraft/route while Flight gameplay/readiness stay inactive; no unregistered City R3F mesh or duplicate map/source/layer/Canvas; runtime-ready status follows exact-head source and browser proof |
