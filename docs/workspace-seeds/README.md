...
| `knowgrph-game-city-building-sim-demo.md` | `city-sim` | Geo+XR: native MapLibre below the shared City R3F stage | implementation candidate | Deterministic local grid/economy and parcel input; reuses existing Flight Geo store/layers for a read-only stopped aircraft/route while Flight gameplay/readiness stay inactive; no duplicate map/source/layer/Canvas; runtime-ready status follows exact-head source and browser proof |
