...
`knowgrph/docs/workspace-seeds` is the only authored source for Knowgrph workspace seeds. The XR Physics source is `knowgrph-physics-playground-demo.md`; its `run_ready_demo.id`, source identity, scene composition, Motion Control, and optional Game Mode projection are edited here first. The Flight Sim source is `knowgrph-game-flight-sim-demo.md`; its `run_ready_demo.id` (`flight-sim`), source identity, native flight demo, asset pipeline, shared Camera catalog, Motion Control handoff, and Flight Sim projection are edited here first. It is an XR Mode overlay on the Physics-authored world and supplies a pure aircraft follow/framing descriptor to the canonical Physics controller camera; it owns no second rendered XR world, scene owner, Canvas, or camera driver. The City Sim source is `knowgrph-game-city-building-sim-demo.md`; its `run_ready_demo.id` (`city-sim`), source-authored initial grid and geographic profile, Geo+XR composition, deterministic civic state, local persistence, and projections across the existing Media, Animation, Motion Control, Game Mode, Flight Sim, and Camera panels are edited here first. City retains one real native MapLibre basemap, projects live parcels from source-authored meter dimensions and gaps exactly once through the City-owned `kg-city-sim:geo-overlay` source and fill/extrusion/outline/selection layers, frames those parcels inside the visible map aperture, and wraps the map in the shared semantic `SemanticMediaFigure`. It does not project the selected local XR environment, its source, or its features. A separate read-only aircraft/route overlay is reused above City with presentation owner `city`; it does not activate or borrow Flight bootstrap, camera, gameplay, environment, or readiness ownership. City mounts zero Three Canvas or alternate world. The MMORPG source is `knowgrph-game-mmorpg-demo.md`; its planned `run_ready_demo.id` (`mmorpg`), source identity, native offline RPG world, three-track asset provenance pipeline, camera source, and MMORPG World projection are edited here first. Draft seeds use `planned_run_ready_demo` and do not become activation authorities until their runtime-readiness and browser-smoke gates exist and pass.
...
| `knowgrph-game-city-building-sim-demo.md` | `city-sim` | Geo+XR: one real native MapLibre basemap with meter-scaled City parcel layers, wrapped by `SemanticMediaFigure`; zero local XR environment and zero City Three Canvas | implementation candidate | Applied source authors the initial grid and geographic profile; one City parcel source/layer family renders live zoning and selection at source-authored meter scale, no selected local XR environment source/features are projected, and the independently reused aircraft/route renders above City with owner `city` and no Flight bootstrap/camera/gameplay/readiness; runtime-ready status follows exact-head source and browser proof |
