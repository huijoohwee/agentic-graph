...
ci_scopes:
  ...
  protected_ci_evidence:
    ...
    commands:
      - ["node", "--test", "scripts/__tests__/ci-evidence-inputs.test.mjs"]
  ...
  core_runtime_release:
    ...
    commands:
      ...
      - ["node", "--test", "scripts/__tests__/production-mirror-maintenance-proof.test.mjs", "scripts/__tests__/core-runtime-release.test.mjs", "scripts/__tests__/core-runtime-release-domain.test.mjs", "scripts/__tests__/production-mirror-artifact.test.mjs", "scripts/__tests__/runtime-release-migrations.test.mjs"]
...
- Protected release verification may reuse the exact successful main-push `ci:integration` evidence through the pinned Agentic OS owner and `.agentic-os-ci-evidence.json`. Graph adds the current affected-path/command plan and Python/Chrome versions to its input binding. Current worktree and collaboration checks, release build/parity, isolated and live browsers, and candidate authorization always run fresh. Any missing, stale, failed, mismatched or unavailable evidence executes the original command. The optional producer artifact has one-day retention and grants no authority. See [source evidence](documents/agentic-graph-ci-evidence-reuse.md).
- The owner plan has a 15-minute aggregate bound in addition to the existing per-command limits. It never reuses results whose browser, installed or external inputs are not bound. Broad fallback and unconditional required integration gates remain applicable.
...
- If GitHub misses a protected-main push event, dispatch the existing Integration workflow with `operation=protected-main-recheck`, `branch=main`, `pull_request_number=0`, the exact `expected_head_sha`, and an identifying `operation_id`, on ref `main`. The dispatch rejects any mismatch between that SHA, the Actions ref/revision, checkout HEAD, and freshly observed remote `main`. It runs the same immutable manifest, collaboration, canonical integration, and XR gates with read-only permissions. This recovery creates actual check evidence; it does not authorize a release or replace candidate review. Pull-request refresh continues to require its exact PR metadata.
