...
export async function verifyWorkspaceObservation(page, openDashboard) {
  ...
  try {
    ...
    await page.waitForFunction(async () => (await import('/src/features/monaco/monacoModelRegistry.ts')).readRegisteredTextModelSnapshots()
      .some(model => model.uri.startsWith('inmemory://agent-run/') && model.value.includes('workspace-check-1')))
    generation = 2
    await page.clock.fastForward(15000)
    await page.waitForFunction(async () => (await import('/src/features/monaco/monacoModelRegistry.ts')).readRegisteredTextModelSnapshots()
      .some(model => model.uri.startsWith('inmemory://agent-run/') && model.value.includes('workspace-check-2')))
