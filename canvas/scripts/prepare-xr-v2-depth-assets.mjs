...
async function ensurePinnedModelFile(file) {
  ...
  for (let attempt = 0; attempt < MODEL_DOWNLOAD_ATTEMPTS; attempt += 1) {
    ...
    if (
      ...
      || attempt === MODEL_DOWNLOAD_ATTEMPTS - 1
    ) {
      break
    }
...
export async function prepareXrV2DepthAssets() {
  for (const modelFile of MODEL_FILES) {
    await ensurePinnedModelFile(modelFile)
  }
  await Promise.all(WASM_FILES.map(ensurePinnedWasmFile))
  process.stdout.write(
    `[agentic-graph] prepared pinned ${XR_V2_DEPTH_MODEL_ID}@${XR_V2_DEPTH_MODEL_REVISION} and same-origin ONNX Wasm assets in ${publicRoot}\n`,
  )
}
