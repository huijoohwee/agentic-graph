...
function createSyntheticPullRequestMerge(t) {
  const repositoryRoot = mkdtempSync(resolve(tmpdir(), 'knowgrph-xr-v2-traversal-'))
  t.after(() => rmSync(repositoryRoot, { force: true, recursive: true }))
  git(repositoryRoot, ['init', '--initial-branch=main'])
  git(repositoryRoot, ['config', 'user.email', 'xr-v2-test@knowgrph.invalid'])
  git(repositoryRoot, ['config', 'user.name', 'Knowgrph XR v2 test'])
