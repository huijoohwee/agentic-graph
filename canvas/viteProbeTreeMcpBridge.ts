...
const buildChildEnv = (repoRoot: string): Record<string, string> => {
  const keys = [
    ...
    'TMPDIR',
    'NODE_ENV',
    'AGENTIC_OS_AGENTIC_CANVAS_OS_DOCS_ROOT',
    'AGENTIC_OS_AGENTIC_CANVAS_OS_DOCS_REVISION',
    'AGENTIC_OS_PROBE_TREE_MODEL',
    'AGENTIC_OS_PROBE_TREE_MODEL_PROVIDER',
    'AGENTIC_OS_PROBE_TREE_MODEL_URL',
    'AGENTIC_OS_PROBE_TREE_MODEL_ALLOW_REMOTE',
    'AGENTIC_OS_PROBE_TREE_MODEL_TIMEOUT_MS',
  ]
  const env: Record<string, string> = { AGENTIC_OS_ROOT: repoRoot }
...
export function createProbeTreeMcpBridgePlugin({ repoRoot }: { repoRoot: string }): Plugin {
  return {
    name: 'agentic-graph-probe-tree-mcp-bridge',
    apply: 'serve',
    configureServer(server) {
      server.middlewares.use(AGENTIC_OS_DOCS_MCP_BRIDGE_PATH, async (request, response) => {
        ...
        try {
          ...
          client = new Client({ name: 'agentic-graph-canvas-agentic-os-docs', version: '0.1.0' })
      ...
      server.middlewares.use(PROBE_TREE_MCP_BRIDGE_PATH, async (request, response) => {
        ...
        try {
          ...
          const result = await client.callTool({
            name: AGENTIC_OS_PROBE_TREE_TOOL_NAMES.generate,
            arguments: {
