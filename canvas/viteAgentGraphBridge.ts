...
} from './src/features/agent-graph/agentGraphHostAdapter'
import {
  AgentGraphRepositoryUrlError,
  normalizeAgentGraphRepositoryRemoteUrl,
} from './src/features/agent-graph/agentGraphRepositoryUrl'
import {
  sanitizeAgentGraphImportProgress,
...
const sanitizeImportResult = (value: unknown): Record<string, unknown> => (
  sanitizeAgentGraphImportResult(value, {
...
const sanitizeImportProgress = (value: unknown): Record<string, unknown> => (
  sanitizeAgentGraphImportProgress(value, {
    fail: (code, message, status = 502) => {
      throw new HostBridgeError(code, message, status)
    },
  })
...
async function defaultRuntimeIngest(context: Parameters<RuntimeIngest>[0]): Promise<unknown> {
  return runAgentGraphTool(INGEST_TOOL_NAME, context.args, {
...
export function createAgentGraphBridgeRequestHandler(options: BridgeOptions) {
  ...
  async function handle(request: IncomingMessage, response: ServerResponse) {
    ...
    try {
      ...
      if (!url.pathname.startsWith(AGENT_GRAPH_HOST_ROUTE)) return false
      ...
      if (request.method === 'GET' && route === '/capability') {
        ...
        sendJson(response, 200, {
          schema: AGENT_GRAPH_HOST_CAPABILITY_SCHEMA,
          available: true,
          limits: {
...
export function createAgentGraphBridgePlugin(options: BridgeOptions): Plugin {
  return {
