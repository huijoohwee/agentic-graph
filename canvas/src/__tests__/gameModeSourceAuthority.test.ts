...
test('XR Physics is the only source-backed authority for the Game Mode overlay', () => {
  ...
  assert.equal(
    existsSync(resolve(repoRoot, 'docs/workspace-seeds/agentic-graph-game-fps-demo.md')),
    false,
  )
  assert.doesNotMatch(
...
test('Game Mode browser proof starts explicitly on the XR Physics source', () => {
  ...
  assert.match(launcher, /VITE_AGENTIC_OS_RUN_READY_DEMO \|\|= 'xr-physics'/)
  assert.doesNotMatch(launcher, /VITE_AGENTIC_OS_RUN_READY_DEMO \|\|= 'game-fps'/)
  ...
  assert.match(verifier, /name\.startswith\("agentic_os_xr_empty_world"\)/)
