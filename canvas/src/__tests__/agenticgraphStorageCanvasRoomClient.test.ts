...
export const testAgenticGraphStorageCanvasRoomClientReadsConfigFromStorageEnv = () => {
  ...
  try {
    ...
    if (!config || config.baseUrl !== 'https://airvio.co/agenticgraph' || config.workspaceId !== 'kgws:test-room' || config.sessionToken !== 'sess_test_token' || !config.deviceId.startsWith('dev:')) {
