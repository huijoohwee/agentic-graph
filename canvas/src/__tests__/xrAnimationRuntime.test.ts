...
import {
  readXrMotionReferencePlan, XR_MOTION_REFERENCE_GRAPH_METADATA_KEY,
...
export function testXrAnimationRuntimeIsNativeInvocableAndExportable() {
  ...
  try {
    ...
    useGraphStore.setState({ graphData, markdownDocumentName: 'Shared object.md', markdownDocumentText: '# Shared object' })
    hydrateXrMotionReferenceRuntime({
    ...
    useGraphStore.getState().updateGraphMetadata({ [XR_MOTION_REFERENCE_GRAPH_METADATA_KEY]: serializeXrMotionReferencePlan(readXrMotionReferenceRuntime().plan) })
    ...
    if (!selectedObject.ok
      ...
      throw new Error('expected shared 3D object selection to promote Car into keyboard and Motion Control drivable cast motion without mark focus: ' + JSON.stringify({ ok: selectedObject.ok, message: selectedObject.message, target: readXrSharedMotionTargetId(), actor: objectRuntime.selectedActorId, shot: objectRuntime.selectedShotTargetId, markKind: objectRuntime.selectedMark?.kind, carPosition: carTrack?.marks[0]?.position, tapPosition: selectedObjectTap?.nextPosition, applied: liveObjectMotion.applied, livePosition: liveMotionTrack?.marks[0]?.position }))
    }
    ...
    if (!retimeSource.includes('data-kg-xr-animation-editor-link="individual-lane"') || retimeSource.includes('<XrChoreographyMarkControls')) {
      throw new Error('expected Timeline to link to the Animation authoring owner without duplicate mark controls')
    }
