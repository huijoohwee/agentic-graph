...
export function testXrAnimationRuntimeIsNativeInvocableAndExportable() {
  ...
  for (const marker of ['data-kg-xr-mark-animation-presets="click-appear"', 'data-kg-xr-mark-animation-preset-select={selectedCastMark.id}', '<PanelSelect', '<option key={preset.id} value={preset.id}>', 'XR_ANIMATION_PRESETS.filter(preset => xrAnimationPresetCompatible', "operation: 'apply-animation'"]) {
