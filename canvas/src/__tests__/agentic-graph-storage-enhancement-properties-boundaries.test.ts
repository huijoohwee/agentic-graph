...
import {
  AGENTIC_OS_SOURCE_IMPORT_LIMITS,
  AGENTIC_OS_STORAGE_SYNC_BOUNDS,
} from '@/lib/storage/agentic-graph-storage-bounds'
import type { KgDocumentLocalRecord } from '@/lib/storage/agentic-graph-storage-db'
import {
...
import { pruneStaleSyncEvents } from '../../../cloudflare/workers/agentic-graph-storage/db'
...
// Feature: agentic-graph-storage-sync-enhancement, Property 28: Browser-local field mapping round-trip
...
// Feature: agentic-graph-storage-sync-enhancement, Property 29: sync_events TTL partition
    ...
    async agesMs => {
      ...
      return agesMs.every((ageMs, index) =>
        env.DB.syncEvents.has(`event-${index}`)
          === (ageMs <= AGENTIC_OS_STORAGE_SYNC_BOUNDS.syncEventsTtlMs))
    },
  ), { numRuns: PROPERTY_RUNS })
}

// Feature: agentic-graph-storage-sync-enhancement, Property 30: Generated artifacts split bytes and manifest
...
// Feature: agentic-graph-storage-sync-enhancement, Property 31: Extensions gated on complete readiness
export function testStorageEnhancementProperty31ExtensionsRequireCompleteReadiness() {
  const schemaExtensionDoc = sourceText('../docs/documents/agentic-graph-storage-schemas-extensions-document.md')
  assert(
...
// Feature: agentic-graph-storage-sync-enhancement, Property 32: Server-managed relay fails closed
export function testStorageEnhancementProperty32ServerManagedRelayFailsClosed() {
  const chatAuthSource = sourceText('../cloudflare/workers/agentic-graph-storage/chatAuth.ts')
  assert(
...
// Feature: agentic-graph-storage-sync-enhancement, Property 33: Bounded ingest
export function testStorageEnhancementProperty33BoundedIngest() {
  ...
  assert(
    ingestSource.includes('timeoutMs: AGENTIC_OS_SOURCE_IMPORT_LIMITS.urlTimeoutMs')
      && ingestSource.includes('maxBytes: AGENTIC_OS_SOURCE_IMPORT_LIMITS.maxBytes'),
    'expected URL imports to apply both timeout and byte bounds',
  )
  fc.assert(fc.property(
    fc.string({ maxLength: 4_096 }),
    fc.integer({ min: 0, max: AGENTIC_OS_SOURCE_IMPORT_LIMITS.maxBytes + 128 }),
    (text, boundedInputSize) => {
      const measuredSize = new TextEncoder().encode(text).byteLength
      const accepted = boundedInputSize <= AGENTIC_OS_SOURCE_IMPORT_LIMITS.maxBytes
      return readSourceImportUtf8ByteLength(text) === measuredSize
        && accepted === (boundedInputSize <= 10_485_760)
        && AGENTIC_OS_SOURCE_IMPORT_LIMITS.urlTimeoutMs === 30_000
    },
  ), { numRuns: PROPERTY_RUNS })
}

// Feature: agentic-graph-storage-sync-enhancement, Property 34: Recomposition consistency
...
// Feature: agentic-graph-storage-sync-enhancement, Property 35: Parse round-trip preserves structure
...
// Feature: agentic-graph-storage-sync-enhancement, Property 36: Per-source error isolation with continuation
...
// Feature: agentic-graph-storage-sync-enhancement, Property 37: Version snapshot retention keeps the most recent fifty
export function testStorageEnhancementProperty37VersionSnapshotRetentionKeepsRecentFifty() {
  fc.assert(fc.property(
    ...
    (pathId, count) => {
      ...
      const expectedStart = Math.max(0, count - AGENTIC_OS_STORAGE_SYNC_BOUNDS.maxVersionSnapshots)
...
// Feature: agentic-graph-storage-sync-enhancement, Property 38: Cost boundary rejects remote mutation
...
// Feature: agentic-graph-storage-sync-enhancement, Property 39: Origin guard for mutating actions
