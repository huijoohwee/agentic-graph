...
import { buildProbeTreeStructuredResponse, AGENTIC_OS_PROBE_TREE_TOOL_NAMES, PROBE_TREE_LLM_RESPONSE_CONTRACT_VERSION } from '@/features/agent-ready/probeTreeContract.mjs'
...
export async function testProbeTreeWidgetRunInvokesMcpAndProjectsRelevantProviderCards() {
  ...
  if (
    ...
    || !['/sme-care-agent', '@source.frontmatter', '#runtime-ready', '/agentic-graph.probe-tree'].every(token => (mcpRequest!.invocationTokens as string[]).includes(token))
    ...
    || !published.outputText.includes('mcp=agentic-graph.probe.generate')
  ) {
...
export async function testProbeTreeWidgetRunRefusesGenericNoModelFallback() {
  const authoredRequest = '/agentic-graph.probe-tree invest in China, India, SE Asia?'
    ...
    nodes: [
      {
        ...
        properties: {
          summary: '/agentic-graph.probe-tree Assess SME cyber and supply-chain coverage.',
          probeTreeDepth: 0,
        },
      },
      {
...
export function testProbeTreeContinuationMetadataRoutesWithoutVisibleSlashToken() {
  ...
  if (invocationText.includes('/agentic-graph.probe-tree') || resolveStoryboardWidgetProbeTreeInvocationTokenForNode(node, invocationText) !== '/agentic-graph.probe-tree') {
...
export async function testProbeTreeGenerateRequestStopsContinuationAndPublishesDeliverable() {
  ...
  const rootNode: GraphNode = {
    ...
    type: 'TextGeneration',
    label: 'SME risk review',
    properties: { summary: '/agentic-graph.probe-tree Assess the root SME risk scope.' },
  }
...
export async function testProbeTreeWidgetRunStopsBeforeMcpAtDepthLimit() {
  const graphData: GraphData = {
    ...
    nodes: [{
      ...
      properties: {
        cardTypeLabel: 'Probe-Tree Card',
        prompt: '/agentic-graph.probe-tree',
