...
import { RETIRED_XR_WORKSPACE_SEED_PATH, preserveRetiredXrSeed } from '@/features/workspace-fs/workspaceXrSeedMigration'
...
export async function testXrSeedConsolidationPreservesAuthoredBytes() {
  ...
  const oldText = '---\nrun_ready_demo:\n  id: xr-physics\n---\n# My edited scene\n'
  const retired: WorkspaceEntry = { path: RETIRED_XR_WORKSPACE_SEED_PATH, parentPath: '/docs/workspace-seeds', name: 'agentic-graph-physics-playground-demo.md', kind: 'file', text: oldText, updatedAtMs: 1 }
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.resolve(process.cwd(), '..', RETIRED_XR_WORKSPACE_SEED_PATH.slice(1))), false)
    ...
    const memory = createMemoryWorkspaceFs({ initialEntries: [retired, imported] })
    await memory.ensureSeed()
    const memoryEntries = await memory.listEntries()
    assert.equal(memoryEntries.some(entry => entry.path === retired.path), false)
