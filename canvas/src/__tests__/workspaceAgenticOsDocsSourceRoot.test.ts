...
export function testWorkspaceDocsMirrorLocalRootsSelectsSoleAgenticRootWhenPrimaryRootIsAbsent(): void {
  const seedOnly = resolveWorkspaceDocsMirrorLocalRootRequests({
    docsAbsRoot: '',
    agenticDocsAbsRoot: '',
    workspaceSeedsReadAbsRoot: '/tmp/agentic-graph/docs/workspace-seeds',
  })
  if (
    seedOnly.length !== 1
  ...
  const combined = resolveWorkspaceDocsMirrorLocalRootRequests({
    docsAbsRoot: '/tmp/agentic-graph/docs',
    outputDocsAbsRoot: '/tmp/huijoohwee/docs_',
    agenticDocsAbsRoot: '/tmp/agentic-canvas-os/docs',
    workspaceSeedsReadAbsRoot: '/tmp/agentic-graph/docs/workspace-seeds',
  })
  if (combined[0]?.excludedRelPathRoots?.[0] !== 'workspace-seeds') {
    throw new Error(`expected the general docs root to exclude the agentic-graph-owned seed subtree, got ${JSON.stringify(combined)}`)
  }
  if (combined[1]?.workspaceRootName !== 'workspace-seeds') {
    throw new Error(`expected the agentic-graph seed root to retain its Explorer namespace, got ${JSON.stringify(combined)}`)
  }
