...
export function testAgenticOsPromptContractDocsUseCanonicalImplementedNames() {
  ...
  if (!existsSync(mainPath) || !existsSync(companionPath)) {
    throw new Error('Expected canonical AGENTIC_OS prompt contract docs to exist without proposed suffix')
  }
  ...
  required.forEach(snippet => {
    if (!referenceDocs.includes(snippet)) {
      throw new Error(`Expected canonical AGENTIC_OS prompt contract docs to include ${JSON.stringify(snippet)}`)
    }
  })
  if (referenceDocs.includes(staleBaseName)) {
    throw new Error('Expected AGENTIC_OS prompt contract references to avoid stale proposed document names')
  }
