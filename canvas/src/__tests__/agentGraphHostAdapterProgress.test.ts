...
export async function testAgentGraphBrowserAdapterStreamsRepositoryProgress() {
  const frames = [
    {
      type: 'progress',
      progress: {
  ...
  const bridge = createAgentGraphHostAdapter({
    fetchImpl: async input => {
      ...
      if (route === `${AGENT_GRAPH_HOST_ROUTE}/capability`) {
      ...
      if (route === `${AGENT_GRAPH_HOST_ROUTE}/repositories/stream`) {
