...
export function testMiroMindServerManagedProxyEnvNamesStayAligned() {
  ...
  const expectedSourceSnippets = [
    ...
    'requiresMiroMindKey',
    'process.env.AGENTIC_OS_CHAT_PROXY_MIROMIND_API_KEY || process.env.MIROMIND_API_KEY',
...
export function testGeminiServerManagedProxyEnvNamesStayAligned() {
  ...
  const requiredViteSnippets = [
    ...
    'process.env.AGENTIC_OS_CHAT_PROXY_GEMINI_API_KEY',
  ...
  if (!envSource.includes("'AGENTIC_OS_CHAT_PROXY_GEMINI_API_KEY'")) {
    throw new Error('expected the server-managed environment loader to whitelist AGENTIC_OS_CHAT_PROXY_GEMINI_API_KEY')
  }
...
export function testOpenAiServerManagedProxyLoadsLocalEnvFiles() {
  ...
  const expectedViteSnippets = [
    ...
    'process.env.AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_TOKEN || process.env.AI_GATEWAY_TOKEN || process.env.CLOUDFLARE_API_TOKEN',
    'cf-aig-cache-ttl',
    'cf-aig-metadata',
