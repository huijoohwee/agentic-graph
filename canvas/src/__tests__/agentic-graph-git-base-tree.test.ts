...
import {
  ...
  parseGitTree,
} from '../lib/storage/git/agentic-graph-git-object-codec'
import {
...
test('path-scoped commit replaces its authority snapshot and preserves the fetched base root', async () => {
  const baseDocuments = [
    document('agentic-graph/README.md', 'README.md', '# repository\n'),
    document('agentic-graph/config.json', 'config.json', '{"outside":true}\n'),
    document('agentic-graph/docs/stale.md', 'docs/stale.md', '# stale\n'),
    document('agentic-graph/docs/old.md', 'docs/old.md', '# old\n'),
  ]
  const baseRequest = request('agentic-graph', baseDocuments)
  ...
  const nextDocuments = [
    document('agentic-graph/docs/current.md', 'docs/current.md', '# current\n'),
    document('agentic-graph/docs/index.json', 'docs/index.json', '{"current":true}\n'),
  ]
  const nextRequest = {
    ...request('agentic-graph/docs', nextDocuments),
...
test('authority canonical-to-repository scope mapping fails closed when paths disagree', () => {
  const documents = [
    document('agentic-graph/docs/a.md', 'docs/a.md', '# a\n'),
    document('agentic-graph/docs/b.md', 'content/b.md', '# b\n'),
  ]
  assert.throws(
    () => deriveAgenticGraphGitRepositoryPathScope('agentic-graph/docs', documents),
    /ambiguous/,
  )
  assert.throws(
    () => deriveAgenticGraphGitRepositoryPathScope(
      'agentic-graph/docs',
      [document('agentic-graph/other/a.md', 'docs/a.md', '# a\n')],
    ),
    /outside/,
  )
})
