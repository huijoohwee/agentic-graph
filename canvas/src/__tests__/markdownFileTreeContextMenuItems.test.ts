...
export async function testMarkdownFileTreeShareUrlAwaitsAsyncPublishedUrlBeforeCopy() {
  ...
  const items = buildMarkdownFileTreeContextMenuItems({
    ...
    buildShareUrl: async () => 'https://airvio.co/agentic-graph/share/kg-public-token',
    copyToClipboard: async text => {
      copied.push(text)
      return true
    },
  ...
  if (copied.join(',') !== 'https://airvio.co/agentic-graph/share/kg-public-token') {
...
export async function testMarkdownFileTreeShareActionsReuseCodePanelContract() {
  ...
  if (panelSummary !== 'Share URL:plaintext:https://airvio.co/agentic-graph/share/kg-public-token|Copy Path:plaintext:/docs/public.md|Copy Relative Path:plaintext:docs/public.md') {
    throw new Error(`expected URL and path actions to use the shared code-panel contract, got ${panelSummary}`)
  }
  if (copied.join(',') !== 'https://airvio.co/agentic-graph/share/kg-public-token,/docs/public.md,docs/public.md') {
...
export async function testMarkdownFileTreeShareCanvasEmbedAwaitsAsyncPublishedUrlBeforeCopy() {
  ...
  const items = buildMarkdownFileTreeContextMenuItems({
    ...
    buildCanvasEmbedUrl: async () => 'https://airvio.co/agentic-graph/share/kg-public-token?kgPreview=1',
    copyToClipboard: async text => {
      copied.push(text)
      return true
    },
  ...
  const expectedIframe = buildCanvasEmbedIframeMarkup('https://airvio.co/agentic-graph/share/kg-public-token?kgPreview=1')
  if (copied.join(',') !== expectedIframe) {
    throw new Error(`expected async published canvas iframe markup to be copied, got ${copied.join(',')}`)
  }
  if (selected.join(',') !== '/docs/public.md:https://airvio.co/agentic-graph/share/kg-public-token?kgPreview=1') {
...
export async function testMarkdownFileTreeShareUrlPromptsWhenClipboardUnavailable() {
  ...
  const items = buildMarkdownFileTreeContextMenuItems({
    ...
    buildShareUrl: () => 'https://airvio.co/agentic-graph/share/kg-public-token',
  ...
  await items[0]?.onSelect()
  await new Promise(resolve => setTimeout(resolve, 0))
  if (prompted.join(',') !== 'https://airvio.co/agentic-graph/share/kg-public-token') {
