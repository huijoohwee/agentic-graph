...
import { AgentGraphProjectionError } from '@/features/agent-graph/agentGraphCanvasProjection'
import {
  buildAgentGraphWorkspaceArtifactFileName,
  AGENT_GRAPH_WORKSPACE_ARTIFACT_DIRECTORY,
  materializeAgentGraphWorkspaceArtifact,
} from '@/features/agent-graph/agentGraphWorkspaceArtifact'
...
function agentGraphArtifactPath(timestampMs = ARTIFACT_TIMESTAMP_MS): string {
  return `${AGENT_GRAPH_WORKSPACE_ARTIFACT_DIRECTORY}/${buildAgentGraphWorkspaceArtifactFileName(timestampMs)}`
}

export async function testAgentGraphRepositoryImportMaterializesSourceFilesArtifact() {
  ...
  try {
    if (buildAgentGraphWorkspaceArtifactFileName(ARTIFACT_TIMESTAMP_MS) !== ARTIFACT_FILE_NAME) {
    ...
    if (!entries.some(entry => entry.path === AGENT_GRAPH_WORKSPACE_ARTIFACT_DIRECTORY && entry.kind === 'folder')) {
    ...
    await materializeAgentGraphWorkspaceArtifact({
      repositoryUrl,
...
export async function testIncompleteAgentGraphRepositoryDoesNotMaterializeSourceFilesArtifact() {
  const { restore } = initJsdomHarness()
  const artifactPath = agentGraphArtifactPath()
  try {
    resetWorkspaceFsForTests()
    useGraphStore.getState().resetAll()
    const incomplete = agentGraphResult()
    ...
    try {
      await runLaunchImportUrl({
        ...
        bridge: {
          ...
          materializeAgentGraphImport: async args => {
            materializationCalls += 1
            return materializeAgentGraphWorkspaceArtifact(args)
          },
        },
        fallback: async () => {
