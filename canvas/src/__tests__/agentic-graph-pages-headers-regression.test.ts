...
export function testAgenticGraphReportOnlyCspAvoidsIgnoredUpgradeDirective() {
  ...
  if (routeStart < 0) {
    throw new Error('expected huijoohwee/_headers to define a /agentic-graph/* header block')
  }
  ...
  if (!reportOnlyBlock.includes('Content-Security-Policy-Report-Only:')) {
    throw new Error('expected /agentic-graph/* header block to define a report-only CSP')
  }
  if (reportOnlyBlock.includes('upgrade-insecure-requests')) {
    throw new Error('expected /agentic-graph/* report-only CSP to avoid ignored upgrade-insecure-requests noise')
  }
}
