...
} from '../lib/storage/agentic-graph-storage-file-sync-relay'
import {
  ...
  sha256Hex,
  type RelayEntry,
} from '../lib/storage/agentic-graph-storage-file-sync-relay-support'
...
class StatefulRelay {
  ...
  private async read(payload: Record<string, unknown>): Promise<Response> {
    ...
    return new Response(node.bytes, {
      ...
      headers: {
        ...
        'x-agentic-graph-file-sync-meta': encodeRelayJsonHeader({
  ...
  private async write(headers: Headers, body: BodyInit | null | undefined) {
    ...
    const rawMetadata = headers.get('x-agentic-graph-file-sync-meta')
    ...
    this.lastWriteSha256 = String(headers.get('x-agentic-graph-content-sha256'))
