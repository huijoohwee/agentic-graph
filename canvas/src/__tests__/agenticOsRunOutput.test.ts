...
import { emitAgenticOsRunOutput, resolveAgenticOsRunOutputPreference } from '@/features/chat/agenticOsRunOutput'

const buildAgenticOsFixture = (formatLine = ''): string => [
  '---',
...
const buildGenericAgenticOsFixture = (): string => [
  '---',
...
export function testResolveAgenticOsRunOutputPreferenceReadsOutputNodeFormat() {
  const pngPref = resolveAgenticOsRunOutputPreference({
    canonicalPath: '/chat-log/agenticOs_20260420231505.md',
    canonicalText: buildAgenticOsFixture('format: "png"'),
  })
  ...
  const svgPref = resolveAgenticOsRunOutputPreference({
    canonicalPath: '/chat-log/agenticOs_20260420231505.md',
    canonicalText: buildAgenticOsFixture('file: "agentic-os-output_20260420231505.svg"'),
  })
  ...
  const videoPref = resolveAgenticOsRunOutputPreference({
    canonicalPath: '/chat-log/agenticOs_20260420231505.md',
    canonicalText: buildAgenticOsFixture('format: "video"'),
  })
...
export function testResolveAgenticOsRunOutputPreferenceReadsCanonicalMediaKeys() {
  const imagePref = resolveAgenticOsRunOutputPreference({
    canonicalPath: '/chat-log/agenticOs_20260420231505.md',
    canonicalText: buildAgenticOsFixture('image: "https://example.com/output.png"'),
  })
  ...
  const videoPref = resolveAgenticOsRunOutputPreference({
    canonicalPath: '/chat-log/agenticOs_20260420231505.md',
    canonicalText: buildAgenticOsFixture('videoUrl: "https://example.com/output.mp4"'),
  })
  if (videoPref.kind !== 'video' || videoPref.extension !== 'mp4') {
...
export async function testEmitAgenticOsRunOutputWritesCanonicalMarkdownSection() {
  ...
  try {
    ...
    const result = await emitAgenticOsRunOutput({
      canonicalPath: '/chat-log/agenticOs_20260420231505.md',
      canonicalText: buildAgenticOsFixture('format: "markdown"'),
      getStore: () => ({
        captureCanvasPngSnapshot: async () => null,
        captureCanvasSvgSnapshot: async () => null,
      }),
    })
    if (result.path !== '/chat-log/20260420T231505Z/agenticOs_20260420T231505Z.md') {
      throw new Error(`expected markdown run output path, got ${String(result.path)}`)
    }
    const fs = await getWorkspaceFs()
    const written = await fs.readFileText('/chat-log/20260420T231505Z/agenticOs_20260420T231505Z.md')
    if (!written) {
    ...
    for (const snippet of forbiddenSnippets) {
      if (written.includes(snippet)) {
        throw new Error(`expected markdown run output to avoid copying AGENTIC_OS scaffold content: ${snippet}`)
      }
    }
  } finally {
    resetWorkspaceFsForTests()
...
export async function testEmitAgenticOsRunOutputStaysGenericForArbitraryNamedTerms() {
  const storage = new MemoryStorage()
  const { restore: restoreWindow } = initWindowHarness({ storage })
  const { restore: restoreDom } = initJsdomHarness()
  const originalFetch = globalThis.fetch
  try {
    ...
    globalThis.fetch = (async () => ({ ok: true } as Response)) as typeof fetch
    const result = await emitAgenticOsRunOutput({
      canonicalPath: '/chat-log/agenticOs_20260605050000.md',
      canonicalText: buildGenericAgenticOsFixture(),
      getStore: () => ({
        captureCanvasPngSnapshot: async () => null,
        captureCanvasSvgSnapshot: async () => null,
      }),
...
export async function testEmitAgenticOsRunOutputFallsBackToMarkdownForVideoPreference() {
  const storage = new MemoryStorage()
  const { restore: restoreWindow } = initWindowHarness({ storage })
  const { restore: restoreDom } = initJsdomHarness()
  const originalFetch = globalThis.fetch
  try {
    ...
    if (!written || !written.includes('## Solution') || written.includes('# Knowledge Graph Canvas · AI Pipeline')) {
      throw new Error('expected video markdown fallback to stay query-responsive instead of copying the runnable AGENTIC_OS scaffold')
    }
  } finally {
    resetWorkspaceFsForTests()
    globalThis.fetch = originalFetch
