...
import {
  ...
  AGENT_GRAPH_HOST_CAPABILITY_SCHEMA,
  AGENT_GRAPH_HOST_ROUTE,
  normalizeAgentGraphRepositoryUrl,
} from '@/features/agent-graph/agentGraphHostAdapter'
import { registerAgentGraphLaunchHostBridge } from '@/features/agent-graph/agentGraphLaunchHostBridge'
...
const SOURCE_CATALOG = [
  {
    ...
    mcpTool: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.agentGraphIngest,
    mcpTools: [AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.agentGraphIngest],
...
async function startHost(
  ...
  const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'agentic-graph-host-adapter-test-'))
...
export async function testAgentGraphHostUsesOpaqueContentAddressedFolderGrant() {
  const host = await startHost()
  try {
    const capabilityResponse = await host.fetchImpl('/__agentic_graph_agent_graph/capability')
    const capability = await capabilityResponse.json() as { schema?: unknown; available?: unknown }
    if (
      capabilityResponse.status !== 200
      || capability.schema !== AGENT_GRAPH_HOST_CAPABILITY_SCHEMA
      || capability.available !== true
    ) {
      throw new Error(`expected available host capability, got ${JSON.stringify(capability)}`)
    }

    const createResponse = await host.fetchImpl('/__agentic_graph_agent_graph/grants', {
      method: 'POST',
    ...
    await host.fetchImpl(
      `/__agentic_graph_agent_graph/grants/${grant.grantId}/files?path=src%2Findex.ts&offset=0&complete=1`,
      { method: 'PUT', body: new TextEncoder().encode('export const answer = 42\\n') },
    )
    const commitResponse = await host.fetchImpl(
      `/__agentic_graph_agent_graph/grants/${grant.grantId}/commit`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export async function testAgentGraphHostStrictFolderCommitCompletesMultipartRuntime() {
  ...
  try {
    ...
    const upload = await host.fetchImpl(
      `/__agentic_graph_agent_graph/grants/${grant.grantId}/files?path=large.md&offset=0&complete=1`,
...
export async function testAgentGraphBrowserAdapterStreamsFolderAndSkipsGeneratedTrees() {
  const host = await startHost()
  let uploadCalls = 0
  try {
    ...
    const bridge = createAgentGraphHostAdapter({
      fetchImpl,
...
export async function testAgentGraphHostRepositoryBoundaryIsStrictAndPathSafe() {
  const host = await startHost()
  const invocation = {
  ...
  try {
    for (const invalid of [
      ...
      try {
        normalizeAgentGraphRepositoryUrl(invalid)
      } catch (error) {
        failure = error
      }
      if (!failure) throw new Error(`expected strict repository rejection for ${invalid}`)
    }
    const response = await host.fetchImpl('/__agentic_graph_agent_graph/repositories', {
      method: 'POST',
      headers: {
    ...
    const accepted = await host.fetchImpl('/__agentic_graph_agent_graph/repositories', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (
      normalizeAgentGraphRepositoryUrl('https://code.example.test/group/project.git')
    ...
    if (
      normalizeAgentGraphRepositoryUrl('https://localhost/group/project.git')
...
export async function testAgentGraphDefaultCanvasBridgeRunsSourceBackedRepositoryIngest() {
  const host = await startHost()
  ...
  try {
    globalThis.fetch = (async (input, init) => {
      ...
      if (requestUrl.startsWith(AGENT_GRAPH_HOST_ROUTE)) {
        return host.fetchImpl(input, init)
      }
      ...
      if (body.method === 'initialize') {
        return new Response(JSON.stringify({
          ...
          headers: {
            'content-type': 'application/json',
            'mcp-session-id': 'agent-graph-launch-host-session',
          },
        })
      }
    ...
    unregister = registerAgentGraphLaunchHostBridge({ enabled: true })
    ...
    if (!result || !('kind' in result) || result.kind !== 'agent-graph' || result.graphId !== GRAPH_ID) {
    ...
    if (
      invocation?.tool !== AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.agentGraphIngest
...
export async function testAgentGraphToolbarLauncherOwnsDefaultHostRegistration() {
  ...
  if (
    !launcherSource.includes("from '@/features/agent-graph/agentGraphLaunchHostBridge'")
    || !launcherSource.includes('useAgentGraphLaunchHostBridge()')
  ) {
    throw new Error('the always-mounted Launch owner must install the default knowledge graph host bridge')
  }
  if (
    editorShellSource.includes('createAgentGraphHostAdapter')
    || editorShellSource.includes('useAgentGraphLaunchHostBridge')
  ) {
    throw new Error('the default knowledge graph host bridge must not depend on the editor-only shell')
  }
}

export async function testAgentGraphHostRejectsOversizedProjectionBeforeBrowserTransfer() {
  const host = await startHost({
    ...runtimeResult,
    projection: {
