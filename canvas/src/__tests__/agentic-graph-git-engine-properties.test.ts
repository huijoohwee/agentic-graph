...
export async function testAgenticGraphGitProperty41AuthorityBatchPreflight() {
  await fc.assert(fc.asyncProperty(
    ...
    async names => {
      ...
      assert.deepEqual(writeCalls[0]!.documents.map(document => document.path), names.map(name => `agentic-graph/docs/${name}.md`))
  ...
  await fc.assert(fc.asyncProperty(fc.boolean(), async exceedBytes => {
    ...
    const engine = createAgenticGraphGitEngine({
      ...
      relay: relayWithFetch(async () => {
        if (!exceedBytes) {
          clockMs += AGENTIC_OS_GIT_OPERATION_BOUNDS.timeoutMs
        ...
        return {
          ...
          refs: [],
          headRefName: 'HEAD',
          transferBytes: AGENTIC_OS_GIT_OPERATION_BOUNDS.maxTransferBytes + 1,
        }
      }),
      deviceId: 'device',
      now: () => clockMs,
