...
export async function testWebMcpRuntimeLateBindsAndUsesSameOriginStoragePaths(): Promise<void> {
  ...
  try {
    ...
    const inspectLocalSettingsChatReadinessTool = registeredTools.get('agentic-graph.inspect_local_settings_chat_readiness')
    const inspectLocalMainPanelTool = registeredTools.get('agentic-graph.inspect_local_mainpanel_state')
    const inspectLocalEditorWorkspaceTool = registeredTools.get('agentic-graph.inspect_local_editor_workspace_state')
    const inspectLocalChatPipelineTool = registeredTools.get('agentic-graph.inspect_local_chat_pipeline_state')
    const inspectLocalPipelineTool = registeredTools.get('agentic-graph.inspect_local_mainpanel_chat_canvas_pipeline')
    const inspectLocalDocumentTool = registeredTools.get('agentic-graph.inspect_local_workspace_document')
    const inspectLocalCanvasTool = registeredTools.get('agentic-graph.inspect_local_canvas_topology')
    const inspectLocalCanvasSnapshotTool = registeredTools.get('agentic-graph.inspect_local_canvas_snapshot')
    const inspectLocal3dCameraPoseTool = registeredTools.get('agentic-graph.inspect_local_3d_camera_pose')
    const inspectLocal3dLayoutPositionsTool = registeredTools.get('agentic-graph.inspect_local_3d_layout_positions')
    const inspectLocalXrSceneAssetsTool = registeredTools.get('agentic-graph.inspect_local_xr_scene_assets')
    const controlLocalXrSceneTool = registeredTools.get('agentic-graph.control_local_xr_scene')
    const inspectLocalCameraTool = registeredTools.get('agentic-graph.inspect_local_camera')
    const controlLocalCameraTool = registeredTools.get('agentic-graph.control_local_camera')
    const inspectLocalAnimationTool = registeredTools.get('agentic-graph.inspect_local_animation')
    const controlLocalAnimationTool = registeredTools.get('agentic-graph.control_local_animation')
    const inspectLocal2dZoomViewportTool = registeredTools.get('agentic-graph.inspect_local_2d_zoom_viewport')
    const inspectLocalSourceFilesSnapshotTool = registeredTools.get('agentic-graph.inspect_local_source_files_snapshot')
    const readLocalRuntimeIdentityTool = registeredTools.get('agentic-graph.read_local_runtime_identity')
    ...
    useGraphStore.setState({
      ...
      markdownDocumentSourceUrl: '/agentic-graph/share/local-only',
      graphData: MOCK_CANVAS_GRAPH_DATA as never,
      graphDataRevision: 7,
      canvasRenderMode: '2d',
      canvas2dRenderer: 'd3',
    ...
    if ((localCamera as { schema?: unknown; optics?: { stateOwner?: unknown; sensors?: unknown[] } }).schema !== 'agentic-graph-shared-camera-mcp/v1'
    ...
    if (
      (localRuntimeIdentity as { identity?: { schema?: unknown } }).identity?.schema !== 'agentic-graph-runtime-identity/v1'
      || (localRuntimeIdentity as { gate?: { schema?: unknown } }).gate?.schema !== 'agentic-graph-runtime-identity-gate/v1'
    ) {
    ...
    if (!fetchCalls.some((url) => url.endsWith('/agentic-graph/health'))) {
      throw new Error(`expected inspect_agent_surface to fetch the agent-ready health route, got ${fetchCalls.join(', ')}`)
    }
    if (!fetchCalls.some((url) => url.endsWith('/agentic-graph/.well-known/mcp/server-card.json'))) {
