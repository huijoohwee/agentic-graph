...
import {
  AGENTIC_OS_STORAGE_GIT_RELAY_API_VERSION,
  AGENTIC_OS_STORAGE_GIT_RELAY_PATH,
  createAgenticGraphStorageGitRelay,
} from '../lib/storage/agentic-graph-storage-git-relay'
...
const buildFixture = async () => {
  const documents = [
    resolvedDocument('agentic-graph/README.md', 'README.md', '# outside\n'),
    resolvedDocument('agentic-graph/docs/old.md', 'docs/old.md', '# old\n'),
    resolvedDocument('agentic-graph/docs/stale.md', 'docs/stale.md', '# stale\n'),
  ]
  const request = {
    workspaceId,
    repositoryId: 'repo',
  ...
  const nextDocuments = [
    resolvedDocument('agentic-graph/docs/current.md', 'docs/current.md', '# current\n'),
  ]
  const nextRequest = {
    ...request,
...
export async function testAgenticGraphGitRelayRecursiveCanonicalFetch() {
  ...
  const fetcher = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    assert.equal(url.pathname, AGENTIC_OS_STORAGE_GIT_RELAY_PATH)
...
export async function testAgenticGraphGitRelayConflictAndBounds() {
  ...
  const oversizedRelay = createAgenticGraphStorageGitRelay({
    ...
    fetcher: async () => jsonResponse(
      ...
      { 'content-length': String(AGENTIC_OS_GIT_OPERATION_BOUNDS.maxTransferBytes + 1) },
    ),
  })
  await assert.rejects(
