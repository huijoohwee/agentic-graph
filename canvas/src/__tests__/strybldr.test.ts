...
export function testStrybldrImportImageAndStoryboardOwnersAreWired() {
  ...
  const choiceText = readSource('features', 'immersive-media', 'importedImageChoiceRuntime.ts')
  const mediaText = readSource('features', 'immersive-media', 'ImmersiveMediaPanelProjection.tsx')

  assert(launchText.includes('Import Image') && launchText.includes('importLocalImages'), 'expected Launch to expose the workspace image importer')
  ...
  assert(imageBridgeText.includes('runLaunchImportLocalFiles'), 'expected Import Image to use the local-files import owner')
  assert(bridgeText.includes('importLocalImages'), 'expected workspace action bridge to expose image import')
  assert(actionsText.includes('return handleImportLocalFiles(images)'), 'expected image files to enter the same Source Files path')
  assert(choiceText.includes('createStoryboardForImportedImage') && choiceText.includes('buildStrybldrStoryboardDocument'), 'expected selected storyboard creation through the feature owner')
  assert(mediaText.includes('Image imported. Choose the next step.'), 'expected explicit post-import choices')
...
export async function testStrybldrVideoHandoffReusesBytePlusOwnerWithFallbackArtifact() {
  ...
  assert(handoffArtifactText.includes('publishGeneratedWorkspaceEntriesToAgenticGraphStorage'), 'expected generated Strybldr handoff artifacts to publish through the shared storage helper when runtime sync is enabled')
  assert(handoffArtifactText.includes('readAgenticGraphStorageRuntimeSyncEnabled'), 'expected generated Strybldr handoff storage publication to stay runtime-sync gated')
