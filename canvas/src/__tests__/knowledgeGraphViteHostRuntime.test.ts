...
export async function testKnowledgeGraphViteHostUsesStartupBoundDefaultRuntime() {
  ...
  const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'knowgrph-default-vite-host-'))
  ...
  const handler = createKnowledgeGraphBridgeRequestHandler({
    ...
    env: {
      ...
      KNOWGRPH_KNOWLEDGE_GRAPH_OUTPUT_ROOT: path.join(temporaryRoot, 'output'),
    },
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${port}/__knowgrph_knowledge_graph`
...
export async function testKnowledgeGraphViteHostKeepsUnknownFailureDetailsServerOnly() {
  const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'knowgrph-vite-host-failure-'))
  ...
  try {
    const port = await listen(server)
    const response = await fetch(
      `http://127.0.0.1:${port}/__knowgrph_knowledge_graph/repositories`,
      {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ repositoryUrl: 'https://github.com/example/repository' }),
