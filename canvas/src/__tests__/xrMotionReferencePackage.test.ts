...
export async function testXrMotionReferencePackageIsNativeDeterministicAndGraphBacked() {
  ...
  if (first.referenceBoundary.runtimeDependency !== false || first.referenceBoundary.implementation !== 'native-agentic-graph') {
  ...
  if (!xrCameraMotionSource.includes('saveDisabled={!graphData || savingScene}') || !xrCameraMotionSource.includes('persistXrSceneToAuthoredSource'))
