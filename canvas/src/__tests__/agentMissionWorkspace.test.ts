...
export async function testAgentMissionWorkspaceLifecycle() {
  ...
  assert.equal(resolveAgentMissionSource(start, '/agent-mission/agent-mission.md'), projection.markdownPath)
  ...
  assert.equal(resolveAgentMissionSource(next, projection.manifestPath), agentMissionWorkspace(next).markdownPath)
