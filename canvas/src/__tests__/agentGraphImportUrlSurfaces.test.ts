...
const assertAgentGraphSummary = (value: unknown, surface: string) => {
  ...
  if (
    ...
    || counts?.nodes !== 2
    || counts?.edges !== 1
    || result.projectionToken !== agentGraphResult.projection.token
...
const assertAgentGraphLaunchResult = (value: unknown) => {
  const result = value as Record<string, unknown>
  const counts = result.counts as Record<string, unknown> | undefined
  ...
  if (
    ...
    || result.graphId !== GRAPH_ID
    || result.snapshotDigest !== SNAPSHOT_DIGEST
    || result.parserRegistryDigest !== agentGraphResult.parserRegistryDigest
    || result.complete !== true
    || counts?.nodes !== 2
    || counts?.edges !== 1
    || projection?.token !== agentGraphResult.projection.token
...
export async function testAgentGraphCanonicalRepositoryRunsThroughNativeAndWebMcpImportUrl(): Promise<void> {
  const originalFetch = globalThis.fetch
  const exactInvocationRequests: string[][] = []
  const repositoryCalls: Array<{
    url: string
    invocation: WorkspaceAgentGraphInvocation | undefined
  }> = []
  let legacyImportCalls = 0
  resetSkillsCommandsMcpTargetForTests()
  resetAgenticOsRemoteGrammarCatalogForTests()
  useGraphStore.getState().resetAll()
  const unregister = registerMarkdownWorkspaceActionBridge('agent-graph-native-webmcp-import-url-test', {
    importUrl: async () => {
      legacyImportCalls += 1
  ...
  try {
      ...
      if (body.method === 'initialize') {
        return new Response(JSON.stringify({
          ...
          headers: {
            ...
            'mcp-session-id': 'agent-graph-native-webmcp-session',
          },
        })
      }
    ...
    assertAgentGraphLaunchResult(launchResult)

    const nativeResult = await executeStructuredImportUrl({ url })
    assertAgentGraphSummary(nativeResult, 'native Import URL')
    ...
    assertAgentGraphSummary(webMcpResult, 'WebMCP Import URL')
    ...
    if (
      ...
      || repositoryCalls.some(call => call.invocation?.tool !== AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.agentGraphIngest)
