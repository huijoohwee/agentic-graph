...
const validateGeneratedAgenticOs = (md: string, label: string): void => {
...
export function testAgenticOsFallbackPreservesNamedTermsForUnfamiliarPromptWithoutFamilyBranches() {
  const requestText = [
  ...
  const md = normalizeAgenticOsAssistantBodyForStorage({
    ...
    workspacePath: '/chat-log/20260605T041520Z/agenticOs_20260605T041520Z.md',
    requestText,
  ...
  validateGeneratedAgenticOs(md, 'universal AGENTIC_OS fallback')
}

export function testAgenticOsFallbackGeneratesFlowDiagramsForArbitraryPromptDynamicPanels() {
  const requestText = [
    'Draft a concise implementation memo for `QNX-42 gateway`,',
    'BlueLark adapter, 17ms jitter budget, DeltaSync handoff,',
    'outputSrcDoc chart panel, and audioUrl review notes.',
  ...
  ], 'universal AGENTIC_OS fallback flow_diagrams')
  assertOmits(md, [
    'agentic-graph-research-agent-demo',
    'agentic-graph-missalph-demo',
    'documentVersionGraph',
    'version control GitGraph',
    'flow-diagram-gitgraph-source:',
    'flow-diagram-gantt-panel:',
  ], 'universal AGENTIC_OS fallback flow_diagrams')
  validateGeneratedAgenticOs(md, 'universal AGENTIC_OS fallback flow_diagrams')

  const parsed = tryParseMarkdownFrontmatterFlowGraph('agentic-os-universal-flow-diagrams.md', md)
  if (!parsed) throw new Error('Expected universal AGENTIC_OS fallback flow_diagrams to parse as frontmatter-flow')
...
export function testAgenticOsFallbackPreservesDenseSymbolicTermsWithoutPromptFamilyTemplate() {
  const requestText = [
  ...
  const md = normalizeAgenticOsAssistantBodyForStorage({
    ...
    workspacePath: '/chat-log/20260605T041630Z/agenticOs_20260605T041630Z.md',
    requestText,
    assistantText: buildBaseTemplateSample(),
  })
  ...
  assertIncludes(md, [
    ...
    'Named terms',
    'Term coverage',
  ], 'dense symbolic AGENTIC_OS fallback')
  ...
  validateGeneratedAgenticOs(md, 'dense symbolic AGENTIC_OS fallback')
}

export function testAgenticOsFallbackKeepsAnalysisAsTermNotArtifactFamily() {
  const requestText = [
  ...
  const md = normalizeAgenticOsAssistantBodyForStorage({
    ...
    workspacePath: '/chat-log/20260605T011843Z/agenticOs_20260605T011843Z.md',
    requestText,
    assistantText: buildBaseTemplateSample(),
  })
  ...
  validateGeneratedAgenticOs(md, 'dense financial term coverage')
}
