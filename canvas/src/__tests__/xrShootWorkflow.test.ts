...
export function testXrShootWorkflowMarksRigsRetimeAndExports() {
  ...
  const keyboardChoreographySource = readSource('features', 'three', 'XrKeyboardChoreographyRuntime.tsx')
  ...
  const timelineRulerDenseCssSource = readSource('components', 'timeline', 'VideoSequenceTimelineDenseFbf.css')
  ...
  const ganttSurfaceModelSource = readSource('features', 'gitgraph', 'useGanttTimelineTransportSurfaceModel.ts')
  ...
  for (const marker of ['xr-camera-motion-retime-lane-label', 'xr-camera-motion-retime-lane-mark', 'position: absolute', 'inset: 0', 'top: 50%', 'transform: translate(-50%, -50%)', 'translate: 0', 'max-width: 16px', 'cursor: ew-resize', 'xr-camera-motion-mark-selection-controls--lane', '--kg-xr-mark-editor-translate-x', '[aria-pressed="true"]', 'xr-shot-target-timeline-bar', 'data-kg-xr-timeline-lane-selected', 'pointer-events: none', 'pointer-events: auto', 'background: transparent']) {
  ...
  const laneMarkCssStart = retimeCssSource.indexOf('.timeline-video-sequence-ruler-content .timeline-video-sequence-inserted-lane .xr-shot-target-timeline-lane .xr-camera-motion-retime-lane-mark.timeline-transport-time-axis-mark {')
  const laneMarkCss = retimeCssSource.slice(laneMarkCssStart, retimeCssSource.indexOf('}', laneMarkCssStart))
  if (laneMarkCssStart < 0 || !laneMarkCss.includes('width: 16px') || !laneMarkCss.includes('height: 16px') || !laneMarkCss.includes('max-width: 16px') || !laneMarkCss.includes('translate: 0') || !laneMarkCss.includes('background: transparent')) {
    throw new Error('expected XR choreography marks to keep only the draggable circle hitbox instead of inheriting compact bar chrome')
  }
  const laneMarkEditorCssStart = retimeCssSource.indexOf('\n.xr-camera-motion-mark-selection-controls--lane {')
  const laneMarkEditorCss = retimeCssSource.slice(laneMarkEditorCssStart, retimeCssSource.indexOf('}', laneMarkEditorCssStart))
  if (laneMarkEditorCssStart < 0 || !laneMarkEditorCss.includes('border: 1px solid var(--kg-canvas-accent') || !laneMarkEditorCss.includes('box-shadow: none') || laneMarkEditorCss.includes('0 2px 8px')) {
    throw new Error('expected selected choreography mark controls to keep the dot and use line-only shared selected-border chrome')
  }
  const activeXrBarCssStart = retimeCssSource.indexOf('.xr-shot-target-timeline-bar[data-kg-video-sequence-active-track="1"]')
  const activeXrBarCss = retimeCssSource.slice(activeXrBarCssStart, retimeCssSource.indexOf('}', activeXrBarCssStart))
  if (activeXrBarCssStart < 0 || !activeXrBarCss.includes('border-width: 2px') || !activeXrBarCss.includes('box-shadow: none') || activeXrBarCss.includes('0 0 0 2px')) {
    throw new Error('expected selected XR timeline bars to reuse the shared 2px timeline clip border instead of a heavier outer ring')
  }
  for (const marker of ['.xr-shot-target-timeline-bar[data-kg-video-sequence-selected-bar="1"]', '.xr-shot-target-timeline-bar[data-kg-xr-timeline-lane-selected="1"]', '.xr-shot-target-timeline-bar[aria-pressed="true"]']) {
    if (!activeXrBarCss.includes(marker)) throw new Error(`expected selected XR bars to share the same active border treatment through ${marker}`)
  }
  for (const source of [retimeCssSource, timelineSource, keyboardChoreographySource]) {
    if (source.includes('data-kg-xr-shot-target-selected')) throw new Error('expected XR timeline selection to avoid the stale parent selected-lane variant')
  }
  for (const marker of ['<CameraMotionMarkRetime', 'layout="lane"', 'laneTarget={{ kind:', 'timelineInsertedLanes={[', "insertAfterLaneId: 'scene'", "selectRowKey: 'xr-simulation'", 'selectRowKey: `xr-object:${target.id}`', 'content: ({ selected, selectRowKey }) =>', 'aria-pressed={selected}', 'data-kg-gantt-timeline-track-row-key={selectRowKey}', 'data-kg-video-sequence-active-track={selected ?', 'data-kg-video-sequence-selected-bar={selected ? ', 'data-kg-video-sequence-ruler-scrub-target="1"', 'data-kg-video-sequence-ruler-scrub-row-key="xr-simulation"', 'data-kg-video-sequence-ruler-scrub-row-key={selectRowKey}', 'includeChoreographyCues: false', '<TimelineTransportTimeAxisClip', '<TimelineTransportMiniActionBar', 'data-kg-xr-choreography-shared-axis-rail', 'data-kg-xr-choreography-cast-lane-label', 'data-kg-xr-choreography-camera-lane-label', 'data-kg-xr-timeline-control-lane="scene-clip"', 'data-kg-xr-timeline-control-bar="scene-clip"', 'data-kg-xr-timeline-shot-target="scene-clip"', 'aria-label="XR timeline scene or 3D object shot target"', 'renderXrSceneStageClipOverlay', "args.span.rowKey.includes('xr_stage_scene')", "selectedShotTarget.id !== XR_MOTION_REFERENCE_SCENE_SHOT_TARGET_ID", 'data-kg-xr-motion-scene-controls="click-appear"', 'data-kg-xr-motion-scene-control-strip="click-appear"', 'className="xr-camera-motion-mark-selection-controls xr-camera-motion-mark-selection-controls--lane xr-timeline-scene-stage-control xr-timeline-scene-stage-control--selected"', 'style={sceneEditorStyle}', 'renderClipOverlay={renderXrSceneStageClipOverlay}', 'data-kg-xr-motion-stage-field="scene-clip"', '<PanelSelect', 'className="xr-timeline-scene-stage-select"', 'data-kg-xr-motion-stage-select="scene-clip"', 'data-kg-xr-motion-stage-select-lane="scene"', 'onChange={event => applyStage(event.target.value)}', '<option key={preset.id} value={preset.id}>', 'data-kg-xr-motion-stage-summary="scene-clip"', 'xr-timeline-scene-stage-summary-chip', 'data-kg-xr-shot-target-lane', 'data-kg-xr-shot-target-bar', 'onSelectedRowKeyChange', 'onClickCapture', '[data-kg-gantt-timeline-track-row-key*="xr_stage_scene"]', 'XR_MOTION_REFERENCE_SCENE_SHOT_TARGET_ID', 'data-kg-xr-timeline-playhead-control="scene-clip"', 'data-kg-xr-timeline-playhead-input="scene-clip"', 'aria-label="XR timeline playhead seconds"', 'data-kg-xr-timeline-seconds-control="time-axis"', 'aria-label="XR timeline seconds"', 'data-kg-xr-timeline-fps-control="time-axis"', 'aria-label="XR timeline FPS"', 'runtimeDurationSeconds={runtime.plan.durationSeconds}', 'runtimeFrameRate={runtime.plan.fps}', 'data-kg-xr-timeline-transport="reused-gantt-player"', '<GanttTimelineTransportPanel', 'timeAxisControls={']) {
  ...
  for (const forbidden of ['layout="controls"', 'layout="ruler"', 'layout="time-axis"', 'supplementalLanes={', 'data-kg-xr-timeline-consolidated-lane="stage-output-ruler"', 'data-kg-xr-timeline-control-bar="stage-output"', 'data-kg-xr-timeline-shot-target="1"', 'data-kg-xr-timeline-playhead-control="1"', 'data-kg-xr-motion-stage-select="1"', '<TimelineTransportInlineClip', 'data-kg-xr-timeline-control-bar="marks"', 'timeRulerOverlay={<CameraMotionMarkRetime', '--kg-xr-timeline-marks-height', '>Marks</span>', 'xr-timeline-scene-mini-action']) {
  ...
  for (const marker of ['timeline-transport-supplemental-lanes', 'TimelineTransportInlineClip', 'TimelineTransportTimeAxisClip', 'TimelineTransportTimeAxisMark', 'TimelineTransportMiniActionBar', 'timeline-transport-track-clip--lane-${laneStyle}', 'timeline-transport-track-clip-label', 'timeline-transport-time-axis-clip', 'timeline-transport-time-axis-mark', 'timeline-transport-mini-action-bar', 'timeline-transport-mini-action']) {
  ...
  for (const marker of ['VideoSequenceTimelineInsertedLaneRenderArgs', 'content: React.ReactNode | ((args: VideoSequenceTimelineInsertedLaneRenderArgs) => React.ReactNode)', 'lane.content({ selected, selectRowKey: laneSelectRowKey })', 'renderClipOverlay?: VideoSequenceTimelineClipOverlayRenderer', 'const clipOverlay = renderClipOverlay?.({', '{clipOverlay}', 'timeAxisControls?: React.ReactNode', 'timeRulerOverlay?: React.ReactNode', 'timelineInsertedLanes?: readonly VideoSequenceTimelineInsertedLane[]', 'selectRowKey?: string', '<VideoSequenceTimeAxisControls>{timeAxisControls}</VideoSequenceTimeAxisControls>', '{timeRulerOverlay}', 'visibleLanes.flatMap(lane', 'data-kg-video-sequence-inserted-lane-content']) {
    if (!timelineRulerSource.includes(marker)) throw new Error(`expected the shared time ruler to expose ${marker}`)
  }
  const insertedLaneShellStart = timelineRulerSource.indexOf('data-kg-video-sequence-inserted-lane-content')
  const insertedLaneShellSource = timelineRulerSource.slice(Math.max(0, insertedLaneShellStart - 360), timelineRulerSource.indexOf('>', insertedLaneShellStart) + 1)
  for (const forbidden of ['timeline-video-sequence-inserted-lane--selected', 'data-kg-gantt-timeline-track-row-key=', 'data-kg-video-sequence-active-track={selected ?', 'data-kg-video-sequence-inserted-lane-selectable', 'onPointerDownCapture={laneSelectRowKey', 'onClickCapture={laneSelectRowKey']) {
    if (insertedLaneShellSource.includes(forbidden)) throw new Error(`expected inserted-lane row shells to pass selected state to their bar instead of rendering a selected row wrapper through ${forbidden}`)
  }
  for (const marker of ['resolveTimelineRulerScrubRowKey', 'data-kg-video-sequence-ruler-scrub-row-key', 'data-kg-video-sequence-ruler-scrub-target="1"', 'isTimelineRulerInteractiveControl', 'state.selectedRowKey']) {
    if (!ganttInteractionsText.includes(marker)) throw new Error(`expected shared ruler scrub to preserve selectable inserted lanes through ${marker}`)
  }
  const insertedScrubStart = ganttInteractionsText.indexOf('if (state.selectedRowKey) {')
  const insertedScrubSource = ganttInteractionsText.slice(insertedScrubStart, ganttInteractionsText.indexOf('return', insertedScrubStart))
  if (insertedScrubStart < 0 || insertedScrubSource.indexOf('args.setSelectedRowKey') < 0 || insertedScrubSource.indexOf('args.setTransportPlaybackPosition') < 0 || insertedScrubSource.indexOf('args.setSelectedRowKey') > insertedScrubSource.indexOf('args.setTransportPlaybackPosition')) {
    throw new Error('expected scrub-target inserted lanes to select their row before scrubbing the shared playhead')
  }
  for (const marker of ['timelineInsertedLanes?: readonly VideoSequenceTimelineInsertedLane[]', 'positionOnlySelectedRowKeys', 'setSelectedRowKey: handleSelectedRowKeyChange']) {
    if (!ganttSurfaceModelSource.includes(marker)) throw new Error(`expected shared transport surface to route inserted-lane selection through ${marker}`)
  }
  ...
  for (const marker of ['[data-kg-timeline-clip-compact="1"]:not(.timeline-transport-track-clip--milestone):not(.timeline-transport-time-axis-mark)', ':not(.timeline-transport-track-clip--milestone):not(.timeline-transport-time-axis-mark):not([data-kg-video-sequence-dense-fbf="1"])']) {
    if (!timelineRulerDenseCssSource.includes(marker)) throw new Error(`expected compact dense clip chrome to skip XR time-axis mark hitboxes through ${marker}`)
  }
  ...
  const sceneClipControlStart = timelineSource.indexOf('data-kg-xr-timeline-control-bar="scene-clip"')
  const sceneClipControlSource = timelineSource.slice(sceneClipControlStart, timelineSource.indexOf('</section>', sceneClipControlStart))
  if (sceneClipControlSource.includes('>FPS</span>') || sceneClipControlSource.includes('>Seconds</span>')) {
    throw new Error('expected Seconds and FPS to leave the SCENE clip controls and align with the shared time axis')
  }
  ...
  for (const marker of ['<XrSceneLibrarySubject', 'selected={runtime.selectedShotTargetId === subject.id}', "controlXrSharedAssetControls({ operation: 'select-target', targetId: subjectId })", 'if (!result.ok) selectBoundXrShotTarget(subjectId)']) {
