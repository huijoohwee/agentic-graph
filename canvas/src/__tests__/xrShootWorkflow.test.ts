...
export function testXrShootWorkflowMarksRigsRetimeAndExports() {
  ...
  for (const forbidden of ['layout="controls"', 'layout="ruler"', 'layout="time-axis"', 'supplementalLanes={', 'data-kg-xr-timeline-consolidated-lane="stage-output-ruler"', 'data-kg-xr-motion-stage-buttons="1"', 'data-kg-xr-motion-stage-button={preset.id}', 'data-kg-xr-timeline-control-bar="marks"', 'timeRulerOverlay={<CameraMotionMarkRetime', '--kg-xr-timeline-marks-height', '>Marks</span>']) {
  ...
  if (!stageOutputBarSource.includes('<PanelSelect') || !stageOutputBarSource.includes('<option')) {
    throw new Error('expected BottomPanel XR Control to render the stage control as a dropdown list inside its individual lane')
  }
