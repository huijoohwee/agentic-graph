...
export function testXrShootWorkflowMarksRigsRetimeAndExports() {
  ...
  for (const forbidden of ['layout="controls"', 'layout="ruler"', 'layout="time-axis"', 'supplementalLanes={', 'data-kg-xr-timeline-consolidated-lane="stage-output-ruler"', 'data-kg-xr-motion-stage-buttons="1"', 'data-kg-xr-motion-stage-button={preset.id}', 'data-kg-xr-motion-stage-select="1"', 'data-kg-xr-motion-stage-field="1"', 'data-kg-xr-timeline-control-bar="marks"', 'timeRulerOverlay={<CameraMotionMarkRetime', '--kg-xr-timeline-marks-height', '>Marks</span>']) {
  ...
  const stageOutputBarSource = timelineSource.slice(stageOutputBarStart, timelineSource.indexOf('</TimelineTransportTimeAxisClip>', stageOutputBarStart))
  ...
  if (stageOutputBarSource.includes('data-kg-xr-motion-stage-select') || stageOutputBarSource.includes('aria-label="XR grey-box stage"')) {
    throw new Error('expected BottomPanel XR Control to move the stage dropdown into the SCENE clip')
  }
