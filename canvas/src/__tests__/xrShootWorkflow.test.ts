...
export function testXrShootWorkflowMarksRigsRetimeAndExports() {
  ...
  const agentReadyToolContractSource = readSource('features', 'agent-ready', 'agentic-graph-agent-ready-tool-contract.mjs')
  for (const marker of [
  ...
  if (retimeSource.includes('data-kg-xr-mark-animation-preset={preset.id}')) {
    throw new Error('expected selected mark XR animation presets to use the gait-style dropdown, not duplicate preset buttons')
  }
  ...
  for (const marker of ['xr-camera-motion-retime-lane-label', 'xr-camera-motion-retime-lane-mark', 'position: absolute', 'inset: 0', 'pointer-events: auto', 'top: 50%', 'transform: translate(-50%, -50%)', 'cursor: ew-resize', '[data-kg-xr-lane-mark-shape="circle-only"]', 'height: 16px', 'min-height: 16px', 'translate: 0', 'xr-camera-motion-mark-selection-controls--lane', '--kg-xr-mark-editor-translate-x', '[aria-pressed="true"]', 'xr-shot-target-timeline-bar', 'cursor: grab', 'touch-action: none', 'user-select: none', '[data-kg-xr-timeline-lane-dragging="1"]', 'cursor: grabbing', 'data-kg-xr-timeline-lane-selected="1"', 'timeline-transport-time-axis-clip.timeline-transport-track-clip--selected', '[data-kg-video-sequence-inserted-lane-selected="1"] > .xr-camera-motion-retime-time-axis-rail', 'border-color: transparent', 'border-width: 0', 'xr-shot-target-timeline-bar--camera', 'xr-camera-motion-mark-animation-presets', '[data-kg-xr-ruler-mark-editor="cast"]', 'flex-wrap: nowrap', 'xr-timeline-scene-stage-control.xr-camera-motion-mark-selection-controls--lane', 'xr-timeline-scene-stage-select', 'xr-timeline-scene-selection-label', '[data-kg-gantt-timeline-track-row-key*="xr_stage_scene"].timeline-transport-track-clip--selected .timeline-transport-track-clip-label', 'visibility: hidden']) {
  ...
  const sceneClipControlSource = timelineSource.slice(sceneClipControlStart, timelineSource.indexOf('data-kg-xr-motion-save="1"', sceneClipControlStart))
  ...
  if (!sceneClipControlSource.includes('data-kg-xr-timeline-playhead-control="scene-clip"')) {
    throw new Error('expected BottomPanel SCENE clip controls to own the playhead input')
  }
  ...
  for (const marker of ['data-kg-xr-camera-aspect-mask="1"', 'data-kg-camera-optics-projection="xr-viewport"', 'framing.claimed', 'cameraFramingSourceOwnsViewportMask(framing.source)', "source === 'panel' || source === 'document'", "'selected-timeline-mark'", "'timeline-playback'", "'floating-panel-camera'"]) if (!aspectMaskSource.includes(marker)) throw new Error(`expected the XR viewport aspect-mask projection to expose ${marker}`)
  ...
  if (aspectMaskSource.includes('framing.claimed ? framing.settings : null')) throw new Error('expected canvas and axis camera mutations not to own the XR aspect mask')
  ...
  if (!nativeCameraInspection.invocationGrammar
    || nativeCameraInspection.invocationGrammar.source !== 'native-agentic-graph-invocation-catalog') {
