...
export function testXrShootWorkflowMarksRigsRetimeAndExports() {
  ...
  for (const forbidden of ['layout="controls"', 'layout="ruler"', 'layout="time-axis"', 'supplementalLanes={', 'data-kg-xr-timeline-consolidated-lane="stage-output-ruler"', 'data-kg-xr-motion-stage-select="1"', 'data-kg-xr-timeline-control-bar="marks"', 'timeRulerOverlay={<CameraMotionMarkRetime', '--kg-xr-timeline-marks-height', '>Marks</span>']) {
  ...
  if (stageOutputBarSource.includes('<PanelSelect') || stageOutputBarSource.includes('<option')) {
    throw new Error('expected BottomPanel XR Control to render as an individual lane without drop-down lists')
  }
