...
import { installSourceCatalogFetchMock } from '@/__tests__/agentGraphSkillsCommandsLaunch.test'
import { ToastHost } from '@/components/ui/ToastHost'
import {
  registerMarkdownWorkspaceActionBridge,
  type WorkspaceAgentGraphImportProgress,
  type WorkspaceAgentGraphImportResult,
  type WorkspaceAgentGraphInvocation,
...
const AGENT_GRAPH_RESULT: WorkspaceAgentGraphImportResult = {
  handled: true,
  kind: 'agent-graph',
...
const AGENT_GRAPH_PROGRESS: WorkspaceAgentGraphImportProgress = {
  schema: 'agentic-graph-agent-graph-import-progress/v1',
  kind: 'source-parsed',
  graphId: AGENT_GRAPH_RESULT.graphId,
  parserRegistryDigest: AGENT_GRAPH_RESULT.parserRegistryDigest,
  ...
  graphData: {
    context: 'agentic-graph-agent-graph-projection',
    type: 'Graph',
    nodes: [AGENT_GRAPH_RESULT.projection.graphData.nodes[0]!],
    edges: [],
  },
}

export async function testAgentGraphLaunchImportUrlInputRunsVisibleCanonicalGraph() {
  const fetchMock = installSourceCatalogFetchMock()
  const { dom, restore } = initJsdomHarness()
  const container = dom.window.document.createElement('section')
  dom.window.document.body.appendChild(container)
  const root = createRoot(container)
  const repositoryCalls: Array<{ url: string; invocation: WorkspaceAgentGraphInvocation | undefined }> = []
  let closeCalls = 0
  let releaseImport: (() => void) | undefined
  let emitProgress: ((progress: WorkspaceAgentGraphImportProgress) => void) | undefined
  const pendingImport = new Promise<WorkspaceAgentGraphImportResult>(resolve => {
    releaseImport = () => resolve(AGENT_GRAPH_RESULT)
  })
  const unregister = registerMarkdownWorkspaceActionBridge('agent-graph-launch-input-interaction-test', {
    agentGraph: {
  ...
  try {
    ...
    const busyToast = useGraphStore.getState().uiToasts.find(
      toast => toast.id === 'launch:import:agent-graph-url',
    )
    ...
    await act(async () => {
      emitProgress?.(AGENT_GRAPH_PROGRESS)
        ...
        bridge: {
          agentGraph: {
            importRepositoryUrl: async (_url, _opts, _invocation, onProgress) => {
              onProgress?.(AGENT_GRAPH_PROGRESS)
              previewSeen = (useGraphStore.getState().graphData.metadata?.agentGraphPreview as { complete?: unknown } | undefined)?.complete === false
...
export async function testAgentGraphLaunchImportUrlOffersRateLimitedRepositoryRecovery() {
  const fetchMock = installSourceCatalogFetchMock()
  const { dom, restore } = initJsdomHarness()
  const container = dom.window.document.createElement('section')
  dom.window.document.body.appendChild(container)
  ...
  const repositoryCalls: Array<{ url: string; invocation: WorkspaceAgentGraphInvocation | undefined }> = []
  let closeCalls = 0
  const unregister = registerMarkdownWorkspaceActionBridge('agent-graph-launch-rate-limit-recovery-test', {
    importUrl: async url => {
...
export async function testAgentGraphRepositoryImportDedupeIsExactProofBoundAndClears() {
  ...
  const hostInvocations: WorkspaceAgentGraphInvocation[] = []
  const bridge = {
    agentGraph: {
      importRepositoryUrl: async (
        _url: string,
        _opts: unknown,
        proof: WorkspaceAgentGraphInvocation | undefined,
      ) => {
        assert.ok(proof)
  ...
  const run = (proof: WorkspaceAgentGraphInvocation) => runLaunchImportUrl({
    urlRaw: REPOSITORY_URL,
    forceAgentGraphRepository: true,
    bridge,
    fallback: async () => undefined,
