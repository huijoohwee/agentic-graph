...
export async function testSettingsApplyCommitsAgenticGraphStorageTargetAndWorkspacePathIntoFloatingChatPipelineInspection() {
  ...
  try {
    ...
    if (draftStorageTarget !== 'chatAgenticGraph' || draftChatAgenticGraphPath !== nextAgenticGraphPath) {
      throw new Error(`expected Settings draft agentic-graph storage values to update before apply, got ${JSON.stringify({ draftStorageTarget, draftChatAgenticGraphPath })}`)
    }
    ...
    if (
      ...
      throw new Error(`expected FloatingPanel Chat pipeline agentic-graph storage state to update after Settings apply, got ${JSON.stringify(appliedChatInspection)}`)
    }
    if (
      ...
      throw new Error(`expected canonical store agentic-graph storage settings to commit after Settings apply, got ${JSON.stringify({
