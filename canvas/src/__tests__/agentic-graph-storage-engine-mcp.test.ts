...
const webName = (toolId: string): string => `agentic-graph.${toolId}`
...
export function testStorageEngineInvocationAndStructuredInputsStayEquivalent() {
  const gitInvocation = normalizeAgenticGraphGitControlInput({
    invocation: '/git.run @local-git-repository @git-remote #git-remote operation=commit remote=origin path=agentic-graph%2Fdocs base-ref=refs%2Fheads%2Fmain message=storage%20sync',
  })
...
export async function testStorageEngineInputsRejectCredentialAndRuntimeOverrides() {
  ...
  for (const codePoint of CONTROL_CODE_POINTS) {
    ...
    const gitPath = `agentic-graph/docs${character}blocked`
    ...
    assertRejects(
      () => normalizeAgenticGraphGitControlInput({
        invocation: `/git.run @local-git-repository @git-remote #git-remote operation=fetch remote=origin path=agentic-graph%2Fdocs${encoded}blocked base-ref=refs%2Fheads%2Fmain`,
      }),
    ...
    assertRejects(
      () => normalizeAgenticGraphGitControlInput({
        operation: 'fetch',
        remoteId: 'origin',
        canonicalPathScope: `agentic-graph/docs${character}`,
        baseRef: 'refs/heads/main',
      }),
  ...
  process.env.VITE_AGENTIC_OS_STORAGE_WORKSPACE_ID = 'kgws:mcp-control-character-test'
  process.env.VITE_AGENTIC_OS_STORAGE_CHAT_SESSION_TOKEN = 'mcp-control-character-session'
  ...
  try {
    for (const codePoint of CONTROL_CODE_POINTS) {
      ...
      await assertInvalidControl(controlLocalGitRepository({
        operation: 'fetch',
        remoteId: 'origin',
        canonicalPathScope: `agentic-graph/docs${character}blocked`,
        baseRef: 'refs/heads/main',
