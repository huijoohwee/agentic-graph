...
export function testRuntimeIdentityAttestationKeepsCanonicalOwnership(): void {
  const runtimeRoot = readFileSync(resolve(process.cwd(), 'src/features/runtime-identity/agentic-graph-runtime-identity-runtime.tsx'), 'utf8')
  const reporter = readFileSync(resolve(process.cwd(), 'src/features/runtime-identity/use-agentic-graph-runtime-identity-attestation-runtime.ts'), 'utf8')
  const settings = readFileSync(resolve(process.cwd(), 'src/features/panels/views/CrossDeviceIdentitySettingsRows.tsx'), 'utf8')
  const room = readFileSync(resolve(process.cwd(), '..', 'cloudflare/workers/agentic-graph-storage/canvasSyncRoom.ts'), 'utf8')
