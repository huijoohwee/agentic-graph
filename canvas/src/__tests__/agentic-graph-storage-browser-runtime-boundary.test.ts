...
import { createAgenticGraphGitPersistedCache } from '@/lib/storage/agentic-graph-storage-engine-adapters'
import {
...
const RELAY_ENV_KEYS = [
  'VITE_AGENTIC_OS_STORAGE_BASE_URL',
  'VITE_AGENTIC_OS_STORAGE_WORKSPACE_ID',
  'VITE_AGENTIC_OS_STORAGE_CHAT_SESSION_TOKEN',
] as const
...
const configureRelayEnv = (): void => {
  ...
  process.env.VITE_AGENTIC_OS_STORAGE_WORKSPACE_ID = WORKSPACE_ID
  process.env.VITE_AGENTIC_OS_STORAGE_CHAT_SESSION_TOKEN = SESSION_SECRET
}
  ...
  try {
    await withoutBrowserPersistence(async () => {
      ...
      await gitCache.putRepository({
        id: `${WORKSPACE_ID}\0agentic-graph-docs`,
        workspaceId: WORKSPACE_ID,
        repositoryId: 'agentic-graph-docs',
        remoteId: 'origin',
      ...
      await gitCache.putObjects([{
        id: `${WORKSPACE_ID}\0agentic-graph-docs\0aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa`,
        workspaceId: WORKSPACE_ID,
        repositoryId: 'agentic-graph-docs',
      ...
      await gitCache.putRefs([{
        id: `${WORKSPACE_ID}\0agentic-graph-docs\0refs/heads/main`,
        workspaceId: WORKSPACE_ID,
        repositoryId: 'agentic-graph-docs',
      ...
      assert.equal(gitInspection.schema, 'agentic-graph-storage-git-inspection/v1')
      assert.equal(fileInspection.schema, 'agentic-graph-storage-file-sync-inspection/v1')
...
export async function testAgenticGraphStorageBrowserControlsRejectInvalidInputBeforeRuntime(): Promise<void> {
  ...
  assert.deepEqual(git, {
    schema: 'agentic-graph-storage-git-control/v1',
    ok: false,
    status: 'invalid-input',
  })
  assert.deepEqual(files, {
    schema: 'agentic-graph-storage-file-sync-control/v1',
    ok: false,
    status: 'invalid-input',
  })
...
export async function testAgenticGraphStorageWebMcpBuildersExecuteBrowserBoundaries(): Promise<void> {
  await withoutBrowserPersistence(async () => {
    ...
    assert.equal(gitInspect.name, 'agentic-graph.inspect_local_git_repository')
    assert.equal(fileInspect.name, 'agentic-graph.inspect_local_file_sync')
    assert.equal(
      (await gitInspect.execute({} as Record<string, unknown>) as Record<string, unknown>).schema,
      'agentic-graph-storage-git-inspection/v1',
    )
    assert.equal(
      (await fileInspect.execute({} as Record<string, unknown>) as Record<string, unknown>).schema,
      'agentic-graph-storage-file-sync-inspection/v1',
    )
    assert.equal(
