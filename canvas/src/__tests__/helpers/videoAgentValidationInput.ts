...
export function readVideoAgentValidationUrlsFromEnv(): string[] {
  const plural = String(process.env.AGENTIC_OS_VIDEO_AGENT_TEST_URLS || '').trim()
  if (plural) return splitVideoAgentValidationUrls(plural)
  return splitVideoAgentValidationUrls(String(process.env.AGENTIC_OS_VIDEO_AGENT_TEST_URL || '').trim())
}
