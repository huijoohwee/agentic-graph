...
export function testWorkspaceInitializationDocsAbsRootDefaultStaysOutOfProductionBuilds() {
  ...
  if (!text.includes("'import.meta.env.VITE_AGENTIC_OS_WORKSPACE_SEEDS_READ_ABS_ROOT': JSON.stringify(workspaceInitializationWorkspaceSeedsReadAbsRootForDev)")) {
  ...
  if (text.includes("path.resolve(configuredDocsRoot, '..', '..', 'agentic-graph', 'docs', 'workspace-seeds')")) {
    throw new Error('expected Dev workspace seeds never to drift through another agentic-graph checkout')
  }
...
export function testProductionHtmlInlinesGeneratedStylesheetAssets() {
  ...
  if (!text.includes("name: 'agentic-graph-inline-html-stylesheet-assets'")) {
