...
export function testXrModeUsesCanonicalFloatingPanel() {
  ...
  for (const forbiddenPresetLane of ['compatibleSharedAssetPresets.map(preset => {', 'data-kg-xr-shared-asset-preset-lane={preset.id}', 'data-kg-xr-shared-asset-preset-lane-label={preset.id}', "id: `xr-preset:${preset.id}`"]) {
    if (xrCameraMotion.includes(forbiddenPresetLane)) throw new Error(`expected XR animation presets to stay tied to the XR Assets lane, found ${forbiddenPresetLane}`)
  }
