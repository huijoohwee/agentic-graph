...
export function testAgenticGraphReportOnlyCspAvoidsIgnoredUpgradeDirective() {
  ...
  const routeMarker = '/agenticgraph/*'
  const routeStart = headersText.indexOf(routeMarker)
  if (routeStart < 0) {
    throw new Error('expected huijoohwee/_headers to define a /agenticgraph/* header block')
  }
  ...
  if (!reportOnlyBlock.includes('Content-Security-Policy-Report-Only:')) {
    throw new Error('expected /agenticgraph/* header block to define a report-only CSP')
  }
  if (reportOnlyBlock.includes('upgrade-insecure-requests')) {
    throw new Error('expected /agenticgraph/* report-only CSP to avoid ignored upgrade-insecure-requests noise')
  }
}
...
export function testAgenticGraphAppShellHtmlAddsNoTransformForCloudflareJsd() {
  const headersPath = resolve(process.cwd(), '..', '..', 'huijoohwee', '_headers')
  const headersText = readFileSync(headersPath, 'utf8')
  for (const route of ['/content/agenticgraph/index.html', '/agenticgraph', '/agenticgraph/', '/agenticgraph/index.html']) {
