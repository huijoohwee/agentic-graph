...
export async function testKnowledgeGraphBrowserHostParserRegistryDigestBoundary() {
  const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'knowgrph-parser-digest-test-'))
  ...
  try {
    ...
    const request = async () => fetch(`http://127.0.0.1:${address.port}/__knowgrph_knowledge_graph/repositories`, {
      method: 'POST',
