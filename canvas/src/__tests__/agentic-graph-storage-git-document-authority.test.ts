...
} from '@/lib/storage/agentic-graph-storage-git-document-authority'

const SCOPE = 'agentic-graph/docs'
const REPOSITORY_ID = 'agentic-graph-docs'
...
const sourceFiles: SourceFile[] = [
  ...
  },
  {
    ...
    enabled: true,
    status: 'parsed',
    source: { kind: 'local', path: 'workspace:/agentic-graph/docs/beta.json' },
  },
]
    ...
    const authority = createSaveBridgeDocumentAuthority({
      ...
      fetcher: async (input, init) => {
        ...
        return Response.json({
          ...
          githubPath: String(body.documentKey).replace(/^agentic-graph\//, ''),
