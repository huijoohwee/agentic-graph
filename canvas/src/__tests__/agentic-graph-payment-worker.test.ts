  ...
  try {
    ...
    const response = await worker.fetch(
      new Request(`https://example.com${STRIPE_PAYMENT_ROUTE_PATHS.checkoutSession}`, {
        ...
        body: JSON.stringify({
          successUrl: 'https://example.com/agentic-graph?stripeCheckout=success',
          cancelUrl: 'https://example.com/agentic-graph?stripeCheckout=cancel',
  ...
  try {
    ...
    const response = await worker.fetch(
      new Request(`https://example.com${STRIPE_PAYMENT_ROUTE_PATHS.checkoutSession}`, {
        ...
        body: JSON.stringify({
          successUrl: 'https://attacker.example/agentic-graph?stripeCheckout=success',
          cancelUrl: 'https://attacker.example/agentic-graph?stripeCheckout=cancel',
          workspaceId: 'workspace-payment',
        }),
      }),
      env as never,
  ...
  try {
    ...
    const response = await worker.fetch(
      new Request(`https://worker.example${STRIPE_PAYMENT_ROUTE_PATHS.checkoutSession}`, {
        ...
        body: JSON.stringify({
          successUrl: 'https://app.example/agentic-graph?stripeCheckout=success',
          cancelUrl: 'https://app.example/agentic-graph?stripeCheckout=cancel',
          workspaceId: 'workspace-payment',
        }),
      }),
      env as never,
    ...
    if (fetchCalls.length !== 1) throw new Error(`expected one Stripe API call, got ${fetchCalls.length}`)
    const params = new URLSearchParams(String(fetchCalls[0]?.init?.body || ''))
    if (!String(params.get('success_url') || '').startsWith('https://app.example/agentic-graph?')) {
  ...
  try {
    ...
    const response = await worker.fetch(
      new Request(`https://example.com${STRIPE_PAYMENT_ROUTE_PATHS.checkoutSession}`, {
        ...
        body: JSON.stringify({
          successUrl: 'https://example.com/agentic-graph?stripeCheckout=success&session_id={CHECKOUT_SESSION_ID}',
          cancelUrl: 'https://example.com/agentic-graph?stripeCheckout=cancel',
        }),
      }),
      env as never,
...
export async function testAgenticGraphPaymentWorkerRejectsStripeSubscriptionWithoutPriceId() {
  ...
  env.STRIPE_CHECKOUT_PRODUCT_NAME = 'agentic-graph'
  const response = await worker.fetch(
    new Request(`https://example.com${STRIPE_PAYMENT_ROUTE_PATHS.checkoutSession}`, {
      method: 'POST',
      headers: {
