...
import { AGENTIC_OS_STORAGE_SYNC_LIMITS } from '@/lib/storage/agentic-graph-storage-sync-contract'
import {
  AGENTIC_OS_STORAGE_ROUTE_UNAVAILABLE_RETRY_MS,
...
export const getClientFetch = (value?: AgenticGraphStorageFetchLike): AgenticGraphStorageFetchLike => {
  ...
  if (!fetchImpl) throw new Error('fetch is not available for agentic-graph storage sync')
  return (input, init) => {
  ...
  const timeout = new Promise<never>((_resolve, reject) => {
    timeoutId = globalThis.setTimeout(() => {
      ...
      reject(new AgenticGraphStorageRetryableTransportError(
        `agentic-graph storage request timed out after ${timeoutMs}ms`,
      ))
    }, timeoutMs)
  })
  try {
    ...
    if (name === 'aborterror' || isNetworkLoadFailure(error)) {
      throw new AgenticGraphStorageRetryableTransportError(
        error instanceof Error ? error.message : 'agentic-graph storage network request failed',
      )
    }
    throw error
  } finally {
...
export const buildApiOriginKey = (baseUrl?: string | null): string => {
  try {
    return new URL(resolveAgenticGraphStorageApiUrl(AGENTIC_OS_STORAGE_ROUTE_PATHS.push, baseUrl)).origin
  } catch {
...
export const markRouteUnavailableForApiOrigin = (apiOrigin: string, nowMs = Date.now()): void => {
  ...
  routeUnavailableUntilByApiOrigin.set(apiOrigin, nowMs + AGENTIC_OS_STORAGE_ROUTE_UNAVAILABLE_RETRY_MS)
  if (shouldLog) {
    console.warn(`[agentic-graph-storage] route unavailable for ${apiOrigin} — retry in ${AGENTIC_OS_STORAGE_ROUTE_UNAVAILABLE_RETRY_MS}ms`)
  }
}
