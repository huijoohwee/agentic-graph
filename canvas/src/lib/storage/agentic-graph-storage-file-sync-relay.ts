...
} from './agentic-graph-storage-file-sync-relay-support'
import {
  RelaySnapshotCache,
  toRelayFileSyncEntry,
  type RelayMapping,
  type RelaySnapshot,
} from './agentic-graph-storage-file-sync-relay-snapshot'
import {
...
} from './agentic-graph-storage-file-sync-relay-protocol'
...
class AgenticGraphStorageFileSyncRelayProvider implements FileSyncProvider {
  ...
  read(key: string, signal: AbortSignal) {
    return this.run(signal, async (operationSignal, budget) => {
      ...
      const metadataHeader = response.headers.get('x-agentic-graph-file-sync-meta')
    ...
    return this.run(signal, async (operationSignal, budget) => {
      ...
      const response = await this.request({
        ...
        headers: {
          ...
          'x-agentic-graph-content-sha256': sha256,
          'x-agentic-graph-file-sync-meta': encodedMetadata,
        },
        signal: operationSignal,
      })
  ...
  private requestUrl(): string | URL {
    ...
    try {
      ...
      const parsed = new URL(String(value), 'http://agentic-graph.local')
