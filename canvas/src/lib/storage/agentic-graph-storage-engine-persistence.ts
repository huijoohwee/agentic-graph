...
import { createAgenticGraphStorageEngineOutboxPersistence } from './agentic-graph-storage-engine-outbox-persistence'
import type {
  ...
  StoredEngineRecord,
} from './agentic-graph-storage-engine-persistence-contract'
import {
...
} from './agentic-graph-storage-engine-persistence-support'
...
export const AGENTIC_OS_STORAGE_ENGINE_DB_NAME = 'kg:agentic-graph-storage-engines'
export const AGENTIC_OS_STORAGE_ENGINE_MAX_BYTES = 10_485_760
export const AGENTIC_OS_STORAGE_ENGINE_BINARY_CHUNK_BYTES = 256 * 1_024
...
export const createAgenticGraphStorageEnginePersistence = async (args: {
  ...
  const raw = new AgenticGraphStorageEngineDexie(args.databaseName || AGENTIC_OS_STORAGE_ENGINE_DB_NAME)
  ...
  return {
    ...
    binary: {
      async put(record) {
        ...
        if (bytes.byteLength > AGENTIC_OS_STORAGE_ENGINE_MAX_BYTES) {
        ...
        for (let offset = 0, chunkIndex = 0; offset < bytes.byteLength; chunkIndex += 1) {
          const chunk = bytes.subarray(offset, offset + AGENTIC_OS_STORAGE_ENGINE_BINARY_CHUNK_BYTES)
          chunks.push({
