...
export function HierarchyGuides(props: {
  depth: number; visibleDepths: readonly number[]; index: number
  hasChildren: boolean; expanded?: boolean; origin?: number; step?: number; span?: boolean; maxDepth?: number
}) {
  const guides = Math.min(props.depth, props.maxDepth ?? 8)
  const origin = props.origin ?? 62, step = props.step ?? 24
  return <>
    {Array.from({ length: guides }, (_, level) => {
      ...
      return <span key={level} aria-hidden="true" data-hierarchy-guide="" {...(props.span ? { 'data-span-guide': '' } : {})}
        className="pointer-events-none absolute top-0 border-l"
        style={{ left: origin + level * step, height: branch && last ? '50%' : '100%', borderColor: '#a8a29e', opacity: branch || !last ? 1 : 0 }}>
    ...
    {props.hasChildren && props.expanded && <span aria-hidden="true" className="pointer-events-none absolute bottom-0 h-1/2 border-l"
      style={{ left: origin + guides * step, borderColor: '#a8a29e' }} />}
  </>
}
...
export function HierarchyTreeRow(props: {
  ...
  return <div role="treeitem" tabIndex={props.focused ? 0 : -1}
    ...
    style={{ gridTemplateColumns: props.gridTemplateColumns || 'minmax(0, 1fr)', borderLeftColor: props.selected ? '#3b82f6' : 'transparent', background: props.selected ? 'color-mix(in srgb, #3b82f6 22%, var(--kg-panel-bg, white))' : undefined }}>
