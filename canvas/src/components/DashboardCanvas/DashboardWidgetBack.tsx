...
export default function DashboardWidgetConfiguration(props: DashboardWidgetEditorProps & { onClose: () => void }) {
  ...
  const chartKind = props.template === 'metric' || props.template === 'tree' || props.template === 'codebase' ? undefined : props.template
  ...
  return <section aria-label="Widget settings" className="h-full min-h-0 min-w-0 overflow-y-auto overscroll-contain space-y-3 rounded-lg border border-[var(--kg-border)] bg-[var(--kg-panel-bg)] p-4 shadow-sm"
    ...
    <form aria-label="Widget configuration" className="space-y-3" onSubmit={event => {
      ...
      if (!missionId && !source) { setError('Choose a data source.'); return }
      const id = selected || `graph:widget-${crypto.randomUUID()}`
      const settings = Object.fromEntries(Object.entries({ ...draft, ...(missionId ? {} : { source: sourceId }),
    ...
    {!missionId && <label className="block text-xs">Data source<select aria-label="Data source" className={control} value={sourceId} required onChange={event => change({ source: event.target.value })}>
    ...
    {props.template !== 'metric' && !missionId && <label className="block text-xs">Display<select aria-label="Display" className={control} value={draft.kind ?? (source as DashboardCard)?.kind ?? props.template} onChange={event => change({ kind: event.target.value as DashboardCard['kind'] })}>
