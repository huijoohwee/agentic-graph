...
const runNodeOnlyUiTests = async (results: TestResult[]) => {
  ...
  await execTest(
    ...
    const modSemanticSpace = await import('../features/xr-v2/__tests__/semanticSpaceRuntime.test')
    await execTest(results, 'xr.semanticSpace.package.roundTrip', modSemanticSpace.testSemanticSpaceRoundTripAndGuards)
    await execTest(results, 'xr.semanticSpace.webMcp.invocation', modSemanticSpace.testSemanticSpaceWebMcpAndInvocation)
    await execTest(results, 'xr.semanticSpace.projection.geometry', modSemanticSpace.testSemanticSpaceRelativeDepthProjection)
