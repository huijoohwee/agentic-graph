...
export function compileImageRegions(doc: SpaceDocument, action: ConfirmImageRegions): SpaceDocument {
  ...
  const entities: SpaceEntity[] = action.proposals.map((proposal, index) => ({
    ...
    label: proposal.label, category: 'visual-region', region: proposal.region,
  ...
  const objects = entities.map((entity, index) => {
    ...
    const width = clamp(region.width * twin.room.width, 0.1, Math.min(5, twin.room.width))
    const depth = 0.4 // Explicit fixed extrusion assumption, not an inferred depth.
    ...
    const binding = buildSemanticTwinBinding({ entity, observation: action.observation, room: twin.room,
      template: 'box', size: [width, clamp(region.height * 4, 0.1, 5), depth], position: [x, 0, z] })
