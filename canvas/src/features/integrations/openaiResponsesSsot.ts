...
const OPENAI_DOC_ROW_BY_ROW_KEY: Readonly<Record<string, OpenAiApiDocRow>> = {
  ...
  'openaiApi.auth_mode': {
    ...
    notes: 'Server-managed auth reads AGENTICGRAPH_CHAT_PROXY_OPENAI_API_KEY or OPENAI_API_KEY from the proxy runtime; BYOK stays memory-only.',
    searchHints: ['chatAuthMode auth byok serverManaged api key Cloudflare secret AGENTICGRAPH_CHAT_PROXY_OPENAI_API_KEY OPENAI_API_KEY'],
  ...
  'openaiApi.api_key': {
    ...
    notes: 'Server-managed mode leaves this field empty and lets the proxy use AGENTICGRAPH_CHAT_PROXY_OPENAI_API_KEY or OPENAI_API_KEY.',
    searchHints: ['chatApiKey api key authentication bearer memory-only server-managed proxy AGENTICGRAPH_CHAT_PROXY_OPENAI_API_KEY OPENAI_API_KEY'],
    modules: ['canvas/src/hooks/store/uiSlice.ts', 'canvas/src/features/settings/registry-ui.ui.ts', 'canvas/src/components/StoryboardWidgetCanvas.tsx'],
    classes: ['GraphState', 'RunGenerationConfig'],
