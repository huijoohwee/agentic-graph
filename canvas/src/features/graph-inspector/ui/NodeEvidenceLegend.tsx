...
export function NodeEvidenceLegend({ graph }: { graph: GraphData }) {
  ...
  return <details className="mt-2"><summary>Legend</summary>
    {native && <>
      <h4 className="mt-2 font-semibold">Node size</h4>
      <p>Radius grows with distinct incident relationships in the loaded projection, including each self-link once. Isolated nodes remain visible; hubs are capped at 36 canvas units. Size stays stable when the render budget hides other nodes. Partial imports give lower-bound connectivity.</p>
