...
export function SpatialWorkspaceReview() {
  ...
  return <section aria-label="Spatial change review" className="grid min-w-0 gap-2 border-t p-2 text-xs" style={{ color: 'var(--kg-text-primary)', background: 'var(--kg-panel-bg)' }} data-kg-spatial-review>
    ...
    {!proposal && <fieldset disabled={working || review.preparing || !snapshot?.ok} className="grid min-w-0 gap-2">
      ...
      <button type="button" className="min-h-11 rounded border px-3" onClick={() => void run(async () => {
        if (!subject || !snapshot || !('identity' in snapshot)) return { ok: false, message: 'Inspect a loaded scene first.' }
        return proposeSpatialWorkspace({ expectedToken: snapshot.identity.token,
          edits: [{ subjectId: subject.id, position: [subject.position[0] + 1, subject.position[1], subject.position[2]] }] })
      })}>Preview +1 m on X</button>
    </fieldset>}
    ...
    {result?.durability === 'indeterminate' && result.receipt && <button type="button" disabled={working} className="min-h-11 rounded border px-3" onClick={() => void run(() => recheckSpatialReceipt(result.receipt!.id))}>Recheck saved receipt</button>}
  </section>
}
