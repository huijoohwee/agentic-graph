...
function CarGeometry({ color, size }: { color: string; size: VehicleSize }) {
  ...
  return (
    <group name="agentic_os_xr_procedural_car" userData={{ vehicleKind: 'car' }}>
      <mesh name="agentic_os_xr_car_chassis" position={[0, 0, height * 0.34]} castShadow receiveShadow>
      ...
      <mesh name="agentic_os_xr_car_glass" position={[0, depth * 0.01, height * 0.7]} scale={[width * 0.38, depth * 0.25, height * 0.25]} castShadow>
      ...
      {[-1, 1].flatMap(x => [-1, 1].map(y => (
        <group key={`${x}:${y}`} position={[x * wheelX, y * wheelY, height * 0.2]}>
          <mesh name="agentic_os_xr_car_wheel" rotation={[0, 0, Math.PI / 2]} castShadow>
...
function HelicopterGeometry({ color, size }: { color: string; size: VehicleSize }) {
  ...
  return (
    <group name="agentic_os_xr_procedural_helicopter" userData={{ vehicleKind: 'helicopter' }}>
      <mesh name="agentic_os_xr_helicopter_cabin" position={[0, -depth * 0.24, height * 0.48]} scale={[cabinWidth * 0.5, depth * 0.18, height * 0.3]} castShadow>
        <sphereGeometry args={[1, 20, 14]} />
        <Paint color={color} metalness={0.12} roughness={0.4} />
      </mesh>
      <mesh name="agentic_os_xr_helicopter_windshield" position={[0, -depth * 0.4, height * 0.54]} scale={[cabinWidth * 0.38, depth * 0.045, height * 0.2]}>
        <sphereGeometry args={[1, 18, 12]} />
        <meshStandardMaterial color="#15364c" roughness={0.18} metalness={0.2} />
      </mesh>
      <mesh name="agentic_os_xr_helicopter_tail_boom" position={[0, depth * 0.16, height * 0.5]} rotation={[0.04, 0, 0]} castShadow>
      ...
      <mesh name="agentic_os_xr_helicopter_mast" position={[0, -depth * 0.16, height * 0.86]} rotation={[Math.PI / 2, 0, 0]}>
        <cylinderGeometry args={[height * 0.035, height * 0.045, height * 0.28, 10]} />
        <Paint color="#475569" metalness={0.5} roughness={0.35} />
      </mesh>
      <group name="agentic_os_xr_helicopter_main_rotor" position={[0, -depth * 0.16, height]}>
        <mesh castShadow>
      ...
      <group name="agentic_os_xr_helicopter_tail_rotor" position={[cabinWidth * 0.09, depth * 0.49, height * 0.62]} rotation={[0, Math.PI / 2, 0]}>
      ...
      {[-1, 1].map(side => (
        <React.Fragment key={side}>
          <mesh name="agentic_os_xr_helicopter_skid" position={[side * skidX, -depth * 0.2, height * 0.1]} castShadow>
