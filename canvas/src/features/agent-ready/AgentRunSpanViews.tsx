...
export function AgentRunSpanViews({ rows, timing, selectedId, onSelect, search = '' }: {
  rows: ReturnType<typeof visibleSpanTree>; timing: boolean; selectedId: string | null; onSelect: (id: string) => void; search?: string
}) {
  ...
  const visible = rows.filter((row, index) => {
    if (timing || search.trim()) return true
    let depth = row.depth
  ...
  return <ul ref={container} role={timing ? 'list' : 'tree'} aria-label={timing ? 'Span timing' : 'Span hierarchy'} className="min-w-0 py-2">
    {visible.map(({ span, depth, missingParent }, index) => {
      ...
      const guides = Math.min(depth, 8), width = timing ? 0 : guides * 24
      ...
      return <li key={span.spanId} role={timing ? undefined : 'none'} className="min-w-0">
        <div role={timing ? 'button' : 'treeitem'} tabIndex={timing || focusedId === span.spanId ? 0 : -1}
          ...
          className="relative flex w-full min-w-0 cursor-pointer items-center gap-3 border-l-4 px-3 py-3 text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-500"
          ...
          {!timing && Array.from({ length: guides }, (_, level) => {
          ...
          {!timing && hasChildren && expanded && <span aria-hidden="true" className="pointer-events-none absolute bottom-0 h-1/2 border-l" style={{ left: 62 + guides * 24, borderColor: '#a8a29e' }} />}
          <span className="relative flex min-w-0 flex-1 items-center gap-3" style={{ paddingLeft: width }}>
            {!timing && <span className="flex w-5 shrink-0 justify-center">{hasChildren && <button type="button" tabIndex={-1}
            ...
            <span className="min-w-0"><span className="block break-words text-sm font-medium" title={span.operation}>{span.operation}</span>
              <span className="flex flex-wrap items-center gap-x-1 text-xs opacity-80">
                ...
                {timing && <span>· exclusive observed {numberLabel(span.timing.exclusive, ' ms')}</span>}
