...
export const buildStrybldrLocalImageDataUri = (args: {
  ...
  const provider = cleanText(args.provider) || 'agentic-graph-local-image'
...
export const buildStrybldrLocalAnimaticHtml = (handoff: Pick<StrybldrVideoHandoff, 'cards' | 'referenceImageUrl' | 'sourceVideoUrl' | 'renderVideoUrl'>): string => {
  ...
  return [
    ...
    '<section><div class="kg-label">Generator</div><div class="kg-value">agentic-graph local animatic</div></section>',
    ...
    nodes: nodes.map(node => {
      ...
      return {
        ...
        properties: {
          ...
          provider: cleanText(args.provider) || cleanText(props.provider) || 'agentic-graph-local-animatic',
    ...
    nodes: nodes.map(node => {
      ...
      return {
        ...
        properties: {
          ...
          provider: cleanText(args.provider) || cleanText(props.provider) || 'agentic-graph-local-image',
...
export const buildStrybldrImageHandoffMarkdown = (args: {
  ...
  return [
    ...
    'status: "generated"',
    `provider: ${yamlQuote(cleanText(args.provider) || 'agentic-graph-local-image')}`,
    args.model ? `model: ${yamlQuote(cleanText(args.model))}` : '',
    `elapsedMs: ${Math.max(0, Math.round(args.elapsedMs))}`,
    `paidCallCount: ${Math.max(0, Math.round(args.paidCallCount))}`,
    `cacheHit: ${args.cacheHit === true ? 'true' : 'false'}`,
