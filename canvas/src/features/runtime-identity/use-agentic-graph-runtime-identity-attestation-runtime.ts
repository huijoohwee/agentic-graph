...
import {
  ...
  AGENTIC_OS_RUNTIME_IDENTITY_REQUIRED_DEVICE_COUNT,
...
import { serializeAgenticGraphRuntimeIdentity, type AgenticGraphRuntimeIdentity } from './agentic-graph-runtime-identity'
...
const publishGateState = (overrides: Partial<AgenticGraphRuntimeIdentityGateSnapshot>): void => {
  publishAgenticGraphRuntimeIdentityGateSnapshot({
    ...
    requiredDeviceCount: AGENTIC_OS_RUNTIME_IDENTITY_REQUIRED_DEVICE_COUNT,
...
export function useAgenticGraphRuntimeIdentityAttestationRuntime(identity: AgenticGraphRuntimeIdentity): void {
  ...
  React.useEffect(() => {
    ...
    const socketUrl = buildAgenticGraphStorageCanvasRoomWebSocketUrl(config, AGENTIC_OS_RUNTIME_IDENTITY_ROOM_ID)
    if (!socketUrl) {
      publishGateState({
        status: 'blocked',
