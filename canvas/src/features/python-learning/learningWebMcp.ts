...
const deadlineMessage = 'Request acknowledgement exceeded two seconds.'
function withinDeadline<T>(operation: (deadline: AbortController) => Promise<T>): Promise<T> {
  const deadline = new AbortController()
  let timer: ReturnType<typeof setTimeout>
  const timeout = new Promise<never>((_, reject) => { timer = setTimeout(() => { deadline.abort(); reject(new Error(deadlineMessage)) }, 2000) })
  return Promise.race([operation(deadline), timeout]).finally(() => clearTimeout(timer))
}
export function createLearningToolExecutor(runtime: LearningRuntime = pythonLearningRuntime, digest = digestLearningSource) {
  ...
  const identity = async () => {
    ...
    const [sourceDigest, sceneDigest] = await Promise.all([digest(document.source), digest(learningSceneDescriptor(document.lessonId))])
  ...
  const inspect = () => withinDeadline(async deadline => {
    ...
    if (deadline.signal.aborted) throw new Error(deadlineMessage)
  ...
  const execute = async (input: Record<string, unknown>) => {
    ...
    const operation = async (deadline: AbortController) => {
    ...
    const result = withinDeadline(operation)
