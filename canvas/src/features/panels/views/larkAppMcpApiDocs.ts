...
const LARK_APP_MCP_DOC_ROWS: ReadonlyArray<LarkAppMcpDocRow> = [
  {
    ...
    responsibility: 'Default MCP server key a Lark app backend or host can use when registering the deployed agentic-graph MCP.',
    searchHints: ['mcpServers', 'agentic-graph', 'server key'],
  },
  {
    key: 'remote.url',
    typeLabel: 'url',
    ...
    searchHints: ['https://airvio.co/agentic-graph/mcp', 'pages http mcp', 'remote endpoint'],
  },
  {
    key: 'transport.type',
    typeLabel: 'string',
    value: LARK_APP_MCP_TRANSPORT_TYPE,
    responsibility: 'Transport label for the deployed agentic-graph remote MCP surface.',
  ...
  },
  {
    ...
    notes: 'Open agentic-graph Canvas when imported or reviewed content should be validated before graph apply.',
    searchHints: ['agentic-graph canvas', 'review surface', 'graph visualization'],
  },
  {
    key: 'import.command',
    typeLabel: 'command',
...
export const LARK_APP_MCP_DOC_ENTRIES: ReadonlyArray<VirtualSettingsEntry> =
  LARK_APP_MCP_DOC_ROWS.map(row => {
    ...
    return {
      ...
      searchHints: ['lark app mcp to canvas', 'agentic-graph canvas', row.key, ...(row.searchHints || [])],
      details,
    }
  })
