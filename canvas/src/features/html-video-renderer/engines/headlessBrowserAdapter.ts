...
<body>
${spec.html}
<script type="application/json" id="agentic-graph-html-video-data">${dataJson}</script>
<script>
(() => {
  const readData = () => {
    const element = document.getElementById('agentic-graph-html-video-data');
    try {
  ...
  const data = readData();
  window.__AGENTIC_OS_HTML_VIDEO_DATA__ = data;
  window.__AGENTIC_OS_RENDER_TIME_MS__ = 0;
  window.__agenticGraphRenderFrame = async (timeMs) => {
    const seconds = timeMs / 1000;
    window.__AGENTIC_OS_RENDER_TIME_MS__ = timeMs;
    ...
    window.dispatchEvent(new CustomEvent('agentic-graph:render-frame', { detail: { timeMs, seconds, data } }));
...
export const headlessBrowserAdapter: RenderEngine = {
  ...
  async render(spec) {
    const frameCount = Math.max(1, Math.ceil((spec.durationMs / 1000) * spec.fps))
    const maxFrames = readPositiveIntegerEnv(AGENTIC_OS_HTML_VIDEO_MAX_FRAMES, DEFAULT_MAX_FRAMES)
    if (frameCount > maxFrames) {
      throw new Error(`${AGENTIC_OS_HTML_VIDEO_MAX_FRAMES} blocks ${frameCount} requested frames; raise the runtime limit deliberately to render this spec`)
    }
    ...
    const workspace = await mkdtemp(join(tmpdir(), 'agentic-graph-html-video-'))
    ...
    try {
      ...
      await runFfmpeg(readEnv(AGENTIC_OS_HTML_VIDEO_FFMPEG_BIN) || DEFAULT_FFMPEG_BIN, [
        '-y',
        '-hide_banner',
        '-loglevel',
        'error',
        ...
        '-an',
        '-c:v',
        readEnv(AGENTIC_OS_HTML_VIDEO_FFMPEG_VIDEO_CODEC) || DEFAULT_MP4_VIDEO_CODEC,
        '-pix_fmt',
        'yuv420p',
        '-movflags',
        '+faststart',
      ...
      return {
        ...
        renderLog: [
          `frames=${frameCount}`,
          `ffmpeg=${readEnv(AGENTIC_OS_HTML_VIDEO_FFMPEG_BIN) || DEFAULT_FFMPEG_BIN}`,
          `codec=${readEnv(AGENTIC_OS_HTML_VIDEO_FFMPEG_VIDEO_CODEC) || DEFAULT_MP4_VIDEO_CODEC}`,
        ],
      }
    } finally {
