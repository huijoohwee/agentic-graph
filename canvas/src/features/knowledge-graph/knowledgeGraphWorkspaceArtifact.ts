...
const CODEBASE_GRAPH_DOCUMENT_NAME = 'codebase-graph.md'
...
export const KNOWLEDGE_GRAPH_WORKSPACE_ARTIFACT_PATH =
  `${KNOWLEDGE_GRAPH_WORKSPACE_ARTIFACT_DIRECTORY}/${CODEBASE_GRAPH_DOCUMENT_NAME}` as const
...
function assertCompletedKnowledgeGraphArtifactRequest(args: WorkspaceKnowledgeGraphArtifactRequest): void {
  if (
    ...
    || args.result.projection.complete !== true
  ) {
...
/**
 * Maintains a single Source Files artifact for the most recent completed
 * repository graph import. It intentionally does not select the document or
 * re-parse it, so the authoritative graph canvas remains active.
 */
export async function materializeKnowledgeGraphWorkspaceArtifact(
  ...
  const path = await upsertWorkspaceMarkdownSourceFile({
    ...
    name: CODEBASE_GRAPH_DOCUMENT_NAME,
