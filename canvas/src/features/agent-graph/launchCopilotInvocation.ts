...
import { launchSourceIdentity, publishLaunchWorkspace, reopenLaunchWorkspace, exportLaunchWorkspace, launchProbeBinding, launchProbeDecisions, retainLaunchProbeResponse, replaceLaunchWorkspaceDraft, type LaunchRecord } from './launchCopilotWorkspace'
import { AGENT_GRAPH_HOST_ROUTE, agentGraphHostFetch, connectAgentGraphRelay, disconnectAgentGraphRelay } from './agentGraphHostAdapter'
...
let unregisterPairedBridge: (() => void) | undefined
async function generateLaunchText(args: FloatingPanelChatSubmitArgs, controller: AbortController, prompt: string, maxTokens: number) {
  ...
  const send = createChatSubmitRequestSender({ requestUrl, controller, fetchFn: agentGraphHostFetch, submitArgs: {
    ...args, chatMaxCompletionTokens: maxTokens, chatStream: false, chatMessagesJson: null,
    chatToolsJson: null, chatToolChoiceJson: null, chatResponseFormatJson: null,
  } })
  const response = await send(args.chatModel, [{ role: 'user', content: prompt }])
...
export async function invokeLaunchCopilot(args: FloatingPanelChatSubmitArgs) {
  ...
  const probe = input.match(/^\/launch-copilot\s+probe\s+([a-z0-9-]+)$/)
  const refine = input.match(/^\/launch-copilot\s+refine\s+([a-z0-9-]+)$/)
  ...
  try {
    const connect = input.match(/^\/launch-copilot\s+connect\s+(\S+)$/)
    if (connect || input === '/launch-copilot disconnect') {
      unsubscribe(); args.setInput('')
      unregisterPairedBridge?.(); unregisterPairedBridge = undefined
      disconnectAgentGraphRelay()
      if (!connect) { report('Graph host disconnected. Retained proposals remain available offline.'); return }
      const expires = await connectAgentGraphRelay(connect[1])
      const { registerAgentGraphLaunchHostBridge } = await import('./agentGraphLaunchHostBridge')
      unregisterPairedBridge = registerAgentGraphLaunchHostBridge({ enabled: true })
      report(`Graph host connected until ${new Date(expires).toLocaleTimeString()}. Use Launch → Import URL, select the source cluster, then outline or draft your business requirement. The pairing credential is held only for this connection.`)
      return
    }
    if (handoff) {
      ...
      const response = await agentGraphHostFetch(`${AGENT_GRAPH_HOST_ROUTE}/proposal`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, signal: controller.signal, body })
    ...
    if (probe) {
      if (!navigator.onLine) throw new Error('Reconnect to generate source-grounded questions; retained answers remain editable offline')
      const selected = useGraphStore.getState().selectedNodeIds[0]
      unsubscribe()
      const record = await reopenLaunchWorkspace(probe[1]), graph = useGraphStore.getState().graphData
      unsubscribe = useGraphStore.subscribe(state => { if (state.graphData !== graph) controller.abort() })
      const anchor = record.overlay!.nodes.find(node => node.id === selected && node.properties?.launchProbe === true) || record.overlay!.nodes[0]
      if (Number(anchor.properties.probeTreeDepth || 0) >= 5) throw new Error('Probe-Tree depth limit reached')
      const { invokeProbeTreeMcpBridge } = await import('@/features/agent-ready/probeTreeMcpClient')
      ...
      const response = await invokeProbeTreeMcpBridge({ sourceBinding: launchProbeBinding(record), contextText: buildStoryboardWidgetProbeTreeContextText({ graphData: record.overlay!, node: anchor, prompt: record.evidence.requirement }),
        threadRootId: record.overlay!.nodes[0].id, currentNodeId: anchor.id, invocationTokens: [], optionCount: 3,
        probeTreeDepth: Number(anchor.properties.probeTreeDepth || 0) + 1, recallTopK: 0, tokenBudget: 1200,
      }, (url, init) => agentGraphHostFetch(url, { ...init, signal: controller.signal }))
      const { buildStoryboardWidgetProbeTreeProviderPrompt } = await import('@/components/StoryboardWidgetCanvas/runtime/storyboardWidgetProbeTreeProviderPrompt')
      const generated = await generateLaunchText(args, controller, buildStoryboardWidgetProbeTreeProviderPrompt({ contextText: response.groundedContext!, currentNodeId: anchor.id,
        mcpResult: response.result, mcpInvoked: true, probeTreeDepth: Number(anchor.properties.probeTreeDepth || 0) + 1 }), 2000)
      controller.signal.throwIfAborted()
      await retainLaunchProbeResponse(record, response, anchor.id, { text: generated.assistantText, model: generated.modelId || args.chatModel })
      report(`Source-bound native Probe-Tree questions retained for ${probe[1]} using the current OpenAI connection (${args.chatModel}). Answer on the cards; select a question and run probe again to continue, or /launch-copilot refine ${probe[1]} to apply your decisions to the five documents.`)
      args.setInput(`/launch-copilot refine ${probe[1]}`)
      return
    }
    if (!action && !refine) throw new Error('Use /launch-copilot outline owned <business requirement> (or draft/reference). Select an imported source cluster first. Reopen/export/probe/refine uses the returned CID.')
    let prior: LaunchRecord | undefined
    if (refine) {
      unsubscribe(); prior = await reopenLaunchWorkspace(refine[1])
    ...
    const request = prior ? { ...prior.request, action: 'ground', decisions: launchProbeDecisions(prior) } : {
    ...
    const host = async (body: Record<string, unknown>): Promise<LaunchRecord> => {
      const response = await agentGraphHostFetch(`${AGENT_GRAPH_HOST_ROUTE}/proposal`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal: controller.signal })
    ...
    if (refine || action[1] === 'draft') {
      try {
        // One owner transport attempt; no automatic provider/model fallback or hidden repair.
        const result = await generateLaunchText(args, controller, record.prompt, 6000)
        ...
        controller.signal.throwIfAborted()
        if (refine) throw error // Preserve reviewed documents and answers if refinement fails.
        drafting = `AI drafting unverified: ${String(error)}. Retained the labelled editable outline.`
      }
    }
    controller.signal.throwIfAborted()
    if (prior) {
      record.overlay = prior.overlay; record.sourceWorkspacePath = prior.sourceWorkspacePath
      await replaceLaunchWorkspaceDraft(record)
    } else await publishLaunchWorkspace(record)
