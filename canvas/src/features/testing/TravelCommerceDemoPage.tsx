...
function isExecutedDemoReport(value: unknown): value is ExecutedDemoReport {
  try {
    if (!isRecord(value) || value.schema !== 'knowgrph-travel-commerce-demo-evidence/v1' || value.status !== 'passed' || value.mode !== DEMO_MODE || value.deployLane !== 'Dev_Lane' || value.providerRequests !== 0 || value.realPaymentCalls !== 0 || value.productionMutations !== 0 || !Array.isArray(value.beats) || value.beats.length !== 8) return false
