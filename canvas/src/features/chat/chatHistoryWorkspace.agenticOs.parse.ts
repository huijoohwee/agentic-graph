...
} from './chatAgenticOsFrontmatter'
import { hasResponseOnlyAgenticOsBody, isResponseOnlyAgenticOsFrontmatter } from './chatAgenticOsResponseOnlyContract'
...
export const isAgenticOsStructuredMarkdown = (raw: string): boolean => {
  ...
  if (frontmatterKeys.has('agenticOsResponseOnly') && isResponseOnlyAgenticOsFrontmatter(parsedFrontmatterBody.frontmatter)) {
    return hasResponseOnlyAgenticOsBody(markdownBody, BASE_TEMPLATE_REQUIRED_BODY_SECTIONS)
  }
  ...
  const parsed = tryParseMarkdownFrontmatterFlowGraph('chatAgenticGraph.agenticOs.base.md', text)
  if (!parsed) return false
