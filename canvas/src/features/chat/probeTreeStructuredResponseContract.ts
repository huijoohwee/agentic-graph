import {
  AGENTIC_OS_PROBE_TREE_GENERATE_TOOL_NAME,
  AGENTIC_OS_PROBE_TREE_INVOCATION_TOKENS,
  AGENTIC_OS_PROBE_TREE_MAX_DEPTH,
  AGENTIC_OS_PROBE_TREE_SELECT_TOOL_NAME,
...
const PROBE_TREE_DIRECT_TOOL_PATTERN = /\bagentic-graph\.probe\.(?:generate|select)\b/i
...
const hasProbeTreeInvocation = (userQuery: string): boolean => {
  ...
  return AGENTIC_OS_PROBE_TREE_INVOCATION_TOKENS.some(token => tokens.has(token.toLowerCase()))
}
...
export function buildProbeTreeCardMaterializationPrompt(userQuery: string): string {
  ...
  return [
    ...
    '- Resolve authored /agentic-graph.probe-tree, #agentic-graph.probe-tree, or @agentic-graph.probe-tree metadata through agentic-graph.agentic_canvas_os.docs.invoke when that MCP tool is connected.',
    '- When the local agentic-graph MCP tools are connected, invoke agentic-graph.probe.generate once with thread_root_id, current_node_id, context_text, k between 2 and 4, and the bounded token_budget.',
    ...
    `- Put the model-generated probe question in question so the card renders it as Summary; the runtime derives bounded depth at or below ${AGENTIC_OS_PROBE_TREE_MAX_DEPTH}, leaves output empty for the user-owned selection, and makes every numbered choice a concise answer to that card's question.`,
    '- On continuation, the selected child card and its committed multi-selection own the next topic. The runtime persists that commitment through agentic-graph.probe.select; the provider must not emit the action. Use preceding cards only as lineage context and never substitute the thread root or a same-id root alias.',
...
const readProbeTreeDepth = (record: Record<string, unknown>): number => {
  ...
  return Math.min(AGENTIC_OS_PROBE_TREE_MAX_DEPTH, Math.max(1, Math.floor(value)))
}
...
export function buildProbeTreeStructuredResponseProperties(args: {
  ...
  const nextAction = readFirstString(args.record, ['nextAction', 'next_action']) || AGENTIC_OS_PROBE_TREE_SELECT_TOOL_NAME
  ...
  return {
    ...
    invocationTokens: [...AGENTIC_OS_PROBE_TREE_INVOCATION_TOKENS],
    invocation: AGENTIC_OS_PROBE_TREE_GENERATE_TOOL_NAME,
