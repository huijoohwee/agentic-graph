...
export const deriveOutputTargetFileName = (fileName: string): string => {
  ...
  return raw && /^agenticOs_/i.test(raw) ? raw : 'agenticOs.md'
}

export const buildRequestSummary = (profile: ReturnType<typeof analyzeAgenticOsRequest>): string => {
...
export const buildFlowContextSummary = (profile: ReturnType<typeof analyzeAgenticOsRequest>): string => {
...
export const buildObjectiveSummary = (profile: ReturnType<typeof analyzeAgenticOsRequest>): string => {
  if (profile.signals.creativeScript) {
