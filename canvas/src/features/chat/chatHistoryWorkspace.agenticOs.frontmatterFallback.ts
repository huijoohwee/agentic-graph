...
import { buildFlowDiagramsBlock } from './chatHistoryWorkspace.agenticOs.flowDiagramsFallback'
import { AGENTIC_OS_RESPONSE_ONLY_SCHEMA } from './chatAgenticOsResponseOnlyContract'
...
const buildRequiredSectionLabels = (profile: ReturnType<typeof analyzeAgenticOsRequest>): string[] => {
  return [
...
const buildFrontmatterContextSummary = (profile: ReturnType<typeof analyzeAgenticOsRequest>): string => {
...
const buildFrontmatterValidationFocus = (profile: ReturnType<typeof analyzeAgenticOsRequest>): string[] => {
  return [
...
const buildResponseOnlyFrontmatter = (args: {
  ...
  return [
    ...
    `$schema: ${JSON.stringify(AGENTIC_OS_RESPONSE_ONLY_SCHEMA)}`,
    ...buildStoryboardTemplateProjectionFrontmatterLines({
      profile: args.profile,
      assistantText: args.assistantText,
    }),
    'spec:',
    '  format:        agentic-os-response',
    '  version:       "1.0.0"',
...
export const buildFrontmatter = (args: {
  ...
  return [
    ...
    'kgMultiDimTableModeEnabled: false',
    'kgDocumentStructureBaselineLock: false',
    ...(args.responseOnly ? ['agenticOsResponseOnly: true'] : []),
