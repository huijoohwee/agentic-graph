...
async function activateChatAgenticOsWorkspacePath(path: string): Promise<boolean> {
...
export async function applyChatAgenticOsDocumentTextToCanvas({
  name,
  text,
}: {
  name: string
...
export async function applyChatAgenticOsWorkspaceDocumentToCanvas(path: string): Promise<boolean> {
  const workspacePath = normalizeWorkspacePath(path)
  ...
  if (!await activateChatAgenticOsWorkspacePath(workspacePath)) return false
  const name = workspaceDocumentKey(workspacePath) || workspaceBasename(workspacePath) || workspacePath
  return await applyChatAgenticOsDocumentTextToCanvas({ name, text })
}
