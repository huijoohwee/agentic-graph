...
const buildFrameAnalysisRuntimeScript = (timing: string): string => [
  ...
  `window.addEventListener('resize',schedule,{passive:true});window.addEventListener('agenticgraph:render-frame',function(event){sync(Number(event&&event.detail&&event.detail.timeMs)||0);});sync(Number(window.__AGENTICGRAPH_RENDER_TIME_MS__)||0);}());`,
].join('')
...
export const projectVideoAgentFrameAnalysisSrcDoc = (args: {
  ...
  if (srcDoc.includes('data-composition-id="agenticgraph-video-agent-runtime"')) return srcDoc
