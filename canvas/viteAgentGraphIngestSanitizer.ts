...
const MAX_PROJECTION_BYTES = AGENT_GRAPH_PROJECTION_MAX_BYTES
const MAX_PROJECTION_NODES = 2_000
const MAX_PROJECTION_EDGES = 5_000
const AGENT_GRAPH_IMPORT_PROGRESS_SCHEMA = 'agentic-graph-agent-graph-import-progress/v1'
...
export type AgentGraphIngestSanitizerFailure = (
  code: string,
  message: string,
  status?: number,
) => never
...
type SanitizerOptions = {
  fail: AgentGraphIngestSanitizerFailure
...
export function sanitizeAgentGraphImportResult(
  value: unknown,
...
export function sanitizeAgentGraphImportProgress(
  value: unknown,
  options: SanitizerOptions,
): Record<string, unknown> {
  const expectedParserRegistryDigest = options.expectedParserRegistryDigest || SOURCE_PARSER_REGISTRY.digest
  ...
  if (byteBounded.byteLength > AGENT_GRAPH_PROJECTION_GRAPH_DATA_MAX_BYTES) {
