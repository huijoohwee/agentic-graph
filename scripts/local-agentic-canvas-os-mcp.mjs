...
export const createLocalAgenticCanvasOsMcpServer = ({
  ...
  const initialize = (response, body) => {
    ...
    writeJson(response, 200, {
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'knowgrph-local-agentic-canvas-os-docs', version: '1' },
      },
  ...
  return createServer(async (request, response) => {
    ...
    if (request.method === 'GET' && requestUrl.pathname === HEALTH_PATH) {
      writeJson(response, 200, {
        schema: 'knowgrph-local-agentic-canvas-os-mcp-health/v1',
        status: 'ready',
      })
      return
    }
...
if (invokedFile === currentFile) {
  ...
  server.listen(port, '127.0.0.1', () => {
    process.stdout.write(`[knowgrph] local Agentic Canvas OS MCP ready on http://127.0.0.1:${port}${MCP_PATH}\n`)
  })
}
