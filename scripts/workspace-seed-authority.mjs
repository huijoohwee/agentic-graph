...
const requireXrV2RuntimeIdentity = source => {
  ...
  for (const [label, actual, expected] of [
    ...
    ['run_ready_demo.validation_seed_path', runReady.validation_seed_path, `/${XR_V2_SEED_RELATIVE_PATH}`],
...
export async function verifyWorkspaceSeedAuthority({
  ...
  if (!await isFile(xrV2Path)) throw new Error(`canonical XR v2 workspace seed is missing: ${xrV2Path}`)
  requireXrV2RuntimeIdentity(await readFile(xrV2Path, 'utf8'))
