...
const readLocalReport = () => {
  ...
  for (const key of Object.keys(env)) {
    if (key.startsWith('AGENTIC_OS_PR_') || key === 'AGENTIC_OS_GITHUB_TOKEN') delete env[key]
  }
...
test('source revision resolves from the explicit CI value or repository HEAD', () => {
  ...
  assert.equal(
    resolveCollaborationRuntimeSourceRevision({ environment: { AGENTIC_OS_SOURCE_REVISION: configuredRevision } }),
    configuredRevision,
  )
  ...
  assert.throws(
    () => resolveCollaborationRuntimeSourceRevision({ environment: { AGENTIC_OS_SOURCE_REVISION: 'A'.repeat(40) } }),
...
test('schema command emits the canonical schema without path knowledge', async () => {
  ...
  assert.equal(commandSchema.$id, 'https://agentic-graph.dev/schemas/collaboration-runtime-report/v1')
})
...
test('validation schema command emits the canonical envelope schema without path knowledge', async () => {
  ...
  assert.deepEqual(commandSchema, fileSchema)
  assert.deepEqual(sharedSchema, fileSchema)
  assert.equal(commandSchema.$id, 'https://agentic-graph.dev/schemas/collaboration-runtime-validation/v1')
})
...
test('example command emits the canonical local report without pull request context', async () => {
  const result = spawnSync('npm', ['run', '--silent', 'collaboration:report:example'], {
    ...
    env: {
      ...process.env,
      AGENTIC_OS_PR_NUMBER: '999',
      AGENTIC_OS_PR_BASE_REF: 'invalid',
    },
  })
  assert.equal(result.status, 0, result.stderr)
...
test('report validator accepts the canonical example through stdin and rejects mutations', async () => {
  ...
  assert.deepEqual(success, {
    ...
    status: 'passed',
    schemaId: 'https://agentic-graph.dev/schemas/collaboration-runtime-report/v1',
...
test('validation-result CLI accepts a stored artifact and rejects schema drift', async () => {
  const temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), 'agentic-graph-validation-result-'))
...
test('artifact validator CLI accepts canonical output and rejects a mutated file', async () => {
  const temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), 'agentic-graph-collaboration-report-'))
  ...
  try {
    ...
    assert.notEqual(invalidResult.status, 0)
    assert.match(invalidResult.stderr, /invalid agentic-graph\.collaboration-runtime-report\/v1/)
  } finally {
    await rm(temporaryDirectory, { recursive: true, force: true })
  }
})
