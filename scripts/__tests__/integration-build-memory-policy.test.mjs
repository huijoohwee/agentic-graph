...
test('integration workflow authorizes the exact protected refresh candidate', () => {
  ...
  assert.match(source, /AGENTICGRAPH_PR_BODY<<\$\{delimiter\}/)
  assert.match(source, /AGENTICGRAPH_REQUIRE_REMOTE_SCOPE_CHECK: \$\{\{ github\.event_name == 'pull_request' \|\| github\.event_name == 'workflow_dispatch' \}\}/)
})
