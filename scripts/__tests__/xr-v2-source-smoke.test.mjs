...
test('XR v2 workspace browser smoke prefers explicit then bundled Chromium and omits a missing executable', () => {
  for (const verifier of [
    ...
    assert.match(
      source,
      /const executablePath = findLocalChromiumExecutable\(\s*process\.env\.AG_XR_V2_CHROMIUM_EXECUTABLE,\s*chromium\.executablePath\(\),?\s*\)/u,
    )
...
test('XR v2 browser source rejects weakened explicit AC-11/AC-12 evidence flow', async t => {
  ...
  const mutations = [
    ...
    ['software GLES ANGLE driver', "'--use-angle=swiftshader'", "'--use-angle=default'"],
    ['software WebGL fallback regression', "'--use-angle=swiftshader'", "'--use-angle=swiftshader-webgl'"],
...
test('XR v2 workspace seed rejects evidence and cross-device claim drift', async t => {
  const mutations = [
    ...
    ['shared XR source authority', 'source_authority: /docs/workspace-seeds/agentic-graph-ar-vr-xr-runtime-readiness-demo.md', 'source_authority: /docs/workspace-seeds/alternate-world.md'],
    ['shared XR world ownership', 'world_ownership: source-authored', 'world_ownership: standalone'],
