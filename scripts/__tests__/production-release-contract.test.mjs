...
const productionServiceWorkerUpgradeScript = ['verify-production-service-worker-upgrade.mjs', 'production-service-worker-profile.mjs'].map(file => fs.readFileSync(path.resolve(repoRoot, 'scripts', file), 'utf8')).join('\n')
...
test('Pages release dependencies use the canonical storage origin', () => {
  ...
  assertNoneMatch(storageOriginSources.join('\n'), [/https:\/\/agentic-graph-storage\.huijoohwee\.workers\.dev/, /https:\/\/agentic-graph-storage\.huijoohwee\.workers\.dev/])
