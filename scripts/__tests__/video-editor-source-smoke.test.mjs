...
async function createFixture(t) {
  const root = await mkdtemp(path.join(os.tmpdir(), 'agentic-graph-video-editor-boundary-'))
...
test('source smoke returns a structured, network-free repository contract', async () => {
  ...
  assert.equal(report.schema, 'agentic-graph-video-editor-source-smoke/v1')
  assert.equal(report.status, 'pass')
  assert.equal(report.checks[0].schema, 'agentic-graph-video-editor-independence-source-contract/v1')
  assert.equal(
