...
test("resolveHostedSubmissionFlowConfig derives deterministic output paths", () => {
  const config = resolveHostedSubmissionFlowConfig({
    FRONTEND_URL: "https://airvio.co/agentic-graph",
    MCP_ENDPOINT: "https://airvio.co/agentic-os/control-plane/mcp",
    ARTIFACTS_DIR: "./tmp-artifacts",
  });

  assert.equal(config.frontendUrl, "https://airvio.co/agentic-graph");
  assert.equal(config.mcpEndpoint, "https://airvio.co/agentic-os/control-plane/mcp");
