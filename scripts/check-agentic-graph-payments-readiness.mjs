...
if (json) {
  ...
  if (report.ok) {
    console.log('[agentic-graph] payments are ready for protected integration; no canonical or deployment claim is implied.')
  } else {
    console.error(`[agentic-graph] payments runtime readiness is blocked by ${report.blockers.length} explicit blocker(s).`)
  }
}
