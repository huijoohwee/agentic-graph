...
const requireCanonicalSource = (state, source) => {
  if (source.task_divergence_allowed) {
    ...
    const canonicalOwnerPath = path.resolve(state.canonicalOwnerPath || '')
    if (!state.root || !state.canonicalRoot || !state.canonicalOwnerPath) {
      throw new Error(`${source.id} canonical Dev source topology is incomplete`)
    }
    if (canonicalOwnerPath !== canonicalRoot || invocationRoot !== canonicalRoot) {
      throw new Error(
        `blocked-canonical-path: canonical ${source.id} Dev must run from ${canonicalRoot}; `
        + `the registered ${source.canonical_branch} owner is ${canonicalOwnerPath} and this command ran from ${invocationRoot}. `
