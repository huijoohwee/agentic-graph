...
const candidateTag = sourceSha => `agentic-graph-${sourceSha}`
const candidateMessage = (sourceSha, candidateDigest) => `agentic-graph candidate ${sourceSha} ${candidateDigest}`
...
const secretFile = (entry, secrets, environment) => {
  ...
  const file = path.join(path.resolve(environment.RUNNER_TEMP || os.tmpdir()),
    `agentic-graph-travel-secrets-${entry.id}-${crypto.randomUUID()}.json`)
...
const activateCandidate = async ({ entry, unit, sourceSha, run }) => {
  await run(activationArguments(entry, unit.candidate.versionId, `activate agentic-graph candidate ${sourceSha}`))
