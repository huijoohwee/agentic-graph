...
const liveOrigin = String(process.env.AGENTIC_OS_AI_GATEWAY_PAGES_ORIGIN || defaultOrigin).replace(/\/+$/g, '');
...
const inspectPagesSecretList = () => {
  ...
  const acceptedSecretNames = [
    'AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_TOKEN',
    'AI_GATEWAY_TOKEN',
...
const inspectPagesProjectConfig = () => {
  ...
  try {
    ...
    return {
      baseUrlPresent: configText.includes('AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_BASE_URL'),
      gatewayIdPresent: configText.includes('AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_GATEWAY_ID'),
    };
  } finally {
...
const runLiveTransportSmoke = async () => {
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      model: 'gpt-5-nano',
      input: [{ role: 'user', content: 'hello from agentic-graph ai gateway readiness smoke' }],
      stream: false,
    }),
  ...
  if (proxyMissingKey) {
    throw new Error(
      ...
      'Create a fresh Pages deployment after setting AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_TOKEN, ' +
...
process.stdout.write(`AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_TOKEN=${readEnvPresence('AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_TOKEN')}\n`);
...
process.stdout.write(`AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_BASE_URL=${readEnvPresence('AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_BASE_URL')}\n`);
process.stdout.write(`AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_GATEWAY_ID=${readEnvPresence('AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_GATEWAY_ID')}\n`);
const projectConfig = inspectPagesProjectConfig();
process.stdout.write('\n[ai-gateway-readiness] Cloudflare Pages project AI Gateway vars\n');
process.stdout.write(`AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_BASE_URL=${projectConfig.baseUrlPresent ? 'present' : 'missing'}\n`);
process.stdout.write(`AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_GATEWAY_ID=${projectConfig.gatewayIdPresent ? 'present' : 'missing (optional)'}\n`);

const pagesSecrets = inspectPagesSecretList();
const blockers = [];
if (!projectConfig.baseUrlPresent) {
  blockers.push(
    'Cloudflare Pages project joohwee is missing AGENTIC_OS_CHAT_PROXY_AI_GATEWAY_BASE_URL in the downloaded project config.',
  );
}
