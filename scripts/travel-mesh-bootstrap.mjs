...
export const createBootstrapDesiredState = (environment, beforeInventory = null) => {
  requiredEnvironment(environment); const routes = routeSpecFor(environment), runtime = bootstrapRuntimeConfiguration(environment),
    mcpTransition = bootstrapMcpTransitionFor(beforeInventory)
  return Object.freeze({
    accountId: environment.CLOUDFLARE_ACCOUNT_ID, zoneId: environment.TRAVEL_PUBLIC_ZONE_ID,
    resources: bootstrapResourceSpecFor(environment), migrations: bootstrapMigrationSpec(beforeInventory?.appliedMigrations ?? []),
    ...
    deploymentOrder: BOOTSTRAP_DEPLOY_ORDER, mcpTransition,
    ...
    environment: Object.freeze({ variables: Object.freeze(Object.entries(runtime.variables).map(([name, value]) => ({ name, valueDigest: digest(String(value)) })).sort((left, right) => left.name.localeCompare(right.name))),
...
const packetMatchesRuntime = (packet, environment) => {
  ...
  for (const { name, valueDigest } of [...normalized.variables, ...normalized.secrets]) if (!(name in environment)
    || digest(String(environment[name])) !== valueDigest) throw new Error(`provider packet digest does not match protected runtime value ${name}`)
...
export const planBootstrap = async ({ adapter, environment, packet, sourceSha, sourceTree, controllerDigest, workflowDigest, wranglerDigest, issuedAt, expiresAt }) => {
  if (![sourceSha, sourceTree].every(value => SHA.test(value)) || ![controllerDigest, workflowDigest, wranglerDigest].every(value => DIGEST.test(value))) throw new Error('bootstrap protected source evidence is invalid')
...
const journalEffect = ({ id, expected, observed, disposition, at }) => { const body = { effectId: id, expectedDigest: digest(expected), observedDigest: digest(observed), disposition, attemptedAt: at }; return Object.freeze({ ...body, effectDigest: digest(body) }) }
const projectOne = async ({ adapter, plan, journal, id, expected, at, pending = null }) => {
  ...
  if (pending && adapter.responseLossAdoptable?.(id) === true) { let observedPending
    try { observedPending = await adapter.observe(id, plan) } catch (error) { if (adapter.pendingReplayProjectable?.(id) !== true) throw error }
    if (canonical(observedPending) === canonical(expected)) { const next = appendBootstrapJournal(journal, journalEffect({ id, expected, observed: observedPending, disposition: exactResponseLossDisposition({ expected, observed: observedPending, error: new Error('durable pending effect survived controller interruption') }), at })); await adapter.persistJournal(next); return next } }
  let disposition = 'projected'
  try { await adapter.project(id, expected, plan) } catch (error) {
    if (adapter.responseLossAdoptable?.(id) !== true) throw error
    let observedAfterLoss; try { observedAfterLoss = await adapter.observe(id, plan) }
  ...
  const next = appendBootstrapJournal(journal, journalEffect({ id, expected, observed, disposition, at })); await adapter.persistJournal(next); return next
}
...
export const applyBootstrap = async ({ adapter, environment, packet, plan, authorization: authorizationText,
  ...
  if (typeof adapter.verifyTerminalOwnedState !== 'function') throw new Error('terminal bootstrap evidence verifier is required')
  ...
  if (sealedCompletion) { const completion = normalizeBootstrapCompletion(sealedCompletion, normalized.planDigest)
    await adapter.verifyTerminalOwnedState(normalized, completion.receipt); return completion }
  ...
  if (persisted) {
    ...
    if (pending && (Object.keys(pending).sort().join(',') !== 'effectId,expectedDigest'
      || pending.effectId !== BOOTSTRAP_EFFECT_ORDER[journal.effects.length] || !DIGEST.test(pending.expectedDigest))) throw new Error('durable pending bootstrap envelope is invalid')
    authorization = journal.authorization
    if (journal.planDigest !== normalized.planDigest || authorization.actor !== actor) throw new Error('durable bootstrap journal belongs to a different plan or actor')
  } else {
    ...
    const stable = await requireStableCompleteInventory(() => adapter.readCompleteInventory()); if (stable.inventoryDigest !== normalized.beforeInventoryDigest) throw new Error('provider inventory changed after bootstrap planning')
    journal = createBootstrapJournal({ plan: normalized, authorization }); await adapter.persistJournal(journal)
  }
  ...
  for (const id of BOOTSTRAP_EFFECT_ORDER.slice(0, -2)) {
    ...
    const completed = journal.effects.find(effect => effect.effectId === id); if (completed) {
      let superseded = id === 'deploy:mcp-shell' && journal.effects.some(effect => effect.effectId === 'deploy:mcp'), replayOptions = {}
      if (id === 'deploy:mcp-shell' && !superseded && pending?.effectId === 'deploy:mcp') { const finalExpected = effectExpectation('deploy:mcp', normalized.desired, null, normalized)
        if (pending.expectedDigest === digest(finalExpected)) { replayOptions = { pendingFinalMcp: true }; try { superseded = canonical(await adapter.observe('deploy:mcp', normalized)) === canonical(finalExpected) } catch {} } }
      if (completed.expectedDigest !== digest(expected) || (!superseded
        && canonical(await adapter.observe(id, normalized, replayOptions)) !== canonical(expected))) {
        throw new Error(`completed bootstrap effect ${id} drifted during replay`)
      }
      continue
    }
    journal = await projectOne({ adapter, plan: normalized, journal, id, expected, at: attemptedAt, pending }); pending = null
  }
  const finalEvidence = await adapter.readCompletionEvidence(normalized)
  const receipt = buildReceipt({ plan: normalized, authorization, journal, finalEvidence, provisionedAt: authorization.consumedAt })
  if (!journal.effects.some(effect => effect.effectId === 'enable-release')) preflightBootstrapTerminalCarriers({ journal, receipt })
  for (const [id, expected] of [
    ...
    if (completed && (completed.expectedDigest !== digest(expected)
      || canonical(await adapter.observe(id, normalized)) !== canonical(expected))) throw new Error(`completed bootstrap effect ${id} drifted during replay`)
    if (!completed) { journal = await projectOne({ adapter, plan: normalized, journal, id, expected, at: attemptedAt, pending }); pending = null }
  }
  await adapter.verifyTerminalOwnedState(normalized, receipt)
...
const safeRepository = value => { const repository = requireText(value, 'GITHUB_REPOSITORY'); if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repository)) throw new Error('GITHUB_REPOSITORY is malformed'); return repository }
const parseJsonResult = (result, label) => { try { return JSON.parse(result.stdout) } catch { throw new Error(`${label} did not return JSON`) } }
...
export const createProductionBootstrapAdapter = ({ environment, journalPath, run = null, apiFetch = fetch,
  ...
  if (path.relative(runnerRoot, journalFile).startsWith('..') || path.relative(runnerRoot, journalFile) === '') throw new Error('bootstrap journal must be an external RUNNER_TEMP child')
  ...
  const privateMcpCandidateStatus = async (entry, version, plan) => {
    ...
    if (version.annotations?.['workers/tag'] !== annotation.tag || version.annotations?.['workers/message'] !== annotation.message
      || bindingEvidence(version, 'private MCP dependency shell candidate').length) throw new Error('private MCP dependency shell candidate is not exact and binding-free')
    const secrets = await wranglerJson(['--no-install', 'wrangler', 'secret', 'list', ...flags(entry), '--format', 'json'])
    if (!Array.isArray(secrets) || secrets.length) throw new Error('private MCP dependency shell candidate inherited script secrets')
    const subdomain = (await api(`/accounts/${account}/workers/scripts/${encodeURIComponent(entry.worker)}/subdomain`)).result
    const routes = (await api(`/zones/${zone}/workers/routes`)).result, domains = (await api(`/accounts/${account}/workers/domains`)).result
    if (subdomain?.enabled !== false || subdomain?.previews_enabled !== false || !Array.isArray(routes) || !Array.isArray(domains)
      || routes.some(item => item.script === entry.worker) || domains.some(item => item.service === entry.worker)) throw new Error('private MCP dependency shell candidate is exposed')
    try { return activeDeployment(await wranglerJson(['--no-install', 'wrangler', 'deployments', 'status', ...flags(entry), '--json']), entry.worker) }
    catch (error) { if (absentWorker(error)) return null; throw error }
  }
  const verifyBindings = (entry, version, plan) => {
    ...
    if (!unit || digest(evidence) !== unit.bindingSpecDigest || canonical(evidence) !== canonical(unit.bindings)) throw new Error(`${entry.id} active binding inventory drifted from the complete sealed plan`)
    const annotation = expectedVersion(entry, plan)
    if (version.annotations?.['workers/tag'] !== annotation.tag || version.annotations?.['workers/message'] !== annotation.message) throw new Error(`${entry.id} active version is not bound to the bootstrap plan`)
    return new Map((version.resources?.bindings ?? []).map(binding => [binding.name, binding]))
  }
  const stableMcpProjection = async plan => { const { inventory } = await requireStableCompleteInventory(readCompleteInventory)
    assertBootstrapTargetsUnexposed(inventory, plan.desired); return { inventory, projection: mcpPrivateProjection(inventory) } }
  const exactMcpPredecessor = async (plan, allowFinalSecrets = false) => { const { projection } = await stableMcpProjection(plan), transition = plan.desired.mcpTransition,
    unit = projection.unit, baseline = mcpPrivateProjection(plan.beforeInventory).unit, sealedSecrets = baseline.secretNames ?? [], finalSecrets = plan.desired.units.find(item => item.id === 'mcp').secretNames,
    secretsExact = canonical(unit.secretNames) === canonical(sealedSecrets) || (allowFinalSecrets && canonical(unit.secretNames) === canonical(finalSecrets)),
    annotation = expectedVersion(entryFor('mcp-shell'), plan, 'mcp-shell'), normalizedProjection = { ...projection, unit: { ...unit, secretNames: sealedSecrets } }
    if (!secretsExact || (transition.mode === 'adopt-existing-private' ? digest(normalizedProjection) !== transition.beforeProjectionDigest : (unit.absent === true
      || !unit.deployment?.deploymentId || unit.deployment.versionId !== unit.versionId || unit.deployment.percentage !== 100 || canonical(unit.annotations) !== canonical({ 'workers/tag': annotation.tag,
        'workers/message': annotation.message }) || unit.bindings.length))) throw new Error('active MCP predecessor drifted from its exact sealed baseline or shell')
    return { deploymentId: unit.deployment.deploymentId, versionId: unit.versionId, normalizedProjectionDigest: digest(normalizedProjection) } }
  const observeDeployment = async (id, plan, { pendingFinalMcp = false } = {}) => {
    if (id === 'mcp-shell') { if (pendingFinalMcp) { const entry = entryFor('mcp'), annotation = expectedVersion(entry, plan), matches = (await versions(entry)).filter(item => item.annotations?.['workers/tag'] === annotation.tag && item.annotations?.['workers/message'] === annotation.message)
        if (matches.length !== 1) throw new Error('pending final MCP candidate is absent or duplicated'); verifyBindings(entry, matches[0], plan) }
      await exactMcpPredecessor(plan, pendingFinalMcp); return effectExpectation('deploy:mcp-shell', plan.desired, null, plan) }
    ...
    if (id === 'mcp') { const { projection } = await stableMcpProjection(plan), evidence = projection.unit
      if (evidence.versionId !== status.versionId || evidence.deployment?.versionId !== status.versionId || evidence.deployment?.percentage !== 100
        || canonical(evidence.bindings) !== canonical(unit.bindings) || canonical(evidence.secretNames) !== canonical(unit.secretNames)) throw new Error('active final MCP is not the exact private desired unit') }
  ...
  const deploy = async (id, plan) => {
    ...
    if (id === 'mcp-shell' && plan.desired.mcpTransition.mode === 'adopt-existing-private') { await observeDeployment(id, plan); return }
    ...
    let versionId = matches[0]?.id, responseLoss = null, config = null, secrets = null, shell = null, sealedMcpPredecessor = null
    if (id === 'mcp') { if (versionId) verifyBindings(entry, matches[0], plan)
      const active = activeDeployment(await wranglerJson(['--no-install', 'wrangler', 'deployments', 'status', ...flags(entry), '--json']), entry.worker), final = matches.find(item => item.id === active.versionId)
      if (final) await observeDeployment('mcp', plan); else if ((sealedMcpPredecessor = await exactMcpPredecessor(plan, Boolean(versionId))).versionId !== active.versionId) throw new Error('active MCP predecessor changed before candidate upload') }
    try {
      if (!versionId) {
        ...
        if (!shell && config.contentDigest !== plan.desired.units.find(unit => unit.id === id)?.configDigest) throw new Error(`${id} effective route-free configuration drifted from the sealed plan`)
        secrets = ['marketplace', 'mcp-shell'].includes(id) ? null : secretFile(entry, runtime, environment)
        if (id === 'mcp-shell' && digest((await stableMcpProjection(plan)).projection)
          !== plan.desired.mcpTransition.beforeProjectionDigest) throw new Error('sealed absent MCP baseline changed before shell creation')
        const args = id === 'marketplace'
          ? ['--no-install', 'wrangler', 'versions', 'upload', ...flags(entry, config.file), '--strict', '--keep-vars', '--tag', annotation.tag, '--message', annotation.message]
          : id === 'mcp-shell'
            ? ['--no-install', 'wrangler', 'deploy', ...flags(entry, config.file), '--strict', '--tag', annotation.tag, '--message', annotation.message]
      ...
      const selected = (await versions(entry)).filter(item => item.id === versionId)
      if (selected.length !== 1) throw new Error(`${id} plan-bound version detail is absent or duplicated`)
      let status = id === 'mcp-shell' ? await privateMcpCandidateStatus(entry, selected[0], plan) : null
      if (id !== 'mcp-shell') {
        verifyBindings(entry, selected[0], plan)
      ...
      if (id === 'mcp-shell' && status && status.versionId !== versionId) throw new Error(`${id} has a competing active version`)
      if (id === 'mcp' && status?.versionId !== versionId && (!sealedMcpPredecessor || canonical(await exactMcpPredecessor(plan, true)) !== canonical(sealedMcpPredecessor)
        || status?.versionId !== sealedMcpPredecessor.versionId)) throw new Error('active MCP predecessor changed before final activation')
  ...
  const resourcesFromInventory = (inventory, plan) => {
    const resources = plan.desired.resources
    for (const id of [resources.balanceCacheKvNamespaceId, resources.mcpDefinitionKvNamespaceId]) if (!inventory.kv.some(item => item.id === id)) throw new Error(`required KV namespace ${id} is absent`)
    for (const name of [resources.mcpMediaR2Bucket, resources.provenanceArchiveR2Bucket, resources.storageR2Bucket]) if (!inventory.r2.some(item => (item.name ?? item.bucket_name) === name)) throw new Error(`required R2 bucket ${name} is absent`)
  ...
  const observeResources = async plan => resourcesFromInventory(await readCompleteInventory(), plan)
  ...
  const migrationsFromInventory = (names, plan) => {
    if (!Array.isArray(names) || new Set(names).size !== names.length
      || canonical([...names].sort()) !== canonical([...plan.desired.migrations.names].sort())) throw new Error('D1 migration inventory does not exactly equal the sealed baseline and repository union')
    return plan.desired.migrations
  }
  const observeMigrations = async plan => migrationsFromInventory([...(await appliedMigrations())], plan)
  ...
  const observeRoutes = async plan => { const inventory = await readCompleteInventory()
    validateRouteInventory(inventory.routes, inventory.domains, environment); return plan.desired.routes }
  const exposureFromInventory = (inventory, plan) => {
  ...
  const observeExposure = async plan => exposureFromInventory(await readCompleteInventory(), plan)
  ...
  const project = async (id, expected, plan) => {
    if (id === 'resources') {
      ...
      for (const target of [resources.balanceCacheKvNamespaceId, resources.mcpDefinitionKvNamespaceId]) if (!inventory.kv.some(item => item.id === target)) throw new Error(`bootstrap cannot synthesize sealed KV identity ${target}`)
      if (!inventory.d1.some(item => (item.uuid ?? item.id) === ACTUAL_STORAGE_D1_ID
    ...
    if (id === 'storage-migrations') {
      const applied = await appliedMigrations(), desiredNames = new Set(plan.desired.migrations.names)
      if (plan.beforeInventory.appliedMigrations.some(name => !applied.has(name)) || [...applied].some(name => !desiredNames.has(name))) throw new Error('D1 migration inventory drifted outside the sealed baseline and repository union')
      const spec = assertAdditiveBootstrapMigrations(applied)
      if (canonical(spec.names) !== canonical(plan.desired.migrations.names)) throw new Error('D1 migration projection drifted from its sealed exact union')
    ...
    if (id === 'persist-receipt') { setEnvironmentValue('variable', 'TRAVEL_MESH_BOOTSTRAP_RECEIPT_JSON', bootstrapReceiptCarrier(expected)); return }
  ...
  const observe = async (id, plan, options = {}) => {
    if (id === 'resources') return observeResources(plan)
    if (id === 'storage-migrations') return observeMigrations(plan)
    if (id.startsWith('deploy:')) return observeDeployment(id.slice('deploy:'.length), plan, options)
  ...
  const providerInventoryDigest = inventory => digest({ ...inventory, environment: { ...inventory.environment,
    variables: inventory.environment.variables.filter(item => !['TRAVEL_MESH_BOOTSTRAP_JOURNAL_JSON', 'TRAVEL_MESH_BOOTSTRAP_RECEIPT_JSON', 'TRAVEL_MESH_RELEASE_ENABLED'].includes(item.name)) } })
  const readCompletionEvidence = async plan => {
    const opening = await requireStableCompleteInventory(readCompleteInventory), versions = {}, bindings = {}, secretNames = {}
    resourcesFromInventory(opening.inventory, plan); versions.providerInventoryDigest = providerInventoryDigest(opening.inventory)
    for (const unit of plan.desired.units) {
      const evidence = opening.inventory.units.find(item => item.id === unit.id), annotation = expectedVersion(entryFor(unit.id), plan)
      if (!evidence || evidence.absent === true || evidence.deployment?.versionId !== evidence.versionId
        || evidence.deployment?.percentage !== 100 || canonical(evidence.annotations) !== canonical({
          'workers/tag': annotation.tag, 'workers/message': annotation.message })
        || canonical(evidence.bindings) !== canonical(unit.bindings)
        || canonical(evidence.secretNames) !== canonical(unit.secretNames)) throw new Error(`${unit.id} closing unit evidence drifted`)
    ...
    const routeEvidence = validateRouteInventory(opening.inventory.routes, opening.inventory.domains, environment)
    const evidence = { versions, bindings, secretNames, routes: routeEvidence.routes, domains: routeEvidence.domains,
      migrations: migrationsFromInventory(opening.inventory.appliedMigrations, plan),
      exposure: exposureFromInventory(opening.inventory, plan), probes: await observeProbes(plan) }
    const closing = await requireStableCompleteInventory(readCompleteInventory)
    if (closing.inventoryDigest !== opening.inventoryDigest) throw new Error('complete provider inventory drifted while collecting terminal evidence')
    return evidence
  }
  const assertTerminalEnvironmentProjection = async (plan, receipt) => { if (canonical(await observe('persist-receipt', plan)) !== canonical(receipt) || canonical(await observe('enable-release', plan)) !== canonical(effectExpectation('enable-release', plan.desired))) throw new Error('terminal bootstrap environment projection drifted') }
  return {
    ...
    responseLossAdoptable: id => !['project-environment-packet', 'live-probes'].includes(id), pendingReplayProjectable: id => id === 'deploy:mcp',
    readCompletionEvidence,
    verifyTerminalOwnedState: async (plan, receipt, evidence = null) => {
      await assertTerminalEnvironmentProjection(plan, receipt)
      const closingEvidence = evidence ?? await readCompletionEvidence(plan)
      if (digest(closingEvidence) !== receipt.finalEvidenceDigest) throw new Error('terminal bootstrap provider evidence drifted from its sealed receipt')
      await assertTerminalEnvironmentProjection(plan, receipt)
    },
  }
}
