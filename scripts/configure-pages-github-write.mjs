...
const DEFAULT_ROUTE = 'https://airvio.co/agentic-graph/api/workspace/github/write'
...
const options = {
  ...
  branch: readArgValue('--branch', readEnv('AGENTIC_OS_GITHUB_WRITE_BRANCH') || DEFAULT_BRANCH),
  confirm: readArgValue('--confirm', ''),
  json: hasFlag('--json'),
  projectName: readArgValue('--project-name', DEFAULT_PROJECT),
  repository: readArgValue('--repository', readEnv('AGENTIC_OS_GITHUB_WRITE_REPOSITORY') || DEFAULT_REPOSITORY),
...
const redact = (text) => {
  const token = readEnv('AGENTIC_OS_GITHUB_WRITE_TOKEN')
...
const validateRepository = () => {
  ...
  addAction('AGENTIC_OS_GITHUB_WRITE_REPOSITORY', 'cloudflare-pages-secret')
  return true
}
...
const validateBranch = () => {
  ...
  addAction('AGENTIC_OS_GITHUB_WRITE_BRANCH', 'cloudflare-pages-secret')
  return true
}
...
const validateToken = (tokenSecretExists) => {
  ...
  if (!token) {
    ...
    addCheck(
      'token-input',
      'fail',
      'Missing AGENTIC_OS_GITHUB_WRITE_TOKEN in the process environment. Use a fine-grained GitHub token limited to the target repository with Contents read/write.',
    )
    return false
  }
  ...
  addAction('AGENTIC_OS_GITHUB_WRITE_TOKEN', 'cloudflare-pages-secret')
  return true
}
...
for (const name of ['AGENTIC_OS_GITHUB_WRITE_REPOSITORY', 'AGENTIC_OS_GITHUB_WRITE_BRANCH', 'AGENTIC_OS_GITHUB_WRITE_TOKEN']) {
...
const tokenOk = validateToken(existingSecrets.has('AGENTIC_OS_GITHUB_WRITE_TOKEN'))

if (options.apply && canApply) {
  if (repositoryOk) putSecret('AGENTIC_OS_GITHUB_WRITE_REPOSITORY', options.repository)
  if (branchOk) putSecret('AGENTIC_OS_GITHUB_WRITE_BRANCH', options.branch)
  if (tokenOk) putSecret('AGENTIC_OS_GITHUB_WRITE_TOKEN', readEnv('AGENTIC_OS_GITHUB_WRITE_TOKEN'))
}
...
if (!options.apply) {
  addCheck(
    'apply-mode',
    'skip',
    `Dry run only. To mutate Pages secrets, export AGENTIC_OS_GITHUB_WRITE_TOKEN and pass --apply --yes --confirm=${APPLY_CONFIRMATION}. Add --write-smoke only when a real GitHub test commit is intended.`,
  )
}
