...
} from './agentic-graph-payments-bundle-secrets.mjs'
import {
  runAgenticGraphPaymentsLocalVcc,
} from './agentic-graph-payments-local-vcc.mjs'
import {
  inspectAgenticGraphPaymentsCanonicalRuntime,
  inspectAgenticGraphPaymentsSourceIdentity,
} from './agentic-graph-payments-delivery-gates.mjs'
import {
  inspectAgenticGraphAgenticPurchaseReadiness,
} from './agentic-graph-agentic-purchase-readiness.mjs'
import {
...
} from './agentic-graph-payments-provider-proof.mjs'
import {
...
} from './agentic-graph-payments-source-evidence.mjs'

export const AGENTIC_OS_PAYMENTS_READINESS_SCHEMA_ID = 'agentic-graph-payments-readiness/v1'
export {
  AGENTIC_OS_PAYMENTS_PROVIDER_PROOF_SCHEMA_ID,
  validateAgenticGraphPaymentsProviderProof,
}
export const AGENTIC_OS_PAYMENTS_MANIFEST_SCHEMA_ID =
...
const inspectSource = async ({ root, requireTracked, executeLocalVcc }) => {
  ...
  if (manifest.schemaId === AGENTIC_OS_PAYMENTS_MANIFEST_SCHEMA_ID) {
    addCheck(checks, 'manifest-schema', 'pass', `Manifest uses ${AGENTIC_OS_PAYMENTS_MANIFEST_SCHEMA_ID}.`, [
      MANIFEST_PATH,
    ])
  } else {
  ...
  if (!executeLocalVcc) {
    ...
    try {
      ...
      addCheck(
        ...
        [MANIFEST_PATH, 'scripts/lib/agentic-graph-payments-local-vcc.mjs'],
      )
      localVcc = {
      ...
      addCheck(
        ...
        ['scripts/lib/agentic-graph-payments-local-vcc.mjs'],
      )
      localVcc = { status: 'fail', attestation: null }
    }
  }
...
export async function inspectAgenticGraphPaymentsReadiness({
  ...
  return {
    schemaId: AGENTIC_OS_PAYMENTS_READINESS_SCHEMA_ID,
