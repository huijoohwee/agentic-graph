...
export const AGENTIC_OS_PAYMENTS_LOCAL_VCC_SCHEMA_ID =
  'agentic-graph-payments-local-vcc-attestation/v1'
...
export const AGENTIC_OS_PAYMENTS_LOCAL_VCC_SUITES = Object.freeze([
  Object.freeze({
  ...
  Object.freeze({
    ...
    args: Object.freeze([
      ...
      '--test',
      '--test-reporter=tap',
      'cloudflare/workers/agentic-graph-payment/__tests__/agentic-purchase-safety.test.ts',
      'cloudflare/workers/agentic-graph-payment/__tests__/payment-runtime-service.test.ts',
      'cloudflare/workers/agentic-graph-payment/__tests__/payment-rail-adapters.test.ts',
      'cloudflare/workers/agentic-graph-payment/__tests__/payment-event-ingress.test.ts',
      'cloudflare/workers/agentic-graph-payment/__tests__/payment-runtime-routes.test.ts',
    ]),
  }),
  Object.freeze({
  ...
  Object.freeze({
    ...
    args: Object.freeze([
      '--test',
      '--test-reporter=tap',
      'scripts/__tests__/agentic-graph-payments-local-vcc.test.mjs',
    ]),
  }),
])
...
export const AGENTIC_OS_PAYMENTS_LOCAL_VCC_INVENTORY_DIGEST = sha256(JSON.stringify({
  preparation: commandIdentity(PREPARATION),
  suites: AGENTIC_OS_PAYMENTS_LOCAL_VCC_SUITES.map(commandIdentity),
}))
...
export function validateAgenticGraphPaymentsLocalVccAttestation(
  ...
  if (attestation?.schemaId !== AGENTIC_OS_PAYMENTS_LOCAL_VCC_SCHEMA_ID) {
  ...
  if (attestation?.inventoryDigest !== AGENTIC_OS_PAYMENTS_LOCAL_VCC_INVENTORY_DIGEST) {
  ...
  const expectedSuiteIds = AGENTIC_OS_PAYMENTS_LOCAL_VCC_SUITES.map(suite => suite.id)
...
export async function runAgenticGraphPaymentsLocalVcc({
  ...
  const suites = preparation.status === 'pass'
    ? await Promise.all(AGENTIC_OS_PAYMENTS_LOCAL_VCC_SUITES.map(suite =>
        executeSuite({ root, suite, executor })))
    : AGENTIC_OS_PAYMENTS_LOCAL_VCC_SUITES.map(skippedSuiteResult)
  const attestation = Object.freeze({
    schemaId: AGENTIC_OS_PAYMENTS_LOCAL_VCC_SCHEMA_ID,
    sourceEvidenceDigest,
    inventoryDigest: AGENTIC_OS_PAYMENTS_LOCAL_VCC_INVENTORY_DIGEST,
    preparation,
