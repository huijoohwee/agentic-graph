...
This repository-tracked design is part of the normative `.kiro/specs/agentic-graph-game-flight-sim` source of truth. It grounds those constraints in the tracked Agentic ECS contract (`docs/documents/agentic-graph-agentic-entity-component-system-prd-tad.md`), the canonical `contracts/cost-log.schema.js`, and the source module cluster under `canvas/src/features/game-flight-sim/`. The Flight Sim PRD/TAD (`docs/documents/agentic-graph-game-flight-sim-prd-tad.md`) and workspace seed (`docs/workspace-seeds/agentic-graph-game-flight-sim-demo.md`) are derived implementation/proof projections; any workspace-root Kiro copy is a byte-identical local projection only.
