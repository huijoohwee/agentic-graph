...
related:
  ...
  - "canvas/src/lib/storage/knowgrphStorageDb.ts"
...
read-only chain-evidence boundary inside the existing `knowgrph-payment` Worker, one hosted adapter over it, one
...
|---|---|---|
| Payment_Trust_Boundary Worker | `cloudflare/workers/knowgrph-payment/index.ts`: route predicate plus handler pairs (`isPaymentRuntimeRoute` / `handlePaymentRuntimeRoute`), `readDb(env)` from `../shared/d1`, env typed `Record<string, unknown> & { DB }` | One new module set behind the existing runtime route handler; no new route family or env access pattern |
| D1 binding, migrations, cost ledger | `wrangler.toml` binding `DB`, `migrations_dir = ../../d1/migrations`, latest `0010_knowgrph_agentic_purchase_lifecycle.sql`; `payment_cost_entries` (0009) with `appendCostEntry` / `listCostEntries` in `paymentRuntimePersistence.ts` | One new migration `0011_xsgd_chain_evidence.sql`; the cost ledger is extended, not forked, by a chain-scoped table with the same field vocabulary |
...
| Readiness gate | `payment:runtime:readiness` -> `scripts/check-knowgrph-payments-readiness.mjs` -> `scripts/lib/knowgrph-payments-readiness.mjs` `gates` map, digest-bound local VCC in `scripts/lib/knowgrph-payments-local-vcc.mjs`, manifest `scripts/knowgrph-payments-readiness-properties.json` | Two new statuses inside the existing `gates` map and manifest; no new command |
| Browser-local storage and funding surface | `canvas/src/lib/storage/knowgrphStorageDb.ts`, IndexedDB database `kg:knowgrph-storage` with collections including `paymentIntentQueue` and `paymentReceiptDocuments`; `AgenticPurchaseLifecycleView.tsx` inside `PaywallOverlay.tsx` | One new collection `paymentChainEvidence`, origin and profile scoping from IndexedDB itself; projection rows render inside the existing funding phase item, no second surface |
...
| Trust boundary, secret custody, rail selection, buyer product authority, data minimization | knowgrph-payments R1, R2, R12 |
| Provider event authentication and replay-safe settlement; record document, parser, printer, round trip | knowgrph-payments R5, R7 |
| Funding tuple validation, signer authority, funding reservation, provider credit gate | knowgrph-payments R14 |
| Discovery, disposable-card issuance and Approval_Gate, execution | knowgrph-payments R15, R16, R17 |
...
flowchart TB
  subgraph Client["Payment_Client (browser, no credential)"]
    ...
    DB0[("kg:knowgrph-storage IndexedDB<br/>origin + profile scoped")]
    PW --> EC --> DB0
  end
  ...
  subgraph Worker["knowgrph-payment Worker = Payment_Trust_Boundary"]
...
One new collection `paymentChainEvidence` in the existing `kg:knowgrph-storage` database. Origin and profile scoping
...
knowgrph-payments R7 contract that this design cites rather than redefines.
...
`cloudflare/workers/knowgrph-payment/__tests__/*.test.ts` under `node --import tsx --test`; rendering tests in the
canvas unit runner. Each property test carries the tag comment `Feature: xsgd-onchain-verification, Property {n}:
{statement}`. Both new suites join the existing `KNOWGRPH_PAYMENTS_LOCAL_VCC_SUITES` allowlist and
`scripts/knowgrph-payments-readiness-properties.json`, so the digest-bound attestation covers them; no new command.
