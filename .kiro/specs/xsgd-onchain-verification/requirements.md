...
Knowgrph's Singapore agentic-purchase lifecycle funds a non-custodial XSGD position on Avalanche C-Chain before an
...
| Concern | Owner |
|---|---|
| Paywall surface, lifecycle identifier, phase projection, cancellation | knowgrph-payments R13 |
| Rail selection, credential custody, provider egress, data minimization | knowgrph-payments R1, R2, R12 |
| Funding tuple validation, signer authority, funding reservation, provider credit gate | knowgrph-payments R14 |
| Bounded discovery, card issuance mechanics, merchant checkout execution | knowgrph-payments R15, R16, R17 |
| Payment record document, parser, printer, round-trip contract | knowgrph-payments R7 |
...
| TCO-zero | Runs on the existing `knowgrph-payment` Worker, its D1 binding, and browser-local cache. No new store, proxy tier, or default metered egress. | R1, R6, R9 |
...
Reused unchanged from knowgrph-payments: **Payment_Trust_Boundary**, **Payment_Client**, **Payment_Intent_Record**,
...
owned by knowgrph-payments R14 and does not replace it.
...
Operator TTV excludes provider account approval and Data API plan approval, which are outside Knowgrph control and
...
3. THE Payment_Trust_Boundary SHALL be the only Knowgrph component that sends a Chain_Evidence_Adapter request to an external source, and no Chain_Evidence_Adapter implementation SHALL be reachable from Payment_Client bundle output.
...
3. WHILE the projection does not report Evidence_State `chain_confirmed` with a reported agreement and an evidence freshness label of `fresh`, THE Discovery, Issuance, and Execution gates owned by knowgrph-payments R15, R16, and R17 SHALL remain closed.
...
5. THE Funding_Verification_Projection SHALL grant no approval, and spend authorization SHALL remain with the existing Approval_Gate owned by knowgrph-payments R16.
...
6. THE Readiness_Gate SHALL report the upstream blocked funding gates owned by knowgrph-payments R14 with their existing blocked status.
...
- Rail selection, credential custody, the Paywall surface, card issuance mechanics, and merchant checkout, owned by
  knowgrph-payments R1, R2, R13, R16, and R17.
...
|---|---|---|
| Existing `knowgrph-payment` Worker and its D1 binding | Zero-TCO (existing free-tier binding) | Already the Payment_Trust_Boundary; reuse avoids a new tier and keeps secret custody in one place. |
...
| Existing browser-local Knowgrph storage owner | FOSS / platform | Zero egress while offline; no new service and no second store. |
...
Ids continue the shared `OQ-N` space owned by `.kiro/specs/knowgrph-payments/requirements.md` and its companion
...
1. The existing `knowgrph-payment` Worker remains the only server-side payment trust boundary; this increment adds a read path inside it rather than a new tier.
2. knowgrph-payments R13 through R17 remain the owners of the Paywall, funding tuple validation, discovery, issuance, and execution; this increment supplies evidence and a projection they consume.
