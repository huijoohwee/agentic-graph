---
title: "agentic-graph Payments - Requirements"
doc_type: "Requirements"
id: "agentic-graph-payments-requirements"
spec: "agentic-graph-payments"
version: "0.4.0"
...
agentic-graph needs one payments capability with two payment rails: Stripe for card and
...
- **Payment_Trust_Boundary**: The server-side context that holds provider credentials and alone may call provider APIs, realized by the existing `agentic-graph-payment` Cloudflare Worker.
- **Payment_Client**: The browser-first, local-first agentic-graph canvas surface; holds no payment credential.
...
- **Payment_Intent_Record**: The agentic-graph-owned requested-payment record containing local identity, minor-unit amount, currency, selected Rail, intent state, and Client_Intent_Key; distinct from any provider object.
...
- **Payment_Surface**: The buyer-facing agentic-graph surface that starts a payment and displays payment state.
...
|---|---|---|---|---|
| JB-Trigger | Buyer decides to unlock a paid agentic-graph capability | Payment_Surface | Unclear which currency and method apply | Rail chosen from locale and currency without asking |
...
Operator TTV excludes provider account approval time, which is outside agentic-graph
...
| MCP Gateway-ready | Yes | The hosted Stripe MCP server at `https://mcp.stripe.com` SHALL be federated as one external transport alongside existing agentic-graph transports ([Stripe MCP](https://docs.stripe.com/mcp)). No new monolithic proxy tier SHALL be introduced. |
...
1. THE Payment_Trust_Boundary SHALL be the only agentic-graph component that sends a
...
keeps card data out of agentic-graph. TCO-zero - no added infrastructure. Token
...
5. IF Stripe returns an error of type `idempotency_error` because retried parameters
   ...
   [Idempotent requests](https://docs.stripe.com/api/idempotent_requests)).
6. THE Stripe_Rail SHALL store the agentic-graph Payment_Intent_Record identifier on the
...
than integrating a bank directly. TCO-zero - no added agentic-graph infrastructure; fees
...
10. THE Agent_Payment_Surface SHALL add no new proxy tier and SHALL reuse the existing
   agentic-graph MCP transports and the existing Approval_Gate owner.
...
1. THE Payment_Trust_Boundary SHALL map every Stripe failure into a typed agentic-graph
...
3. THE Payment_Trust_Boundary SHALL map every StraitsX failure into a typed agentic-graph
...
1. THE Payment_Trust_Boundary SHALL store no card number, card verification value,
   or full bank account number in any agentic-graph store.
...
2. A token contract address SHALL never be accepted as the destination deposit
   address, and agentic-graph SHALL never store a private key, seed phrase, or raw signed
   transaction.
...
6. Cancellation or failure before authorization SHALL release the unused local
   ...
   account; agentic-graph SHALL create no automatic return transfer.
...
- agentic-graph custody of buyer funds, a agentic-graph-operated wallet, or a agentic-graph-operated
  exchange.
...
- **OQ-12 - Intent_Queue durability medium (resolved).** The queue extends the
  existing browser-local agentic-graph storage owner with an explicit maximum depth of
...
- **OQ-19 - Secure card credential broker.** Select an approved provider-hosted or
  PCI-scoped injection mechanism that keeps PAN, CVV, and expiry outside agentic-graph
