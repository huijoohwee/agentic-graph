# Design: agentic-graph VideoDB API Reference

## Overview

This spec produces two Dev artifacts:

`docs/documents/agentic-graph-api-reference/agentic-graph-videodb-api-reference.md`

`$GITHUB_ROOT/huijoohwee/docs/agentic-graph-strybldr-demo.md`

The reference document is the codebase map and runtime-ready reference for the VideoDB integration in agentic-graph's `MainPanel Integrations` surface. The TypeScript SSOT lives at `canvas/src/features/integrations/videodbSsot.ts`; the CLI generator remains planned at `canvas/src/cli/generate-videodb-reference.ts`. The demo document is a runnable local renderer-neutral workspace that exercises the agentic-graph brief -> generated video -> indexed review -> publish packet path without storing credentials, IDs, job IDs, stream URLs, download URLs, or publish URLs.

## Architecture

The document follows the established agentic-graph API reference architecture:
...
Runnable Dev Demo
  └── $GITHUB_ROOT/huijoohwee/docs/agentic-graph-strybldr-demo.md
      ...
      ├── flow nodes reference the API table keys and local agentic-graph handoff keys
...
| field | value |
| --- | --- |
| `title` | `"agentic-graph — VideoDB API Reference (SSOT + Codebase Map)"` |
| `doc_type` | `"api-reference"` |
| `version` | `"0.1.0"` |
| `status` | `"runtime-ready"` |
| `created` | ISO 8601 date |
| `updated` | ISO 8601 date |
| `author` | `"agentic-graph"` |
| `domain` | `"integrations"` |
| `lang` | `"en"` |
| `frontmatter_contract` | `"agentic-graph-api-reference-v1"` |
...
| `related` | sequence: `agentic-graph-byteplus-openark-video-generation-api-reference.md`, `agentic-graph-integrations-ssot-sync-directives.md` |
...
**Tag format**: `Feature: agentic-graph-videodb-api-reference, Property {N}: {property_text}`
...
- The `json strybldr-storyboard` payload parses and contains the same agentic-graph-native workflow beats
