---
title: "agentic-graph Visual Annotation Engine — Requirements"
id: "spec:agentic-graph-visual-annotation-engine"
...
traceability:
  ...
  spec_path: ".kiro/specs/agentic-graph-visual-annotation-engine/requirements.md"
...
`agentic-graph-visual-annotation-engine` embeds image and video annotation directly into the
agentic-graph canvas. Users drop an image or video asset onto the canvas (or select an existing
...
(`agentic-graph.annotate.image` and `agentic-graph.annotate.video_frame`) so agents can invoke it
...
- **Visual_Annotation_Engine**: The agentic-graph feature that accepts an image or video-frame
...
2. THE Visual_Annotation_Engine SHALL resolve the active model identifier from the
   `AGENTIC_OS_ANNOTATION_MODEL` environment variable at invocation time; it SHALL NOT cache
...
3. WHEN a `modelHint` that is present and contains at least one non-whitespace character is
   ...
   model identifier, overriding `AGENTIC_OS_ANNOTATION_MODEL`; IF the hint value does not
...
4. IF `AGENTIC_OS_ANNOTATION_MODEL` is absent, empty, or its value does not equal any value
...
8. IF `modelHint` is present but consists entirely of whitespace characters, THE
   ...
   `AGENTIC_OS_ANNOTATION_MODEL` resolution as per criterion 4.
...
4. WHEN a valid Annotation_Spec is received by the Annotation_Worker and inference
   ...
   (positive integer), and `schemaVersion: "agentic-graph-annotation/v1"`.
...
on the agentic-graph canvas so that I can see labels, bounding boxes, and captions alongside
...
**User Story:** As a coding agent using MCP, I want `agentic-graph.annotate.image` and
`agentic-graph.annotate.video_frame` tools so that I can trigger annotation jobs programmatically
...
1. THE MCP_Annotate_Image_Tool SHALL be registered in `buildAgenticGraphLocalMcpToolDefinitions()`
   in `mcp/local-tool-contract.js` under the name `agentic-graph.annotate.image`, following the
   same registration pattern as `agentic-graph.html_video.render`.
...
3. THE MCP_Annotate_Video_Frame_Tool SHALL be registered under the name
   `agentic-graph.annotate.video_frame` with the same required fields as the image tool plus
...
4. THE MCP tool output schema for both tools SHALL be a JSON object with required fields
   ...
   `"agentic-graph-annotation/v1"` on success), and `tasks` (object); plus optional `error`
...
7. THE tool names `agentic-graph.annotate.image` and `agentic-graph.annotate.video_frame` SHALL be
   added to `AGENTIC_OS_LOCAL_MCP_TOOL_NAMES` in `agentic-graph-vdeoxpln-contract.mjs` so that
...
1. THE Visual_Annotation_Engine SHALL define an `annotationEngineSsot` module that exports:
   ...
   the `AGENTIC_OS_ANNOTATION_MODEL` string constant with value `"AGENTIC_OS_ANNOTATION_MODEL"`
...
`agentic-graph-visual-annotation-engine` entry in the skill registry so that the router can
...
1. THE Visual_Annotation_Engine SHALL have an entry in `RAW_VDEOXPLN` in
   `canvas/src/features/agent-ready/agentic-graph-vdeoxpln-contract.mjs` with the required keys:
   `id` (`"agentic-graph-visual-annotation-engine"`), `title`, `purpose`, `scope`, `mutation`,
...
2. THE vdeoxpln entry SHALL declare `scope: "browser-local"`,
   `mutation: "local-approval-gated"`, include `agentic-graph.annotate.image` and
   `agentic-graph.annotate.video_frame` in its `tools.local` array, and set
...
1. THE Visual_Annotation_Engine SHALL read all configurable values from environment
   ...
   `AGENTIC_OS_ANNOTATION_MODEL`, inference backend preference from
   `AGENTIC_OS_ANNOTATION_BACKEND` (accepted values: `"webgpu"`, `"wasm"`), and
   cache-storage key prefix from `AGENTIC_OS_ANNOTATION_CACHE_PREFIX`; no configurable
