---
title: "AgenticGraph Visual Annotation Engine — Design"
id: "spec:agenticgraph-visual-annotation-engine:design"
...
`agenticgraph-visual-annotation-engine` embeds in-browser image annotation directly into the
agenticgraph canvas. Users supply an image URL (or select an existing canvas node); the engine
...
| `annotationMcpTools.ts` | MCP tool handlers for `agenticgraph.annotate.image` and `agenticgraph.annotate.video_frame` | Surface |
...
| `canvas/src/features/agent-ready/agenticgraphVdeoxplnContract.mjs` | Adds annotation tool names + vdeoxpln entry (additive) | Registry |
...
flowchart TD
    ...
    subgraph ModelResolution["Model Resolution\nannotationOrchestrator.ts"]
        C1["resolveAnnotationModel()\nreads AGENTICGRAPH_ANNOTATION_MODEL\nor modelHint"]
        C2{model registered?}
...
flowchart LR
    subgraph SSOT["annotationEngineSsot.ts (contract only)"]
        ...
        S3["AGENTICGRAPH_ANNOTATION_MODEL\n(env-var name constant)"]
    ...
    subgraph VContract["agenticgraphVdeoxplnContract.mjs (additive)"]
        V1["AGENTICGRAPH_LOCAL_MCP_TOOL_NAMES.annotateImage\n= 'agenticgraph.annotate.image'"]
        V2["AGENTICGRAPH_LOCAL_MCP_TOOL_NAMES.annotateVideoFrame\n= 'agenticgraph.annotate.video_frame'"]
        V3["AGENTICGRAPH_VDEOXPLN_IDS.visualAnnotationEngine\n= 'agenticgraph-visual-annotation-engine'"]
    end

    subgraph MCPContract["mcp/local-tool-contract.js (additive)"]
        M1["buildAgenticGraphLocalMcpToolDefinitions()\n→ includes agenticgraph.annotate.image\n  + agenticgraph.annotate.video_frame"]
    end
...
export type AnnotationSpec = {
  ...
  modelHint?:       string          // max 255 chars; overrides AGENTICGRAPH_ANNOTATION_MODEL
...
export type AnnotationResult = {
  ...
  schemaVersion: 'agenticgraph-annotation/v1'
...
- Backend selection: read `AGENTICGRAPH_ANNOTATION_BACKEND` env hint first; else probe
 ...
 * Priority: non-whitespace modelHint > AGENTICGRAPH_ANNOTATION_MODEL env var > florence2Base default.
...
/** Handler for agenticgraph.annotate.image MCP tool invocation. */
...
/** Handler for agenticgraph.annotate.video_frame MCP tool invocation. */
...
**`canvas/src/features/agent-ready/agenticgraphVdeoxplnContract.mjs`** — additive entries:

```javascript
// 1. Add to AGENTICGRAPH_LOCAL_MCP_TOOL_NAMES (additive):
annotateImage:      "agenticgraph.annotate.image",
annotateVideoFrame: "agenticgraph.annotate.video_frame",

// 2. Add to AGENTICGRAPH_VDEOXPLN_IDS (additive):
visualAnnotationEngine: "agenticgraph-visual-annotation-engine",
```
...
type AnnotationSpec = {
  ...
  // Optional
  modelHint?:        string   // max 255 chars; overrides AGENTICGRAPH_ANNOTATION_MODEL when non-whitespace
  frameTimestampMs?: number   // integer ≥ 0; required when assetType === 'video_frame'
}
```
...
```javascript
{
  id: AGENTICGRAPH_VDEOXPLN_IDS.visualAnnotationEngine,  // "agenticgraph-visual-annotation-engine"
  title: "AgenticGraph Visual Annotation Engine",
  ...
  tools: {
    ...
    browserLocal: [],
    local: [
      AGENTICGRAPH_LOCAL_MCP_TOOL_NAMES.annotateImage,
      AGENTICGRAPH_LOCAL_MCP_TOOL_NAMES.annotateVideoFrame,
      AGENTICGRAPH_LOCAL_MCP_TOOL_NAMES.vdeoxplnList,
    ],
  },
  workflow: [
    "Validate the Annotation_Spec before any model resolution or inference.",
    "Resolve model identifier from modelHint, AGENTICGRAPH_ANNOTATION_MODEL, or florence2Base default.",
...
- `result.schemaVersion === "agenticgraph-annotation/v1"`
- `result.durationMs > 0`
...
2. `AGENTICGRAPH_ANNOTATION_MODEL` environment variable read at invocation time — fallback.
...
Tag format: `// Feature: agenticgraph-visual-annotation-engine, Property N: <property_text>`
...
- `mcp/local-tool-contract.js`: `buildAgenticGraphLocalMcpToolDefinitions()` result contains
  `agenticgraph.annotate.image` and `agenticgraph.annotate.video_frame`
- `agenticgraphVdeoxplnContract.mjs`: `validateAgenticGraphVdeoxplnRegistry()` returns `{ ok: true, errors: [] }`
