...
2. A token contract address SHALL never be accepted as the destination deposit
   address, and Knowgrph SHALL never store a private key, seed phrase, or raw signed
   transaction.
...
6. Cancellation or failure before authorization SHALL release the unused local
   ...
   account; Knowgrph SHALL create no automatic return transfer.
...
- **OQ-12 - Intent_Queue durability medium (resolved).** The queue extends the
  existing browser-local Knowgrph storage owner with an explicit maximum depth of
...
- **OQ-19 - Secure card credential broker.** Select an approved provider-hosted or
  PCI-scoped injection mechanism that keeps PAN, CVV, and expiry outside Knowgrph
