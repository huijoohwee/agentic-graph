---
title: "agentic-graph HTML Video Renderer — Requirements"
id: "spec:agentic-graph-html-video-renderer"
...
traceability:
  ...
  spec_path: ".kiro/specs/agentic-graph-html-video-renderer/requirements.md"
...
(`agentic-graph-html-video-renderer`), and an MCP tool (`agentic-graph.html_video.render`). All owners,
...
- **Html_Video_Renderer**: The agentic-graph feature that accepts a Render_Spec and produces an MP4
...
- **Render_Engine**: A stateless, pluggable adapter that accepts a Render_Spec and returns a
  ...
  `AGENTIC_OS_HTML_VIDEO_ENGINE` at runtime; the resolver never falls through to a hardcoded
  engine name.
...
- **Engine_Resolver**: The pure function that reads `AGENTIC_OS_HTML_VIDEO_ENGINE` (or
...
- **MCP_Html_Video_Tool**: The local MCP tool registered as `agentic-graph.html_video.render` through
  `buildAgenticGraphLocalMcpToolDefinitions()`. Follows the memory-layer and showrunner tool
  registration patterns.
- **Source_Files**: The agentic-graph workspace document store used for Render_Manifest and artifact
...
2. THE Engine_Resolver SHALL read the active engine identifier from the
   `AGENTIC_OS_HTML_VIDEO_ENGINE` environment variable at invocation time; it SHALL NOT cache the
   value across invocations.
3. WHEN `AGENTIC_OS_HTML_VIDEO_ENGINE` is set to a registered engine identifier, THE
   Engine_Resolver SHALL return an object `{ ok: true, engine: <Render_Engine adapter> }`
   without error.
4. IF `AGENTIC_OS_HTML_VIDEO_ENGINE` is absent, empty, or resolves to an engine identifier not
...
5. WHEN a Render_Spec carries a non-empty `engineHint` and the caller explicitly provides it as
   ...
   hint value as the engine identifier and ignore `AGENTIC_OS_HTML_VIDEO_ENGINE`; IF the hint
...
**User Story:** As a coding agent using MCP, I want a `agentic-graph.html_video.render` tool, so that
...
1. THE MCP_Html_Video_Tool SHALL be registered in `buildAgenticGraphLocalMcpToolDefinitions()` in
   `mcp/local-tool-contract.js` under the name `agentic-graph.html_video.render`, following the same
...
6. IF the Engine_Resolver returns a structured error, THEN THE MCP_Html_Video_Tool SHALL return
   ...
   Render_Engine.
7. THE MCP_Html_Video_Tool name `agentic-graph.html_video.render` SHALL be added to
   `AGENTIC_OS_LOCAL_MCP_TOOL_NAMES` in `agentic-graph-vdeoxpln-contract.mjs` so that the vdeoxpln
...
**User Story:** As an AI agent using vdeoxpln routing, I want a `agentic-graph-html-video-renderer`
...
1. THE Html_Video_Renderer SHALL have an entry in `RAW_VDEOXPLN` in
   `canvas/src/features/agent-ready/agentic-graph-vdeoxpln-contract.mjs` with the following required
   keys: `id` (`"agentic-graph-html-video-renderer"`), `title`, `purpose`, `scope`, `mutation`,
   `triggers`, `inputs`, `outputs`, `owners`, `tools`, `workflow`, `aiPolicy`, `artifactPolicy`,
   and `validation` — matching the object structure of `agentic-graph-memory-layer` and
   `agentic-graph-ai-showrunner` entries.
2. THE vdeoxpln entry SHALL declare `scope: "local-stdio-and-browser-local"`,
   `mutation: "local-approval-gated"`, include `agentic-graph.html_video.render` in its
...
1. THE Html_Video_Renderer SHALL define a `htmlVideoRendererSsot` module (following the
   ...
   the string constant `AGENTIC_OS_HTML_VIDEO_ENGINE` (value `"AGENTIC_OS_HTML_VIDEO_ENGINE"`),
...
4. IF the `AGENTIC_OS_HTML_VIDEO_ENGINE` environment variable is absent or empty at invocation
