# Design Document: AgenticGraph Native Flight Simulator

## Overview

The AgenticGraph Native Flight Simulator (`Flight_Sim`) is a browser-local, single-player, deterministic flight mission that mounts as a `FloatingPanel` mode over the composed Geo+XR surface: the existing native MapLibre view renders the geospatial world plus georeferenced Flight route, waypoint, and aircraft layers, while the one existing transparent React Three Fiber XR Canvas retains simulation, input, and readiness without painting Flight or XR world geometry. The shared DOM HUD remains above both. It composes existing repository owners — the AgenticGraph renderers, the native Agentic ECS, browser-local WorkspaceFs, and the shared camera catalog — with a small cluster of new, in-repo flight owners: an in-repo `Flight_Model`, an in-repo AABB `Collision_Resolver`, an `Asset_Loader` over `img2threejs`-style diffable specs, a strict `Invocation_Parser`, two browser-local WebMCP tools, and a Decisions-only persistence path.
...
- **Strict invocation and bounded MCP surface.** Exactly `/flight.sim @canvas #flight`; browser-local tools `agenticgraph.inspect_local_flight_sim` and `agenticgraph.control_local_flight_sim`; the private Agentic ECS stdio lane stays at exactly three tools.
...
This repository-tracked design is part of the normative `.kiro/specs/agenticgraph-game-flight-sim` source of truth. It grounds those constraints in the tracked Agentic ECS contract (`docs/documents/agenticgraph-agentic-entity-component-system-prd-tad.md`), the canonical `contracts/cost-log.schema.js`, and the source module cluster under `canvas/src/features/game-flight-sim/`. The Flight Sim PRD/TAD (`docs/documents/agenticgraph-game-flight-sim-prd-tad.md`) and workspace seed (`docs/workspace-seeds/agenticgraph-game-flight-sim-demo.md`) are derived implementation/proof projections; any workspace-root Kiro copy is a byte-identical local projection only.
...
interface FlightSimAircraftAssetSpec {
  schema: "agenticgraph.img2threejs-scene/v1"; id: "vehicle-airplane";
...
Each property MUST be implemented as a single property-based test running at least 100 iterations, tagged in the form `Feature: agenticgraph-game-flight-sim, Property <number> - <property text>`.
...
*For any* Flight_Sim state, invoking `agenticgraph.inspect_local_flight_sim` returns read-only state (or an error result when state is unavailable) and leaves all mission state unchanged.
...
- Each test is tagged with a comment in the form `Feature: agenticgraph-game-flight-sim, Property <number> - <property text>`.
