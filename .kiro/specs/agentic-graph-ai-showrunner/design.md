---
title: "agentic-graph AI Showrunner — Technical Design"
id: "spec:agentic-graph-ai-showrunner:design"
...
# agentic-graph AI Showrunner — Technical Design

## Overview

The **AI Showrunner** is a multi-agent creative pipeline orchestration layer for agentic-graph.
...
graph TD
  subgraph "MCP Surface — mcp/server.js"
    MCP_START["agentic-graph.showrunner.start_run"]
    MCP_STATUS["agentic-graph.showrunner.run_status"]
    MCP_CHOICE["agentic-graph.showrunner.post_choice"]
    MCP_CRITIQUE["agentic-graph.showrunner.submit_critique"]
    MCP_APPROVE["agentic-graph.showrunner.approve_stage"]
    MCP_ARTIFACT["agentic-graph.showrunner.get_artifact"]
  end
  ...
  subgraph "Showrunner Core — canvas/src/features/ai-showrunner/"
    ...
    SCRIPT_SCHEMA["scriptSchema.ts\nScript parse/print + agentic-graph-script/v1"]
  ...
  subgraph "Existing Owners — REUSED, NOT MODIFIED"
    ...
    SUPERAGENT["agentic_graph_parser/superagent_harness.py\nSuperAgent harness"]
  end
...
| File | Responsibility |
|---|---|
| `briefParser.ts` | Parse / print `ShowrunnerBriefSpec` ↔ `agentic-graph-showrunner-brief/v1` Markdown |
...
| `scriptSchema.ts` | Parse / print `Script` ↔ `agentic-graph-script/v1` Markdown |
...
The names are then added to `AGENTIC_OS_LOCAL_MCP_TOOL_NAMES` in
`canvas/src/features/agent-ready/agentic-graph-vdeoxpln-contract.mjs` — no new keys invented,
...
export const SHOWRUNNER_WIDGET_ENTRY: WidgetRegistryEntry = {
  ...
  widgetTypeId: "agentic-graph-showrunner",
...
A new entry `AGENTIC_OS_VDEOXPLN_IDS.aiShowrunner = "agentic-graph-ai-showrunner"` is added to
`agentic-graph-vdeoxpln-contract.mjs` following the `memoryLayer` pattern. The entry declares
...
export type ShowrunnerBriefSpec = {
  schema: "agentic-graph-showrunner-brief/v1"
...
export interface IBriefParser {
  /** Parse a agentic-graph-showrunner-brief/v1 Markdown document into ShowrunnerBriefSpec.
  ...
  /** Format a ShowrunnerBriefSpec back into a agentic-graph-showrunner-brief/v1 document.
...
The orchestrator calls `agentic-graph.memory.add` and `agentic-graph.memory.search` before and after each
...
### Vdeoxpln Registry — `canvas/src/features/agent-ready/agentic-graph-vdeoxpln-contract.mjs`

A single new `RAW_VDEOXPLN` entry `agentic-graph-ai-showrunner` is appended. Its `tools.local`
...
sequenceDiagram
  ...
  participant MCP as agentic-graph.showrunner.start_run
  participant Orch as ShowrunnerOrchestrator
  participant Mem as agentic-graph.memory.*
  ...
  Scriptwriter-->>Orch: Script (agentic-graph-script/v1)
...
sequenceDiagram
  ...
  participant MCP_Choice as agentic-graph.showrunner.post_choice
  participant Orch as ShowrunnerOrchestrator
...
`agentic-graph.memory.assemble_prompt` token-estimation contract from the memory layer.
...
`showrunnerVdeoxpln.ts` exports the `agentic-graph-ai-showrunner` entry. The `vdeoxpln:check` script
(`scripts/check-vdeoxpln.mjs`) validates it automatically when the entry is added to `RAW_VDEOXPLN`
in `agentic-graph-vdeoxpln-contract.mjs`. No extra registration step is needed.
...
```markdown
---
schema: agentic-graph-showrunner-failure-report/v1
...
If `agentic-graph.memory.search` returns an empty result, the orchestrator proceeds with the base
...
```typescript
// Feature: agentic-graph-ai-showrunner, Property 1: Brief Round-Trip
...
- [ ] `npm run vdeoxpln:check` — `agentic-graph-ai-showrunner` present in registry
