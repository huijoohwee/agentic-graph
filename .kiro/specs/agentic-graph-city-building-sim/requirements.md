...
This file is the normative requirements source of truth for the agentic-graph City
...
be source-authored for agentic-graph. The feature may use only existing
...
2. The feature shall add zero runtime dependencies and shall use the existing
   agentic-graph MapLibre Geo, Flight Geo overlay, FloatingPanel, MCP, and
   WorkspaceFs owners.
...
1. Schema `agentic-graph-city-sim-mcp/v1` shall expose exactly
   `agentic-graph.inspect_local_city_sim` and
   `agentic-graph.control_local_city_sim`.
