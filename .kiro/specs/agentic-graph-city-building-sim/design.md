...
interface CitySnapshot {
  schemaId: "agentic-graph-city-grid/v1";
  cityName: string;
  tick: number;
...
```markdown
---
schema_id: agentic-graph-city-grid/v1
...
MCP schema `agentic-graph-city-sim-mcp/v1` registers exactly:

- `agentic-graph.inspect_local_city_sim`: immutable snapshot;
- `agentic-graph.control_local_city_sim`: approval-gated structured operation.
