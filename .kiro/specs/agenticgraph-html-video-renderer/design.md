---
title: "AgenticGraph HTML Video Renderer — Design"
id: "spec:agenticgraph-html-video-renderer:design"
...
resolves an active `Render_Engine` at runtime via the `AGENTICGRAPH_HTML_VIDEO_ENGINE` environment
...
**Design principles:**
- Runtime-only engine selection — `AGENTICGRAPH_HTML_VIDEO_ENGINE` read at invocation, never cached
...
| `mcp/local-tool-contract.js` | Registers `agenticgraph.html_video.render` tool (existing file, additive) | MCP |
| `canvas/src/features/agent-ready/agenticgraphVdeoxplnContract.mjs` | Adds `htmlVideoRender` to `AGENTICGRAPH_LOCAL_MCP_TOOL_NAMES` + vdeoxpln entry (existing file, additive) | Registry |
...
flowchart LR
    subgraph SSOT["htmlVideoRendererSsot.ts (contract only)"]
        ...
        S2["AGENTICGRAPH_HTML_VIDEO_ENGINE\n(string constant = env var name)"]
 ...
 * 2. Otherwise reads process.env[AGENTICGRAPH_HTML_VIDEO_ENGINE] at call time (never cached).
...
**`canvas/src/features/agent-ready/agenticgraphVdeoxplnContract.mjs`** — additive changes:
...
htmlVideoRenderer: "agenticgraph-html-video-renderer",
```
...
withLocalMcpDescriptorDefaults({
  ...
  description:
    ...
    "real MP4 video blob through the AgenticGraph HTML Video Renderer pipeline.",
...
{
  id: "agenticgraph-html-video-renderer",
  title: "AgenticGraph HTML Video Renderer",
  ...
  tools: {
    published: [],
    browserLocal: [],
    local: [AGENTICGRAPH_LOCAL_MCP_TOOL_NAMES.htmlVideoRender, AGENTICGRAPH_LOCAL_MCP_TOOL_NAMES.vdeoxplnList],
  },
  workflow: [
...
2. `process.env[AGENTICGRAPH_HTML_VIDEO_ENGINE]` read at invocation time — fallback
...
- `buildAgenticGraphLocalMcpToolDefinitions()` returns an entry with name `"agenticgraph.html_video.render"`
...
```typescript
// Feature: agenticgraph-html-video-renderer, Property 1: Render_Spec validator round-trip
...
// Feature: agenticgraph-html-video-renderer, Property 2: Engine swap preserves metadata
...
// Feature: agenticgraph-html-video-renderer, Property 3: Semantic key idempotence
...
// Feature: agenticgraph-html-video-renderer, Property 4: Error conditions → structured errors
...
// Feature: agenticgraph-html-video-renderer, Property 5: Round-trip preserves spec and kind
