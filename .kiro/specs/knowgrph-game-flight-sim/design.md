...
The Knowgrph Native Flight Simulator (`Flight_Sim`) is a browser-local, single-player, offline, deterministic flight mission that mounts as a `FloatingPanel` mode over the composed Geo+XR Canvas surface: the existing Geo renderer as the visible background and the one existing transparent React Three Fiber XR Canvas as the mission/input layer. It composes existing repository owners — the Knowgrph renderers, the native Agentic ECS, browser-local WorkspaceFs, and the shared camera catalog — with a small cluster of new, in-repo flight owners: an in-repo `Flight_Model`, an in-repo AABB `Collision_Resolver`, an `Asset_Loader` over `img2threejs`-style diffable specs, a strict `Invocation_Parser`, two browser-local WebMCP tools, and a Decisions-only persistence path.
...
- **Shared composed Canvas surface.** One local Geo background plus one transparent existing R3F XR Canvas and one camera/input authority; `Camera_Source` offers Fixed_Follow and Free_Orbit; `Motion_Control` is optional input only, never the flight policy.
...
| Rendering | Shared surface policy over the existing R3F Canvas (`ThreeGraph.impl.tsx`) and Geo renderer | Geo background below one transparent XR mission layer + shared HUD |
...
`Flight_Runtime` projects aircraft and navigation on the composed Geo+XR surface. The existing Geo renderer remains visible as the background while the one existing R3F Canvas becomes transparent above it and retains authored subjects, lights, the mission actors, camera, and physics; duplicate XR atmosphere and terrain are suppressed only in this composition (R14.1, R14.2, R14.7). Plain Geo remains a separate exclusive interactive surface with the existing DOM/SVG Flight projection and no mounted R3F Canvas (R14.6). Flight never introduces a second Canvas, alternate world, or camera owner. The existing Media environment cards stage through XR scene control before the shared FloatingPanel bridge opens Geo; Geo projects that same runtime selection, and the source-authored Flight document selects Geo+XR in the same handoff without replacing the Geo panel, deriving its spatial profile and World label without a map-provider or network owner (R25). The combined Geo+XR bridge explicitly selects the Geo host's local-only policy: committed inline basemap geometry renders the selected 2D or 3D Classic/Modern presentation while MapLibre remains unstarted, so 3D Classic and 3D Modern stay visible below Flight without a remote style or tile request. Standalone Geo retains its existing provider-backed path (R25.8). Graph-metadata source-text writes enter the shared Workspace seed-sync task owner, whose explicit settlement barrier also flushes and awaits the persisted workspace layer's debounced local host-mirror tail. Flight entry crosses that barrier before installing its gameplay network fence, so the mirror's local `/__kg_fs_write` cannot outlive the source write and be misclassified as gameplay transport. In Geo+XR, Flight/XR owns pointer input while Geo remains the visible background; City and FPS gameplay retain their established 3D precedence, and the selected environment remains intact (R25.7). One shared Flight surface-control hook owns desktop, touch, gamepad, and Motion Control sampling. The XR mission stage invalidates its demand loop and acknowledges preparation after its committed R3F render; the exclusive Geo overlay acknowledges preparation after its committed DOM render. Both are independent of desktop input ownership, so Geo and live Motion Control handoffs cannot miss a request created during the same React transition or reject valid input paths (R25.5). On exit Flight restores the previous surface controller and simulation state; entry or restoration failure leaves the existing Canvas and prior controller unchanged and surfaces a local error (R14.3–R14.5). `Camera_Source` supplies a pure follow/framing descriptor consumed by the shared controller hook; Fixed_Follow is the default, Free_Orbit is selectable, and Timeline camera-marks temporarily take framing ownership before returning to the last selected option (R15). The Cockpit descriptor derives its eye clearance from the admitted aircraft collision envelope, placing it forward of the nose and above the airframe while keeping its look target on the aircraft forward vector, so the procedural aircraft cannot occlude the playable forward view (R25.6). The HUD consumes the shared FloatingPanel width policy and reserves that panel's default right-side footprint for instrumentation and interactive controls. Its root stays pointer-transparent and retains the established layer required for mobile flight controls to remain above bottom choreography surfaces, so Motion Control and Flight actions do not intercept one another while preserving the shared Canvas and normalized pose-input path (R26).
...
- Shared-Canvas and environment ownership: selecting a Media terrain kit stages it before Geo opens; a source-authored Flight document immediately selects Geo+XR with visible Geo below one transparent retained R3F Canvas, no competing Canvas, and the matching selected spatial profile. Plain Geo remains separately interactive; the composed XR path retains authored subjects and Cockpit framing outside the admitted aircraft collision envelope while suppressing duplicate atmosphere and terrain (R14, R25).
...
- Repository-owned runtime-readiness command (source authority, native ECS, focused tests, TypeScript, production build) and browser-smoke command (Source Files apply, visible Geo below exactly one transparent XR Canvas, playable input, strict WebMCP, lifecycle, Timeline camera round-trip, mobile HUD), both local-only with zero paid model/image-to-3D/Cloudflare/network and no automatic Git or deployment (R22.1, R22.3, R22.5, R22.6).
