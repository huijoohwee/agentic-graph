...
- Shared-Canvas and environment ownership: selecting a Media terrain kit stages it before Geo opens, Geo projects the same selection, and a source-authored Flight document immediately re-enters Flight_Sim without replacing the Geo panel while keeping that authored atmosphere/terrain/scene graph mounted in exactly one Canvas/renderer/camera owner with matching spatial profile, overlay actors, and HUD; Cockpit framing remains outside the admitted aircraft collision envelope with an unobstructed forward view (R14.1, R14.2, R25).
