...
5. WHEN Flight_Sim entry follows Geo inspection or a live Motion_Control handoff, THE mission stage SHALL acknowledge the current preparation request after its committed render even if the stage mounted before that request began, so a mount-order race cannot leave Flight controls disabled.
