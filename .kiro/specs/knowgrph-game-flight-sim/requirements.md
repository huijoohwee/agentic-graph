...
3. WHEN FloatingPanel Geo opens after a successful environment selection, THE Geo surface SHALL visibly project the selected environment identifier, label, kind, and authored dimensions from the shared XR runtime; any source-text write produced by staging SHALL remain under shared Workspace seed-sync ownership and SHALL NOT execute through the Flight gameplay network fence; and IF the active document declares the registered Flight run-ready identity, THEN Flight_Runtime SHALL reactivate the aircraft overlay without replacing the Geo panel.
