...
5. WHEN Flight_Sim entry follows Geo inspection or a live Motion_Control handoff, THE mission stage SHALL wait for and reclaim shared viewport input ownership, invalidate the shared Canvas demand loop, and acknowledge the current preparation request after its committed render even if the stage mounted before that request began, so a mount-order race cannot leave Flight controls disabled.
