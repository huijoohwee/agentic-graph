---
title: "agentic-graph SME Care Agent — Runbook"
...
- typed `agentic-graph-sme-profile/v1` input and `agentic-graph-sme-risk-run/v1` output;
...
```text
$GITHUB_ROOT/agentic-graph-desktop-sme-protection-gap
```

Run commands from that directory unless a command says otherwise:

```bash
cd $GITHUB_ROOT/agentic-graph-desktop-sme-protection-gap
```
...
3. Paste this absolute local path:
   ...
   ```text
   $GITHUB_ROOT/agentic-graph-desktop-sme-protection-gap/sme-agent/demo/sme-care-agent-canvas-evidence.md
   ```
...
The executable local tool is `agentic-graph.superagent.run`. For `/sme-care-agent`, the shared runtime detects `runtimeKernel: "sme.risk.profile"` and invokes the specialized deterministic kernel instead of the generic Python SuperAgent harness.

Start the local stdio MCP server:

```bash
AGENTIC_OS_ROOT="$(pwd)" \
AGENTIC_OS_PYTHON="${AGENTIC_OS_PYTHON:-./.venv/bin/python}" \
node ./mcp/server.js
```

Configure that command in a stdio-capable MCP client, then call `agentic-graph.superagent.run` with:
...
The input must be Markdown with byte-zero YAML frontmatter using `agentic-graph-sme-profile/v1`:
...
```bash
AGENTIC_OS_AGENTIC_CANVAS_OS_DOCS_ROOT=$GITHUB_ROOT/agentic-canvas-os/docs \
...
```bash
export AGENTIC_OS_AGENTIC_CANVAS_OS_DOCS_ROOT=$GITHUB_ROOT/agentic-canvas-os/docs
```
