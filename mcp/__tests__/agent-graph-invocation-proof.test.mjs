...
async function createCanonicalDocsFixture(root) {
  ...
  await Promise.all([
    ...
    fs.writeFile(path.join(docsRoot, "DICTIONARY-COMMAND.md"), dictionary(
      ACTION,
      `| \`${ACTION}\` | Fixture route. | \`${BINDING}\` | \`${SEMANTIC}\` | \`${AGENT_GRAPH_TOOL_NAMES.parserGenerate}\` |`,
    )),
...
test("agent-graph host verifies one exact invocation against the authoritative docs catalog", async (t) => {
  const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-kg-invocation-"));
  ...
  const invocation = {
    ...
    tool: AGENT_GRAPH_TOOL_NAMES.parserGenerate,
    action: ACTION,
    semantics: [SEMANTIC],
    bindings: [BINDING],
  ...
  const accepted = await runAgentGraphTool(
    AGENT_GRAPH_TOOL_NAMES.parserGenerate,
    { descriptors: SOURCE_PARSER_DESCRIPTORS, invocation },
    { rootDir: repositoryRoot, env },
  );
  assert.equal(accepted.ok, true, JSON.stringify(accepted));

  const forged = await runAgentGraphTool(
    AGENT_GRAPH_TOOL_NAMES.parserGenerate,
    {
