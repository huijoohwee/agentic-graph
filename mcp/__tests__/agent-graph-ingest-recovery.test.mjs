...
import { pythonRuntimeGrammarValidationSource } from "../agent-graph/python-syntax-recovery.mjs";
import {
...
async function sourceShard(outputRoot, result, sourcePath) {
  const snapshot = await readAgentGraphSnapshot(pointerPath(outputRoot, result.graphId), {
  ...
  return readAgentGraphSourceShard(snapshot, source);
}

test("discovery keeps invalid UTF-8 NUL payloads out of text parser routes", async (t) => {
  const rootPath = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-text-classification-"));
...
test("embedded-NUL text admission validates UTF-8 beyond the binary sniff prefix", async (t) => {
  const rootPath = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-text-classification-late-invalid-"));
  t.after(() => fs.rm(rootPath, { recursive: true, force: true }));
...
test("strict ingest accepts textual NULs and recovers newer Python grammar locally", async (t) => {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-ingest-recovery-"));
