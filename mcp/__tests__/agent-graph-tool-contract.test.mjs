...
test("local MCP exposes one deterministic agent-graph tool family", () => {
  ...
  const ingest = byName.get(AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.agentGraphIngest);
  const parserGenerate = byName.get(AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.agentGraphParserGenerate);
  const query = byName.get(AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.agentGraphQuery);
  const explain = byName.get(AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.agentGraphExplainEdge);
...
test("tool descriptions and invocation proof schemas keep aliases source-backed and zero-vector", () => {
  ...
  assert.equal(AGENT_GRAPH_INVOCATION_SCHEMA_ID, "agentic-graph-agent-graph-invocation/v1");
  assert.equal(AGENTIC_CANVAS_OS_ROUTING_SCHEMA_ID, "agentic-canvas-os-docs-routing/v1");
  assert.equal(AGENT_GRAPH_PARSER_REGISTRY_SCHEMA_ID, "agentic-graph-agent-graph-parser-registry/v2");
...
test("schemas require digest fencing, source-backed invocation proofs, and typed error details", () => {
  ...
  assert.equal(
    validateParserGenerate({ profile: AGENT_GRAPH_DEFAULT_PARSER_PROFILE }),
    true,
  ...
  const validInput = {
    ...
    mode: "summary",
    invocation: invocationProof(AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.agentGraphQuery),
  };
  ...
  assert.equal(validateInput({
    ...validInput,
    invocation: { ...validInput.invocation, tool: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.agentGraphIngest },
  }), false);
  assert.equal(validateInput({
    ...validInput,
  ...
  assert.equal(validateOutput({
    schema: "agentic-graph-agent-graph-query/v1",
    ok: false,
    operation: "query",
...
test("agent-graph docs retain the complete local tool catalog", () => {
  const documents = [
    "README.md",
    "mcp/README.md",
