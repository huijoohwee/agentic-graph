...
} from "../knowledge-graph/repository-acquisition.mjs";
...
test("host private-network capability is distinct from its repository provider allowlist", async (t) => {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "knowgrph-repository-host-policy-"));
  t.after(() => fs.rm(rootDir, { recursive: true, force: true }));
  const baseEnv = {
    PATH: String(process.env.PATH || ""),
    HOME: String(process.env.HOME || ""),
    KNOWGRPH_KNOWLEDGE_GRAPH_REPOSITORY_HOSTS: "127.0.0.1",
    KNOWGRPH_KNOWLEDGE_GRAPH_OUTPUT_ROOT: "blocked-output",
  };
  const args = {
  ...
  const blocked = await runKnowledgeGraphTool(
    KNOWLEDGE_GRAPH_TOOL_NAMES.ingest,
    args,
    { rootDir, env: baseEnv },
  );
  ...
  const optedIn = await runKnowledgeGraphTool(
    KNOWLEDGE_GRAPH_TOOL_NAMES.ingest,
    args,
    {
      rootDir,
      env: {
        ...baseEnv,
        KNOWGRPH_KNOWLEDGE_GRAPH_OUTPUT_ROOT: "opted-in-output",
        KNOWGRPH_KNOWLEDGE_GRAPH_ALLOW_PRIVATE_REPOSITORY_NETWORK: "1",
      },
    },
  );
