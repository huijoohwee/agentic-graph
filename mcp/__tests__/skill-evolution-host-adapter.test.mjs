...
const envFor = async (file, requestedDigest) => ({
  AGENTIC_OS_SKILL_EVOLUTION_ADAPTER_MODULE: path.relative(repoRoot, file),
  AGENTIC_OS_SKILL_EVOLUTION_ADAPTER_SHA256: requestedDigest ?? await fileDigest(file),
});
...
test("fails closed for missing configuration, digest drift, and external paths", async () => {
  ...
  const outside = createSkillEvolutionHostAdapter({
    rootDir: repoRoot,
    env: {
      AGENTIC_OS_SKILL_EVOLUTION_ADAPTER_MODULE: process.execPath,
      AGENTIC_OS_SKILL_EVOLUTION_ADAPTER_SHA256: "0".repeat(64),
    },
  });
