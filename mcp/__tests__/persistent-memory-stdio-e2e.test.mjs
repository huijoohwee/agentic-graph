...
async function connectClient(stateDirectory) {
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      KNOWGRPH_MEMORY_STATE_DIR: stateDirectory,
      KNOWGRPH_MEMORY_APPROVAL_HMAC_KEY: hostSecret,
...
test("stdio MCP executes an exact ACOS tuple and recalls it after process restart", {
  ...
  const stateDirectory = await fs.mkdtemp(path.join(os.tmpdir(), "knowgrph-persistent-memory-e2e-"));
