...
const createLocalMcpClient = () => {
  const client = new Client({
    name: 'agentic-graph-storage-sync-stdio-e2e',
    version: '0.0.0',
  })
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      AGENTIC_OS_MCP_TIMEOUT_MS: '10000',
    },
    stderr: 'pipe',
  })
  let stderrText = ''
...
test('local stdio lists and calls browser-only storage tools as typed handoffs', async () => {
  ...
  try {
    ...
    const gitResult = await client.callTool({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.gitRun,
      arguments: {
        operation: 'fetch',
        remoteId: 'origin',
    ...
    assert.deepEqual(gitResult.structuredContent, {
      ...
      requiredTool: 'agentic-graph.control_local_git_repository',
      invocation: {
        operation: 'fetch',
        remoteId: 'origin',
        canonicalPathScope: 'agentic-graph/docs',
    ...
    const fileSyncResult = await client.callTool({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.fileSyncRun,
      arguments: {
    ...
    assert.equal(fileSyncResult.structuredContent?.requiredTool, 'agentic-graph.control_local_file_sync')
