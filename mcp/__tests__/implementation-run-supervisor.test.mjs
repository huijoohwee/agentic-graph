...
test("no-worktree cancel succeeds while host registries are unavailable", async (t) => {
  ...
  const token = crypto.randomUUID();
  state = await own(fx.runtime.store, state, token);
  const unavailableEnv = { ...fx.env, AGENTIC_OS_IMPLEMENTATION_ACOS_ROOT: "/missing/acos", AGENTIC_OS_IMPLEMENTATION_RUNNERS_JSON: "not-json", AGENTIC_OS_IMPLEMENTATION_VERIFIERS_JSON: "not-json", AGENTIC_OS_IMPLEMENTATION_REPOSITORIES_JSON: "not-json" };
...
test("active pause parks after execution authority is revoked and replays a lost park response", async (t) => {
  const fx = await fixture(t);
  let state = fx.created.state;
  const { lease, pullRequest } = await provisionLane(fx, state, `agentic-graph-${state.runId}`, "revoked-pause");
  ...
  const revokedEnv = { ...fx.env, AGENTIC_OS_IMPLEMENTATION_RUNNERS_JSON: "{}", AGENTIC_OS_IMPLEMENTATION_VERIFIERS_JSON: "{}" };
...
test("partial provision retries start against the existing task worktree without provisioning again", async (t) => {
  ...
  let state = fx.created.state;
  const token = crypto.randomUUID();
  const sessionId = `agentic-graph-${state.runId}`;
...
`;
  ...
  const artifact = await fs.readFile(path.join(state.spec.repoRoot, ".agentic-graph-workspace", "implementation-runs", state.runId, evidence[0].outputArtifact), "utf8");
