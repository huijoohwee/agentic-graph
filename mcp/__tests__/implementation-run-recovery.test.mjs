...
const plan = { schema: "agentic-graph-implementation-run-plan/v1" };
...
test("recovery relaunches only eligible dead supervisors and ignores idle states", async (t) => {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-recovery-"));
...
test("recovery isolates a corrupt run and still recovers healthy runs", async (t) => {
  ...
  const corrupt = path.join(rootDir, ".agentic-graph-workspace", "implementation-runs", "ir_aaaaaaaaaaaaaaaaaaaaaaaa");
...
test("aggregate listing is byte-bounded, cursor-complete, and isolates oversized state", async (t) => {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-list-page-"));
  t.after(() => fs.rm(rootDir, { recursive: true, force: true }));
  ...
  const oversizedDir = path.join(rootDir, ".agentic-graph-workspace", "implementation-runs", oversizedId);
...
test("detached supervisor receives only its runner allowlist and lifecycle auth", async (t) => {
  ...
  const env = {
    ...
    AGENTIC_OS_IMPLEMENTATION_ACOS_ROOT: rootDir,
    AGENTIC_OS_IMPLEMENTATION_REPOSITORIES_JSON: "[]",
    AGENTIC_OS_IMPLEMENTATION_VERIFIERS_JSON: "{}",
  ...
  const created = await runtime.store.create({
    ...
    plan: { schema: "agentic-graph-implementation-run-plan/v1", runner: { executable: process.execPath, args: ["{{requestPath}}"], environment: ["RUNNER_A_SECRET"] }, verifiers: [] },
  });
