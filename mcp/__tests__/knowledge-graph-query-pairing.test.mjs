...
test("common-token limits keep config-search nodes paired with supporting edges and citations", async (t) => {
  ...
  const materialized = queryKnowledgeGraph({
    ...fragment,
  ...
  const outputRoot = await fs.mkdtemp(path.join(os.tmpdir(), "knowgrph-query-pairing-"));
  ...
  const snapshotResult = await queryKnowledgeGraphSnapshot(snapshot, {
    mode: "search",
    query: "common",
    limit: 2,
  });
