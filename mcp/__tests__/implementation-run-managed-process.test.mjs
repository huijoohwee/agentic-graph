...
test("executable proof rejects a same-inode content overwrite", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-proof-"));
...
test("oversized process spawn input is rejected before a child or artifact exists", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-spawn-bound-"));
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  const runtime = createImplementationRunRuntime({ rootDir: root, recoveryIntervalMs: 0 });
  const created = await runtime.store.create({ spec: { idempotencyKey: "spawn-bound-key", bounds: { maxAttempts: 2 }, workItem: { id: "spawn" } }, plan: { schema: "agentic-graph-implementation-run-plan/v1" } });
  let spawned = 0;
...
test("recovery proves a live managed runner group stopped before relaunch", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-orphan-"));
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  ...
  const created = await runtime.store.create({
    ...
    plan: { schema: "agentic-graph-implementation-run-plan/v1" },
  });
...
test("SIGTERM during the durable starting gate cannot launch an untracked command", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-start-gate-"));
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  ...
  const proof = await executableProof(process.execPath);
  const runtime = createImplementationRunRuntime({ rootDir: root, recoveryIntervalMs: 0 });
  const created = await runtime.store.create({ spec: { idempotencyKey: "starting-gate-key", bounds: { maxAttempts: 2 }, workItem: { id: "gate" } }, plan: { schema: "agentic-graph-implementation-run-plan/v1" } });
...
test("mixed stdout and stderr remain within one durable output bound", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-output-bound-"));
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  ...
  const proof = await executableProof(process.execPath);
  const runtime = createImplementationRunRuntime({ rootDir: root, recoveryIntervalMs: 0 });
  const created = await runtime.store.create({ spec: { idempotencyKey: "output-bound-key", bounds: { maxAttempts: 2 }, workItem: { id: "output" } }, plan: { schema: "agentic-graph-implementation-run-plan/v1" } });
...
test("manager exit is observed before authorization and close drains trailing output", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-exit-window-"));
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  const runtime = createImplementationRunRuntime({ rootDir: root, recoveryIntervalMs: 0 });
  const created = await runtime.store.create({ spec: { idempotencyKey: "exit-window-key", bounds: { maxAttempts: 2 }, workItem: { id: "exit" } }, plan: { schema: "agentic-graph-implementation-run-plan/v1" } });
...
test("manager exit with inherited open pipes fails on the bounded stdio drain grace", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-drain-timeout-"));
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  const runtime = createImplementationRunRuntime({ rootDir: root, recoveryIntervalMs: 0 });
  const created = await runtime.store.create({ spec: { idempotencyKey: "drain-timeout-key", bounds: { maxAttempts: 2 }, workItem: { id: "drain" } }, plan: { schema: "agentic-graph-implementation-run-plan/v1" } });
...
test("signal-fast command returns without losing the managed CLI exit", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-signal-exit-"));
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  const command = path.join(root, "signal.mjs");
  await fs.writeFile(command, "process.kill(process.pid, 'SIGTERM');\n");
  const runtime = createImplementationRunRuntime({ rootDir: root, recoveryIntervalMs: 0 });
  const created = await runtime.store.create({ spec: { idempotencyKey: "signal-exit-key", bounds: { maxAttempts: 2 }, workItem: { id: "signal" } }, plan: { schema: "agentic-graph-implementation-run-plan/v1" } });
...
test("managed CLI terminates a SIGTERM-ignoring descendant in the command process group", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "agentic-graph-process-group-"));
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  ...
  const runtime = createImplementationRunRuntime({ rootDir: root, recoveryIntervalMs: 0 });
  const created = await runtime.store.create({ spec: { idempotencyKey: "process-group-key", bounds: { maxAttempts: 2 }, workItem: { id: "group" } }, plan: { schema: "agentic-graph-implementation-run-plan/v1" } });
