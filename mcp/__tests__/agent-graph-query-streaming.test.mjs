...
test("path, neighbors, and impact stream repositories above 250,000 nodes", async () => {
  ...
  const path = await queryAgentGraphSnapshot(snapshot, {
    mode: "path",
    from: startNode.id,
    to: targetNode.id,
    direction: "outgoing",
  ...
  const neighbors = await queryAgentGraphSnapshot(snapshot, {
    mode: "neighbors",
    nodeId: startNode.id,
    direction: "outgoing",
    edgeLabels: ["links"],
  ...
  const impact = await queryAgentGraphSnapshot(snapshot, {
    mode: "impact",
...
test("streamed shortest-path tie breaking is stable across shard edge order", async () => {
  ...
  const query = () => queryAgentGraphSnapshot({
    ...snapshot,
    manifest: {
      ...snapshot.manifest,
...
test("streamed path traversal reports a typed state limit on high fanout", async () => {
  ...
  const result = await queryAgentGraphSnapshot(boundedSnapshot, {
    mode: "path",
    from: startNode.id,
