...
test("local stdio MCP lists voice studio, replays dry-run, and blocks unconfigured live execution", async () => {
  const client = new Client({ name: "agentic-graph-voice-studio-e2e", version: "0.0.0" });
  ...
  try {
    ...
    const tool = listed.tools.find((entry) => entry.name === AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.voiceStudio);
    ...
    const first = await client.callTool({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.voiceStudio,
      arguments: input,
