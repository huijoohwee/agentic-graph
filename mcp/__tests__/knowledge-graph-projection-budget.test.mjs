...
const CONTEXT = "knowgrph-knowledge-graph-projection";
...
test("snapshot projection deterministically fits byte-bounded records with endpoint closure", async () => {
  ...
  const forward = await projectKnowledgeGraphSnapshot(snapshot, 10, {
    projectionByteLimit,
    iterateSnapshotShards: iterableSnapshotShards(nodes, edges),
  });
  const reverse = await projectKnowledgeGraphSnapshot(snapshot, 10, {
    projectionByteLimit,
