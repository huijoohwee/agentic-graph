...
import {
  AGENT_GRAPH_DEFAULT_PARSER_PROFILE,
  AGENT_GRAPH_DECLARATIVE_GRAMMAR_SCHEMA_ID,
} from "../agent-graph-parser-contract.js";
import {
  EVIDENCE_FIELDS,
  AGENT_GRAPH_CANONICAL_NODE_OUTPUT_REVISION,
  sha256,
} from "../agent-graph/contract.mjs";
import {
  compileDeclarativeGrammar,
} from "../agent-graph/declarative-grammar-parser.mjs";
import { compileParserDispatch, compileParserRegistry } from "../agent-graph/parser-generator.mjs";
...
} from "../agent-graph/source-parser-registry.mjs";

const declarativeGrammar = () => ({
  schema: AGENT_GRAPH_DECLARATIVE_GRAMMAR_SCHEMA_ID,
  start: "document",
  tokens: [
...
test("public parser generator returns only a digest-bound inert native registry", () => {
  const builtIn = generateAgentGraphParser({
  ...
  const generated = generateAgentGraphParser({ descriptors });
  ...
  const bothSelections = generateAgentGraphParser({
    profile: AGENT_GRAPH_DEFAULT_PARSER_PROFILE,
    descriptors,
  });
  ...
  const unsupportedProfile = generateAgentGraphParser({ profile: "other-source" });
  assert.equal(unsupportedProfile.ok, false);
  assert.equal(unsupportedProfile.error.code, "parser_profile_unsupported");

  const unsupported = generateAgentGraphParser({
    descriptors: [{
      id: "unsupported",
      kind: "unsupported",
      adapter: "not-native",
...
test("generated declarative grammar ingests unknown syntax as a deterministic explained AST", async (t) => {
  ...
  const generated = runtime.generateAgentGraphParser({
    descriptors: [declarativeDescriptor()],
  });
  assert.equal(generated.ok, true, JSON.stringify(generated));
  assert.equal(generated.parserRegistry.descriptors[0].grammar.schema, AGENT_GRAPH_DECLARATIVE_GRAMMAR_SCHEMA_ID);
  ...
  for (const edge of edges) {
    ...
    assert.match(
      ...
      new RegExp(`^1\\.0\\.0\\+${AGENT_GRAPH_CANONICAL_NODE_OUTPUT_REVISION}\\+grammar-[a-f0-9]{16}$`),
    );
  ...
  const regenerated = generateAgentGraphParser({
...
test("declarative grammar rejects ambiguity, left recursion, unsafe fields, and hard-bound overrun", () => {
  ...
  const ambiguousResult = generateAgentGraphParser({
  ...
  const leftRecursiveResult = generateAgentGraphParser({
  ...
  const unsafeResult = generateAgentGraphParser({
  ...
  const nullablePrefixResult = generateAgentGraphParser({
  ...
  const boundedResult = generateAgentGraphParser({
