...
- Snapshots use bounded content-addressed source, deterministically chunked repository-resolution, index, and manifest shards behind one atomic current pointer. No repository-wide resolution payload bypasses the per-object ceiling; an oversized single record fails with `artifact_too_large` before pointer replacement, preserving the prior ready snapshot.
