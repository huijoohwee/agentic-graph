export const AGENTIC_SDLC_OBSERVABILITY_TOOL_NAME = "knowgrph.agentic_sdlc.observe";
...
export const AGENTIC_SDLC_OBSERVATION_SCHEMA = "knowgrph-agentic-sdlc-observation/v1";
...
const CONFORMANCE_SCHEMA = Object.freeze({
  ...
  properties: {
    schema: { const: "knowgrph-agentic-sdlc-conformance-summary/v1" },
    runId: NONEMPTY_TEXT,
    valid: { const: true },
    runtimeReady: { type: "boolean" },
...
export function buildAgenticSdlcObservabilityToolDefinitions({ toolNames, withDefaults }) {
  ...
  const definition = {
    ...
    description:
      "Use this read-only local tool to project one exact revision- and ledger-digest-fenced agentic-sdlc-run/v1 into deterministic paged Knowgrph Canvas evidence without model calls, network calls, deployment, or a second graph store.",
