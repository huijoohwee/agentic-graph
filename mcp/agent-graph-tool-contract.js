import {
  AGENT_GRAPH_PARSER_GENERATE_INPUT_SCHEMA,
  AGENT_GRAPH_PARSER_REGISTRY_SCHEMA,
} from "./agent-graph-parser-contract.js";
export {
  AGENT_GRAPH_PARSER_REGISTRY_SCHEMA_ID,
  NATIVE_AGENT_GRAPH_PARSER_ADAPTER_IDENTITIES,
} from "./agent-graph-parser-contract.js";

export const AGENT_GRAPH_INVOCATION_SCHEMA_ID = "agentic-graph-agent-graph-invocation/v1";
...
const TOOL_BY_OPERATION = Object.freeze({
  ...
  explain: "agentic-graph.agent_graph.explain_edge",
});
...
const invocationSchema = (tool) => ({
  ...
  properties: {
    schema: { const: AGENT_GRAPH_INVOCATION_SCHEMA_ID },
    tool: tool
      ? { const: tool }
...
export const AGENT_GRAPH_INVOCATION_PROOF_SCHEMA = Object.freeze(invocationSchema());
...
const commonOutputSchema = (operation) => ({
  ...
  properties: {
    schema: { type: "string", pattern: "^agentic-graph-agent-graph(?:-[a-z-]+)?/v[0-9]+$" },
    ok: { type: "boolean" },
    operation: { const: operation },
    graphId: GRAPH_ID_SCHEMA,
    snapshotDigest: SNAPSHOT_DIGEST_SCHEMA,
    parserRegistryDigest: SNAPSHOT_DIGEST_SCHEMA,
    parserRegistry: AGENT_GRAPH_PARSER_REGISTRY_SCHEMA,
    error: {
      type: "object",
      additionalProperties: false,
...
const PARSER_GENERATE_INPUT_SCHEMA = Object.freeze({
  ...AGENT_GRAPH_PARSER_GENERATE_INPUT_SCHEMA,
  properties: {
    ...AGENT_GRAPH_PARSER_GENERATE_INPUT_SCHEMA.properties,
...
export const AGENT_GRAPH_INPUT_SCHEMAS = Object.freeze({
...
export function buildAgentGraphToolDefinitions({ toolNames, withDefaults, readOnlyAnnotations, processAnnotations }) {
  return [
    withDefaults({
      name: toolNames.agentGraphParserGenerate,
    ...
    withDefaults({
      name: toolNames.agentGraphIngest,
    ...
    withDefaults({
      name: toolNames.agentGraphQuery,
    ...
    withDefaults({
      name: toolNames.agentGraphExplainEdge,
