...
import {
  KNOWLEDGE_GRAPH_SOURCE_BUNDLE_SCHEMA,
  KNOWLEDGE_GRAPH_SOURCE_PART_SCHEMA,
...
export async function writeKnowledgeGraphSourceShard(pointerPath, source, fragment, options = {}) {
  ...
  const makePart = (kind) => (records, ordinal, totalParts) => ({
    schema: KNOWLEDGE_GRAPH_SOURCE_PART_SCHEMA,
    kind,
    ...identity,
    ordinal,
    totalParts,
  ...
  const bundle = {
    schema: KNOWLEDGE_GRAPH_SOURCE_BUNDLE_SCHEMA,
    ...identity,
    diagnostics,
...
export async function writeKnowledgeGraphSnapshotAtomic(pointerPath, {
  ...
  const storeRoot = knowledgeGraphStoreRoot(pointerPath);
  for (const entry of sourceEntries) {
    validateSourceEntryStorage(entry, KNOWLEDGE_GRAPH_REPOSITORY_INDEX_SCHEMA);
  }
...
function validateSourceEntryStorage(entry, repositoryIndexSchema) {
  const bundled = repositoryIndexSchema === KNOWLEDGE_GRAPH_REPOSITORY_INDEX_SCHEMA;
  if (bundled) {
...
export async function readKnowledgeGraphSourceBundle(snapshot, sourceEntry) {
  validateSourceEntryStorage(sourceEntry, KNOWLEDGE_GRAPH_REPOSITORY_INDEX_SCHEMA);
  ...
  const identityMatches = bundle.schema === KNOWLEDGE_GRAPH_SOURCE_BUNDLE_SCHEMA
...
function validateLegacySourceShard(shard, entry) {
  if (shard.schema !== KNOWLEDGE_GRAPH_SOURCE_SHARD_SCHEMA
    ...
    throw new KnowledgeGraphError("source_shard_invalid", `Source shard is invalid: ${entry.sourcePath}`);
  }
  const nodeIds = new Set(shard.nodes.map((node) => node?.id));
  if (nodeIds.size !== shard.nodes.length) {
    throw new KnowledgeGraphError("source_shard_invalid", `Source shard has duplicate node ids: ${entry.sourcePath}`);
  }
...
export async function* readKnowledgeGraphSourceParts(snapshot, sourceEntry) {
  if (hasOwn(sourceEntry, "shardDigest")) {
    validateSourceEntryStorage(sourceEntry, KNOWLEDGE_GRAPH_REPOSITORY_INDEX_SCHEMA_V2);
  ...
  const bundle = await readKnowledgeGraphSourceBundle(snapshot, sourceEntry);
  const sequence = [
  ...
  const readNext = async () => {
    ...
    if (part.schema !== KNOWLEDGE_GRAPH_SOURCE_PART_SCHEMA
      || part.kind !== kind
...
export async function readKnowledgeGraphSourceShard(snapshot, sourceEntry) {
  if (hasOwn(sourceEntry, "shardDigest")) {
    const iterator = readKnowledgeGraphSourceParts(snapshot, sourceEntry);
  ...
  const nodes = [];
  const edges = [];
  for await (const part of readKnowledgeGraphSourceParts(snapshot, sourceEntry)) {
...
export async function* readKnowledgeGraphResolutionShards(snapshot, repositoryIndex) {
  const sharded = repositoryIndex.schema === KNOWLEDGE_GRAPH_REPOSITORY_INDEX_SCHEMA
    || repositoryIndex.schema === KNOWLEDGE_GRAPH_REPOSITORY_INDEX_SCHEMA_V2;
