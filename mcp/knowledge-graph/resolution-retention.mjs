...
export function retainResolutionFragment(budget, sourcePath, fragment) {
  ...
  let sourceBytes;
  try {
    sourceBytes = computeKnowledgeGraphArtifactDigestBounded(fragment, remainingBytes).byteLength;
  } catch (error) {
...
export function retainResolutionRecord(budget, sourcePath, record, details = {}) {
  ...
  let recordBytes;
  try {
    recordBytes = computeKnowledgeGraphArtifactDigestBounded(record, remainingBytes).byteLength;
  } catch (error) {
    if (error?.code !== "artifact_too_large") throw error;
    throw retentionError(
      "resolution_byte_limit_exceeded",
