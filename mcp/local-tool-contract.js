...
import { AGENTIC_OS_MEMORY_LAYER_MCP_TOOL_NAMES, MEMORY_ADD_INPUT_SCHEMA, MEMORY_ADD_OUTPUT_SCHEMA, PROCEDURAL_MEMORY_EXTRACT_INPUT_SCHEMA, PROCEDURAL_MEMORY_EXTRACT_OUTPUT_SCHEMA, PROMPT_ASSEMBLER_INPUT_SCHEMA, PROMPT_ASSEMBLER_OUTPUT_SCHEMA, USER_MODEL_MATERIALIZE_INPUT_SCHEMA, USER_MODEL_MATERIALIZE_OUTPUT_SCHEMA } from "../canvas/src/features/memory/aiAgentsMemoryLayerContract.mjs";
...
export const buildAgenticGraphLocalMcpToolDefinitions = (args = {}) => {
  ...
  return [
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.uiLaunch,
      description:
        "Use this when a local MCP host needs to launch the agentic-graph Canvas UI (Vite dev server) and return a URL pre-configured for Canvas / Workspace Editor / Geospatial mode.",
      inputSchema: {
        type: "object",
        additionalProperties: false,
        properties: {
          ...
          host: {
            type: "string",
            description: `Host to bind and open. Default: ${defaultUiHost} (or AGENTIC_OS_UI_HOST).`,
          },
          port: {
            type: "number",
            description: `Port for the dev server. Default: ${String(defaultUiPort)} (or AGENTIC_OS_UI_PORT).`,
          },
          waitForReady: {
            type: "boolean",
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.uiStop,
      description: "Use this when a local MCP host needs to stop the running agentic-graph Canvas dev server if it was started by agentic-graph.ui.launch.",
      inputSchema: {
        type: "object",
        additionalProperties: false,
        properties: {},
    ...
    withLocalMcpDescriptorDefaults(
      buildGeospatialLayerToolDefinition(AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.geospatialCommand),
      LOCAL_IDEMPOTENT_PROCESS_TOOL_ANNOTATIONS,
    ),
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.pipeline,
      description:
        "Use this when a local MCP host needs to run the agentic-graph pipeline (GraphData -> A0 CSV/JSON-LD + codebase index artifacts).",
      inputSchema: {
        type: "object",
        additionalProperties: false,
        properties: {
          ...
          outputDir: {
            type: "string",
            description:
              "Directory for outputs. If omitted, defaults to agentic_graph_parser's configured output directory.",
          },
          timeoutMs: {
            type: "number",
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.graphragPipeline,
      description:
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.videoRemixRun,
      description:
    ...
    withLocalMcpDescriptorDefaults({ name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.sealionDetectLanguageVariant, description: "Use this when a local MCP host needs SEA-LION sidecar language, regional variant, register, and code-switching detection before routing Southeast Asian language work.", outputSchema: SEALION_TOOL_OUTPUT_SCHEMA, inputSchema: SEALION_TEXT_INPUT_SCHEMA }, LOCAL_IDEMPOTENT_PROCESS_TOOL_ANNOTATIONS),
    withLocalMcpDescriptorDefaults({ name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.sealionTranslateLocalize, description: "Use this when a local MCP host needs SEA-LION sidecar translation plus Southeast Asian localization notes from the hosted API.", outputSchema: SEALION_TOOL_OUTPUT_SCHEMA, inputSchema: SEALION_TRANSLATE_INPUT_SCHEMA }, LOCAL_IDEMPOTENT_PROCESS_TOOL_ANNOTATIONS),
    withLocalMcpDescriptorDefaults({ name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.sealionSafetyCheck, description: "Use this when a local MCP host needs SEA-LION sidecar advisory SEA-Guard safety classification for Southeast Asian language or culture-sensitive content.", outputSchema: SEALION_TOOL_OUTPUT_SCHEMA, inputSchema: SEALION_SAFETY_INPUT_SCHEMA }, LOCAL_IDEMPOTENT_PROCESS_TOOL_ANNOTATIONS),
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.htmlVideoRender,
      description:
        "Use this when a local MCP host needs to render an HTML + CSS + data document into an MP4 video artifact through the agentic-graph HTML Video Renderer pipeline.",
      outputSchema: HTML_VIDEO_RENDER_OUTPUT_SCHEMA,
      inputSchema: HTML_VIDEO_RENDER_INPUT_SCHEMA,
    }, LOCAL_PROCESS_TOOL_ANNOTATIONS),
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.annotateImage,
      description:
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.annotateVideoFrame,
      description:
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_MEMORY_LAYER_MCP_TOOL_NAMES.add,
      description:
        "Use this when a local MCP host needs to persist an explicitly scoped user, agent, run, or app memory through the agentic-graph memory harness without storing credentials or hardcoded IDs.",
      outputSchema: MEMORY_ADD_OUTPUT_SCHEMA,
      inputSchema: MEMORY_ADD_INPUT_SCHEMA,
    }, LOCAL_PROCESS_TOOL_ANNOTATIONS),
    ...buildPersistentMemoryToolDefinitions({
      toolNames: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES,
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_MEMORY_LAYER_MCP_TOOL_NAMES.assemblePrompt,
      description:
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_MEMORY_LAYER_MCP_TOOL_NAMES.materializeUserModel,
      description:
    ...
    withLocalMcpDescriptorDefaults({
      ...
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.agenticCanvasOsDocsInvoke,
    }, READ_ONLY_TOOL_ANNOTATIONS),
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.showrunnerStartRun,
      description:
        "Use this when a local MCP host needs to validate a agentic-graph AI Showrunner Creative_Brief and start a bounded dry-run or approval-gated Pipeline_Run.",
      outputSchema: SHOWRUNNER_TOOL_OUTPUT_SCHEMA,
      inputSchema: {
        type: "object",
        additionalProperties: false,
        properties: {
          brief_path: { type: "string", description: "Workspace Source_File path for a agentic-graph-showrunner-brief/v1 document." },
          brief_markdown: { type: "string", description: "Inline agentic-graph-showrunner-brief/v1 Markdown." },
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.showrunnerRunStatus,
      description:
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.showrunnerPostChoice,
      description:
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.showrunnerSubmitCritique,
      description:
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.showrunnerApproveStage,
      description:
    ...
    withLocalMcpDescriptorDefaults({
      name: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.showrunnerGetArtifact,
      description:
    ...
    ...buildAgentSandboxPolicyToolDefinitions({ toolNames: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES, withDefaults: withLocalMcpDescriptorDefaults, readOnlyAnnotations: READ_ONLY_TOOL_ANNOTATIONS }),
    ...
    withLocalMcpDescriptorDefaults(
      buildVdeoxplnLocalToolDefinition(AGENTIC_OS_LOCAL_MCP_TOOL_NAMES.vdeoxplnList),
      READ_ONLY_TOOL_ANNOTATIONS,
    ),
    ...buildAgentApplicationToolDefinitions({ toolNames: AGENTIC_OS_LOCAL_MCP_TOOL_NAMES, withDefaults: withLocalMcpDescriptorDefaults }),
