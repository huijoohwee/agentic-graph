...
export const LOCAL_AGENT_TEAM_ADAPTER_ID = "agentic-graph.agent-team.local-ollama/v1";
...
const systemMessage = (actor) => [
  "You are executing one text-only branch inside the local agentic-graph Agent Team runtime.",
...
export function createLocalAgentTeamModelAdapter({
  ...
  const config = readLocalOllamaConfig(env, {
    envPrefix: "AGENTIC_OS_AGENT_TEAM_MODEL",
    defaultTimeoutMs: 25_000,
    maximumTimeoutMs: 28_000,
  });
  const outputTokens = Math.max(
    128,
    Math.min(4_096, Math.floor(Number(env.AGENTIC_OS_AGENT_TEAM_MODEL_MAX_OUTPUT_TOKENS) || DEFAULT_OUTPUT_TOKENS)),
  );
  ...
  return Object.freeze({
    ...
    readiness: Object.freeze({
      ...
      loopbackOnly: !String(env.AGENTIC_OS_AGENT_TEAM_MODEL_ALLOW_REMOTE || "").trim().match(/^1$/),
    }),
