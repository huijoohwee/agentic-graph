...
export class ImplementationRunStore {
  constructor({
    ...
    this.baseDir = path.join(this.rootDir, ".agentic-graph-workspace", "implementation-runs");
    this.now = now;
  ...
  async assertBaseDirectory() {
    ...
    for (const directory of [path.join(this.rootDir, ".agentic-graph-workspace"), this.baseDir]) {
