...
export const APPLICATION_COMPONENT_PACK_SCHEMA_ID = "agentic-graph.application-component-pack/v1";
export const APPLICATION_COMPONENT_PACK_POLICY_SCHEMA_ID = "agentic-graph.application-component-pack-policy/v1";

const PACK_ID = /^[a-z0-9]+(?:[._-][a-z0-9]+)*$/;
const EXACT_REVISION = /^[0-9]+\.[0-9]+\.[0-9]+$/;
const ID_SEGMENT = "[a-z0-9]+(?:[._-][a-z0-9]+)*";
const SOURCE_URI = new RegExp(`^(?:workspace:/${ID_SEGMENT}(?:/${ID_SEGMENT})*|kgdoc:${ID_SEGMENT}(?:/${ID_SEGMENT})*|urn:agentic-graph:(?!(?:http|https|file|ftp|ws|wss)(?::|$))${ID_SEGMENT}(?::${ID_SEGMENT})*)$`);
