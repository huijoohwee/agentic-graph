...
async function readGitignorePolicy(repositoryRootPath, rootPath, repositoryPath, options = {}) {
  ...
  try {
    ...
    if (error instanceof AgentGraphError && error.details?.causeCode === "ENOENT") {
      return { digest: sha256(""), rules: [] };
    }
    if (error instanceof AgentGraphError && error.code.startsWith("ignore_policy_")) throw error;
    throw new AgentGraphError(
...
export async function hydrateKnowledgeSource(source, {
  ...
  checkAgentGraphBudget({ abortSignal, deadline, stage: "source-hydration" });
  ...
  if (!opened.bytes || opened.stat.size !== source.byteSize || sha256(opened.bytes) !== source.contentHash) {
    throw new AgentGraphError("source_changed_after_discovery", `Source changed after admission: ${source.relativePath}`, {
...
export async function resolveRealDirectory(rootPathRaw, options = {}) {
  checkAgentGraphBudget({ ...options, stage: options.stage || "directory-resolution" });
  if (!String(rootPathRaw || "").trim()) throw new AgentGraphError("root_path_required", "rootPath is required.");
  const resolved = path.resolve(String(rootPathRaw));
  let real;
  try {
    real = await fs.realpath(resolved);
  } catch {
    throw new AgentGraphError("root_not_found", `Knowledge graph root does not exist: ${resolved}`);
  }
  checkAgentGraphBudget({ ...options, stage: options.stage || "directory-resolution" });
  const stat = await fs.stat(real);
  checkAgentGraphBudget({ ...options, stage: options.stage || "directory-resolution" });
  if (!stat.isDirectory()) throw new AgentGraphError("root_not_directory", `Knowledge graph root is not a directory: ${resolved}`);
  return real;
}
...
export async function discoverKnowledgeSources(args) {
  ...
  const deadline = args.deadline || createAgentGraphDeadline(args.maxDurationMs, { now: args.now });
  ...
  async function walk(
    ...
    for (const entry of entries) {
      ...
      if (counts.filesVisited > maxFiles) {
        throw new AgentGraphError("max_files_exceeded", `File count exceeds configured maximum ${maxFiles}.`, {
          maxFiles,
          counts: { ...counts },
          complete: false,
        });
      ...
      if (counts.bytesVisited > maxTotalBytes) {
        throw new AgentGraphError("max_total_bytes_exceeded", `Discovered files exceed configured total byte maximum ${maxTotalBytes}.`, {
          maxTotalBytes,
          counts: { ...counts },
          complete: false,
        });
