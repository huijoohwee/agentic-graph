...
export const PYTHON_PARSER_VERSION = versionAgentGraphParserOutput("1.0.0+python-runtime-probed");
export const MARKDOWN_PARSER_ID = "local-markdown-structure";
export const MARKDOWN_PARSER_VERSION = versionAgentGraphParserOutput("1.0.0");
...
export const JSON_CONFIG_PARSER_VERSION = versionAgentGraphParserOutput(`1.3.0+typescript-${JSON_TYPESCRIPT_VERSION}`);
export const STRUCTURAL_CONFIG_PARSER_ID = "local-config-structure";
export const STRUCTURAL_CONFIG_PARSER_VERSION = versionAgentGraphParserOutput("1.0.0");
export const SOURCE_INVENTORY_PARSER_ID = "local-source-inventory";
export const SOURCE_INVENTORY_PARSER_VERSION = versionAgentGraphParserOutput("1.0.0");
export const PDF_PARSER_ID = "local-pdf-markdown-adapter";
export const PDF_PARSER_VERSION = versionAgentGraphParserOutput("1.2.0");
export const DECLARATIVE_GRAMMAR_PARSER_ID = "local-declarative-grammar";
export const DECLARATIVE_GRAMMAR_PARSER_VERSION = versionAgentGraphParserOutput("1.0.0");
...
function boundedParserOptions(source, options) {
  ...
  const retainRecord = (kind, stage = "parser-output") => {
    ...
    if (retained.nodes > limits.maxNodes
      ...
      throw new AgentGraphError("parser_record_limit_exceeded", `Parser output exceeded its graph limits for ${source.relativePath}.`, {
  ...
  const checkpoint = (stage = "parser") => {
    ...
    if (operations > maxOperations) {
      throw new AgentGraphError("parser_operation_limit_exceeded", `Parser traversal exceeded its operation limit for ${source.relativePath}.`, {
        sourcePath: source.relativePath,
...
function runPythonAstFacts({ pythonBin, sourcePath, text, timeoutMs = 10_000, abortSignal }) {
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () => {
      child.kill("SIGKILL");
      finish(new AgentGraphError("aborted", "Python AST extraction was aborted."));
    };
...
export async function probePythonParserRuntime(options = {}) {
  const facts = await runPythonAstFacts({
    ...
    timeoutMs: Math.max(1, Math.min(
      ...
      remainingAgentGraphDuration(options.deadline),
    )),
    abortSignal: options.abortSignal,
  });
...
async function parsePythonSource(source, options) {
  ...
  try {
    ...
    if (error instanceof AgentGraphError && ["aborted", "max_duration_exceeded"].includes(error.code)) throw error;
    return {
      ...sourceOnlyFragment(source, descriptor),
...
async function parsePdfSource(source, options) {
  ...
  try {
    ...
    checkAgentGraphBudget({ ...options, stage: "pdf-conversion" });
    ...
    if (Buffer.byteLength(markdown, "utf8") > 16 * 1024 * 1024) {
      throw new AgentGraphError("parser_record_limit_exceeded", `PDF conversion output is too large for ${source.relativePath}.`, {
        sourcePath: source.relativePath,
    ...
    if (lines.length > options.maxRecords) {
      throw new AgentGraphError("parser_record_limit_exceeded", `PDF conversion produced too many records for ${source.relativePath}.`, {
        sourcePath: source.relativePath,
...
const PARSER_DISPATCH = createAgentGraphParserDispatch(SOURCE_PARSER_REGISTRY);
