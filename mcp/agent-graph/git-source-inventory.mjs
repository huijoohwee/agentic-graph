...
function decodeUtf8(bytes, code = "repository_inventory_path_unsafe") {
  try {
    ...
    throw new AgentGraphError(code, "Repository inventory contains a non-UTF-8 path.", {
      complete: false,
    });
  }
}
...
function nulRecords(stdout) {
  ...
  while (offset < bytes.length) {
    ...
    if (end < 0) {
      throw new AgentGraphError("repository_inventory_invalid", "Repository inventory is not NUL-terminated.", {
        complete: false,
      });
    }
...
async function runGit(repositoryRootPath, args, options = {}) {
  checkAgentGraphBudget({ ...options, stage: "source-discovery-repository-inventory" });
  try {
    const result = await execFileAsync("git", [
      ...
      timeout: Math.max(1, remainingAgentGraphDuration(options.deadline)),
      windowsHide: true,
    });
    checkAgentGraphBudget({ ...options, stage: "source-discovery-repository-inventory" });
