...
import { normalizeAgentGraphProjectionByteLimit } from "./projection-budget.mjs";
...
export function unavailableProjection(snapshot, limitRaw, projectionByteLimitRaw) {
  ...
  const limit = Number.isFinite(numericLimit)
    ...
    : 200;
  const projectionByteLimit = normalizeAgentGraphProjectionByteLimit(projectionByteLimitRaw);
  return {
