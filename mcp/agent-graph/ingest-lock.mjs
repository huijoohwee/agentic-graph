...
// Preserve the lock identity so an upgrade cannot ignore an active predecessor lock.
...
async function acquireIngestLock(pointerPath, options) {
  const directory = await ensureAgentGraphStorageRoot(path.dirname(pointerPath));
  ...
  try {
    while (true) {
      checkAgentGraphBudget({ ...options, stage: "ingest-lock-acquire" });
      try {
...
export async function withAgentGraphIngestLock(pointerPath, options, operation) {
