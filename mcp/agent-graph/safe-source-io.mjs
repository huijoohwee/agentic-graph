...
export async function readStableSourceFile(
  ...
  const checkBudget = () => checkAgentGraphBudget({
  ...
  try {
    ...
    if (!openedStat.isFile()) {
      throw new AgentGraphError("source_not_regular_file", `Source is not a regular file: ${relativePath}`);
    }
    ...
    throw new AgentGraphError("source_read_failed", `Could not safely read source: ${relativePath}`, {
