import { AgentGraphError, compareStableStrings, sha256, stableStringify } from "./contract.mjs";
...
const asUniqueSorted = (values, validate, field) => {
  if (!Array.isArray(values) || values.length > MAX_MATCHERS_PER_DESCRIPTOR) {
    throw new AgentGraphError("parser_descriptor_invalid", `${field} must be a bounded array.`);
  }
  const normalized = values.map((value) => String(value || "").trim());
  if (normalized.some((value) => !validate.test(value))) {
    throw new AgentGraphError("parser_descriptor_invalid", `${field} contains an unsafe matcher.`);
  }
...
function normalizeDescriptor(raw, index, adapterFidelities) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    throw new AgentGraphError("parser_descriptor_invalid", `Parser descriptor ${index} must be an object.`);
  }
  ...
  if (unexpected.length) {
    throw new AgentGraphError("parser_descriptor_invalid", `Parser descriptor ${index} has unsupported keys.`, {
  ...
  if (![id, kind, adapter, fidelity].every((value) => SAFE_TOKEN.test(value))) {
    throw new AgentGraphError("parser_descriptor_invalid", `Parser descriptor ${index} has an invalid identity token.`);
  }
  ...
  if (!extensions.length && !basenames.length && !basenameFamilies.length) {
    throw new AgentGraphError("parser_descriptor_invalid", `Parser descriptor ${id} has no inert matcher.`);
  }
  const priority = Number(raw.priority ?? 0);
  if (!Number.isSafeInteger(priority) || priority < -1000 || priority > 1000) {
    throw new AgentGraphError("parser_descriptor_invalid", `Parser descriptor ${id} has an invalid priority.`);
  }
...
export function compileParserRegistry(rawDescriptors, { adapterFidelities = null } = {}) {
  if (!Array.isArray(rawDescriptors) || !rawDescriptors.length || rawDescriptors.length > MAX_DESCRIPTORS) {
    throw new AgentGraphError("parser_registry_invalid", `Parser registry must contain 1-${MAX_DESCRIPTORS} descriptors.`);
  }
  ...
  for (const descriptor of descriptors) {
    if (ids.has(descriptor.id)) throw new AgentGraphError("parser_registry_invalid", `Duplicate parser descriptor id: ${descriptor.id}`);
    if (kinds.has(descriptor.kind)) throw new AgentGraphError("parser_registry_invalid", `Duplicate parser descriptor kind: ${descriptor.kind}`);
    ...
    for (const extension of descriptor.extensions) {
      ...
      if (owner && owner.priority === descriptor.priority) {
        throw new AgentGraphError("parser_registry_ambiguous", `Extension ${extension} has equal-priority parser descriptors.`);
      }
    ...
    for (const basename of descriptor.basenames) {
      ...
      if (owner && owner.priority === descriptor.priority) {
        throw new AgentGraphError("parser_registry_ambiguous", `Basename ${basename} has equal-priority parser descriptors.`);
      }
    ...
    for (const family of descriptor.basenameFamilies) {
      ...
      if (owner && owner.priority === descriptor.priority) {
        throw new AgentGraphError("parser_registry_ambiguous", `Basename family ${family} has equal-priority parser descriptors.`);
      }
...
export function compileParserDispatch(registry, adapters) {
  if (!registry?.digest || !adapters || typeof adapters !== "object" || Array.isArray(adapters)) {
    throw new AgentGraphError("parser_dispatch_invalid", "Parser dispatch requires a compiled registry and adapter record.");
  }
  const dispatch = new Map();
  for (const descriptor of registry.descriptors) {
    const adapter = adapters[descriptor.adapter];
    if (typeof adapter !== "function") {
      throw new AgentGraphError("parser_adapter_missing", `Parser adapter is missing: ${descriptor.adapter}`);
    }
  ...
  return Object.freeze({
    ...
    parse(source, options = {}) {
      ...
      if (!adapter) throw new AgentGraphError("parser_adapter_missing", `No parser adapter is registered for ${String(source?.kind || "")}.`);
