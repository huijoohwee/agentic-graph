...
export const APPLICATION_ADAPTER_POLICY_SCHEMA_ID = "agentic-graph.application-adapter-policy/v1";
...
const APPLICATION_OWNER_EVIDENCE = deepFreezeApplicationValue({ ownerId: "agentic-graph.application-runtime", implementationRevision: ADAPTER_REVISION, implementationDigest: MODULE_SOURCE_DIGEST });
const AGENT_OWNER_EVIDENCE = deepFreezeApplicationValue({ ownerId: "agentic-graph.agent-runtime", implementationRevision: ADAPTER_REVISION, implementationDigest: sourceDigest(["../contracts/agent-runtime.schema.js", "../data/config/agents/agent-definitions.json"]) });
...
const createDescriptor = (id, ownerEvidence, capabilities, execute, { sideEffect = "none", replay = "safe" } = {}) => ({
  id: `agentic-graph.${id}`, revision: ADAPTER_REVISION, adapterKind: id, interfaceId: APPLICATION_ADAPTER_INTERFACE_ID, interfaceRevision: APPLICATION_ADAPTER_INTERFACE_REVISION,
...
export function createDefaultApplicationAdapterRegistry(options = {}) {
  ...
  if (!gatewayEvidence || gatewayEvidence.ownerId !== "agentic-graph.external-tool-gateway" || !EXACT_REVISION.test(gatewayEvidence.implementationRevision) || !SHA256.test(gatewayEvidence.implementationDigest)) throw new TypeError("External gateway must expose exact owner implementation evidence.");
  ...
  const resolveNodeOwnerEvidence = (node, component, adapter) => {
    if (component.id === "agent.registered") {
      ...
      return definition ? { ok: true, evidence: deepFreezeApplicationValue({ contractId: "agentic-graph.agent-definition", ownerId: adapter.ownerId, revision: definition.version, digest: digestApplicationValue(definition) }) } : { ok: false, error: { code: "agent_definition_unavailable", message: "The exact registered agent definition is unavailable." } };
    }
    if (component.id === "integration.external-artifact") {
      const resolved = resolveIntegration(node.config);
      return resolved.ok ? { ok: true, evidence: deepFreezeApplicationValue({ contractId: "agentic-graph.integration-profile", ownerId: adapter.ownerId, revision: resolved.evidence.integrationProfileRevision, digest: digestApplicationValue(resolved.evidence), integration: resolved.evidence }) } : resolved;
    }
    return { ok: true, evidence: deepFreezeApplicationValue({ contractId: "agentic-graph.application-runtime", ownerId: adapter.ownerId, revision: adapter.ownerImplementationRevision, digest: adapter.ownerImplementationDigest }) };
  };
