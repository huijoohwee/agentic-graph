...
const parseRecord = (text, effectId, inputDigest) => {
  ...
  if (
    !isRecord(record)
    || record.schema !== "knowgrph.agent-team-local-effect/v1"
...
export class LocalAgentTeamEffectStore {
  constructor({ rootDir } = {}) {
    ...
    this.baseDir = path.join(this.rootDir, ".knowgrph-workspace", "agent-team-effects");
  }
  ...
  async begin(effectId, inputDigest) {
    ...
    const filePath = this.effectPath(effectId);
    const pending = {
      schema: "knowgrph.agent-team-local-effect/v1",
      status: "pending",
      effectId,
      inputDigest,
    };
