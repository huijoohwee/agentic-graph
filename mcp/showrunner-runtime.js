...
const validateBrief = (markdown) => {
  ...
  const errors = [];
  if (text(meta.schema) !== "agentic-graph-showrunner-brief/v1") errors.push("schema must be agentic-graph-showrunner-brief/v1");
...
const safeRootPath = (rootDir, relativePath) => {
  ...
  if (!rel || rel.startsWith("..") || path.isAbsolute(rel)) throw new Error(`Path must stay inside AGENTIC_OS_ROOT: ${relativePath}`);
  return resolved;
};
