...
const STATE_SCHEMA = "agentic-graph-persistent-memory-store/v1";
...
const assertStateDirectoryOutsideRepository = (directory, rootDir, gitCommonDirectory) => {
  ...
  for (const repositoryPath of new Set([
    ...
    if (isInside(repositoryPath, physicalDirectory)) {
      throw new TypeError("Persistent-memory state must remain outside the agentic-graph repository.");
    }
  }
...
export function resolvePersistentMemoryStateDirectory(env = process.env, rootDir = process.cwd()) {
  ...
  const configured = String(env.AGENTIC_OS_MEMORY_STATE_DIR || "").trim();
  const namespace = String(env.AGENTIC_OS_MEMORY_NAMESPACE || "local-operator").trim() || "local-operator";
...
export function createLocalPersistentMemoryStore({
  ...
  const namespace = normalizeStoreId(String(env.AGENTIC_OS_MEMORY_STORE_ID || "local").trim() || "local");
