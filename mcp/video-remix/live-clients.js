...
// (agentic-graph-acos-mcp-connector runtime-readiness path, task 12.5).
//
...
//
//   * LIVE only when the operator opts in (`AGENTIC_OS_LIVE_CLIENTS` truthy) OR a
...
const DIRECTOR_TOOL_NAME = "agentic-graph.video_remix.run";
 ...
 *
 * LIVE is enabled when `AGENTIC_OS_LIVE_CLIENTS` is truthy OR an `EXA_API_KEY` is
...
export function resolveStageClients(env = {}, deps = {}) {
  ...
  const exaApiKey =
    typeof source.EXA_API_KEY === "string" && source.EXA_API_KEY.length > 0
      ? source.EXA_API_KEY
      : undefined;
  const live = isTruthyFlag(source.AGENTIC_OS_LIVE_CLIENTS) || Boolean(
  ...
  if (renderProvider === "strytree") {
    ...
    if (aiGatewayVideoUrl && bytePlusKey) {
      const mediaPersister = deps.mediaPersister || (
        deps.r2Client
          ? createMediaPersister({ r2Client: deps.r2Client, bucket: cleanEnv(source.AGENTIC_OS_MEDIA_BUCKET) })
          : null
      );
  ...
  const paymentUrl = cleanEnv(source.AGENTIC_OS_PAYMENT_URL);
  const commerceClient = paymentUrl
    ? createStripeCommerceClients({
        fetchImpl: deps.fetchImpl,
        endpoint: paymentUrl,
        apiKey: cleanEnv(source.AGENTIC_OS_PAYMENT_API_KEY),
      })
    : null;
 ...
 *
 * For a Director (`agentic-graph.video_remix.run`) call, when a live Exa client is
