...
// Default demo endpoints (Cloudflare-only). Frontend = Pages app at airvio.co/agentic-graph;
// Worker = agentic-graph-mcp Worker base endpoint + its open `GET /health` liveness route.
const DEFAULT_FRONTEND_URL = "https://airvio.co/agentic-graph";
const DEFAULT_WORKER_URL = "https://airvio.co/agentic-graph/mcp";
const DEFAULT_WORKER_HEALTH_URL = "https://airvio.co/agentic-graph/mcp/health";
...
function buildSectionEvidence(id, ctx) {
  ...
  switch (id) {
    case "agent_overview":
      return `Single autonomous Director (agentic-graph.video_remix.run) drove the ` +
...
const URL_KIND_TO_SECTION = Object.freeze({
  ...
  // The embedded agentic-graph canvas backs the Actions & Tool Use dimension.
